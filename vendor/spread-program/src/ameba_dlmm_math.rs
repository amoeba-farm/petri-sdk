...
pub fn fill_bin_exact_input_without_fee(direction: AmoebaDlmmSwapDirection, remaining: u64,
    price: u64, bin: &AmoebaDlmmBinLiquidity) -> MathResult<AmoebaDlmmBinFill> {
    if remaining == 0 || price == 0 { return Err(AmoebaDlmmMathError::InvalidAmount); }
    let (input, output, options, quote) = match direction {
        AmoebaDlmmSwapDirection::QuoteForOption => {
            if bin.option_reserve == 0 { return Err(AmoebaDlmmMathError::InvalidRoute); }
            let full = ceil_mul_div(bin.option_reserve, price, AMOEBA_DLMM_PRICE_SCALE)?;
            let input = min(remaining, full);
            let output = if remaining >= full { bin.option_reserve }
                else { floor_mul_div(remaining, AMOEBA_DLMM_PRICE_SCALE, price)? };
            (input, output, bin.option_reserve.checked_sub(output), bin.quote_reserve.checked_add(input))
        }
        AmoebaDlmmSwapDirection::OptionForQuote => {
            if bin.quote_reserve == 0 { return Err(AmoebaDlmmMathError::InvalidRoute); }
            let full = ceil_mul_div(bin.quote_reserve, AMOEBA_DLMM_PRICE_SCALE, price)?;
            let input = min(remaining, full);
            let output = if remaining >= full { bin.quote_reserve }
                else { floor_mul_div(remaining, price, AMOEBA_DLMM_PRICE_SCALE)? };
            (input, output, bin.option_reserve.checked_add(input), bin.quote_reserve.checked_sub(output))
        }
    };
    Ok(AmoebaDlmmBinFill { bin_id: bin.bin_id, trade_input: input, amount_out: output, lp_fee: 0,
