...
impl WriterDlmmPolicyV1 {
    pub const LEN: usize = 6 + 6 * 32 + 3 * 8 + 2 + 3 + 6 * 8
        + WRITER_SERIES_STORAGE_CAPACITY * (WriterDlmmSeriesPolicyV1::LEN + 2 * 8) + 32;
    ...
    pub fn has_current_layout(&self) -> bool {
        ...
        self.account_discriminator == Self::ACCOUNT_DISCRIMINATOR
            ...
            && count > 0 && count <= WRITER_LIVE_SERIES_LIMIT && appended <= count
            && (!self.sealed || (appended == count
                && self.rolling_policy_hash == self.expected_policy_hash))
            && self.total_uncommitted_quote_atoms <= self.total_pool_quote_atoms
            && self.series_monthly_spent_atoms.iter().try_fold(0u64, |sum, value| sum.checked_add(*value))
                == Some(self.monthly_spent_atoms)
            && self.monthly_spent_atoms <= self.monthly_buyback_cap_atoms
            && self.series_monthly_spent_atoms.iter().zip(self.series.iter())
                .all(|(spent, terms)| *spent <= terms.monthly_buyback_cap_atoms)
            && self.series[appended..].iter().all(|entry| *entry == WriterDlmmSeriesPolicyV1::default())
            && self.series_monthly_spent_atoms[count..].iter().all(|value| *value == 0)
            && self.series_pool_inventory_atoms[count..].iter().all(|value| *value == 0)
            && self.reserved == [0; 32]
    }
}
...
impl WriterDlmmPositionV1 {
    pub const LEN: usize = 6 + 4 * 32 + 2 + 4 * 8
        + WRITER_DLMM_POSITION_BINS * WriterDlmmBinV1::LEN + 32;
    ...
    pub fn has_current_layout(&self) -> bool {
        ...
        if self.account_discriminator != Self::ACCOUNT_DISCRIMINATOR
            ...
            || count > WRITER_DLMM_POSITION_BINS || self.reserved != [0; 32]
            || self.bins[count..].iter().any(|bin| *bin != WriterDlmmBinV1::default())
        {
            return false;
        }
        ...
        for bin in &self.bins[..count] {
            if bin.bin_id == 0 || bin.bin_id > crate::constants::MAX_AMOEBA_DLMM_BIN_COUNT
            ...
            let Some(next_options) = options.checked_add(bin.option_atoms) else { return false; };
            let Some(next_quote) = quote.checked_add(bin.quote_atoms) else { return false; };
...
pub fn derive_writer_dlmm_policy_pda(program_id: &Pubkey, sleeve: &Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[CURRENT_STATE_NAMESPACE_SEED, WRITER_DLMM_POLICY_SEED,
        sleeve.as_ref()], program_id)
}

pub fn derive_writer_dlmm_position_pda(
    program_id: &Pubkey, pool: &Pubkey, sleeve: &Pubkey,
) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[CURRENT_STATE_NAMESPACE_SEED, WRITER_DLMM_POSITION_SEED,
        pool.as_ref(), sleeve.as_ref()], program_id)
}
