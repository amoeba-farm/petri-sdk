...
fn share_and_swap_overflows_fail_closed() {
    ...
    let lp_fee_add_overflow = [AmoebaDlmmBinLiquidity {
        bin_id: 1,
        option_reserve: 1,
...
#[test]
fn fees_round_against_the_trader_and_protocol_share_rounds_down() {
    assert_eq!(
        calculate_fees(101, 25, 2_500),
...
#[test]
fn lp_fee_remainder_belongs_to_final_bin() {
    ...
    .unwrap();
    assert_eq!(quote.total_fee, 4);
    assert_eq!(quote.protocol_fee, 1);
    assert_eq!(quote.fills.iter().map(|fill| fill.lp_fee).sum::<u64>(), 3);
    assert_eq!(quote.fills.last().unwrap().lp_fee, 2);
}
...
fn shared_json_fixtures_match_quote_and_post_swap_state() {
    ...
    for fixture in fixtures.cases {
        ...
        let quote = quote_exact_in(
            ...
            fixture.pool.swap_fee_bps,
            fixture.pool.protocol_fee_share_bps,
...
proptest! {
    ...
    fn randomized_fees_and_share_withdrawals_are_bounded(
        ...
        let fees = calculate_fees(amount, fee_bps, protocol_bps).unwrap();
    ...
    fn zero_fee_same_bin_round_trip_never_creates_quote_value(
        ...
        let buy = quote_exact_in(
            ...
            0,
            1,
            &[AmoebaDlmmBinLiquidity { bin_id, option_reserve, quote_reserve: 0 }],
        ).unwrap();
        let sell = quote_exact_in(
            AmoebaDlmmSwapDirection::OptionForQuote,
            buy.amount_out,
