...
pub fn derive_collective_settlement_delegate(program_id: &Pubkey, owner: &Pubkey, mint: &Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[CURRENT_STATE_NAMESPACE_SEED,
        COLLECTIVE_SETTLEMENT_DELEGATE_SEED, owner.as_ref(), mint.as_ref()], program_id)
}

/// A create-once permission record for one exact LP position and its owner.
pub fn derive_position_settlement_authority(program_id: &Pubkey, owner: &Pubkey, position: &Pubkey) -> (Pubkey, u8) {
    Pubkey::find_program_address(&[CURRENT_STATE_NAMESPACE_SEED,
        POSITION_SETTLEMENT_AUTHORITY_SEED, owner.as_ref(), position.as_ref()], program_id)
}
