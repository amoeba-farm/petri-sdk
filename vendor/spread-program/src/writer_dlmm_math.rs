...
pub fn writer_dlmm_price_bounds(seller_floor: u64, tick: u64, separation_ticks: u16,
    swap_fee_bps: u16, primary_fee_bps: u16) -> Result<(u64, u64, u64)> {
    if seller_floor == 0 || tick == 0 || separation_ticks == 0 || swap_fee_bps >= 10_000
    ...
    let one_taker_leg = mul_ceil(ask, u64::from(swap_fee_bps), 10_000 - u64::from(swap_fee_bps))?;
    let round_trip = add(primary, add(one_taker_leg, one_taker_leg)?)?;
    let gap = tick.checked_mul(u64::from(separation_ticks)).ok_or(WriterDlmmAdmissionError::Arithmetic)?;
    let bid = seller_floor.checked_sub(gap).and_then(|value| value.checked_sub(round_trip))
...
pub fn writer_dlmm_primary_fee(gross_writer_premium: u64, primary_fee_bps: u16) -> Result<u64> {
    if primary_fee_bps > 10_000 { return Err(WriterDlmmAdmissionError::InvalidPolicy); }
...
fn reserve(book: &[WriterSeries], limits: &WriterDlmmRiskLimits) -> Result<WriterReserveSummary> {
    exact_reserve(book, limits.lower_tail_max_settlement_atomic,
        limits.upper_tail_min_settlement_atomic).map_err(WriterDlmmAdmissionError::Envelope)
}
...
fn check_drawdown(assets: u64, principal: u64, liability: u64, limit: u64) -> Result<()> {
    ...
    let equity = assets.checked_sub(liability).ok_or(WriterDlmmAdmissionError::Insolvent)?;
...
pub fn admit_writer_dlmm_cash(
    book: &[WriterSeries], cash: WriterDlmmCash, limits: &WriterDlmmRiskLimits,
    ...
    let free_cash = cash.assets_atoms.checked_sub(protected)
    ...
    check_drawdown(cash.assets_atoms, cash.principal_atoms, summary.reserve_atoms,
        limits.worst_drawdown_ppm)?;
    check_drawdown(cash.assets_atoms, cash.principal_atoms, summary.lower_tail_reserve_atoms,
        limits.lower_drawdown_ppm)?;
    check_drawdown(cash.assets_atoms, cash.principal_atoms, summary.upper_tail_reserve_atoms,
...
pub fn admit_writer_dlmm_retirement(
    book: &[WriterSeries], cash: WriterDlmmCash, risk: &WriterDlmmRiskLimits,
    policy: &WriterDlmmBuybackLimits, series_limits: &[WriterDlmmSeriesLimits],
    retirements: &[WriterDlmmRetirement], month_spent_atoms: u64,
    allocated_lp_quote_after_atoms: u64, close_pending: bool,
) -> Result<WriterDlmmAdmission> {
    if close_pending {
        return Err(WriterDlmmAdmissionError::CloseHasPriority);
    }
    if book.is_empty() || book.len() > WRITER_MAX_SERIES || series_limits.len() != book.len()
        || retirements.is_empty() || retirements.len() > book.len()
    {
        return Err(WriterDlmmAdmissionError::InvalidBook);
    }
    if policy.tick_size_quote_atoms == 0 || policy.price_separation_ticks == 0
    ...
    let tick_gap = policy.tick_size_quote_atoms.checked_mul(u64::from(policy.price_separation_ticks))
    ...
    for retirement in retirements {
        ...
        after[i].external_oi_atoms = after[i].external_oi_atoms.checked_sub(retirement.retired_atoms)
        ...
        let separated_price = limits.seller_floor_quote_atoms.checked_sub(tick_gap)
            .ok_or(WriterDlmmAdmissionError::PriceSeparation)?;
        let price_bound = mul_floor(retirement.retired_atoms, separated_price,
            WRITER_CONTRACT_ATOMIC_SCALE)?.checked_sub(mul_ceil(retirement.retired_atoms,
                policy.round_trip_fee_quote_atoms, WRITER_CONTRACT_ATOMIC_SCALE)?)
            .ok_or(WriterDlmmAdmissionError::PriceSeparation)?;
        ...
        conservative_value = add(conservative_value, mul_floor(retirement.retired_atoms,
            limits.conservative_claim_value_atoms, WRITER_CONTRACT_ATOMIC_SCALE)?)?;
    }
    let assets_after = cash.assets_atoms.checked_sub(total_cost)
        .ok_or(WriterDlmmAdmissionError::Insolvent)?;
    let (summary, free_cash, exposure) = admit_writer_dlmm_cash(&after[..book.len()],
        WriterDlmmCash { assets_atoms: assets_after, principal_atoms: cash.principal_atoms,
            allocated_lp_quote_atoms: allocated_lp_quote_after_atoms }, risk)?;
    let released = before.reserve_atoms.checked_sub(summary.reserve_atoms)
        .filter(|value| *value > 0).ok_or(WriterDlmmAdmissionError::NoReserveReduction)?;
    ...
    let post_retirement_available = cash.assets_atoms.checked_sub(add(summary.reserve_atoms,
        risk.operational_buffer_atoms)?).ok_or(WriterDlmmAdmissionError::Insolvent)?;
    if total_cost > conservative_value
        || total_cost > mul_floor(released, policy.reserve_release_spend_ratio_ppm,
    ...
    Ok(WriterDlmmAdmission { assets_after_atoms: assets_after,
        reserve_before_atoms: before.reserve_atoms, reserve_after: summary,
        released_reserve_atoms: released, cost_atoms: total_cost,
        monthly_spent_after_atoms: monthly_after, free_cash_after_atoms: free_cash,
