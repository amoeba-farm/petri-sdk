use super::*;
use crate::state::{WriterDlmmBinV1, WriterDlmmPolicyV1, WriterDlmmPositionV1,
    WriterDlmmSeriesPolicyV1, WRITER_DLMM_POSITION_BINS};
...
impl FixedField for [WriterDlmmSeriesPolicyV1; WRITER_SERIES_STORAGE_CAPACITY] {
    ...
    fn write(&self, output: &mut FixedWriter<'_>) {
        for entry in self { entry.write(output); }
    }
}
...
impl FixedField for [WriterDlmmBinV1; WRITER_DLMM_POSITION_BINS] {
    ...
    #[inline(always)]
    fn write(&self, output: &mut FixedWriter<'_>) {
        for bin in self { bin.write(output); }
    }
}
