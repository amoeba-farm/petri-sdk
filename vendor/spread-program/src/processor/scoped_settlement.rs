...
pub(super) fn load_scoped_holder_token_account(
    program_id: &Pubkey, account: &AccountInfo, owner: &Pubkey, mint: &Pubkey,
    ...
    let delegation_ok = match token.delegate {
        ...
        COption::Some(delegate) => delegate == derive_collective_settlement_delegate(program_id, owner, mint).0,
    };
    if token.owner != *owner || token.mint != *mint || token.state != AccountState::Initialized
        || !delegation_ok || token.is_native != COption::None || token.close_authority != COption::None {
        return Err(VaultError::InvalidLightTokenAccount.into());
    }
    Ok(token)
}
...
pub(super) fn authorize_collective_settlement(program_id: &Pubkey, accounts: &[AccountInfo], revoke: bool) -> ProgramResult {
    ...
    if !owner.is_signer || !owner.is_writable { return Err(ProgramError::MissingRequiredSignature); }
    ...
    if !revoke && source.owner == &system_program::id() {
        load_or_create_light_associated_token_account(owner, owner, mint, source,
            light, &accounts[6], &accounts[7], &accounts[8])?;
    }
    ...
    invoke(&instruction, &[source.clone(), delegate.clone(), owner.clone(), light.clone()])?;
    let token = load_scoped_holder_token_account(program_id, source, owner.key, mint.key)?;
    if (revoke && token.delegate != COption::None)
        || (!revoke && (token.delegate != COption::Some(*delegate.key) || token.delegated_amount != u64::MAX)) {
        return Err(VaultError::InvalidLightTokenAccount.into());
    }
    Ok(())
}
