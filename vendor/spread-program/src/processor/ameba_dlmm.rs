    ...
    for (index, account) in accounts.iter().enumerate() {
        ...
        let expected_writable = match tag {
            ...
            AmoebaDlmmInstructionTag::SwapCollectiveDlmmExactInV1 => {
                ...
                matches!(index, 0 | 2 | 4 | 6 | 7 | 9 | 11 | 12 | 13 | 14 | 17 | 18 | 22 | 24 | 26 | 27 | 28 | 29 | 31)
...
pub(super) fn load_writer_dlmm_pool(program_id: &Pubkey, info: &AccountInfo)
    -> Result<AmoebaDlmmPoolV1, ProgramError> { load_pool(program_id, info) }

pub(super) fn store_writer_dlmm_pool(info: &AccountInfo, pool: &AmoebaDlmmPoolV1) -> ProgramResult {
    store_light_state(info, pool)
}

pub(super) fn writer_dlmm_vault_amounts(program_id: &Pubkey, pool_info: &AccountInfo,
    pool: &AmoebaDlmmPoolV1, authority: &AccountInfo, option: &AccountInfo, quote: &AccountInfo)
    -> Result<(u64, u64), ProgramError> {
    validate_pool_vault_amounts(program_id, pool_info, pool, authority, option, quote)
}

pub(super) fn validate_writer_dlmm_pool_binding(config: &VaultConfig, market: &AccountInfo,
    month: &AccountInfo, context: &super::writer_sleeve::CollectiveDlmmContext,
    pool: &AmoebaDlmmPoolV1) -> ProgramResult {
