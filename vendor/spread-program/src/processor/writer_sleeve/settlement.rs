...
pub(super) fn process_scoped_collective_settlement(
    ...
    if action == 0 || action == 2 {
        return super::super::scoped_settlement::authorize_collective_settlement(program_id, accounts, action == 2);
    }
    ...
    if !keeper.is_signer || !keeper.is_writable { return Err(ProgramError::MissingRequiredSignature); }
    let source = super::super::scoped_settlement::load_scoped_holder_token_account(
        program_id, &accounts[7], accounts[0].key, accounts[6].key)?;
    ...
    claim_collective_long(program_id, &accounts[..20],
        ClaimCollectiveLongV1Params { claim_atoms: source.amount }, Some((keeper, delegate)))
}

fn claim_collective_long<'a>(
    program_id: &Pubkey, accounts: &[AccountInfo<'a>], params: ClaimCollectiveLongV1Params,
    ...
    let (expected_delegate, delegate_bump) = crate::scoped_settlement::derive_collective_settlement_delegate(
        program_id, holder_info.key, contract_mint_info.key);
    ...
    let delegate_seeds: &[&[u8]] = &[CURRENT_STATE_NAMESPACE_SEED,
        crate::scoped_settlement::COLLECTIVE_SETTLEMENT_DELEGATE_SEED,
        holder_info.key.as_ref(), contract_mint_info.key.as_ref(), &bump];
    ...
    invoke_light_token_account_transfer_with_signer_seeds(
        ...
        token_program_info,
        system_program_info,
        if scoped.is_some() { scoped_signers } else { &[] },
    )?;
    if payout != 0 {
