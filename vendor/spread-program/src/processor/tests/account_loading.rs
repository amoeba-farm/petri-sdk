...
#[test]
fn canonical_collateral_position_ledger_and_receipt_loaders_reject_type_confusion() {
    let program_id = Pubkey::new_unique();
    ...
    let (ledger_key, ledger_bump) = derive_oracle_player_ledger_pda(&program_id, &owner);
    let ledger = OraclePlayerLedger {
        is_initialized: true,
        bump: ledger_bump,
        owner,
        major_tokens: 41,
    ...
    assert!(with_test_account_info(
        &ledger_key,
        &program_id,
        exact_test_state_data(&ledger, OraclePlayerLedger::LEN),
        |info| load_canonical_oracle_player_ledger(&program_id, info, &owner)
    )
    .is_ok());
    assert_eq!(
        with_test_account_info(&ledger_key, &program_id, collateral_data, |info| {
            load_canonical_oracle_player_ledger(&program_id, info, &owner).unwrap_err()
        }),
        ProgramError::Custom(VaultError::InvalidOraclePlayerLedger as u32)
    );
    let mut wrong_ledger_bump = ledger.clone();
    wrong_ledger_bump.bump = wrong_ledger_bump.bump.wrapping_add(1);
    assert_eq!(
        with_test_account_info(
            &ledger_key,
            &program_id,
            exact_test_state_data(&wrong_ledger_bump, OraclePlayerLedger::LEN),
            |info| load_canonical_oracle_player_ledger(&program_id, info, &owner).unwrap_err()
        ),
        ProgramError::Custom(VaultError::InvalidOraclePlayerLedger as u32)
    );
}
