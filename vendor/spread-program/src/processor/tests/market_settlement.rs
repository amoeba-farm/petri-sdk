...
#[test]
fn emergency_snapshot_uses_samba_supply_and_locks_mint_and_burn() {
    let mut pool = OracleStakingPool {
        active_amba_backing: 700,
        samba_supply: 500,
        ..OracleStakingPool::default()
    };
    assert_eq!(
        prepare_oracle_samba_voting_snapshot(&mut pool, 500, 10),
        Ok(500)
    );
    assert_eq!(pool.governance_lock_count, 1);

    // External burns cannot inflate voting power and do not change the frozen denominator.
    assert_eq!(
        prepare_oracle_samba_voting_snapshot(&mut pool, 400, 11),
        Ok(500)
    );
    assert_eq!(pool.governance_lock_count, 2);
}
