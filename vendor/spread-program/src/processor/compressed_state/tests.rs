use super::{
    ...
    materialized_support_bytes, materialized_winning_vote_bytes,
    reference_validate_typed_state_data, required_access_contract, validate_compact_state_data,
    validate_typed_state_data, CompactOracleSambaVoteSettlementReceipt,
    CompactOracleSambaWinningVote, CompactOracleSkuCoverageRecord, CompactOracleSourceDescriptor,
    CompactOracleSourceState, CompactOracleSupportPosition, CompactOracleUsdcRewardReceipt,
    CompactOracleUsdcRewardRegistration, CompactOracleUsdcSkuPool, CompactOracleUsdcSourceReward,
    RequiredAccessKind, RequiredAccessSpec,
};
use crate::{
    constants::{
    ...
    state::{
        ...
        CompressedStateDomain, OracleEscrowDisposition, OracleSambaVoteSettlementReceipt,
        OracleSambaWinningVote, OracleSkuCoverageRecord, OracleSourceState, OracleSourceStatus,
        OracleSupportPosition, OracleUsdcRewardKind, OracleUsdcRewardReceipt,
...
fn reference_validate_compact_state_data(
    ...
    match domain {
        ...
        CompressedStateDomain::OracleSambaWinningVote => {
            let _: CompactOracleSambaWinningVote =
                decode_compact_state(data, VaultError::InvalidOracleEmergencyDispute)?;
        }
        CompressedStateDomain::OracleSambaVoteSettlementReceipt => {
            let state: CompactOracleSambaVoteSettlementReceipt =
                decode_compact_state(data, VaultError::InvalidOracleEmergencyDispute)?;
            if state.disposition == OracleEscrowDisposition::Unsettled {
