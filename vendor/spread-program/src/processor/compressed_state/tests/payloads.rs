...
fn compact_payloads_are_exact_and_bounded() {
    ...
    let winning = OracleSambaWinningVote {
        base_entitlement: 27,
        registered_slot: 28,
        ..OracleSambaWinningVote::default()
    };
    let vote_receipt = OracleSambaVoteSettlementReceipt {
        amount: 29,
        disposition: OracleEscrowDisposition::Transferred,
        settled_slot: 30,
        ..OracleSambaVoteSettlementReceipt::default()
    };
    ...
    let winning_compact = CompactOracleSambaWinningVote::from_full(&winning);
    let vote_receipt_compact = CompactOracleSambaVoteSettlementReceipt::from_full(&vote_receipt);
    ...
    let winning_data = encode_compact_state(&winning_compact).unwrap();
    let vote_receipt_data = encode_compact_state(&vote_receipt_compact).unwrap();
    ...
    let voting_power = 73;
    assert_eq!(
        materialized_winning_vote_bytes(
            bump,
            &pot,
            &dispute,
            &vote,
        ...
        fixed_account_bytes(
            &winning_compact
                .clone()
                .into_full(bump, pot, dispute, vote, voter, voting_power),
    ...
    assert_eq!(winning_data.len(), 16);
    assert_eq!(vote_receipt_data.len(), 17);
    assert_eq!(source_data.len(), 205);
    ...
    *zero_padded_sku_data.last_mut().unwrap() = 1;
    ...
    assert_eq!(
        decode_compact_state::<CompactOracleSambaWinningVote>(
            &winning_data,
            crate::error::VaultError::InvalidOracleEmergencyDispute,
        )
        .unwrap(),
        winning_compact
    );
    assert_eq!(
        decode_compact_state::<CompactOracleSambaVoteSettlementReceipt>(
            &vote_receipt_data,
            crate::error::VaultError::InvalidOracleEmergencyDispute,
        )
        .unwrap(),
        vote_receipt_compact
    );
    assert_eq!(
