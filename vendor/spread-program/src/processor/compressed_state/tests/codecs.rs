...
fn compact_leaf_codecs_are_byte_identical_to_borsh() {
    ...
    assert_compact_codec_matches_borsh(CompactOracleSambaWinningVote::from_full(
        &OracleSambaWinningVote::default(),
    ));
    assert_compact_codec_matches_borsh(CompactOracleSambaVoteSettlementReceipt::from_full(
...
fn every_supported_domain_binds_the_exact_current_typed_pda() {
    ...
    let (winning_key, winning_bump) =
        derive_oracle_samba_winning_vote_pda(&program_id, &pot, &vote);
    ...
    let (vote_receipt_key, vote_receipt_bump) =
        derive_oracle_samba_vote_settlement_pda(&program_id, &pot, &vote);
    let vote_receipt = OracleSambaVoteSettlementReceipt {
        ...
        amount: 0,
        disposition: OracleEscrowDisposition::Slashed,
        settled_slot: 30,
    };
    assert_valid(
        &program_id,
