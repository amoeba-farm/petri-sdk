...
pub(super) fn process_liquidity_change(
    ...
    if matches!(&change, LiquidityChange::Add(_)) {
        if accounts.len() < 19 { return Err(VaultError::InvalidAccountList.into()); }
        let normalized: Vec<_> = accounts[..16].iter().chain(accounts[17..].iter()).cloned().collect();
        process_liquidity_change_core(program_id, &normalized, change, None)?;
        super::scoped_position::process_scoped_position_settlement(program_id, &[
            accounts[0].clone(), accounts[1].clone(), accounts[2].clone(), accounts[16].clone(),
            accounts[15].clone(),
        ], 0)
    } else {
...
pub(super) fn process_scoped_liquidity_cleanup<'a>(
    program_id: &Pubkey, accounts: &[AccountInfo<'a>], params: RemoveAmoebaDlmmLiquidityV1Params,
    permit: &super::scoped_position::ScopedCleanup<'_, 'a>,
) -> ProgramResult {
    process_liquidity_change_core(program_id, accounts, LiquidityChange::Remove(params), Some(permit))
}

fn process_liquidity_change_core<'a>(
    program_id: &Pubkey, accounts: &[AccountInfo<'a>], change: LiquidityChange,
    ...
    if let Some(permit) = permit {
        ...
        if is_add || position.owner != *owner_info.key || pool.liquidity_manager != *owner_info.key
            || position.position_nonce != position_nonce || pool.status != AmoebaDlmmPoolStatus::Settled {
            return Err(VaultError::UnauthorizedAmoebaDlmmManager.into());
        }
    } else {
