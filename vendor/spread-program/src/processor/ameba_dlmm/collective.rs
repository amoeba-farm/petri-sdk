...
pub(super) fn process_set_collective_pool_status(
    ...
    let writer_sides = super::super::writer_sleeve::dlmm::activation_sides(program_id, accounts, &pool)?;
...
pub(super) fn process_collective_swap_exact_in(
    ...
    validate_collective_pool_binding(&config, &accounts[2], &accounts[3], &context, &pool)?;
    if context.group_status != WriterSettlementGroupStatus::Active
        || !matches!(context.sleeve_status, WriterSleeveStatus::Active | WriterSleeveStatus::CloseStaging)
        || context.anchor_month_settled
    {
        return Err(VaultError::AmoebaDlmmMarketNotTradable.into());
    }
    let mut writer = super::super::writer_sleeve::dlmm::load_swap_state(program_id, accounts, &pool)?;
    let normalized: Vec<_> = accounts[..4].iter().chain(accounts[7..23].iter())
        .chain(accounts[32..].iter()).cloned().collect();
    process_collective_swap_exact_in_core_with_writer(program_id, &normalized, params, &mut writer, accounts)?;
    ...
    super::super::scoped_settlement::authorize_collective_settlement(program_id, &[
        accounts[0].clone(), accounts[2].clone(), accounts[9].clone(), accounts[13].clone(),
        accounts[23].clone(), accounts[15].clone(), accounts[21].clone(), accounts[22].clone(),
        accounts[20].clone(),
    ], false)
}
