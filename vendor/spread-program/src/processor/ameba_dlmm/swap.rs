...
pub(super) fn process_collective_swap_exact_in_core_with_writer(
    ...
    let _ = load_user_transfer_account(program_id, input_user_info, trader_info.key, input_mint_info.key)?;
    if output_user_info.owner == &system_program::id() {
        ...
        let _ =
            load_user_transfer_account(program_id, output_user_info, trader_info.key, output_mint_info.key)?;
    }
    ...
    for (offset, page_info) in accounts[FIXED_ACCOUNTS..].iter().enumerate() {
        ...
        expected_page = next_set_page(&route_bitmap, expected_page.ok_or(VaultError::InvalidAmoebaDlmmRoute)?, ascending);
    }
    let unloaded_ordinary_boundary = if pages.is_empty() {
        let best = if ascending { pool.best_ask_bin_id } else { pool.best_bid_bin_id };
        (best != AMOEBA_DLMM_EMPTY_BIN_ID).then_some(best)
    } else { expected_page.map(|page| {
        let first = page_first_bin(page).map_err(math_error)?;
        Ok::<u16, ProgramError>(if ascending { first } else { first.saturating_add(31).min(pool.maximum_bin_id) })
    }).transpose()? };
    let writer_policy = writer.as_ref().map(|state| state.quote_policy(pool.tick_size_quote_atomic));
    let route = crate::writer_dlmm_quote::quote_writer_dlmm_exact_in(
        crate::writer_dlmm_quote::WriterDlmmRouteConfig { direction, amount_in: params.amount_in,
            minimum_amount_out: params.minimum_amount_out, limit_bin_id: params.limit_bin_id,
            tick_size_quote_atomic: pool.tick_size_quote_atomic, maximum_bin_id: pool.maximum_bin_id,
            swap_fee_bps: pool.swap_fee_bps, protocol_fee_share_bps: pool.protocol_fee_share_bps,
            maximum_bins: pool.maximum_bins_per_swap, unloaded_ordinary_boundary }, &bins,
        writer.as_ref().map_or(&[], |state| state.bins()), writer_policy.as_ref())
    .map_err(math_error)?;
    ...
    let (last_fill_page_index, _) = bin_to_page(last_ordinary_fill.bin_id).map_err(math_error)?;
    ...
    *input_reserve = input_reserve
    ...
    if let Some(state) = writer.as_mut() {
        super::super::writer_sleeve::dlmm::finish_swap(program_id, writer_accounts, state, &route,
            direction, pool.as_mut())?;
    }
    ...
    let physical_delta_ok = after_input
        == before_input
            ...
            .and_then(|value| value.checked_sub(match direction {
                AmoebaDlmmSwapDirection::QuoteForOption => route.writer.gross_premium_atoms.checked_add(route.writer.lp_fee_atoms)?,
...
#[cfg(test)]
pub(super) fn process_collective_swap_exact_in_core(program_id: &Pubkey, accounts: &[AccountInfo],
    params: SwapAmoebaDlmmExactInV1Params) -> ProgramResult {
