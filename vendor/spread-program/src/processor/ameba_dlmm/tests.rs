...
fn dlmm_p0_effective_privilege_matrix_accepts_dynamic_limits_only() {
    let liquidity = privilege_pattern("X W W R R R W W W W R R W W R R W W");
    let mut four_page_pairs = liquidity[..16].to_vec();
    ...
    let mut eight_swap_pages = privilege_pattern("X R R R R R R W R R R W W W W R R W W R R R W W");
    eight_swap_pages.extend(core::iter::repeat_n((false, true), 7));
    assert_eq!(eight_swap_pages.len(), 31);
    assert_eq!(
        validate_pack_dlmm_account_privileges(
...
fn fixed_event_codecs_preserve_borsh_bytes() {
    fn assert_event(event: AmoebaDlmmEvent) {
        let reference = match &event {
            ...
            AmoebaDlmmEvent::Fees(value) => borsh::to_vec(value).unwrap(),
    ...
    assert_event(AmoebaDlmmEvent::Fees(FeesEvent {
        pool,
        option_amount: 23,
        quote_amount: 24,
        slot: 25,
