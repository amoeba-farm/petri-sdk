...
#[test]
fn delayed_council_source_keep_source_reopens_coverage_for_a_fresh_schedule() {
    ...
    oracle_council::reopen_coverage_after_delayed_emergency_resolution(
        &mut month,
        &mut coverage,
        scramble_end - 1,
    )
...
#[test]
fn v5_source_emergency_resolution_requires_council_source_but_other_kinds_use_council_non_source() {
    let v5 = OracleMonthState {
        ..OracleMonthState::default()
    };
    assert!(oracle_council::ensure_emergency_resolver_coverage_lane(
        &v5,
    ...
    .is_err());
    assert_eq!(
        oracle_council::ensure_emergency_resolver_coverage_lane(
            &v5,
            OracleEmergencyDisputeKind::Source,
            true
        ),
    ...
    for kind in [
        ...
        assert_eq!(
            oracle_council::ensure_emergency_resolver_coverage_lane(&v5, kind, false),
            Ok(())
        );
        assert!(oracle_council::ensure_emergency_resolver_coverage_lane(&v5, kind, true).is_err());
    }
}

#[test]
fn council_source_removes_sku_coverage_only_for_the_first_contributing_terminalization() {
    let mut month = OracleMonthState {
        source_count: 1,
        ..OracleMonthState::default()
    };
    ...
    assert_eq!(
        oracle_council::remove_emergency_supported_candidate_coverage(
            &mut month,
            Some(&mut coverage),
            Some(&mut record),
            true,
...
fn stale_cleanup_terminalizes_unopened_council_review_once() {
    let expiry = 10_000_000u64;
