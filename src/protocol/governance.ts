...
export const GOVERNANCE_GATE_V1_BYTES = 192 as const;
export const GOVERNANCE_GATE_V1_DISCRIMINATOR = "AGVGAT01" as const;
export const GOVERNANCE_GATE_V1_VERSION = 1 as const;
export const GOVERNANCE_TAIL_V1_BYTES = 16 as const;
export const GOVERNANCE_TAIL_V1_MAGIC = "AGV1" as const;
export const GOVERNANCE_TAIL_V1_VERSION = 1 as const;
export const GOVERNED_INSTRUCTION_DATA_MAX_BYTES = 16_384 as const;
export const GOVERNANCE_UPGRADE_SEED_DOMAIN_V1 = "ameba-upgrade-v1" as const;
export const BPF_UPGRADEABLE_LOADER_PROGRAM_ID =
...
export type GovernanceGateStatusV1 =
  (typeof GOVERNANCE_GATE_STATUS_V1)[keyof typeof GOVERNANCE_GATE_STATUS_V1];
...
export interface GovernanceGateRpcV1 {
  getMultipleAccountsInfoAndContext?(addresses: PublicKey[], config: { readonly commitment: "finalized"; readonly minContextSlot: number }): Promise<{ readonly context: { readonly slot: number }; readonly value: readonly ({readonly data: Uint8Array; readonly executable: boolean; readonly owner: PublicKey} | null)[] }>;
...
export function deriveGovernanceControllerConfigPdaV1(
...
export function deriveProtocolGovernanceGatePdaV1(
  controllerProgram: PublicKey,
  targetProgram: PublicKey,
): readonly [PublicKey, number] {
  return PublicKey.findProgramAddressSync([
...
export function deriveUpgradeableProgramDataAddressV1(
  targetProgram: PublicKey,
): readonly [PublicKey, number] {
  return PublicKey.findProgramAddressSync(
    [targetProgram.toBytes()],
    new PublicKey(BPF_UPGRADEABLE_LOADER_PROGRAM_ID),
  );
}

export function encodeGovernanceInstructionTailV1(expectedEpoch: bigint): Uint8Array {
...
export function decodeGovernanceInstructionTailV1(
  data: Uint8Array,
...
export function decodeProtocolGovernanceGateV1(
  data: Uint8Array,
  ...
  const status = bytes[GOVERNANCE_GATE_V1_OFFSETS.status] as GovernanceGateStatusV1;
  const statusName = statusNameFor(status);
  const gate: ProtocolGovernanceGateV1 = {
...
export function validateProtocolGovernanceGateAccountV1(
  input: GovernanceGateAccountInput,
): ProtocolGovernanceGateV1 {
  if (input === null || typeof input !== "object") {
    fail("GOVERNANCE_GATE_INPUT_INVALID", "governance gate account input is malformed");
  }
  const identity = validateGovernanceIdentityV1(
    input.identity ?? CURRENT_GOVERNANCE_GENERATION_3,
  );
  ...
  if (
    controllerConfig.toBase58() !== identity.controllerConfigPda
...
export function assertFreshGovernanceGateObservationV1(
  planned: GovernanceGateContextV1,
  observed: GovernanceGateContextV1,
): GovernanceGateContextV1 {
  const plan = validateContext(planned);
  const current = validateContext(observed);
  if (
    plan.genesisHash !== current.genesisHash
...
export function buildGovernedInstructionEnvelopeV1(input: {
  ...
  return appendGovernanceEnvelopeV1(
    snapshot.instruction,
    snapshot.context,
    snapshot.recognizedInstructionTags,
  );
}
...
export async function materializeFreshGovernedInstructionEnvelopeV1(input: {
  ...
  return Object.freeze({
    instruction: appendGovernanceEnvelopeV1(
      snapshot.instruction,
      current,
      snapshot.recognizedInstructionTags,
    ),
...
export async function revalidateFreshGovernedInstructionEnvelopeV1(input: {
  ...
  const recognizedInstructionTags = validateRecognizedInstructionTagsV1(
    input.recognizedInstructionTags,
    instruction.data[0]!,
  );
...
export function inspectGovernedInstructionEnvelopeV1(input: {
  ...
  readonly context: GovernanceGateContextV1;
  readonly recognizedInstructionTags: readonly number[];
}): { readonly legacyData: Uint8Array; readonly tail: GovernanceInstructionTailV1 } {
  if (
    input === null
    || typeof input !== "object"
    ...
    || !(isSdkPublicKey(input.instruction.programId))
    || !Array.isArray(input.instruction.keys)
    || !input.instruction.keys.every((meta) =>
      meta !== null
      && typeof meta === "object"
  ...
  const finalMeta: AccountMeta | undefined = input.instruction.keys.at(-1);
  if (
    matchingGateMetas.length !== 1
    || finalMeta === undefined
  ...
  if (endsWithGovernanceInstructionTailV1(legacyData)) {
    fail(
      "GOVERNED_INSTRUCTION_ALREADY_ENVELOPED",
      "governed instruction contains a nested canonical governance tail",
    );
  }
  return Object.freeze({
    legacyData,
...
export function inspectGovernedInstructionBatchV1(input: {
  ...
  return Object.freeze(input.instructions.map((instruction) =>
    inspectGovernedInstructionEnvelopeV1({
      instruction,
...
interface UngovernedInstructionSnapshotV1 {
  readonly instruction: TransactionInstruction;
  readonly context: GovernanceGateContextV1;
  readonly recognizedInstructionTags: readonly number[];
}
...
function snapshotUngovernedInstructionV1(input: {
  ...
  readonly context: GovernanceGateContextV1;
  readonly recognizedInstructionTags: readonly number[];
}): UngovernedInstructionSnapshotV1 {
  if (
    input === null
    || typeof input !== "object"
    || !(input.instruction instanceof TransactionInstruction)
  ...
  if (
    ...
    fail(
      "GOVERNED_INSTRUCTION_DATA_INVALID",
      "business instruction data plus the governance tail is outside the exact outer bounds",
    );
  }
  const recognizedInstructionTags = validateRecognizedInstructionTagsV1(
    input.recognizedInstructionTags,
  ...
  if (endsWithGovernanceInstructionTailV1(data)) {
    fail(
      "GOVERNED_INSTRUCTION_ALREADY_ENVELOPED",
      "business instruction already carries a canonical governance tail",
    );
  }
  if (input.instruction.keys.some((meta) => meta.pubkey.equals(context.gateAddress))) {
    fail(
      "GOVERNED_INSTRUCTION_GATE_META_INVALID",
      "business instruction already contains the canonical governance gate",
    );
  }
  return Object.freeze({
...
function appendGovernanceEnvelopeV1(
  instruction: TransactionInstruction,
  context: GovernanceGateContextV1,
  recognizedInstructionTags: readonly number[],
): TransactionInstruction {
...
function cloneTransactionInstructionV1(instruction: TransactionInstruction): TransactionInstruction {
...
function cloneAccountMetaV1(meta: AccountMeta): AccountMeta {
  return {
...
function isCanonicalAccountMetaV1(meta: unknown): meta is AccountMeta {
  return meta !== null
    && typeof meta === "object"
    && isSdkPublicKey((meta as AccountMeta).pubkey)
    && typeof (meta as AccountMeta).isSigner === "boolean"
    && typeof (meta as AccountMeta).isWritable === "boolean";
}

function validateRecognizedInstructionTagsV1(
  ...
  if (
    recognized.size !== snapshot.length
...
function endsWithGovernanceInstructionTailV1(data: Uint8Array): boolean {
  if (data.length < GOVERNANCE_TAIL_V1_BYTES) return false;
  try {
    decodeGovernanceInstructionTailV1(data.subarray(-GOVERNANCE_TAIL_V1_BYTES));
    return true;
  } catch (cause) {
    if (cause instanceof GovernanceGateValidationError) return false;
    throw cause;
  }
}

function validateContext(value: GovernanceGateContextV1): GovernanceGateContextV1 {
  try {
    ...
    if (cause instanceof GovernanceGateValidationError) throw cause;
    fail("GOVERNANCE_CONTEXT_INVALID", "governance context mixes identities or contains noncanonical fields");
  }
...
function freezeContext(value: GovernanceGateContextV1): GovernanceGateContextV1 {
  return Object.freeze({
    ...value,
...
function requireRpcMethod<T extends keyof GovernanceGateRpcV1>(
  value: GovernanceGateRpcV1,
  name: T,
): GovernanceGateRpcV1[T] {
...
function statusNameFor(status: number): GovernanceGateStatusNameV1 {
  if (status === GOVERNANCE_GATE_STATUS_V1.Active) return "Active";
  if (status === GOVERNANCE_GATE_STATUS_V1.FrozenForUpgrade) return "FrozenForUpgrade";
  if (status === GOVERNANCE_GATE_STATUS_V1.EmergencyFrozen) return "EmergencyFrozen";
  fail("GOVERNANCE_GATE_STATUS_UNSUPPORTED", "ProtocolGateV1 status is unsupported");
}

function requireCanonicalStatus(gate: ProtocolGovernanceGateV1): void {
  ...
  if (
    (gate.status === GOVERNANCE_GATE_STATUS_V1.Active && !(proposalDefault && clear))
...
function validateGovernanceIdentityV1(value: unknown): GovernanceIdentityV1 {
  if (
    value === CURRENT_GOVERNANCE_GENERATION_3
    || value === CURRENT_GOVERNANCE_GENERATION_1
    || value === REVIEWED_GOVERNANCE_GENERATION_2_CANDIDATE
  ) {
    return value as GovernanceIdentityV1;
  }
  fail(
    "GOVERNANCE_GENERATION_MISMATCH",
    "governance identity is not an exact release-train Generation 1 or reviewed Generation 2 identity",
  );
}

function governanceIdentityForGenerationV1(value: unknown): GovernanceIdentityV1 {
  if (value === 3) return CURRENT_GOVERNANCE_GENERATION_3;
  if (value === 1) return CURRENT_GOVERNANCE_GENERATION_1;
  if (value === 2) return REVIEWED_GOVERNANCE_GENERATION_2_CANDIDATE;
  fail("GOVERNANCE_GENERATION_MISMATCH", "governance context identity generation is unsupported");
}

function canonicalPublicKey(value: PublicKey | string, field: string): PublicKey {
  try {
    const key = canonicalSdkPublicKey(value);
    if (typeof value === "string" && key.toBase58() !== value) throw new Error("noncanonical");
    return key;
  } catch {
...
function publicKeyAt(bytes: Uint8Array, offset: number): PublicKey {
...
function asciiBytes(value: string): Uint8Array {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (code > 0x7f) throw new TypeError("ASCII constants must contain only ASCII bytes");
    bytes[index] = code;
  }
  return bytes;
}
...
function requireNondefault(value: PublicKey, field: string): void {
...
function requireSafeSlot(value: unknown, field: string): number {
