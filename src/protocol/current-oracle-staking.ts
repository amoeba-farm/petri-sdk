...
import { validateCurrentOracleActionRequest, type CurrentOracleActionRequest } from "./current-oracle-public.js";
import { buildQueueStakeAmbaForSambaInstruction, buildActivateQueuedStakeAmbaForSambaInstruction,
  buildRequestUnstakeSambaInstruction, buildCompleteUnstakeSambaInstruction } from "./current-builders.js";

export type CurrentOracleStakingRequest = Extract<CurrentOracleActionRequest, { readonly actionType:
  "queue_stake_amba_for_samba" | "activate_queued_stake_amba_for_samba" | "request_unstake_samba" | "complete_unstake_samba" }>;
type StakingAction = { readonly kind: "queue"; readonly ambaAmount: bigint }
  | { readonly kind: "activate"; readonly minSambaOut: bigint }
  | { readonly kind: "request_unstake"; readonly sambaAmount: bigint; readonly minAmbaOut: bigint }
  | { readonly kind: "complete_unstake" };
type ActorPrerequisite = "PLAYER_LEDGER" | "STAKE_ACTIVATION" | "UNSTAKE_REQUEST" | "SAMBA_TOKEN_ACCOUNT";
const prerequisiteMessages: Readonly<Record<ActorPrerequisite, string>> = {
...
function missingPrerequisite(prerequisite: ActorPrerequisite, address: PublicKey) {
...
function stateUnavailable(message: string) {
...
interface NativeStakingInput {
  readonly action: StakingAction; readonly owner: PublicKey; readonly currentUnixTimestamp: bigint;
  ...
  readonly sambaMintSupply: bigint; readonly ownerSambaBalance: bigint; readonly rewardFunnelBalance: bigint;
}
interface NativeStakingPlan { readonly instructionTag: 138 | 139 | 131 | 133;
  readonly expectedAmbaAmount: bigint; readonly expectedSambaAmount: bigint; readonly earliestExecutionTs: bigint }
interface StakingNativeCapability {
  planCurrentOracleStakingOperationV1(input: NativeStakingInput): NativeStakingPlan;
  decodeOracleMajorTokenConfig(data: Uint8Array): { readonly bump: number; readonly mint: PublicKey; readonly vaultTokenAccount: PublicKey };
}
function capability(): StakingNativeCapability {
  const current = native as unknown as Partial<StakingNativeCapability>;
  ...
  return current as StakingNativeCapability;
}
...
export interface ReadCurrentOracleStakingStateInput {
  readonly connection: Connection; readonly programId: PublicKey;
  readonly request: CurrentOracleStakingRequest; readonly minimumContextSlot?: number;
}
export interface CurrentOracleStakingState {
  readonly request: CurrentOracleStakingRequest; readonly readWindowStartSlot: number; readonly readWindowEndSlot: number;
  readonly currentUnixTimestamp: string; readonly accountFacts: readonly CurrentOracleStakingAccountFact[];
  readonly nativeInput: NativeStakingInput; readonly ambaMint: PublicKey | null; readonly ownerSambaTokenAccount: PublicKey;
}

/** Each optional record is authenticated classic absence; failed reads throw. No global pause predicate. */
export async function readCurrentOracleStakingState(input: ReadCurrentOracleStakingStateInput): Promise<CurrentOracleStakingState> {
  const request = validateCurrentOracleActionRequest(input.request) as CurrentOracleStakingRequest;
  ...
  if (!allowed.includes(request.actionType) || input.programId.toBase58() !== AMOEBA_SPREAD_PROGRAM_ID) throw new Error("Invalid current staking request/program");
  ...
  if (!Number.isSafeInteger(floor) || floor < 0 || start < floor) throw new Error("Finalized staking read floor is unavailable");
  const [genesis, time] = await Promise.all([input.connection.getGenesisHash(), input.connection.getBlockTime(start)]);
  if (genesis !== CURRENT_PROTOCOL_DEVNET_GENESIS_HASH || time === null || !Number.isSafeInteger(time) || time < 0) throw new Error("Finalized Devnet staking clock unavailable");
  const facts: CurrentOracleStakingAccountFact[] = [];
  const read = async (address: PublicKey): Promise<AccountInfo<Buffer> | null> => {
    ...
    if (!Number.isSafeInteger(response.context.slot) || response.context.slot < start || response.context.slot < floor) throw new Error("Staking account read precedes finalized floor");
  ...
  const programData = async (address: PublicKey, optional = false, prerequisite?: ActorPrerequisite): Promise<Buffer | null> => {
    ...
    if (absent && optional) return null;
    if (absent && prerequisite) throw missingPrerequisite(prerequisite, address);
    if (!info || info.executable || !info.owner.equals(input.programId)) throw stateUnavailable(`Canonical staking state unavailable: ${address.toBase58()}`);
    return Buffer.from(info.data);
  };
  const pda = (address: PublicKey, bump: number, seed: Uint8Array, extra: readonly Buffer[] = []) => {
    ...
    if (!expected.equals(address) || expectedBump !== bump) throw new Error("Staking PDA/bump mismatch");
  };
  const vaultAddress = native.deriveVaultConfigPda(input.programId);
  const vaultInfo = await read(vaultAddress);
  if (vaultInfo === null) throw stateUnavailable("Canonical VaultConfig is absent");
  ...
  if (!pool.majorTokenConfig.equals(configAddress) || !pool.sambaMint.equals(sambaMint)
      || !pool.sambaVoteVault.equals(native.deriveOracleSambaVoteVaultPda(input.programId))) throw new Error("Staking pool custody identity mismatch");
  const action: StakingAction = request.actionType === "queue_stake_amba_for_samba" ? { kind: "queue", ambaAmount: BigInt(request.ambaAmountAtomic) }
  ...
  let ambaMint: PublicKey | null = null;
  if (action.kind !== "complete_unstake") {
    ...
    if (config.vaultTokenAccount.equals(PublicKey.default) || config.mint.equals(PublicKey.default)
      || config.mint.equals(vault.usdcMint) || config.vaultTokenAccount.equals(vault.vaultTokenAccount)) throw new Error("AMBA staking and USDC collateral custody must be distinct");
  }
  let playerLedger: NativeStakingInput["playerLedger"] = null;
  let stakeActivation: NativeStakingInput["stakeActivation"] = null;
  let unstakeRequest: NativeStakingInput["unstakeRequest"] = null;
  if (action.kind === "queue" || action.kind === "complete_unstake") {
    ...
    if (bytes !== null) { playerLedger = native.decodeOraclePlayerLedgerBalance(bytes); pda(address, playerLedger.bump, native.ORACLE_PLAYER_LEDGER_PDA_SEED, [owner.toBuffer()]); }
  }
  if (action.kind === "queue" || action.kind === "activate") {
    ...
    if (bytes !== null) { stakeActivation = native.decodeOracleStakeActivation(bytes); pda(address, stakeActivation.bump, native.ORACLE_STAKE_ACTIVATION_PDA_SEED, [owner.toBuffer()]); }
  }
  if (action.kind === "request_unstake" || action.kind === "complete_unstake") {
    ...
    if (bytes !== null) { unstakeRequest = native.decodeOracleUnstakeRequest(bytes); pda(address, unstakeRequest.bump, native.ORACLE_UNSTAKE_REQUEST_PDA_SEED, [owner.toBuffer()]); }
  }
  const ownerSambaTokenAccount = getAssociatedTokenAddressSync(sambaMint, owner, false, native.SPL_TOKEN_PROGRAM_ID);
  let sambaMintSupply = pool.sambaSupply; let ownerSambaBalance = 0n; let rewardFunnelBalance = 0n;
  const token = async (address: PublicKey, mint: PublicKey, tokenOwner: PublicKey, allowAbsent = false, prerequisite?: ActorPrerequisite) => {
    const info = await read(address);
    const absent = info === null || (!info.executable && info.owner.equals(SystemProgram.programId) && info.data.length === 0);
    if (absent && allowAbsent) return { amount: 0n };
    if (absent && prerequisite) throw missingPrerequisite(prerequisite, address);
    if (!info || info.executable || !info.owner.equals(native.SPL_TOKEN_PROGRAM_ID) || info.data.length !== 165) throw stateUnavailable(`Canonical SPL staking account absent/invalid: ${address}`);
    ...
    if (!decoded.mint.equals(mint) || !decoded.owner.equals(tokenOwner) || !decoded.isInitialized || decoded.isFrozen
      || decoded.isNative) throw new Error("Staking token identity/state mismatch");
    return decoded;
  };
  if (action.kind === "activate" || action.kind === "request_unstake") {
    const info = await read(sambaMint);
    if (!info || info.executable || !info.owner.equals(native.SPL_TOKEN_PROGRAM_ID) || info.data.length !== 82) throw stateUnavailable("Canonical sAMBA mint unavailable");
    const mint = unpackMint(sambaMint, info, native.SPL_TOKEN_PROGRAM_ID);
    if (!mint.isInitialized || !mint.mintAuthority?.equals(vaultAddress) || mint.freezeAuthority !== null) throw new Error("Canonical sAMBA mint authority mismatch");
  ...
  if (action.kind === "activate") {
    ...
    if (!funnel.majorTokenConfig.equals(configAddress) || !funnel.ambaMint.equals(ambaMint!) || !funnel.funnelTokenAccount.equals(expected)) throw new Error("Staking reward funnel identity mismatch");
    rewardFunnelBalance = (await token(expected, ambaMint!, funnelAddress)).amount;
  }
  const end = await input.connection.getSlot("finalized");
  if (end < start || facts.some(fact => fact.observedSlot > end)) throw new Error("Staking finalized read window inconsistent");
...
export async function prepareCurrentOracleStakingAction(input: ReadCurrentOracleStakingStateInput) {
