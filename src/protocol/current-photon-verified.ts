...
import { accountCompressionProgram, createBN254, TreeType, type AddressWithTree, type HashWithTree,
  type CompressedAccountWithMerkleContext, type ValidityProofWithContext, type WithContext } from "@lightprotocol/stateless.js";
import { decodeCompressedAmebaStateLeaf, deriveCompressedStateAddressV1, type CompressedStateDomain } from "@amoeba/spread-release-tools/compressed-state";
import { AMOEBA_SPREAD_PROGRAM_ID } from "./identity.js";
import { CURRENT_PROTOCOL_DEVNET_GENESIS_HASH } from "./current.js";
import { CURRENT_COMPRESSED_EVIDENCE_SCHEMA, requireCurrentCompressedEvidenceVerifier,
  type CurrentCompressedEvidenceRequest, type CurrentCompressedEvidenceResult, type CurrentCompressedEvidenceStatement,
  type CurrentCompressedEvidenceVerifier, validateCurrentCompressedEvidenceRequest } from "./current-compressed-evidence-verifier.js";
import type { CurrentPhotonTopology } from "./current-photon.js";

export interface CurrentPhotonRawCompressedAccount {
  readonly owner: string; readonly address: string | null; readonly lamports: string;
  readonly data: { readonly discriminatorHex: string; readonly dataBase64: string; readonly dataHashHex: string } | null;
  readonly tree: string; readonly queue: string; readonly leafIndex: string; readonly proveByIndex: boolean; readonly hashHex: string;
}
...
export interface CurrentPhotonVerifiedCompressedStateEvidence {
  readonly exists: boolean; readonly canonicalPda: string; readonly domain: number; readonly compressedAddress: string;
  readonly providerOriginSha256: string;
  readonly compressedAccount: CurrentPhotonRawCompressedAccount | null;
  readonly leaf: { readonly schemaVersion: number; readonly canonicalPda: string; readonly domain: number; readonly revision: string; readonly dataBase64: string } | null;
  readonly proofBase64: string | null; readonly proofStatementJson: string;
  readonly verificationEvidence: CurrentPhotonNativeVerificationEvidence;
}
const digest = (value: Uint8Array | string) => createHash("sha256").update(value).digest("hex");
const keyHex = (value: PublicKey) => value.toBuffer().toString("hex");
function invalid(message: string): never { throw new Error(`CURRENT_PHOTON_VERIFIED_EVIDENCE_INVALID: ${message}`); }
function fieldHex(value: { isNeg(): boolean; byteLength(): number; toArrayLike(type: typeof Buffer, endian: "be", length: number): Buffer }): string {
  if (value.isNeg() || value.byteLength() > 32) invalid("field value exceeds32bytes");
  return value.toArrayLike(Buffer, "be", 32).toString("hex");
}
function proofHex(value: readonly number[], length: number): string {
  if (value.length !== length || value.some(byte => !Number.isInteger(byte) || byte < 0 || byte > 255)) invalid("proof limb has invalid bytes");
...
export async function readCurrentVerifiedCompressedStateEvidence(input: {
  readonly stateConnection: Connection; readonly providerOriginSha256: string; readonly topology: CurrentPhotonTopology;
  ...
  if (!Number.isSafeInteger(start) || !Number.isSafeInteger(requestedFloor) || requestedFloor < 0 || start < requestedFloor
    || await input.stateConnection.getGenesisHash() !== CURRENT_PROTOCOL_DEVNET_GENESIS_HASH) invalid("finalized Devnet read floor unavailable");
  ...
  let raw: CurrentPhotonRawCompressedAccount | null = null;
  let leaf: CurrentPhotonVerifiedCompressedStateEvidence["leaf"] = null;
  let statement: CurrentCompressedEvidenceStatement;
  let proofBytes: string | null = null;
  let proofStatementJson: string;
  let proofSlot = start;
  if (account !== null) {
    ...
    if (!topology || topology.queue !== account.treeInfo.queue.toBase58() || topology.cpiContext !== account.treeInfo.cpiContext?.toBase58()
      ...
      || typeof account.proveByIndex !== "boolean") invalid("compressed account identity/topology is invalid");
    const decoded = decodeCompressedAmebaStateLeaf(account);
    if (decoded.domain !== input.domain || !decoded.canonicalPda.equals(input.canonicalPda)) invalid("compact domain/PDA mismatch");
    const encodedData = Buffer.from(account.data.data);
    const dataHash = createHash("sha256").update(encodedData).digest(); dataHash[0] = 0;
    if (proofHex(account.data.discriminator, 8) !== digest("CompressedAmebaStateLeaf").slice(0, 16)
      || proofHex(account.data.dataHash, 32) !== dataHash.toString("hex")) invalid("compact discriminator/data hash mismatch");
  ...
  } else statement = { kind: "address_absent", treeKeyHex: keyHex(addressTree), valueHex: keyHex(compressedAddress) };
  if (statement.kind === "state_queue") {
    ...
    if (!Number.isSafeInteger(response.context.slot) || response.context.slot < 0 || proof.compressedProof === null
      || [proof.roots, proof.rootIndices, proof.leaves, proof.leafIndices, proof.proveByIndices, proof.treeInfos].some(values => values.length !== 1)) invalid("single-statement proof response invalid");
    ...
    if (keyHex(tree.tree) !== statement.treeKeyHex || (account !== null ? keyHex(tree.queue) !== statement.queueKeyHex : !tree.queue.equals(addressQueue))
      ...
      || (account === null ? !proof.leaves[0]!.isZero() : fieldHex(proof.leaves[0]!) !== statement.valueHex || proof.leafIndices[0] !== account.leafIndex || proof.proveByIndices[0] !== false)) invalid("proof statement differs from requested account/address");
  ...
  if (!Number.isSafeInteger(snapshot.context.slot) || snapshot.context.slot < floor || snapshot.value.length !== addresses.length) invalid("finalized root/queue snapshot slot invalid");
  const accounts = snapshot.value.map((info, index) => {
    if (!info || info.executable || info.owner.toBase58() !== accountCompressionProgram || info.data.length < 8 || info.data.length > 16 * 1024 * 1024) invalid("root/queue snapshot owner or size invalid");
  ...
  const request: CurrentCompressedEvidenceRequest = Object.freeze({ schema: CURRENT_COMPRESSED_EVIDENCE_SCHEMA,
