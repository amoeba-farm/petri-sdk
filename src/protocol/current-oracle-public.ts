...
type CurrentOracleRequestUpdateEmergencyChoice =
  | "accept_claim"
  | "accept_alternative_state"
  | "reject_update";
type CurrentOracleRequestOpeningEmergencyChoice =
  | "keep_opening"
...
export type CurrentOracleActionRequest =
  | (CurrentOracleRequestCommon & { readonly actionType: "queue_stake_amba_for_samba"; readonly ambaAmountAtomic: string })
  | (CurrentOracleRequestCommon & { readonly actionType: "activate_queued_stake_amba_for_samba"; readonly minSambaOutAtomic: string })
  | (CurrentOracleRequestCommon & { readonly actionType: "request_unstake_samba"; readonly sambaAmountAtomic: string; readonly minAmbaOutAtomic: string })
  | (CurrentOracleRequestCommon & { readonly actionType: "complete_unstake_samba" })
...
type RedactCurrentOracleRequestSecret<T> = T extends { readonly secretSaltHex: string }
...
export const CURRENT_ORACLE_SOURCE_TYPE_HASH_DOMAIN = "ameba-oracle-source-type-v1" as const;
export const CURRENT_ORACLE_SOURCE_DEFINITION_HASH_DOMAIN =
  "ameba-oracle-source-definition-v1" as const;
export const CURRENT_ORACLE_SOURCE_TYPE_MAX_BYTES = 128 as const;
export const CURRENT_ORACLE_SOURCE_DEFINITION_MAX_BYTES = 4_096 as const;
...
export const CURRENT_ORACLE_LOCATOR_MAX_BYTES = 2_048 as const;
...
] as const satisfies readonly CurrentOracleActionRequest["actionType"][]);
...
export type CurrentOracleAction<T extends CurrentOracleActionType> = Extract<
  CurrentOracleActionRequest,
  { readonly actionType: T }
>;
export type CurrentOracleEmergencyDisputeKind = CurrentOracleAction<
  "commit_oracle_emergency_vote_v3"
>["disputeKind"];
export type CurrentOracleSourceEmergencyChoice = Extract<
  CurrentOracleAction<"commit_oracle_emergency_vote_v3">,
  { readonly disputeKind: "source" }
>["choice"];
export type CurrentOracleUpdateEmergencyChoice = Extract<
  CurrentOracleAction<"commit_oracle_emergency_vote_v3">,
  { readonly disputeKind: "update" }
>["choice"];
export type CurrentOracleOpeningEmergencyChoice = Extract<
  CurrentOracleAction<"commit_oracle_emergency_vote_v3">,
  { readonly disputeKind: "opening" }
>["choice"];
export type CurrentOracleEmergencyChoice =
  | CurrentOracleSourceEmergencyChoice
  | CurrentOracleUpdateEmergencyChoice
  | CurrentOracleOpeningEmergencyChoice;
export type CurrentOracleUpdateFinalizationOutcome = CurrentOracleAction<
  "finalize_oracle_update_claim_v2"
>["outcome"];
export type CurrentOracleRewardKind = CurrentOracleAction<
  "claim_oracle_usdc_reward"
>["rewardKind"];

type CurrentOracleJsonRecord = Record<string, unknown>;
const CURRENT_ORACLE_U64_MAX = (1n << 64n) - 1n;
const CURRENT_ORACLE_COMMON_KEYS = ["marketId", "expiryId", "ownerPubkey", "actionType"] as const;
...
function oracleRecord(value: unknown, label: string): CurrentOracleJsonRecord {
  ...
  return value as CurrentOracleJsonRecord;
}

function oracleExactKeys(
  value: CurrentOracleJsonRecord,
  label: string,
  ...
  if (unexpected.length > 0) throw new Error(`${label} exact-key mismatch: extra=[${unexpected.join(",")}].`);
...
function oracleString(value: CurrentOracleJsonRecord, key: string): string {
  if (typeof value[key] !== "string") throw new Error(`${key} must be a string.`);
  return value[key];
}

function oracleProduct(value: unknown): "ramx" | "nandx" {
...
function oracleSeries(value: unknown, product: "ramx" | "nandx"): string {
...
function oracleDecodedBase58Length(value: string): number {
  let numeric = 0n;
...
function oraclePubkey(value: unknown, label: string): string {
  if (
    typeof value !== "string"
    || value.length < 32
    || value.length > 44
...
function oracleU64(value: unknown, label: string, positive = false): string {
...
function oracleU8(value: unknown, label: string): number {
...
function oracleLabel(value: unknown, label: string): string {
  if (typeof value !== "string") throw new Error(`${label} must be a canonical current bytes32 label.`);
...
function oracleHex32(value: unknown, label: string, nonzero = false): string {
  if (typeof value !== "string" || !/^[0-9a-f]{64}$/.test(value)) {
    throw new Error(`${label} must be lowercase bytes32 hex (exactly 32 bytes of lowercase hexadecimal).`);
  }
  if (nonzero && /^0{64}$/.test(value)) throw new Error(`${label} must be nonzero lowercase bytes32 hex.`);
  return value;
}

export function normalizeCurrentOracleSemanticText(
  value: unknown,
  label: string,
  maxBytes: number,
): string {
  ...
  if (normalized.length === 0) throw new Error(`${label} must be nonempty after canonical normalization.`);
  if (bytes.length > maxBytes) throw new Error(`${label} must contain at most ${maxBytes} UTF-8 bytes.`);
...
function oracleArchiveUrl(value: unknown, sourceTime: string): string {
  ...
  if (match === null) throw new Error("archiveUrl must contain an exact 14-digit capture timestamp and archived target.");
...
function oracleCommon(value: CurrentOracleJsonRecord): {
...
function oracleActionKeys(required: readonly string[], optional: readonly string[] = []): {
...
export function validateCurrentOracleActionRequest(request: unknown): CurrentOracleActionRequest {
  ...
  ].includes(String(request.actionType))) throw Object.assign(new Error("Token staking and token voting were retired; use council ballots"), { code: "G3_OPERATION_RETIRED" });
  ...
  const keys = (required: readonly string[], optional: readonly string[] = []): void => {
  ...
  switch (actionType) {
    ...
    case "initialize_oracle_month_v5": {
      ...
      if (BigInt(listingTs) <= BigInt(scrambleStartTs)) throw new Error("listingTs must be later than scrambleStartTs.");
    ...
    case "propose_oracle_source_v3": {
      ...
      if (skuId !== bucketId) throw new Error("propose_oracle_source_v3 requires skuId to exactly equal bucketId.");
    ...
    case "challenge_oracle_source_v2": {
      ...
      if ((reasonCode === 8) !== hasComparison) throw new Error("comparisonSourceId is required exactly when reasonCode is 8.");
    ...
    case "reveal_oracle_update_claim_v3": {
      ...
      const sourceTimeUnix = oracleU64(value.sourceTimeUnix, "sourceTimeUnix", true);
      if (priorStateAtomic === newStateAtomic) throw new Error(`${actionType} requires newStateAtomic to differ from priorStateAtomic.`);
      ...
      return Object.freeze(isCommit ? { ...normalized, actionType: "commit_oracle_update_claim_v3" as const, stakeAtomic: oracleU64(value.stakeAtomic, "stakeAtomic", true) } : { ...normalized, actionType: "reveal_oracle_update_claim_v3" as const });
    }
    ...
    case "finalize_oracle_update_claim_v2": {
      ...
      if (outcome !== "accept_claim" && outcome !== "reject_claim" && outcome !== "rule_review_unresolved") throw new Error("outcome must be accept_claim, reject_claim, or rule_review_unresolved.");
      const hasChallenge = Object.prototype.hasOwnProperty.call(value, "challengeId");
      if (outcome !== "accept_claim" && !hasChallenge) throw new Error("challengeId is required for reject_claim and rule_review_unresolved.");
    ...
    case "reveal_oracle_emergency_vote_v2": {
      ...
      if (disputeKind !== "source" && disputeKind !== "update" && disputeKind !== "opening") throw new Error("disputeKind must be exactly 'source', 'update', or 'opening'.");
      const choice = oracleString(value, "choice") as CurrentOracleEmergencyChoice;
      ...
      return Object.freeze(isCommit ? { ...normalized, actionType: "commit_oracle_emergency_vote_v3" as const, sambaAmountAtomic: oracleU64(value.sambaAmountAtomic, "sambaAmountAtomic", true) } : { ...normalized, actionType: "reveal_oracle_emergency_vote_v2" as const }) as CurrentOracleActionRequest;
    }
    ...
    case "claim_oracle_usdc_reward": {
      ...
      if (rewardKind !== "proposer" && rewardKind !== "support" && rewardKind !== "opening" && rewardKind !== "update") throw new Error("rewardKind must be proposer, support, opening, or update.");
...
export function redactCurrentOracleActionRequest(request: unknown): CurrentOracleRedactedRequest {
  ...
  if ("secretSaltHex" in normalized) {
    ...
    return Object.freeze(redacted) as CurrentOracleRedactedRequest;
  }
  return normalized as CurrentOracleRedactedRequest;
}

export function currentOracleEmergencyChoiceIndex(
  kind: CurrentOracleEmergencyDisputeKind,
  choice: CurrentOracleEmergencyChoice,
): 0 | 1 | 2 {
  switch (kind) {
    case "source":
      if (choice === "keep_source") return 0;
      if (choice === "reject_source") return 1;
      if (choice === "merge_source") return 2;
      break;
    case "update":
      if (choice === "accept_claim") return 0;
      if (choice === "accept_alternative_state") return 1;
      if (choice === "reject_update") return 2;
      break;
    case "opening":
      if (choice === "keep_opening") return 0;
      if (choice === "accept_alternative_opening") return 1;
      if (choice === "source_inactive_for_month") return 2;
      break;
    default:
