...
import { deriveOracleSourcePda, deriveOracleSupportPositionPda, deriveOracleOpeningClaimPda, deriveOracleMonthPda, deriveVaultConfigPda,
  deriveOracleUpdateClaimV2Pda, deriveOracleUsdcRewardSchedulePda, deriveOracleUsdcRewardReceiptPda } from "@amoeba/spread-release-tools/oracle-dlmm";
import { validateCurrentOracleActionRequest } from "./current-oracle-public.js";
import { CurrentOraclePlannerError, readCurrentOracleRewardEntitlementInputs, readUpdateClaimByIdentity,
  readClassicSourceReward, type CurrentOraclePlannerContext, type CurrentOracleRewardRequest } from "./current-oracle-planner.js";
...
export interface CurrentOracleClassicEvidenceAccount {
  readonly address: string; readonly owner: string | null; readonly executable: boolean | null;
  readonly dataBase64: string | null; readonly dataSha256: string | null; readonly observedSlot: number;
}
...
export type CurrentOracleRewardCompressedStateEvidence = Omit<CurrentPhotonVerifiedCompressedStateEvidence, "verificationEvidence"> & {
  readonly verificationEvidence: CurrentPhotonNativeVerificationEvidence | (
    Omit<CurrentPhotonNativeVerificationEvidence, "request"> & {
...
function retainNativeEvidence(evidence: CurrentPhotonVerifiedCompressedStateEvidence, includeSnapshots: boolean): CurrentOracleRewardCompressedStateEvidence {
  if (includeSnapshots) return evidence;
  ...
  return Object.freeze({ ...evidence, verificationEvidence: Object.freeze({ ...evidence.verificationEvidence,
    request: Object.freeze(request), nativeAccountSnapshotsIncluded: false as const }) });
}
function nativeSnapshotPreference(value: boolean | undefined): boolean {
  if (value !== undefined && typeof value !== "boolean") throw new Error("includeNativeAccountSnapshots must be boolean");
  return value !== false;
}
export interface CurrentOracleCompactEvidenceAccount {
  readonly canonicalPda: string; readonly domain: number; readonly dataBase64: string;
  readonly evidence: CurrentOracleRewardCompressedStateEvidence;
}
export interface CurrentOracleRewardReceiptEvidence {
  readonly address: string; readonly subject: string; readonly kind: CurrentOracleRewardRequest["rewardKind"];
  readonly exists: boolean; readonly amountAtomic: string | null; readonly claimedSlot: string | null;
...
export interface CurrentOracleOwnerRewardEvidence {
  ...
  readonly readWindowStartSlot: number; readonly readWindowEndSlot: number;
...
const compactFact = (evidence: CurrentOracleRewardCompressedStateEvidence): CurrentOracleCompactEvidenceAccount => {
  if (!evidence.exists || evidence.leaf === null || evidence.compressedAccount === null) throw new Error("Verified membership payload missing");
...
function captureReads(connection: Connection, floor: number, accounts: CurrentOracleClassicEvidenceAccount[]): Connection {
  ...
  return new Proxy({} as Connection, { get(_target, key) {
    ...
    if (key === "getAccountInfo") return async (address: PublicKey) => {
      ...
      if (!Number.isSafeInteger(response.context.slot) || response.context.slot < floor) throw new Error("Reward account precedes finalized read floor");
      ...
      if (previous && (previous.dataSha256 !== fact.dataSha256 || previous.owner !== fact.owner || previous.executable !== fact.executable)) throw new Error("Reward account changed within read window");
      if (!previous) accounts.push(fact);
      return info;
    };
...
export async function readCurrentOracleOwnerRewardEvidence(input: CurrentOraclePlannerContext & {
  readonly request: CurrentOracleRewardRequest; readonly minimumContextSlot?: number;
  ...
  if (request.actionType !== "claim_oracle_usdc_reward" || input.commitment !== "finalized"
    || input.programId.toBase58() !== AMOEBA_SPREAD_PROGRAM_ID) throw new Error("Exact current finalized reward request required");
  ...
  if (!Number.isSafeInteger(floor) || floor < 0 || start < floor
    || await input.connection.getGenesisHash() !== CURRENT_PROTOCOL_DEVNET_GENESIS_HASH) throw new Error("Finalized Devnet reward read floor unavailable");
  const classicAccounts: CurrentOracleClassicEvidenceAccount[] = [];
  const verified = new Map<string, Promise<CurrentOracleRewardCompressedStateEvidence>>();
  const evidenceRows: CurrentOracleRewardCompressedStateEvidence[] = [];
  const observe = (canonicalPda: PublicKey, domain: CompressedStateDomain) => {
    const key = `${canonicalPda.toBase58()}:${domain}`;
    let pending = verified.get(key);
    if (pending === undefined) {
      const photon = input.photonConnection;
      if (!photon || typeof photon.observeCurrentVerifiedCompressedStateEvidence !== "function") throw new Error("Finalized native compressed verification capability unavailable");
      pending = photon.observeCurrentVerifiedCompressedStateEvidence({ canonicalPda, domain, minimumContextSlot: Math.max(start, floor) }).then(evidence => {
        if (evidence.canonicalPda !== canonicalPda.toBase58() || evidence.domain !== domain) throw new Error("Verified compact business identity mismatch");
        const retained = retainNativeEvidence(evidence, includeSnapshots);
        evidenceRows.push(retained); return retained;
      });
  ...
  const context = { ...input, connection: captureReads(input.connection, Math.max(start, floor), classicAccounts),
    compressedEvidenceReader: async ({ canonicalPda, domain }: { readonly canonicalPda: PublicKey; readonly domain: CompressedStateDomain }) => {
      const evidence = await observe(canonicalPda, domain);
      if (!evidence.exists) return null;
      if (!evidence.leaf) throw new Error("Verified compact leaf unavailable");
  ...
  let receipt: CurrentOracleRewardReceiptEvidence | null = null;
  let status: CurrentOracleOwnerRewardEvidence["status"] = "unavailable";
  let amount: string | null = null; let errorCode: string | undefined;
  let proofFacts: Readonly<Record<string, string | number | boolean | null>> = {};
  try {
    const accountInput = async (address: PublicKey) => {
      const info = await context.connection.getAccountInfo(address, "finalized");
      if (info === null) throw new Error(`Reward anchor absent: ${address}`);
      return { address, data: info.data, owner: info.owner, executable: info.executable,
        namespace: "ameba-spread-v2" as const, programId: input.programId };
    };
    ...
    if (context.market.seriesIdentity.product !== request.marketId || context.market.seriesIdentity.fullSeriesId !== request.expiryId
      ...
      || !deriveVaultConfigPda(input.programId).equals(input.vaultConfigAddress)) throw new Error("Reward request anchor identity mismatch");
    ...
    if (evidence.exists) {
      ...
      if (bytes.length !== 81 || bytes[0] !== kind || !new PublicKey(bytes.subarray(1, 33)).equals(subject)
        || !new PublicKey(bytes.subarray(33, 65)).equals(owner) || bytes.readBigUInt64LE(65) === 0n) throw new Error("Compact reward receipt identity/layout mismatch");
  ...
  if (end < start || classicAccounts.some(value => value.observedSlot > end)
    ...
    status = "unavailable"; errorCode = "CURRENT_ORACLE_EVIDENCE_SLOT_MISMATCH"; amount = null;
  }
  return Object.freeze({ evidenceTrust: "finalized_native_light_verifier" as const, finalizedStateVerified: status !== "unavailable" && receipt !== null,
...
export async function readCurrentOracleOwnerRewardInventory(input: CurrentOraclePlannerContext & {
  readonly ownerPubkey: string; readonly marketId: "ramx" | "nandx"; readonly expiryId: string;
  readonly sourceIds: readonly string[]; readonly minimumContextSlot?: number;
  readonly cursor?: number; readonly limit?: number; readonly includeNativeAccountSnapshots?: boolean;
}) {
  const includeNativeAccountSnapshots = nativeSnapshotPreference(input.includeNativeAccountSnapshots);
  if (input.commitment !== "finalized" || input.sourceIds.length > 256) throw new Error("Bounded finalized source inventory required");
  const cursor = input.cursor ?? 0; const limit = input.limit ?? 16;
  if (!Number.isSafeInteger(cursor) || cursor < 0 || !Number.isSafeInteger(limit) || limit < 1 || limit > 64) throw new Error("Reward inventory cursor/limit invalid");
  const owner = new PublicKey(input.ownerPubkey);
  const start = await input.connection.getSlot("finalized");
  const floor = input.minimumContextSlot ?? start;
  if (!Number.isSafeInteger(floor) || floor < 0 || start < floor) throw new Error("Invalid reward inventory read floor");
  const sources = new Set(input.sourceIds.map(sourceId => {
    if (!/^[0-9a-f]{64}$/.test(sourceId) || /^0+$/.test(sourceId)) throw new Error("Invalid source inventory identity");
    return sourceId;
  }));
  const classicAccounts: CurrentOracleClassicEvidenceAccount[] = [];
  const context = { ...input, connection: captureReads(input.connection, Math.max(start, floor), classicAccounts) };
  ...
  if (updates.value.length > 4096 || retained.value.length > 4096 || updates.context.slot < floor || retained.context.slot < floor) throw new Error("Reward inventory bound or finalized slot invalid");
  ...
  const discoveryContext = { ...context, connection: new Proxy({} as Connection, { get(_target, key) {
    const target = context.connection;
    if (key === "getAccountInfo") return async (address: PublicKey) => {
      const row = discovered.get(address.toBase58());
      if (!row) return target.getAccountInfo(address, "finalized");
      if (!classicAccounts.some(fact => fact.address === address.toBase58())) classicAccounts.push(Object.freeze({ address: address.toBase58(),
  ...
  const updateCandidates: CurrentOracleRewardRequest[] = [];
  for (const row of updates.value) {
    const bytes = Buffer.from(row.account.data);
    const sourceId = bytes.subarray(98, 130); const claimId = bytes.subarray(34, 66);
    const source = deriveOracleSourcePda({ oracleMonthPda: input.oracleMonthAddress, sourceId, programId: input.programId });
    const decoded = await readUpdateClaimByIdentity(discoveryContext, source, sourceId, owner, claimId);
    if (!decoded.address.equals(row.pubkey)) throw new Error("Update inventory PDA mismatch");
    if (decoded.status === "Finalized") updateCandidates.push({ actionType: "claim_oracle_usdc_reward", marketId: input.marketId,
  ...
  for (const row of retained.value) {
    ...
    if (!decoded.address.equals(row.pubkey)) throw new Error("Retained merge inventory PDA mismatch");
    sources.add(decoded.sourceId.toString("hex"));
  }
  if (sources.size > 4096) throw new Error("Expanded reward source inventory exceeds bound");
  const candidates: CurrentOracleRewardRequest[] = [...sources].sort().flatMap(sourceId => (["proposer", "support", "opening"] as const).map(rewardKind => ({
    actionType: "claim_oracle_usdc_reward" as const, marketId: input.marketId, expiryId: input.expiryId,
    ownerPubkey: owner.toBase58(), rewardKind, sourceId,
  })));
  candidates.push(...updateCandidates);
  const key = (request: CurrentOracleRewardRequest) => `${request.rewardKind}:${request.sourceId}:${request.rewardKind === "update" ? request.claimId : ""}`;
  ...
  if (cursor > ordered.length) throw new Error("Reward inventory cursor exceeds candidate count");
  const page = ordered.slice(cursor, cursor + limit);
  const evidence: CurrentOracleOwnerRewardEvidence[] = [];
  for (let index = 0; index < page.length; index += 4) evidence.push(...await Promise.all(page.slice(index, index + 4).map(request =>
    readCurrentOracleOwnerRewardEvidence({ ...input, request, minimumContextSlot: floor, includeNativeAccountSnapshots }))));
  return Object.freeze({ ownerPubkey: owner.toBase58(), candidateCount: ordered.length, cursor, limit,
    ...
    classicAccounts: Object.freeze(classicAccounts), coverage: Object.freeze({ sourceInventory: "caller_supplied" as const,
      ...
      globalInventoryComplete: false as const }) });
}

export interface ReadCurrentOracleOwnerBountyInventoryInput {
  readonly connection: Connection; readonly programId: PublicKey; readonly namespace: "ameba-spread-v2";
  readonly photonConnection: CurrentPhotonConnection;
  readonly marketId: "ramx" | "nandx"; readonly expiryId: string; readonly ownerPubkey: string;
  readonly sourceIds: readonly string[]; readonly minimumContextSlot?: number; readonly cursor?: number; readonly limit?: number;
...
export async function readCurrentOracleOwnerBountyInventory(input: ReadCurrentOracleOwnerBountyInventoryInput) {
  const includeNativeAccountSnapshots = nativeSnapshotPreference(input.includeNativeAccountSnapshots);
  if (input.namespace !== "ameba-spread-v2" || input.programId.toBase58() !== AMOEBA_SPREAD_PROGRAM_ID) throw new Error("Current bounty namespace/program required");
  const start = await input.connection.getSlot("finalized");
  const floor = input.minimumContextSlot ?? start;
  if (!Number.isSafeInteger(start) || !Number.isSafeInteger(floor) || floor < 0 || start < floor) throw new Error("Current bounty finalized floor invalid");
  const anchors: CurrentOracleClassicEvidenceAccount[] = [];
  ...
  if (!marketRead.found || marketRead.market === null) throw new Error("Exact current bounty market is absent");
  const market = marketRead.market; const marketAddress = marketRead.address;
  ...
  const account = async (address: PublicKey) => {
    ...
    if (!info) throw new Error("Current bounty anchor is absent");
