...
export const HISTORICAL_GENERATION_1_DEPLOYMENT = deepFreeze({
  ...
  deploymentReceiptSignature:
    "4YAeu29vSPfm5Gk7xvN5w4u1X7z3qT4tDUxL9Py16xB4Fp3QpN3DsZbdGQeWNLydZxXJn2ejp4Q1qPDBbdf956Lh",
  writeCompatibility: "unavailable-governance-gate-v1",
} as const);

export const CURRENT_LIVE_DEPLOYMENT = deepFreeze({ ...v3.deployment, cluster: "devnet", stateNamespace: "ameba-spread-v2", genesisHash: "EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG" } as const);

export const CURRENT_GOVERNANCE_GENERATION_3 = deepFreeze({ ...v3.governance, identityGeneration: 3 } as const);
...
export const SDK_RELEASE_TRAIN = deepFreeze({
  ...
  writeRelease: {
    ...
    status: SDK_NATIVE_PACKAGE_MATCHES_RETAINED_RELEASE ? "available" as const : "unavailable" as const,
...
export type ReviewedGovernanceGeneration2Identity =
  typeof REVIEWED_GOVERNANCE_GENERATION_2_CANDIDATE;
export type GovernanceIdentityV1 =
  | CurrentGovernanceIdentity
  | typeof CURRENT_GOVERNANCE_GENERATION_1
  | ReviewedGovernanceGeneration2Identity;
...
export class CurrentWriteReleaseUnavailableError extends AmebaProtocolError {
  constructor() {
    super(
      "current program mutation is unavailable until an exact governed-write release is finalized",
      {
...
export function validateSdkReleaseTrain(value: unknown): SdkReleaseTrain {
  let received: string;
  try {
    received = canonicalJson(value);
  } catch (cause) {
    throw new SdkReleaseTrainValidationError(
      `release-train value is not canonical JSON: ${cause instanceof Error ? cause.message : String(cause)}`,
    );
  }
  if (received !== EXPECTED_RELEASE_TRAIN_CANONICAL_JSON) {
    throw new SdkReleaseTrainValidationError(
      "release-train value differs from the exact reviewed reader, deployment, governance, or authorization identities",
    );
  }
  return SDK_RELEASE_TRAIN;
}

export function isCurrentWriteReleaseAvailable(): boolean {
  try { currentGovernedWriteReleaseV1(); return true; }
  catch { return false; }
}

/** Readiness is qualified package evidence, never a caller switch. */
export function assertCurrentWriteReleaseAvailable(): void {
  if (!isCurrentWriteReleaseAvailable()) throw new CurrentWriteReleaseUnavailableError();
}
...
function canonicalJson(value: unknown, seen = new Set<object>()): string {
  ...
  if (seen.has(object)) throw new TypeError("cycles are not allowed");
  seen.add(object);
  try {
    ...
    return `{${(ownKeys as string[]).sort().map((key) =>
      `${JSON.stringify(key)}:${canonicalJson(descriptors[key]!.value, seen)}`
    ).join(",")}}`;
  } finally {
    seen.delete(object);
  }
