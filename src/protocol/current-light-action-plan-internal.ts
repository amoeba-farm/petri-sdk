...
export async function buildRegisteredLightActionPlanV1(input: {
  ...
  if (input.actions.length === 0) fail("a Light-ATA-ready action requires a governed instruction");
  ...
  if (!Number.isSafeInteger(plan.loadBatchCount) || plan.loadBatchCount < 0
    ...
    || plan.instructionBatches.some((batch) => batch.length === 0)) fail("Light planner changed the approved atomic grouping");
...
function sameInstruction(expected: TransactionInstruction, actual: TransactionInstruction | undefined): boolean {
