...
import { validateCurrentWriterDlmmRequest, type CurrentWriterDlmmOperation, type CurrentWriterDlmmRequest } from "./writer-dlmm-public.js";

/** Independent semantic checks against Spread's bounded ManageWriterDlmmV1 wire. */
const SELECTOR: Readonly<Record<CurrentWriterDlmmOperation, number>> = Object.freeze({
...
export function requireWriterDlmmWireBindingV1(operation: CurrentWriterDlmmOperation,
  semantic: Readonly<Record<string, unknown>>, instruction: TransactionInstruction): CurrentWriterDlmmRequest {
  if (Object.hasOwn(semantic, "operation")) invalid("writer liquidity semantic cannot override its operation");
  ...
  const take = (length: number): Uint8Array => {
    if (offset + length > bytes.length) invalid("writer liquidity payload is truncated");
    const result = bytes.subarray(offset, offset + length); offset += length; return result;
  };
  const integer = (size: 1 | 2 | 4): number => {
    const value = take(size); let result = 0;
    for (let index = 0; index < size; index += 1) result += value[index]! * 2 ** (8 * index);
    return result;
  };
  const amount = (expected: string): void => {
    const value = take(8); let result = 0n;
    for (let index = 7; index >= 0; index -= 1) result = result * 256n + BigInt(value[index]!);
    if (result.toString() !== expected) invalid("writer liquidity amount differs from requested semantic");
  };
  const number = (size: 1 | 2 | 4, expected: number): void => {
    if (integer(size) !== expected) invalid("writer liquidity integer differs from requested semantic");
  };
  number(1, 159); number(1, selector);
  if (request.operation === "writer_liquidity_policy_begin") {
    ...
    amount(request.monthlyBuybackCapAtoms); amount(request.transactionBuybackCapAtoms);
    amount(request.reserveReleaseSpendRatioPpm); number(2, request.priceSeparationTicks);
  } else if (request.operation === "writer_liquidity_policy_append") {
    number(1, request.startIndex); number(4, request.entries.length);
    for (const entry of request.entries) {
      amount(entry.conservativeClaimValueAtoms); amount(entry.sellerFloorQuoteAtoms);
      amount(entry.monthlyBuybackCapAtoms); amount(entry.transactionBuybackCapAtoms);
    }
    ...
    if (request.operation === "writer_liquidity_add") {
      amount(request.issueAmountAtoms); number(4, request.entries.length);
      for (const entry of request.entries) { number(2, entry.binId); amount(entry.maximumOptionAmountAtoms); amount(entry.maximumQuoteAmountAtoms); }
    } else if (request.operation === "writer_liquidity_remove") {
      number(4, request.entries.length);
      for (const entry of request.entries) { number(2, entry.binId); amount(entry.optionAmountAtoms); amount(entry.quoteAmountAtoms); }
    }
  }
  if (offset !== bytes.length) invalid("writer liquidity payload has trailing bytes");
  return request;
}
