...
import { inspectBoundCurrentGovernedInstructionV1, prepareBoundCurrentGovernedWriteV1,
  revalidateFreshCurrentGovernedInstructionsV1 } from "./current-governed-write-internal.js";
import {
  advanceOracleRecipeWeightManifestHash,
  fixedBytes32,
  previewOracleRecipeWeightManifest,
  type OracleRecipeWeightManifestPreviewBucket,
...
function membershipInvalid(message: string): never {
  throw new AmebaProtocolError(message, { code: "CURRENT_ORACLE_MEMBERSHIP_INVALID" });
}

function exactHash(bytes: Uint8Array, label: string): Buffer {
  if (bytes.length !== 32 || bytes.every(byte => byte === 0)) membershipInvalid(`${label} must be a nonzero 32-byte value`);
...
function frozenRecipePreimage(input: {
  ...
  const nonzeroHash = (value: Uint8Array | string, label: string): Buffer => exactHash(fixedBytes32(value, label), label);
  const buckets = input.buckets.map(bucket => {
    if (bucket.sources.length === 0 || bucket.sources.length > 65535) membershipInvalid("membership buckets require 1..65535 frozen sources");
  ...
  if (!preview.manifestHash.equals(manifestHash)) membershipInvalid("source index preimage differs from the frozen recipe manifest");
...
export async function readCurrentOracleRecipeIndexProgress(input: {
  ...
  if (!Number.isSafeInteger(input.minimumContextSlot) || input.minimumContextSlot < 0) membershipInvalid("index progress requires a safe finalized slot floor");
  ...
  if (await input.connection.getGenesisHash() !== "EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG") membershipInvalid("index progress requires Devnet");
  ...
  if (!Number.isSafeInteger(observed.context.slot) || observed.context.slot < input.minimumContextSlot) membershipInvalid("index progress is below the finalized floor");
  ...
  const recipe = absent ? null : (() => {
    if (info!.executable || !info!.owner.equals(programId)) membershipInvalid("recipe source index has invalid program ownership");
    const decoded = runtime.decodeOracleRecipeSourceIndex({ ...scope, data: Buffer.from(info!.data) });
    if (!decoded.recipeHash.equals(recipeHash) || !decoded.manifestHash.equals(manifestHash)) membershipInvalid("index progress differs from the independently observed frozen recipe");
    return decoded;
  })();
...
export function prepareOracleRecipeSourceIndexInputs(input: {
  ...
  const steps: Omit<IndexOracleRecipeSourceV1Input, "programId">[] = [];
...
export async function readCurrentOracleRecipeMembership(input: {
  ...
  const fail = (message: string): never => { throw new AmebaProtocolError(message, { code: "CURRENT_ORACLE_MEMBERSHIP_INVALID" }); };
  if (!Number.isSafeInteger(input.minimumContextSlot) || input.minimumContextSlot < 0) fail("membership observation requires a safe finalized slot floor");
  ...
  if (await input.connection.getGenesisHash() !== "EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG") fail("membership observation requires Devnet");
  const observed = await input.connection.getMultipleAccountsInfoAndContext([recipeAddress, bucketAddress], {
    commitment: "finalized", minContextSlot: input.minimumContextSlot,
  });
  if (!Number.isSafeInteger(observed.context.slot) || observed.context.slot < input.minimumContextSlot || observed.value.length !== 2) fail("membership observation is incomplete or below the finalized floor");
  const data = observed.value.map((info, index) => {
    const address = index === 0 ? recipeAddress : bucketAddress;
    if (info === null) return fail(`membership index ${address.toBase58()} is absent; frozen recipe indexing is required`);
    if (info.executable || !info.owner.equals(programId)) fail(`membership index ${address.toBase58()} has invalid program ownership`);
    return Buffer.from(info.data);
  });
  const recipe = runtime.decodeOracleRecipeSourceIndex({ ...scope, data: data[0]! });
  if (!recipe.complete) fail(`frozen recipe index is incomplete: ${recipe.remainingSourceCount} source preimages remain`);
  if (!recipe.recipeHash.equals(recipeHash) || !recipe.manifestHash.equals(manifestHash)) fail("membership index differs from the independently observed frozen recipe");
  ...
  for (let first = 0; first < pageCount; first += 100) {
    ...
    if (page.context.slot < slot || page.value.length !== addresses.length) fail("incomplete membership page response");
    slot = page.context.slot;
    for (const [i, pageIndex] of indices.entries()) {
      const info = page.value[i];
      if (!info || info.executable) fail("membership page is absent or executable");
      ...
      for (const id of decoded.descendingIds) {
        if (sourceIds.length && Buffer.compare(sourceIds.at(-1)!, id) <= 0) fail("membership pages overlap or are unordered");
        sourceIds.push(Buffer.from(id));
      }
    }
  }
  if (sourceIds.length !== bucket.sourceCount) fail("membership pages are truncated");
  return Object.freeze({ observedAtSlot: slot, recipeAddress, bucketAddress, recipe,
    bucket: Object.freeze({ ...bucket, sourceIds: Object.freeze(sourceIds) }), complete: true as const });
}
