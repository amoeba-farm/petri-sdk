...
import { BPF_UPGRADEABLE_LOADER_PROGRAM_ID, validateProtocolGovernanceGateAccountV1, type GovernanceGateRpcV1 } from "./governance.js";
...
export async function observeCurrentV3RuntimeAfterGenesisV1(input: {
  ...
  const fail = (code: string, message: string): never => { throw new AmebaProtocolError(message, {code}); };
  ...
  if (typeof input.rpc.getMultipleAccountsInfoAndContext !== "function") fail("CURRENT_V3_RPC_INVALID", "V3 runtime requires one finalized Program, ProgramData, gate and vault observation");
  ...
  const hash = async (bytes: Uint8Array) => Buffer.from(await globalThis.crypto.subtle.digest("SHA-256", Uint8Array.from(bytes))).toString("hex");
  const loader = new PublicKey(BPF_UPGRADEABLE_LOADER_PROGRAM_ID);
  if (!program || !data || !gateAccount) fail("CURRENT_V3_DEPLOYMENT_ABSENT", "V3 Program, ProgramData or gate is absent");
  ...
  let businessState: "uninitialized" | "paused" | "ready" = "uninitialized";
  if (vault) {
    const b = vault.data;
  ...
  if (input.requireWriteReady && !writeReady) fail(gate.statusName !== "Active" ? "GOVERNANCE_GATE_FROZEN" : "CURRENT_V3_BUSINESS_UNAVAILABLE", "V3 requires an Active gate and initialized canonical business state");
