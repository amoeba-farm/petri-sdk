...
interface DecodedTransfer2Load {
  readonly amountAtoms: bigint;
  readonly inputs: readonly Transfer2InputIdentity[];
}
 ...
 */
export function validateCurrentLightTransfer2LoadSequence(
  transfers: readonly TransactionInstruction[],
  expected: CurrentLightTransfer2Expectation,
): void {
  ...
  for (const transfer of transfers) {
    ...
    if (totalAmount > U64_MAX) fail("cold load amount exceeds the current u64 token domain");
...
function decodeTransfer2Load(
  ...
  const fixed: readonly [PublicKey, boolean, boolean][] = [
  ...
  if (inputCount < 1) fail("Transfer2 contains no compressed token input");
  ...
  for (let index = 0; index < inputCount; index += 1) {
    ...
    if (inputAmount > U64_MAX) fail("Transfer2 input amount exceeds u64");
...
class Transfer2Cursor {
  ...
  constructor(bytes: Buffer) {
    if (bytes.length < 1 || bytes.length > 16_384) fail("Transfer2 data size is outside 1..16384 bytes");
    this.#bytes = bytes;
  }
  ...
  bool(field: string): boolean {
    ...
    if (value > 1) fail(`${field} is not a canonical Borsh bool`);
    return value === 1;
  }
  ...
  option(field: string): boolean {
    ...
    if (value > 1) fail(`${field} has a noncanonical Borsh option`);
    return value === 1;
  }

  vecLength(field: string, maximum: number): number {
    const value = this.u32(`${field} length`);
    if (value > maximum) fail(`${field} exceeds its current bound`);
    return value;
  }

  bytes(length: number, field: string): readonly number[] {
  ...
  finish(): void {
    if (this.#offset !== this.#bytes.length) fail("Transfer2 contains trailing bytes");
  }

  #require(length: number, field: string): void {
    if (this.#offset + length > this.#bytes.length) fail(`Transfer2 ${field} is truncated`);
  }
}
