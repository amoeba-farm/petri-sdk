...
function invalid(): never { throw Object.assign(new Error("Scoped settlement account identity is invalid"), { code: "SCOPED_SETTLEMENT_INVALID" }); }
function programAccount(info: AccountInfo<Buffer> | null, program: PublicKey): AccountInfo<Buffer> {
  if (!info || info.executable || !info.owner.equals(program)) invalid(); return info;
}
export function decodeCurrentScopedLongAuthorization(input: { address: PublicKey; account: AccountInfo<Buffer>; programId: PublicKey }) {
  const { address, account, programId } = input; const data = account.data;
  if (!programId.equals(new PublicKey(AMOEBA_SPREAD_PROGRAM_ID)) || account.executable || !account.owner.equals(LIGHT_TOKEN_PROGRAM_ID)
    ...
    || data.readUInt32LE(129) !== 0 || data[165] !== 2 || data[166] !== 1 || data.readUInt32LE(167) !== 1 || data[171] !== 32) invalid();
  ...
  if (!address.equals(deriveLightAssociatedTokenAddress(mint, owner)) || !delegate.equals(new PublicKey(data.subarray(76, 108))) || allowance < amount) invalid();
  return { owner: owner.toBase58(), mint: mint.toBase58(), amountAtoms: amount.toString(), authority: delegate.toBase58() };
}
export function decodeCurrentScopedPositionAuthorization(input: { address: PublicKey; account: AccountInfo<Buffer>; programId: PublicKey }) {
  const { address, programId } = input; const data = programAccount(input.account, programId).data;
  if (data.length !== 105 || data.subarray(0, 8).toString() !== "APSAUTH1") invalid();
  ...
  if (!expected.equals(address) || data[8] !== bump) invalid();
...
export async function discoverCurrentScopedSettlements(input: { connection: Connection; programId: PublicKey }) {
  const { connection, programId } = input;
  if (!programId.equals(new PublicKey(AMOEBA_SPREAD_PROGRAM_ID))) invalid();
  ...
  if (sleeves.length > 256 || markers.length > 4096) invalid();
  const positions = markers.map(({ pubkey, account }) => decodeCurrentScopedPositionAuthorization({ address: pubkey, account, programId }));
  const longs: { kind: "long_claim"; owner: string; target: string; expiryUnixSeconds: string; request: { owner: string; sleeve: string; claimVariant: "collective_long"; seriesIndex: number; amountAtoms: string } }[] = [];
  for (const entry of sleeves) {
    ...
    if (!book.sleeve.equals(entry.pubkey)) invalid();
    for (let seriesIndex = 0; seriesIndex < book.seriesCount; seriesIndex++) {
      const record = book.records[seriesIndex]!; if (!record.active) continue;
      ...
      if (holders.length > 4096) invalid();
      for (const holder of holders) {
        let authorization; try { authorization = decodeCurrentScopedLongAuthorization({ address: holder.pubkey, account: holder.account, programId }); } catch { continue; }
        if (authorization.amountAtoms === "0") continue;
...
export function currentScopedCollectiveBuilderInput(instruction: TransactionInstruction, keeper: PublicKey) {
  const semantic = semanticCurrentSpreadInstructionV1(instruction); const keys = semantic.keys;
  if (semantic.data[0] !== 246 || keys.length !== 20) invalid();
