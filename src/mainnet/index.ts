...
export class MainnetSdkError extends Error {
  ...
  constructor(readonly code: string) { super(code); this.name = "MainnetSdkError"; this.status = this.statusCode = code === "MAINNET_GATE_NOT_ACTIVE" ? 409 : 503; }
}

/** Validate privately; never include the supplied URL in errors or observations. */
export function assertMainnetHeliusEndpoint(endpoint: string): void {
  let url: URL;
  try { url = new URL(endpoint); } catch { throw new MainnetSdkError("MAINNET_HELIUS_ENDPOINT_REQUIRED"); }
...
export async function observeMainnetDeployment(connection: Connection, minimumContextSlot: number = p.deployedSlot) {
  assertMainnetHeliusEndpoint(connection.rpcEndpoint);
  if (!Number.isSafeInteger(minimumContextSlot) || minimumContextSlot < 0) throw new MainnetSdkError("MAINNET_INVALID_SLOT");
  if (await connection.getGenesisHash().catch(() => { throw new MainnetSdkError("MAINNET_RPC_UNAVAILABLE"); }) !== p.genesisHash) throw new MainnetSdkError("MAINNET_GENESIS_MISMATCH");
  ...
  if (!Number.isSafeInteger(read.context.slot) || read.context.slot < floor || !program || !data || !gateAccount) throw new MainnetSdkError("MAINNET_ACCOUNTS_UNAVAILABLE");
  if (!program.executable || data.executable || !program.owner.equals(BPF_LOADER_UPGRADEABLE_PROGRAM_ID)
    ...
    || data.data.subarray(45 + p.artifactBytes).some(b => b !== 0)) throw new MainnetSdkError("MAINNET_EXECUTABLE_MISMATCH");
  if (gateAccount.executable || !gateAccount.owner.equals(new PublicKey(p.controllerProgramId))) throw new MainnetSdkError("MAINNET_GATE_OWNER_MISMATCH");
  ...
  if (address.toBase58() !== p.gate || gate.bump !== bump || gate.controllerConfig.toBase58() !== p.controllerConfig
    || gate.targetProgram.toBase58() !== p.programId || gate.targetProgramdata.toBase58() !== p.programData) throw new MainnetSdkError("MAINNET_GATE_IDENTITY_MISMATCH");
  return Object.freeze({ network: p.network, genesisHash: p.genesisHash, programId: p.programId,
    ...
    gateActive: gate.status === 0, tradeReady: false as const,
