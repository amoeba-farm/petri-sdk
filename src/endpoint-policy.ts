...
export const AMOEBA_BACKEND_URL = "https://api.amoeba.farm" as const;
 ...
 */
export function normalizeAmoebaBackendUrl(raw: string = AMOEBA_BACKEND_URL): URL {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
  ...
  if (url.protocol !== "https:" && !(url.protocol === "http:" && loopback)) {
    throw new AmebaInputError(
      "backendUrl must use https (http is allowed only for loopback Amoeba development)",
    );
  }
  if (!loopback && hostname !== AMOEBA_BACKEND_HOST) {
    throw new AmebaInputError(
      "backendUrl must target the hosted Amoeba API (or a loopback development server)",
    );
  }
  if (url.username || url.password || url.search || url.hash) {
    throw new AmebaInputError(
      "backendUrl must not contain credentials, a query string, or a fragment",
    );
  }
...
export function amoebaReadGatewayUrl(
  backendUrl: string = AMOEBA_BACKEND_URL,
): string {
 ...
 */
export function normalizeAmoebaReadGatewayUrl(raw: string): string {
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
  ...
  if (url.username || url.password || url.search || url.hash) {
    throw new AmebaInputError(
      "RPC endpoint must not contain credentials, a query string, or a fragment",
    );
  }
  if (url.pathname !== "/rpc") {
