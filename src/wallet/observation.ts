...
export const CURRENT_WALLET_MAX_OBSERVATION_AGE_SECONDS = 110n as const;
export const CURRENT_WALLET_MAX_TRANSACTION_BYTES = 1_232 as const;

export const CURRENT_WALLET_PROTOCOL_IDENTITY = Object.freeze({
  identityKind: "v3-governed-wallet-materializer" as const,
  currentWriteEligible: true as const,
  releaseLabel: live.releaseLabel,
  liveReadProfileId: live.liveReadProfileId,
  programAccountSha256: live.programAccountSha256,
  protocolPlanSdkCommit: "b2cd10739ecb9419115980425920d6b576caf78e" as const,
  protocolRelease: "v0.1.0-rc.44" as const,
  protocolSourceCommit: "1b2230d96e51f6582155d8284900fbfc11ff1f18" as const,
  genesisHash: "EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG" as const,
...
export interface CurrentWalletRpc {
  ...
  getMultipleAccountsInfoAndContext(
    ...
    config?: { readonly commitment?: Commitment; readonly minContextSlot?: number },
  ): Promise<Readonly<{ readonly context: Context; readonly value: readonly (AccountInfo<Uint8Array> | null)[] }>>;
  getBlockTime(slot: number): Promise<number | null>;
  getLatestBlockhashAndContext(
    config?: { readonly commitment?: Commitment; readonly minContextSlot?: number },
  ): Promise<Readonly<{ readonly context: Context; readonly value: BlockhashWithExpiryBlockHeight }>>;
  isBlockhashValid(
    blockhash: string,
    config?: { readonly commitment?: Commitment; readonly minContextSlot?: number },
  ): Promise<Readonly<{ readonly context: Context; readonly value: boolean }>>;
}
...
export async function reobserveCurrentWalletPlan(
  rpc: CurrentWalletRpc,
  admittedObservation: unknown,
): Promise<CurrentWalletReobservation> {
  ...
  if (addresses[0]?.toBase58() !== PROGRAM_ID.toBase58()
    ...
    walletError(
      "CURRENT_WALLET_DEPLOYMENT_INVALID",
      "prepared observation does not begin with the pinned Program and ProgramData identities",
    );
  }
  ...
  const accountInfos = new Map<string, AccountInfo<Uint8Array> | null>();
  ...
  verifyPinnedProgramDeployment(
    requireAccount(accountInfos.get(PROGRAM_ID.toBase58()), "Program"),
    requireAccount(accountInfos.get(PROGRAM_DATA_ID.toBase58()), "ProgramData"),
  );
  const blockTime = await rpc.getBlockTime(observedSlot);
  if (!Number.isSafeInteger(blockTime) || (blockTime as number) <= 0) {
  ...
  const currentBlockTime = BigInt(blockTime as number);
...
function snapshotAccountInfo(
  value: AccountInfo<Uint8Array> | null,
  label: string,
): AccountInfo<Uint8Array> | null {
...
function requireSameObservedAccount(
  admitted: CurrentFinalizedObservation["orderedAccounts"][number],
  actual: AccountInfo<Uint8Array> | null,
  index: number,
): void {
  if (actual === null) {
...
function verifyPinnedProgramDeployment(
  program: AccountInfo<Uint8Array>,
  programData: AccountInfo<Uint8Array>,
): void {
...
function safeSlot(value: string, label: string): number {
...
function canonicalRpcOrigin(endpoint: unknown): string {
...
function requireAccount(
  value: AccountInfo<Uint8Array> | null | undefined,
  label: string,
