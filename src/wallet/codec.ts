...
export function walletError(code: string, message: string): never {
...
export function exactArray(
  ...
  if (!Array.isArray(value) || value.length < minimum || value.length > maximum) {
    walletError(
      "CURRENT_WALLET_RESPONSE_INVALID",
...
export function canonicalPublicKeyString(
  value: unknown,
  label: string,
...
export function canonicalDigest(value: unknown, label: string): string {
...
export function canonicalU64(value: unknown, label: string, positive = false): string {
...
export function canonicalIndex(value: unknown, label: string, maximum: number): number {
...
export function decodeBase64(value: unknown, label: string, maximumBytes = 16_384): Uint8Array {
...
export function snapshotCanonicalJson<Value = unknown>(
  value: unknown,
  label: string,
  ...
  const owned = deepFreezeCanonical(JSON.parse(serialized)) as Value;
...
function canonicalValue(
  ...
  if (Array.isArray(value)) {
    if (ancestors.has(value)) walletError("CURRENT_WALLET_RESPONSE_INVALID", "canonical JSON contains a cycle");
    ancestors.add(value);
  ...
  if (typeof value === "object") {
    const record = value as Readonly<Record<string, unknown>>;
    if (ancestors.has(record)) walletError("CURRENT_WALLET_RESPONSE_INVALID", "canonical JSON contains a cycle");
...
function deepFreezeCanonical(value: unknown): unknown {
  if (value === null || typeof value !== "object") return value;
  if (Array.isArray(value)) {
    value.forEach(deepFreezeCanonical);
    return Object.freeze(value);
  }
  Object.values(value as Record<string, unknown>).forEach(deepFreezeCanonical);
  return Object.freeze(value);
}

function requireRead(bytes: Uint8Array, offset: number, length: number): void {
