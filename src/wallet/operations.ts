...
import { decodeWalletWriterDlmmPolicy, decodeWalletWriterDlmmPosition, deriveWalletWriterDlmmPolicy,
  deriveWalletWriterDlmmPosition, decodeWalletWriterPolicyRegistry, decodeWalletWriterPolicySnapshot } from "./writer-dlmm-accounts.js";
...
import {
  ...
  deriveDlmmAuthority,
  deriveLightAta,
  deriveLightSplInterface,
  deriveWriterBid,
  deriveWriterPda,
  deriveWriterBidIndex,
  deriveWriterCloseFlatEscrow,
  deriveWriterCloseRequest,
  deriveWriterFlatMint,
  deriveWriterFlatBurnCustody,
  deriveWriterFlatStaging,
  deriveWriterRetirementCustody,
  deriveWriterSeriesBook,
  deriveWriterSleeveUsdcVault,
  manifestInstruction,
  pubkey,
  requireAddress,
  requireCanonicalComputeBudget,
  requireCanonicalSetupBatch,
...
import {
  ...
  type CurrentWalletReobservation,
} from "./observation.js";
import type {
  CurrentCollectiveOperationPrepareEnvelope,
  CurrentDecodedCollectivePlan,
} from "./plans.js";
import {
  decodeWalletWriterAuction,
  decodeWalletWriterBidIndex,
  decodeWalletWriterBid,
  decodeWalletWriterCloseRequest,
  decodeWalletWriterSeriesBook,
  decodeWalletWriterSleeve,
} from "./writer-accounts.js";

const R = [false, false] as const;
const RS = [true, false] as const;
const W0 = [false, true] as const;
const W = [true, true] as const;
...
function validateAndMaterializeSemanticPlan(
  envelope: CurrentCollectiveOperationPrepareEnvelope,
  reobservation: CurrentWalletReobservation,
): readonly (readonly TransactionInstruction[])[] {
  const plan = envelope.operationPlan;
  let batches: readonly (readonly TransactionInstruction[])[];
  ...
  for (const [batchIndex, batch] of batches.entries()) {
    ...
    if (batch.length < 1 || batch.length > 32 || signerAddresses.size !== 1 || !signerAddresses.has(owner)) {
      walletError(
        "CURRENT_WALLET_SIGNER_INVALID",
        `execution batch ${batchIndex} must require exactly the selected wallet owner`,
      );
    }
  }
...
function validateWriter(
  ...
  const expectedIdentity = WRITER_NAMES[plan.operation as keyof typeof WRITER_NAMES];
  ...
  switch (plan.operation) {
    case "writer_liquidity_policy_begin": case "writer_liquidity_policy_append": case "writer_liquidity_policy_seal":
    case "writer_liquidity_initialize": case "writer_liquidity_add": case "writer_liquidity_remove": case "writer_liquidity_sweep":
      validateWriterLiquidity(plan, manifest, reobservation); break;
    case "deposit": validateDeposit(plan, manifest, data, reobservation); break;
    case "bid": validateBid(plan, manifest, data, reobservation); break;
    case "withdraw_principal": validatePrincipalWithdrawal(plan, manifest, data, reobservation); break;
    case "auction_refund": validateAuctionRefund(plan, manifest, data, reobservation); break;
    case "close_begin": validateCloseBegin(envelope, manifest, data, reobservation); break;
    case "close_basket": validateCloseBasket(envelope, manifest, data, reobservation); break;
    case "close_finalize": validateCloseFinalize(envelope, manifest, data, reobservation); break;
    case "close_cancel": validateCloseCancel(envelope, manifest, data, reobservation); break;
    case "settlement_claim_flat": validateFlatClaim(plan, manifest, data, reobservation); break;
    case "settlement_claim_collective": validateCollectiveClaim(plan, manifest, data, reobservation); break;
...
function validateWriterLiquidity(plan: CurrentDecodedCollectivePlan, manifest: CurrentWalletInstructionManifest,
  reobservation: CurrentWalletReobservation): void {
  if (!isCurrentWriterDlmmOperation(plan.operation)) walletError("CURRENT_WALLET_PLAN_MISMATCH", "writer liquidity operation is unknown");
  ...
  if (policyAction) {
    ...
    if (request.operation === "writer_liquidity_policy_begin") {
      if (!actor.equals(registry.policyAuthority)) walletError("CURRENT_WALLET_SIGNER_INVALID", "policy begin requires registry authority");
    ...
    if (request.operation === "writer_liquidity_policy_append") {
      if (!actor.equals(policy.committingPolicyAuthority) || request.startIndex !== policy.appendedSeriesCount
        || request.startIndex + request.entries.length > book.seriesCount) walletError("CURRENT_WALLET_PLAN_MISMATCH", "policy append authority or next series differs");
  ...
  if (!("seriesIndex" in request)) walletError("CURRENT_WALLET_PLAN_MISMATCH", "writer liquidity series is missing");
  const policy = decodeWalletWriterDlmmPolicy(policyAddress, accounts);
  ...
  if (!record?.active) walletError("CURRENT_WALLET_ACCOUNT_INVALID", "writer liquidity series is not active in its canonical book");
  ...
  if (!permissionless && !actor.equals(policy.managementAuthority)) walletError("CURRENT_WALLET_SIGNER_INVALID", "writer liquidity actor is not the frozen manager");
  if (initialize) {
    ...
    if ([5, 6, 7].includes(sleeve.status) || [3, 4].includes(pool.status)) walletError("CURRENT_WALLET_PLAN_MISMATCH", "writer position cannot initialize after terminal lifecycle");
...
function requireWriterCreateOnlyTarget(accounts: ReadonlyMap<string, AccountInfo<Uint8Array> | null>, address: PublicKey, label: string): void {
  if (!accounts.has(address.toBase58())) walletError("CURRENT_WALLET_ACCOUNT_INVALID", `${label} was not observed`);
...
function requireOptionalWriterClassicCustody(accounts: ReadonlyMap<string, AccountInfo<Uint8Array> | null>, address: PublicKey,
  mint: PublicKey, owner: PublicKey, label: string): void {
  const info = accounts.get(address.toBase58());
  if (!info || info.owner.equals(SYSTEM_PROGRAM_ID)) requireWriterCreateOnlyTarget(accounts, address, label);
  else requireClassicCustody(accounts, address, mint, owner, 0n, label);
}

function validatePrincipalWithdrawal(plan: CurrentDecodedCollectivePlan, manifest: CurrentWalletInstructionManifest,
  data: Uint8Array, reobservation: CurrentWalletReobservation): void {
  requireFlags(manifest, FLAGS.withdrawPrincipal, "principal withdrawal");
  const amount = BigInt(plan.semantic.amountAtoms as string);
  ...
  if (plan.setupMode !== "cold_load") requireLightCustody(reobservation.accountInfos, source, sleeve.flatMint, owner, amount, "withdrawal Flat source");
}

function validateAuctionRefund(plan: CurrentDecodedCollectivePlan, manifest: CurrentWalletInstructionManifest,
  data: Uint8Array, reobservation: CurrentWalletReobservation): void {
  ...
  if (early ? (!actor.equals(bid.bidder) || !sleeve.activeAuction?.equals(auction.address))
    : (![2, 5].includes(summary.status) && ![5, 6].includes(auction.status))) walletError("CURRENT_WALLET_PLAN_MISMATCH", "auction refund phase/actor does not permit this refund");
...
function requireClassicOutput(plan: CurrentDecodedCollectivePlan, reobservation: CurrentWalletReobservation,
  address: PublicKey, mint: PublicKey, owner: PublicKey): void {
  ...
  if (facts.ata !== address.toBase58() || facts.owner !== owner.toBase58() || facts.mint !== mint.toBase58()
    || facts.payer !== plan.semantic.owner || !deriveClassicAta(mint, owner).equals(address)) walletError("CURRENT_WALLET_PLAN_MISMATCH", "classic output creation changes the native destination");
  const info = reobservation.accountInfos.get(address.toBase58());
...
function validateDeposit(
  ...
  requirePayload(data, concat(Uint8Array.of(227), u64Le(BigInt(plan.semantic.principalAtoms as string))), "writer deposit");
  const owner = pubkey(plan.semantic.owner, "semantic.owner");
  ...
  requireClassicCustody(reobservation.accountInfos, source, sleeve.settlementMint, owner,
    BigInt(plan.semantic.principalAtoms as string), "deposit USDC source");
...
function validateBid(
  ...
  const seriesIndex = plan.semantic.seriesIndex as number;
...
function validateCloseBegin(
  ...
  requirePayload(data, concat(
    Uint8Array.of(240),
    u64Le(BigInt(plan.semantic.flatParAtoms as string)),
    u64Le(BigInt(plan.semantic.minimumWithdrawalAtoms as string)),
    u64Le(expectedDeadline),
  ), "writer close begin");
  const owner = pubkey(plan.semantic.owner, "semantic.owner");
  const sleeveAddress = pubkey(plan.semantic.sleeve, "semantic.sleeve");
  const sleeve = decodeWalletWriterSleeve(sleeveAddress, reobservation.accountInfos);
  ...
  if (plan.setupMode === "none") {
    requireLightCustody(
      reobservation.accountInfos,
      deriveLightAta(sleeve.flatMint, owner),
      sleeve.flatMint,
      owner,
      BigInt(plan.semantic.flatParAtoms as string),
      "close Flat source",
    );
  }
}

function validateCloseBasket(
  envelope: CurrentCollectiveOperationPrepareEnvelope,
  manifest: CurrentWalletInstructionManifest,
  data: Uint8Array,
  reobservation: CurrentWalletReobservation,
  ...
  if (envelope.operationPlan.setupMode === "none") {
    requireLightCustody(
      reobservation.accountInfos,
      deriveLightAta(record.contractMint, request.owner),
      record.contractMint,
      request.owner,
      request.requiredClaimAtoms[stageIndex]!,
      "close basket source",
    );
  }
}

function validateCloseFinalize(
  envelope: CurrentCollectiveOperationPrepareEnvelope,
  manifest: CurrentWalletInstructionManifest,
  data: Uint8Array,
  reobservation: CurrentWalletReobservation,
  ...
  const flags = [RS, R, W0, R, W0, R, W0, W0, W0, R, W0, W0, R, R,
    ...book.records.flatMap(() => [W0, W0, W0])] as const;
  ...
  if (!lpPolicyInfo || lpPolicyInfo.owner.equals(SYSTEM_PROGRAM_ID)) requireWriterCreateOnlyTarget(reobservation.accountInfos, lpPolicyAddress, "historical writer policy");
  else {
    ...
    if (!policy.sealed || !policy.policySnapshot.equals(sleeve.policySnapshot) || policy.totalPoolQuoteAtoms !== 0n
      || policy.seriesPoolInventoryAtoms.some(amount => amount !== 0n)) walletError("CURRENT_WALLET_ACCOUNT_INVALID", "close finalize requires writer pool custody to be unwound");
  }
  requireClassicOutput(envelope.operationPlan, reobservation,
    deriveClassicAta(sleeve.settlementMint, request.owner), sleeve.settlementMint, request.owner);
...
function validateCollectiveClaim(
  plan: CurrentDecodedCollectivePlan,
  manifest: CurrentWalletInstructionManifest,
  data: Uint8Array,
  reobservation: CurrentWalletReobservation,
  ...
  requirePayload(data, concat(Uint8Array.of(246), u64Le(BigInt(plan.semantic.amountAtoms as string))), "collective claim");
  const owner = pubkey(plan.semantic.owner, "semantic.owner");
  const sleeve = decodeWalletWriterSleeve(pubkey(plan.semantic.sleeve, "semantic.sleeve"), reobservation.accountInfos);
  requireSleeveChildren(sleeve);
  const book = decodeWalletWriterSeriesBook(sleeve.seriesBook, reobservation.accountInfos);
  ...
  const record = requireSeries(book, plan.semantic.seriesIndex as number);
  ...
  if (plan.setupMode !== "cold_load") requireLightCustody(reobservation.accountInfos, expected[7]!, record.contractMint, owner,
    BigInt(plan.semantic.amountAtoms as string), "collective claim source");
  requireNativeCreatedLightOutput(reobservation, expected[11]!, sleeve.settlementMint, owner);
}

function validateFlatClaim(
  plan: CurrentDecodedCollectivePlan,
  manifest: CurrentWalletInstructionManifest,
  data: Uint8Array,
  reobservation: CurrentWalletReobservation,
  ...
  requirePayload(data, concat(Uint8Array.of(247), u64Le(BigInt(plan.semantic.amountAtoms as string))), "Flat residual claim");
  const owner = pubkey(plan.semantic.owner, "semantic.owner");
  const sleeve = decodeWalletWriterSleeve(pubkey(plan.semantic.sleeve, "semantic.sleeve"), reobservation.accountInfos);
  requireSleeveChildren(sleeve);
  ...
  if (plan.setupMode !== "cold_load") requireLightCustody(
    reobservation.accountInfos,
    deriveLightAta(sleeve.flatMint, owner),
    sleeve.flatMint,
    owner,
    BigInt(plan.semantic.amountAtoms as string),
    "Flat claim source",
  );
  requireNativeCreatedLightOutput(reobservation, deriveLightAta(sleeve.settlementMint, owner), sleeve.settlementMint, owner);
}

function requireNativeCreatedLightOutput(reobservation: CurrentWalletReobservation, address: PublicKey, mint: PublicKey, owner: PublicKey): void {
  ...
  const info = reobservation.accountInfos.get(address.toBase58());
  if (info === null || (info !== undefined && info.owner.equals(SYSTEM_PROGRAM_ID) && !info.executable && info.data.length === 0)) return;
  requireLightCustody(reobservation.accountInfos, address, mint, owner, 0n, "native Light settlement destination");
}

function validateWriterSetup(
  envelope: CurrentCollectiveOperationPrepareEnvelope,
  actionManifest: CurrentWalletInstructionManifest,
  reobservation: CurrentWalletReobservation,
): void {
  const plan = envelope.operationPlan;
  if (plan.setupMode === "none") return;
  const owner = pubkey(plan.semantic.owner, "semantic.owner");
  let setupBatches = plan.executionInstructionBatches.map((batch, index) =>
    index === plan.actionBatchIndex ? batch.slice(0, batch.length - 1) : batch);
  if (plan.setupMode === "writer_liquidity_compute") {
    if (!isCurrentWriterDlmmOperation(plan.operation) || setupBatches.length !== 1 || setupBatches[0]!.length !== 2
      ...
      || plan.canonicalOutputFacts !== null || plan.classicOutputFacts) walletError("CURRENT_WALLET_PLAN_INVALID", "writer liquidity compute grouping differs");
    const expected = currentWriterDlmmComputeInstructions();
    setupBatches[0]!.forEach((manifest, index) => {
      if (manifest.instructionName !== CURRENT_WRITER_DLMM_TRANSPORT_V1.setupInstructionNames[index]) walletError("CURRENT_WALLET_PLAN_INVALID", "writer liquidity compute name differs");
  ...
  if (plan.classicOutputFacts) {
    const facts = plan.classicOutputFacts;
    const payer = pubkey(facts.payer, "classic output payer"); const outputOwner = pubkey(facts.owner, "classic output owner");
    const mint = pubkey(facts.mint, "classic output mint"); const ata = pubkey(facts.ata, "classic output ATA");
    if (!payer.equals(owner) || !deriveClassicAta(mint, outputOwner).equals(ata)) walletError("CURRENT_WALLET_PLAN_INVALID", "classic output facts are noncanonical");
    ...
    if (finalBatch.length < 2) walletError("CURRENT_WALLET_PLAN_INVALID", "classic output setup is missing");
    ...
    if (plan.setupMode === "classic_output_create") {
      if (setupBatches.length !== 1 || setupBatches[0]!.length !== 1) walletError("CURRENT_WALLET_PLAN_INVALID", "classic output setup has extra instructions");
      const compute = manifestInstruction(setupBatches[0]![0]!);
      requireCanonicalComputeBudget(compute);
      if (readU32Le(compute.data, 1) !== 600_000) walletError("CURRENT_WALLET_PLAN_INVALID", "classic output compute bound mismatch");
      return;
    }
  }
  if (plan.setupMode === "cold_load") {
    ...
    if (!payer.equals(owner) || !deriveLightAta(mint, proofOwner).equals(target)
      || BigInt(facts.amountAtoms as string) < BigInt(facts.minimumAmountAtoms as string)) {
    ...
    const exactMinimum = ["withdraw_principal", "settlement_claim_collective", "settlement_claim_flat"].includes(plan.operation) ? BigInt(plan.semantic.amountAtoms as string) : plan.operation === "close_begin"
      ? BigInt(plan.semantic.flatParAtoms as string)
      : (() => {
          const request = decodeWalletWriterCloseRequest(
            pubkey(plan.semantic.closeRequest, "semantic.closeRequest"),
            reobservation.accountInfos,
          );
          const seriesIndex = requireStage(envelope, "deposit_basket");
          return request.requiredClaimAtoms[seriesIndex] ?? -1n;
        })();
    if (BigInt(facts.minimumAmountAtoms as string) !== exactMinimum) {
    ...
    validateCurrentWalletLightTransfer2LoadSequence(
      nativeSetupBatches.map((batch) => batch[2]!),
      { payer, owner: proofOwner, mint, destination: target, amountAtoms: BigInt(facts.amountAtoms as string) },
    );
    return;
  }
...
function validateFlat(
  ...
  if (plan.sourceState === "hot") {
    if (sourceData === null) walletError("CURRENT_WALLET_PLAN_INVALID", "hot Flat source bytes are absent");
    ...
    if (!state.mint.equals(mint) || !state.owner.equals(owner)
      || state.amount < BigInt(plan.semantic.amountAtoms as string)) {
  ...
  if (plan.destinationState === "hot") {
    if (destinationData === null) walletError("CURRENT_WALLET_PLAN_INVALID", "hot Flat destination bytes are absent");
  ...
  const expectedAction: TransactionInstruction[] = [];
  if (plan.destinationState === "absent") expectedAction.push(buildCreateLightAta(owner, destinationOwner, mint));
  expectedAction.push(buildLightTransfer(source, mint, destination, owner,
    BigInt(plan.semantic.amountAtoms as string), decimals));
  ...
  plan.instructions.forEach((manifest, index) => requireExactInstruction(
    manifestInstruction(manifest), expectedAction[index]!, `Flat action ${index}`));
  if (plan.setupMode === "cold_load") {
    const facts = plan.coldProofFacts!;
    ...
    const minimum = plan.sourceState === "cold" ? BigInt(plan.semantic.amountAtoms as string) : 0n;
    if (!payer.equals(owner) || !target.equals(expectedTarget) || !proofOwner.equals(expectedOwner)
      || !proofMint.equals(mint) || BigInt(facts.amountAtoms as string) < minimum) {
      walletError("CURRENT_WALLET_PLAN_INVALID", "Flat cold proof facts do not bind exact custody semantics");
    }
    const setupBatches = plan.executionInstructionBatches.map((batch, index) =>
      index === plan.actionBatchIndex ? batch.slice(0, batch.length - plan.instructions.length) : batch);
    const nativeSetupBatches = setupBatches.map((batch) => batch.map(manifestInstruction));
    nativeSetupBatches.forEach((batch) => requireCanonicalSetupBatch({
      batch, payer, owner: proofOwner, mint, target, requireCreate: true,
    }));
...
function validateSwap(
  ...
  const reservePages = semantic.reservePageIndices as readonly number[];
  const expectedFlags = [
  ...
  const expected = concat(
    Uint8Array.of(254, direction),
    u64Le(BigInt(semantic.amountIn as string)),
    u64Le(BigInt(semantic.minimumAmountOut as string)),
    u16Le(semantic.limitBinId as number),
    u64Le(BigInt(semantic.deadlineTs as string)),
  );
  ...
  if (sleeveState.settlementGroup.toBase58() !== group.toBase58()
    ...
    || poolState.status !== 1
    || ((semantic.limitBinId as number) !== 0 && (semantic.limitBinId as number) > poolState.maximumBinId)) {
  ...
  if (!snapshot.sleeve.equals(sleeve) || !snapshot.registry.equals(registryAddress)
    || !registry.vaultConfig.equals(sleeveState.vaultConfig)) walletError("CURRENT_WALLET_ACCOUNT_INVALID", "swap policy identity differs");
  ...
  if (!policyInfo || policyInfo.owner.equals(SYSTEM_PROGRAM_ID)) {
    ...
    if (!positionInfo || positionInfo.owner.equals(SYSTEM_PROGRAM_ID)) requireWriterCreateOnlyTarget(reobservation.accountInfos, positionAddress, "uninitialized writer position");
    else {
  ...
  requireLightCustody(reobservation.accountInfos, optionAccount, optionMint, trader,
    semantic.direction === "OptionForQuote" ? BigInt(semantic.amountIn as string) : 0n,
    "swap trader option account");
  requireLightCustody(reobservation.accountInfos, quoteAccount, quoteMint, trader,
    semantic.direction === "QuoteForOption" ? BigInt(semantic.amountIn as string) : 0n,
    "swap trader quote account");
  return instructionsFromBatches(plan.executionInstructionBatches);
}

function requireCloseAdmissionState(
  envelope: CurrentCollectiveOperationPrepareEnvelope,
  request: ReturnType<typeof decodeWalletWriterCloseRequest>,
  sleeve: ReturnType<typeof decodeWalletWriterSleeve>,
  book: ReturnType<typeof decodeWalletWriterSeriesBook>,
  reobservation: CurrentWalletReobservation,
): void {
  ...
  const facts = factsValue as Readonly<Record<string, unknown>>;
  const statuses = ["collecting", "complete", "finalized", "cancelling", "cancelled"] as const;
  ...
  if (!request.sleeve.equals(sleeve.address)
    ...
    walletError(
      "CURRENT_WALLET_ADMISSION_INVALID",
      "writer close admission facts differ from the exact finalized request, sleeve, or book state",
    );
  }
  ...
  const firstDeposited = request.depositedClaimAtoms.findIndex(
    (amount, index) => index >= request.nextCancelIndex && amount > 0n,
  );
...
function requireCloseBeginAdmission(
  envelope: CurrentCollectiveOperationPrepareEnvelope,
  sleeve: ReturnType<typeof decodeWalletWriterSleeve>,
  book: ReturnType<typeof decodeWalletWriterSeriesBook>,
): void {
  ...
  const commitments = commitmentsValue as Readonly<Record<string, unknown>>;
  ...
  const groupObservation = envelope.operationPlan.currentObservation.orderedAccounts.find(
    (account) => account.address === sleeve.settlementGroup.toBase58(),
  );
...
function requireSleeveChildren(
  sleeve: ReturnType<typeof decodeWalletWriterSleeve>,
): void {
...
function requireStage(
  envelope: CurrentCollectiveOperationPrepareEnvelope,
  operation: "deposit_basket" | "finalize" | "cancel_series" | "cancel_flat",
): number {
  const stage = envelope.stage;
  ...
  if (operation === "deposit_basket" || operation === "cancel_series") {
    if (!Number.isInteger(stage.seriesIndex) || (stage.seriesIndex as number) < 0 || (stage.seriesIndex as number) > 19
      || Object.keys(stage).length !== 2) {
      walletError("CURRENT_WALLET_ADMISSION_INVALID", "Lean stage series index is invalid");
    }
    return stage.seriesIndex as number;
  }
...
function requireSeries(book: ReturnType<typeof decodeWalletWriterSeriesBook>, index: number) {
...
function requireWriterLightConstants(
  manifest: CurrentWalletInstructionManifest,
  lightIndex: number,
...
function instructionsFromBatches(
...
function buildLightTransfer(
  ...
  return new TransactionInstruction({
    ...
    data: concat(Uint8Array.of(12), u64Le(amount), Uint8Array.of(decimals)) as TransactionInstruction["data"],
  });
}

function decodeMintDecimals(data: Uint8Array): number {
...
function decodeLightToken(data: Uint8Array, label: string, scoped = false): { readonly mint: PublicKey; readonly owner: PublicKey; readonly amount: bigint } {
  const delegateOkay = data.length === 272 && ((readU32Le(data, 72) === 0 && readU64Le(data, 121) === 0n)
    || (scoped && readU32Le(data, 72) === 1 && new PublicKey(data.subarray(76, 108)).equals(
      deriveCollectiveSettlementDelegatePda(new PublicKey(data.subarray(32, 64)), new PublicKey(data.subarray(0, 32)), CURRENT_PROGRAM_ID)[0])));
...
function requireClassicCustody(
  accounts: ReadonlyMap<string, AccountInfo<Uint8Array> | null>,
  address: PublicKey,
  mint: PublicKey,
  owner: PublicKey,
...
function requirePayload(actual: Uint8Array, expected: Uint8Array, label: string): void {
...
export function validateAndMaterializeCollectivePlan(envelope: CurrentCollectiveOperationPrepareEnvelope, reobservation: CurrentWalletReobservation): readonly (readonly TransactionInstruction[])[] {
  const release = currentGovernedWriteReleaseV1();
  const strip = (manifest: CurrentWalletInstructionManifest): CurrentWalletInstructionManifest => {
    if (manifest.programId !== reobservation.governance.targetProgram.toBase58()) return manifest;
  ...
  return Object.freeze(rebuilt.map((batch,index)=>{
    ...
    if (batch.length !== original.length) walletError("CURRENT_WALLET_GOVERNED_BATCH_INVALID", "reconstructed batch length changed");
    ...
    for (const instruction of original) for (const meta of instruction.keys) {
      if (meta.pubkey.equals(reobservation.governance.gateAddress) && (meta.isSigner || meta.isWritable)) walletError("CURRENT_WALLET_GATE_PRIVILEGES", "transaction elevates the governance gate");
    }
    const governed = original.filter(instruction=>instruction.programId.equals(reobservation.governance.targetProgram));
    if (governed.length) inspectGovernedInstructionBatchV1({instructions:governed, context:reobservation.governance, recognizedInstructionTags:release.assignedInstructionTags});
