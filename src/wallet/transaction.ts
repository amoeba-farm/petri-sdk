...
export function compileCurrentWalletUnsignedBatch(
  owner: PublicKey,
  recentBlockhash: string,
  instructions: readonly TransactionInstruction[],
  lookupTables: readonly AddressLookupTableAccount[] = [],
): CompiledCurrentWalletUnsignedBatch {
  ...
  try {
    ...
    if (error instanceof RangeError || (error instanceof Error && /(?:overrun|too large)/iu.test(error.message))) {
      walletError(
        "CURRENT_WALLET_TRANSACTION_TOO_LARGE",
        `unsigned transaction exceeds ${CURRENT_WALLET_MAX_TRANSACTION_BYTES} bytes`,
      );
    }
