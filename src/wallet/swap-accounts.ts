...
export function decodeCurrentWalletDlmmPool(
  address: PublicKey,
  ...
  const expectedBump = PublicKey.findProgramAddressSync(
    [TEXT.encode("ameba-spread-v2"), TEXT.encode("ameba-dlmm-pool-v1"), market.toBytes()],
    CURRENT_PROGRAM_ID,
  )[1];
...
export function decodeCurrentWalletDlmmPage(
  address: PublicKey,
  accounts: ReadonlyMap<string, AccountInfo<Uint8Array> | null>,
): CurrentWalletDlmmPage {
  const { reader, bump } = start(
    requireProgramAccount(accounts, address, 602, "DLMM page"),
    PAGE_DISCRIMINATOR,
    PAGE_ACCOUNT_DISCRIMINATOR,
    "DLMM page",
  );
  ...
  for (let index = 0; index < BINS_PER_PAGE; index += 1) {
    if (quoteReserve[index]! > 0n) expectedBid = (expectedBid | (1 << index)) >>> 0;
    if (optionReserve[index]! > 0n) expectedAsk = (expectedAsk | (1 << index)) >>> 0;
  }
...
function start(
  ...
  inner: Uint8Array,
  label: string,
): { readonly reader: Reader; readonly bump: number } {
  if (!equalBytes(bytes.subarray(0, 8), outer)) invalid(`${label} outer discriminator is invalid`);
  const reader = new Reader(bytes.subarray(8));
  if (reader.u8() !== 1) invalid(`${label} is uninitialized or terminal`);
...
function bitmapPages(bitmap: bigint): number[] {
  const result: number[] = [];
  for (let index = 0; index < 64; index += 1) if ((bitmap & (1n << BigInt(index))) !== 0n) result.push(index);
  return result;
}
...
class Reader {
  ...
  u16(): number { const value = readU16Le(this.#bytes, this.#offset); this.#offset += 2; return value; }
  u32(): number { const value = readU32Le(this.#bytes, this.#offset); this.#offset += 4; return value; }
  u64(): bigint { const value = readU64Le(this.#bytes, this.#offset); this.#offset += 8; return value; }
