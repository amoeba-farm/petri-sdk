...
export function sanitizeDisplayText(value: string, maximumCharacters = 16_384): string {
...
export class AmebaCurrentStateUnavailableError extends AmebaBackendError {
  readonly wait: CurrentStateUnavailableWait = CURRENT_STATE_UNAVAILABLE_WAIT;
...
export class AmebaTransportError extends AmebaSdkError {
  constructor(message: string, options: { cause?: unknown; context?: AmebaErrorContext } = {}) {
    super(message, {
...
export class AmebaTimeoutError extends AmebaSdkError {
  ...
  constructor(
    timeoutMs: number,
    options: { cause?: unknown; context?: AmebaErrorContext } = {},
  ) {
...
export class AmebaAbortError extends AmebaSdkError {
  ...
  constructor(reason: unknown, options: { cause?: unknown; context?: AmebaErrorContext } = {}) {
...
export class AmebaCurrentBoundaryError extends AmebaProtocolError {
  override readonly code: CurrentBoundaryErrorCode;

  constructor(
    code: CurrentBoundaryErrorCode,
    message: string,
    options: AmebaCurrentBoundaryErrorOptions = {},
  ) {
...
export function createCurrentBoundaryError(
  code: CurrentBoundaryErrorCode,
  message: string,
  options: AmebaCurrentBoundaryErrorOptions = {},
): AmebaCurrentBoundaryError {
  return new AmebaCurrentBoundaryError(code, message, options);
}

export function isCurrentBoundaryErrorCode(value: unknown): value is CurrentBoundaryErrorCode {
  return (
    typeof value === "string" &&
    (CURRENT_BOUNDARY_ERROR_CODES as readonly string[]).includes(value)
  );
}
...
export class AmebaProtocolLayoutError extends AmebaProtocolError {
  ...
  constructor(accountKind: string, reason: string, options: { cause?: unknown; details?: JsonValue } = {}) {
