...
function object(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("G3 object required");
...
function key(value: unknown): PublicKey {
  if (typeof value !== "string") throw new Error("G3 address required");
  const result = new PublicKey(value);
  if (result.toBase58() !== value) throw new Error("G3 noncanonical address");
  return result;
}
function integer(value: unknown): bigint {
  if (typeof value !== "string" || value.length > 20 || !/^(0|[1-9][0-9]*)$/u.test(value)) throw new Error("G3 decimal required");
  const result = BigInt(value);
  if (result > 0xffffffffffffffffn) throw new Error("G3 integer overflow");
  return result;
}
export interface G3RawAccount { address: string; owner: string; executable: boolean; dataBase64: string; observedSlot: number }
export interface G3ResolvedIntent { input: G3OperationInput; observedSlot: number; rawAccounts: G3RawAccount[] }

/** Public intents contain addresses and canonical decimal scalars, never decoded pool/receipt facts. */
export async function resolveG3UserIntent(connection: Connection, ownerText: string,
  operation: G3UserOperation, request: unknown, minimumContextSlot = 0, network: "local-test" | "mainnet-beta" = "local-test"): Promise<G3ResolvedIntent> {
  const programId = new PublicKey(network === "mainnet-beta" ? MAINNET_PROFILE.programId : G3_INTEGRATION_LOCK.profile.programId);
  const owner = key(ownerText), r = object(request), rawAccounts: G3RawAccount[] = [];
  if (!Number.isSafeInteger(minimumContextSlot) || minimumContextSlot < 0) throw new Error("Invalid G3 observation floor");
  let observedSlot = minimumContextSlot;
  async function read(address: PublicKey) {
  ...
  if (operation === "order" && r.kind === "swap") {
    ...
    return { input: { operation, builderInput } as G3OperationInput, observedSlot, rawAccounts };
  }
  if (operation === "order") {
    ...
    for (const [name, value] of Object.entries(supplied)) {
      if (name === "reservePages") {
        if (!Array.isArray(value) || value.length > 8) throw new Error("G3 page window exceeded");
        accounts[name] = value.map(key);
      } else accounts[name] = key(value);
    }
    if (!(accounts.trader as PublicKey).equals(owner)) throw new Error("G3 trader mismatch");
    const poolKey = accounts.pool as PublicKey;
    const pool = decodeAmoebaDlmmPool(poolKey, (await read(poolKey)).data, programId);
    const action = object(r.request), kind = action.kind;
    const shapes: Record<string, string[]> = { initialize: [], closeBook: [],
    ...
    if (typeof kind !== "string" || !Object.hasOwn(shapes, kind)) throw new Error("G3 order action invalid");
    exact(action, ["kind", ...shapes[kind]!]);
    const parsed: Record<string, unknown> = { ...action };
    for (const name of ["expectedSequence", "sequence", "limitPriceAtoms", "quantityAtoms", "amountIn", "minimumAmountOut", "deadlineTs"])
      if (Object.hasOwn(parsed, name)) parsed[name] = integer(parsed[name]);
    if (!Array.isArray(r.orderRecords) || r.orderRecords.length > 24) throw new Error("G3 order witness window exceeded");
    ...
    if (["cancel", "claim", "close", "match", "swap"].includes(kind)) for (const address of orderRecords) await read(address);
    builderInput = { accounts, pool, request: parsed, orderRecords };
  } else {
    const fields: Record<string, string[]> = { receipt_contribute: ["sleeve", "nonce", "amountAtoms"],
    ...
    if (!Object.hasOwn(fields, operation)) throw new Error("G3 receipt operation invalid");
    ...
    switch (operation) {
      case "receipt_contribute": builderInput = { ...context, owner, nonce: integer(r.nonce), amountAtoms: integer(r.amountAtoms) }; break;
      case "receipt_expire": builderInput = { ...context, cranker: owner }; break;
      case "receipt_transfer": builderInput = { lot, newOwner: key(r.newOwner) }; break;
      case "receipt_split": builderInput = { lot, newOwner: key(r.newOwner), nonce: integer(r.nonce), principalAtoms: integer(r.principalAtoms) }; break;
      case "receipt_claim": builderInput = { ...context, lot }; break;
      case "receipt_close": builderInput = { lot }; break;
    }
  }
  return { input: { operation, builderInput } as G3OperationInput, observedSlot, rawAccounts };
}
