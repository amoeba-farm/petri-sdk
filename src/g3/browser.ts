...
export function g3BusinessInstructions(operation: string, instructions: readonly TransactionInstruction[]) {
  if (operation !== "order") {
    if (instructions.length !== 1) throw new Error("G3 receipt requires one instruction");
    return instructions;
  }
  ...
  if (instructions.length !== 2 || !budget?.programId.equals(expected.programId) || budget.keys.length !== 0
    || !Buffer.from(budget.data).equals(expected.data)) throw new Error("G3 order compute budget mismatch");
  return instructions.slice(1);
}
import { assertAmoebaGovernanceEnvelopeV1, RETIRED_AMOEBA_INSTRUCTION_TAGS,
  type GovernanceGateContextV1 } from "@amoeba/spread-release-tools/governance-gate";
...
export function inspectG3Instruction(instruction: TransactionInstruction, context: GovernanceGateContextV1) {
  const { legacyData: data, legacyKeys: accounts } = assertAmoebaGovernanceEnvelopeV1(instruction, context);
  const tag = data[0]!;
  if (RETIRED_AMOEBA_INSTRUCTION_TAGS.includes(tag)) throw new Error("G3_OPERATION_RETIRED");
  if (!currentTags.has(tag)) throw new Error("G3_INSTRUCTION_UNASSIGNED_OR_DISABLED");
  const quantities: Record<string,string> = {};
  let family: G3OperationFamily = "current-protocol";
  let action: string = `tag-${tag}`;
  if (tag === 160) {
    ...
    if (selector < 1 || selector > 6 || data.length !== lengths[selector]) throw new Error("invalid receipt selector/payload");
    ...
    if (selector===1 || selector===3) {
      ...
      if (quantities.principalAtoms==="0") throw new Error("zero receipt principal");
    }
  } else if (tag === 188) {
    family = "public-order";
    ...
    const selector = data[1]!;
    if (selector > 7 || data.length !== lengths[selector]) throw new Error("invalid public-order selector/payload");
    if (selector === 1 && (data[10]! > 1 || data[21]! > 1)) throw new Error("invalid order side/post-only flag");
    action = names[selector]!;
    if (selector===1) {
      ...
      if(quantities.quantityAtoms==="0") throw new Error("zero order quantity");
    }
    if(selector===5 && (data[2]!>1 || data[3]!<1 || data[3]!>8)) throw new Error("invalid matching window");
    if(selector===6) {
      if(data[2]!>1) throw new Error("invalid swap direction");
      ...
      if(quantities.amountIn==="0" || quantities.minimumAmountOut==="0") throw new Error("zero swap amount");
    }
  } else if (tag === 30 && data[1] === 18) {
    family = "oracle-council";
    if (data.length !== 113 || !Buffer.from(data.subarray(2, 6)).equals(Buffer.from("CV01"))) throw new Error("invalid council payload");
    if(data[6]!>3 || data[7]!>3 || data[112]!>2) throw new Error("invalid council selector");
  ...
  return Object.freeze({ generation: 3 as const, family, action, tag,
    businessData: Buffer.from(data), accounts, quantities:Object.freeze(quantities), protocolFeeAtoms: "0" as const });
}
