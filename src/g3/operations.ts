...
import { PublicKey, TransactionMessage, VersionedTransaction, AddressLookupTableAccount,
  AddressLookupTableProgram, ComputeBudgetProgram, type Connection } from "@solana/web3.js";
import type { CurrentGovernedBuilderInputV1 } from "../protocol/current-governed-write.js";
import { G3_INTEGRATION_LOCK, observeG3Candidate, observeG3Mainnet, buildG3CandidateInstruction,
  inspectG3CandidateInstructions } from "./runtime.js";
...
export type G3OperationInput = { [K in G3UserOperation]: {
  operation: K; builderInput: Omit<CurrentGovernedBuilderInputV1<(typeof builders)[K]>, "programId">;
} }[G3UserOperation];
export interface G3OperationPlan {
  schemaVersion: 3; operation: G3UserOperation; owner: string;
  profileSha256: string; sourceCommit: string; epoch: string; observedSlot: number;
  blockhash: string; lastValidBlockHeight: number; serializedTransactionBase64: string;
  messageSha256: string; preparedPlanDigest: string; operationId: string;
  lookupTable?: { address: string; dataBase64: string };
}
const sha = (value: string | Uint8Array) => createHash("sha256").update(value).digest("hex");
function commitment(plan: Omit<G3OperationPlan, "operationId" | "preparedPlanDigest">) {
...
async function readLookup(connection: Connection, address: PublicKey, floor: number) {
  ...
  if (read.context.slot < floor || !read.value || read.value.executable
    || !read.value.owner.equals(AddressLookupTableProgram.programId)) throw new Error("G3 lookup table identity mismatch");
  const table = new AddressLookupTableAccount({ key: address, state: AddressLookupTableAccount.deserialize(read.value.data) });
  if (!table.isActive() || table.state.lastExtendedSlot >= read.context.slot) throw new Error("G3 lookup table is not active and mature");
...
export async function prepareG3UserOperation(connection: Connection, owner: PublicKey,
  input: G3OperationInput, minimumContextSlot: number, lookupTableAddress?: PublicKey, network: "local-test" | "mainnet-beta" = "local-test"): Promise<Readonly<G3OperationPlan>> {
  if (!Object.hasOwn(builders, input.operation)) throw new Error("G3 operation is not supported");
  ...
  if (business.length !== 1) throw new Error("G3 user action requires one canonical instruction");
  ...
  if (!signers.length || signers.some(meta => !meta.pubkey.equals(owner))) throw new Error("G3 operation owner mismatch");
  ...
  if (lifetime.context.slot < context.observedSlot || !Number.isSafeInteger(lifetime.value.lastValidBlockHeight)
    || lifetime.value.lastValidBlockHeight < 0) throw new Error("G3 transaction lifetime is invalid");
  ...
  if (bytes.length > 1232) throw new Error("G3 action exceeds packet size; an admitted lookup-table plan is required");
  const fields: Omit<G3OperationPlan, "operationId" | "preparedPlanDigest"> = {
...
export async function revalidateG3UserOperation(connection: Connection, plan: G3OperationPlan,
  ...
  if (context.epoch !== plan.epoch) throw new Error("G3 gate epoch changed; prepare a new operation");
  ...
  if (lookup && lookup.dataBase64 !== plan.lookupTable!.dataBase64) throw new Error("G3 lookup table changed; prepare again");
  ...
  const expected = { receipt_contribute: "contribute", receipt_transfer: "transfer", receipt_split: "split",
    receipt_claim: "claim", receipt_close: "close", receipt_expire: "expire-never-active" } as const;
  ...
  return Object.freeze({ ...facts, validated: true as const, finalizedObservationSlot: context.observedSlot });
}
