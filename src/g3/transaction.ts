...
export async function validateG3SignedMessage(expected: { serializedTransactionBase64: string; owner: string; blockhash: string },
  signedTransactionBase64: string) {
  const decode = (value: string) => {
    if (typeof value !== "string" || value.length > 1644) throw new Error("G3 packet is oversized");
    const bytes = Buffer.from(value, "base64");
    if (!bytes.length || bytes.length > 1232 || bytes.toString("base64") !== value) throw new Error("G3 packet is not canonical base64");
  ...
  const key = await globalThis.crypto.subtle.importKey("raw", new Uint8Array(signed.message.staticAccountKeys[0]!.toBytes()),
