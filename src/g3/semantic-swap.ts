...
export async function resolveG3SemanticSwap(owner: PublicKey, request: Record<string, unknown>, programId: PublicKey,
  read: (address: PublicKey) => Promise<AccountInfo<Buffer>>) {
  ...
  if (Object.keys(request).length !== fields.length || fields.some(k => !Object.hasOwn(request, k))
    || request.kind !== "swap" || !["QuoteForOption", "OptionForQuote"].includes(String(request.direction))) throw new Error("G3 semantic swap fields invalid");
  const integer = (field: string) => {
    const value = request[field];
    if (typeof value !== "string" || !/^[1-9][0-9]{0,19}$/.test(value) || BigInt(value) > 0xffffffffffffffffn) throw new Error("G3 semantic swap amount invalid");
    return BigInt(value);
  };
  if (typeof request.market !== "string") throw new Error("G3 market required");
  const marketAddress = new PublicKey(request.market);
  if (marketAddress.toBase58() !== request.market) throw new Error("G3 market noncanonical");
  ...
  if (!pool.ordersEnabled || pool.expiryTs !== market.expiryTs || !pool.quoteMint.equals(market.collateralMint)
    ...
    || !market.longContractMint || !pool.optionMint.equals(market.longContractMint)) throw new Error("G3 order market binding invalid");
  ...
  if (!sleeve.settlementGroup.equals(group) || !sleeve.settlementMint.equals(pool.quoteMint)) throw new Error("G3 sleeve binding invalid");
  const reservePages: PublicKey[] = [];
  ...
  for (let i = 0; i < 64; i++) if ((routeBitmap[0]! & (1n << BigInt(i))) !== 0n) reservePages.push(deriveAmoebaDlmmBinPagePda(poolAddress, i, programId)[0]);
  if (reservePages.length > 8) throw new Error("G3 semantic swap page window exceeded");
  if (request.direction === "OptionForQuote") reservePages.reverse();
  for (const page of reservePages) await read(page);
  ...
  const orderRecords: PublicKey[] = [];
  const seen = new Set<string>();
  for (const [side,head] of [["Bid",book.bidHead],["Ask",book.askHead]] as const) {
  let sequence = head;
  let previous = 0n;
  while (sequence !== 0n) {
    if (orderRecords.length >= 24 || seen.has(sequence.toString())) throw new Error("G3 semantic swap record window exceeded");
    ...
    if (record.side !== side || record.previous !== previous) throw new Error("G3 order linked list invalid");
    orderRecords.push(address); previous = sequence; sequence = record.next;
  }
  }
  return { pool, orderRecords, request: { kind: "swap" as const, direction: request.direction as "QuoteForOption" | "OptionForQuote",
