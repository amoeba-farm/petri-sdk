...
export function decodeG3Market(address: PublicKey, data: Buffer, programId: PublicKey) {
  if (data.length !== 279 || data[0] !== 1 || data[98] !== 1 || data[203]! > 1 || data[204] !== 0
    ...
    || data.subarray(252,255).some(b=>b!==0)) throw new Error("G3 market layout invalid");
  const marketId=data.subarray(34,66), underlyingId=data.subarray(131,163), expiryTs=data.readBigUInt64LE(163);
  if (!deriveMarketPda(marketId,programId).equals(address)) throw new Error("G3 market PDA invalid");
  if (PublicKey.findProgramAddressSync([Buffer.from("ameba-spread-v2"),Buffer.from("g3-market"),marketId],programId)[1] !== data[1]) throw new Error("G3 market bump invalid");
  ...
  if (new PublicKey(data.subarray(2,34)).equals(PublicKey.default) || expiryTs===0n || size===0n || width<=0n
    ...
    || data.readBigUInt64LE(271)>data.readBigUInt64LE(263) || data.readBigUInt64LE(263)>data.readBigUInt64LE(255)) throw new Error("G3 market invariants invalid");
