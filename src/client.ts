...
function decodeChainIdentityData(data: JsonObject, context: ValidationContext, path: string): void {
  ...
  if (program.programDataBytes !== CURRENT_LIVE_DEPLOYMENT.programDataAccountBytes) fail(`${programPath}.programDataBytes`, "expected the exact live rc.44 ProgramData allocation", program.programDataBytes, context);
  ...
  if (program.payloadBytes !== CURRENT_LIVE_DEPLOYMENT.programDataPayloadBytes) fail(`${programPath}.payloadBytes`, "expected the pinned rc.44 payload length", program.payloadBytes, context);
  exactString(program.payloadSha256, CURRENT_PROGRAM_PAYLOAD_SHA256, `${programPath}.payloadSha256`, context);

  if (Object.hasOwn(program,"rawAccountSha256")) exactString(program.rawAccountSha256,CURRENT_LIVE_DEPLOYMENT.programDataAccountSha256,`${programPath}.rawAccountSha256`,context);
