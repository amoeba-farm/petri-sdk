import { validateCurrentWriterDlmmRequest, type CurrentWriterDlmmPolicyOperation, type CurrentWriterLiquidityPolicyBeginRequest, type CurrentWriterLiquidityPolicyAppendRequest, type CurrentWriterLiquidityPolicySealRequest } from "./protocol/writer-dlmm-public.js";
import { validateCurrentWriterLiquidityRequest, type CurrentWriterLiquidityOperation, type CurrentWriterLiquidityInitializeRequest, type CurrentWriterLiquidityAddRequest, type CurrentWriterLiquidityRemoveRequest, type CurrentWriterLiquiditySweepRequest } from "./protocol/writer-dlmm-public.js";
...
import {
  CURRENT_LIGHT_PROGRAM_DEPLOYMENTS,
  type CurrentLightProgramDeployment,
} from "./protocol/current-light-deployments.js";
import {
  AmebaAbortError,
  AmebaBackendError,
  AmebaCurrentStateUnavailableError,
  AmebaInputError,
  AmebaResponseValidationError,
  AmebaTimeoutError,
  AmebaTransportError,
  AmebaUnsupportedCapabilityError,
} from "./errors.js";
import type {
  AmebaClientOptions,
  AmebaResponseMetadata,
  ...
  MarketsResponse,
  OracleDraftPrepareRequest,
  OracleDraftPrepareResponse,
  OracleHistoryResponse,
  OracleLatestResponse,
  OracleMarketResponse,
  OracleMarketsResponse,
  OracleStateResponse,
  ProgramRegistryResponse,
  RequestControl,
  CurrentCollectiveOperationStatusResponse,
  CurrentCollectiveOperationSubmitRequest,
  CurrentCollectiveOperationSubmitResponse,
} from "./types.js";
...
class BackendTransport {
  ...
  requireOperator(capability: string): void {
    if (this.clientId !== "ameba-operator-sdk") {
      throw new AmebaInputError(
        `${capability} requires operator client mode; use AmebaOperatorClient`,
      );
    }
  }
  ...
  private async request<T>(
    ...
    try {
      ...
      if (cause instanceof AmebaResponseValidationError) throw cause;
      if (options.signal?.aborted) {
        throw new AmebaAbortError(options.signal.reason, { cause, context });
      }
      if (timedOut) throw new AmebaTimeoutError(timeoutMs, { cause, context });
      throw new AmebaTransportError(
        `Ameba backend request failed: ${errorMessage(cause)}`,
        { cause, context },
      );
    } finally {
    ...
    try {
      ...
      throw new AmebaBackendError(
        `Ameba backend returned HTTP ${response.status} with a non-JSON response`,
        {
    ...
    if (!response.ok || isBackendFailure(payload)) {
      ...
      if (backend.code === "CURRENT_STATE_UNAVAILABLE") {
        if (
          ...
          throw new AmebaResponseValidationError(
            "$",
            "CURRENT_STATE_UNAVAILABLE must use the frozen 503 body and no-store JSON headers",
...
export class AmebaClient {
  ...
  constructor(options: AmebaClientOptions = {}) {
    ...
    this.writerOperations = new CollectiveOperationApi(
      transport,
      "/dlmm/writer-sleeves",
      WRITER_OPERATION_KINDS,
    );
    this.trades = new CollectiveOperationApi(
      transport,
      "/dlmm/trades",
      TRADE_OPERATION_KINDS,
    );
...
export class ChainApi {
  ...
  identity(control: RequestControl = {}): Promise<ChainIdentityResponse> {
    return this.transport.get(
      "/chain/identity",
      currentResponse<ChainIdentityResponse>(decodeChainIdentityData),
      control,
    );
  }
}
...
export class CollectiveOperationApi {
  constructor(
    ...
    private readonly routeBase: "/dlmm/writer-sleeves" | "/dlmm/trades",
  ...
  liquidity(request: CurrentWriterLiquidityReadRequest, control: RequestControl = {}): Promise<CurrentWriterLiquidityResponse> {
    if (this.routeBase !== "/dlmm/writer-sleeves") throw new AmebaInputError("Writer liquidity reads require writerOperations");
    if (!request || typeof request !== "object" || Array.isArray(request) || Object.hasOwn(request, "operation")) throw new AmebaInputError("Writer liquidity read identity is invalid");
    const identity = validateCurrentWriterLiquidityRequest({ ...request, operation: "writer_liquidity_initialize" });
    return this.transport.get(`${this.routeBase}/${identity.sleeve}/liquidity`, currentResponse<CurrentWriterLiquidityResponse>(
      (data, context) => decodeCurrentWriterLiquidity(data, identity, context)),
      { ...control, query: { owner: identity.owner, seriesIndex: identity.seriesIndex } });
  }

  refunds(request: CurrentWriterRefundsRequest, control: RequestControl = {}): Promise<CurrentWriterRefundsResponse> {
    if (this.routeBase !== "/dlmm/writer-sleeves") throw new AmebaInputError("Writer refund reads require writerOperations");
    rejectUnknownInputFields(request, ["owner", "cursor", "limit"], "writer refunds request");
    const owner = request.owner; const cursor = request.cursor; const limit = request.limit ?? 16;
    validateInputBase58(owner, "owner", 32, 44, 32);
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 32) throw new AmebaInputError("Writer refund limit must be1..32");
    ...
    return this.transport.get(`${this.routeBase}/refunds`, currentResponse<CurrentWriterRefundsResponse>(
      (data, context) => decodeCurrentWriterRefunds(data, owner, limit, context)),
      { ...control, query: { owner, cursor, limit } });
  }
  ...
  private prepareLiquidity(operation: CurrentWriterLiquidityOperation, action: "initialize" | "add" | "remove" | "sweep",
    request: CurrentWriterLiquidityInitializeRequest | CurrentWriterLiquidityAddRequest | CurrentWriterLiquidityRemoveRequest | CurrentWriterLiquiditySweepRequest,
    control: RequestControl): Promise<CurrentWriterFundsPrepareResponse> {
    if (this.routeBase !== "/dlmm/writer-sleeves") throw new AmebaInputError("Writer liquidity preparation requires writerOperations");
    if (!request || typeof request !== "object" || Array.isArray(request) || Object.hasOwn(request, "operation")) throw new AmebaInputError("Writer liquidity operation is selected by the method");
    const intent = validateCurrentWriterLiquidityRequest({ ...request, operation });
    const { operation: _operation, ...body } = intent;
    return this.transport.post(`${this.routeBase}/liquidity/${action}/prepare`, body,
      (payload, context) => decodeCurrentWriterPrepareResponse(payload, intent, context), control);
  }
  ...
  private prepareLiquidityPolicy(operation: CurrentWriterDlmmPolicyOperation, action: "begin" | "append" | "seal",
    request: CurrentWriterLiquidityPolicyBeginRequest | CurrentWriterLiquidityPolicyAppendRequest | CurrentWriterLiquidityPolicySealRequest,
    control: RequestControl): Promise<CurrentWriterFundsPrepareResponse> {
    if (this.routeBase !== "/dlmm/writer-sleeves") throw new AmebaInputError("Writer liquidity policy preparation requires writerOperations");
    this.transport.requireOperator("writer liquidity policy preparation");
    if (!request || typeof request !== "object" || Array.isArray(request) || Object.hasOwn(request, "operation")) throw new AmebaInputError("Writer liquidity policy operation is selected by the method");
    const intent = validateCurrentWriterDlmmRequest({ ...request, operation });
    const { operation: _operation, ...body } = intent;
    return this.transport.post(`${this.routeBase}/liquidity/policy/${action}/prepare`, body,
      (payload, context) => decodeCurrentWriterPrepareResponse(payload, intent, context), control);
  }
  ...
  private prepareFunds(operation: "auction_refund" | "withdraw_principal", path: "refunds" | "withdrawals",
    request: CurrentWriterRefundPrepareRequest | CurrentWriterWithdrawalPrepareRequest, control: RequestControl): Promise<CurrentWriterFundsPrepareResponse> {
    if (this.routeBase !== "/dlmm/writer-sleeves") throw new AmebaInputError("Writer funds preparation requires writerOperations");
    ...
    const captured = Object.freeze(Object.fromEntries(fields.map(field => [field, (request as unknown as Record<string, string>)[field]])));
    for (const field of fields) {
      if (field === "amountAtoms") validateInputPositiveU64(captured[field], field);
      else validateInputBase58(captured[field]!, field, 32, 44, 32);
    }
    return this.transport.post(`${this.routeBase}/${path}/prepare`, captured,
      (payload, context) => decodeCurrentWriterPrepareResponse(payload, { operation, ...captured }, context), control);
  }

  submit(
    request: CurrentCollectiveOperationSubmitRequest,
    control: RequestControl = {},
  ): Promise<CurrentCollectiveOperationSubmitResponse> {
    ...
    return this.transport.post(
      `${this.routeBase}/submit`,
      request,
      currentResponse<CurrentCollectiveOperationSubmitResponse>(
        decodeCurrentCollectiveOperationSubmitData,
      ),
      control,
    );
  }
  ...
  status(
    operationId: string,
    control: RequestControl = {},
  ): Promise<CurrentCollectiveOperationStatusResponse> {
    const canonicalOperationId = lowercaseSha256Input(operationId, "operationId");
    return this.transport.get(
      `${this.routeBase}/status/${canonicalOperationId}`,
      currentResponse<CurrentCollectiveOperationStatusResponse>(
        decodeCurrentCollectiveOperationStatusData(this.operationKinds),
      ),
      control,
    );
  }
...
export class ProgramRegistryApi {
  ...
  get(control: RequestControl = {}): Promise<ProgramRegistryResponse> {
    return this.transport.get(
      ...
      currentResponse<ProgramRegistryResponse>(decodeProgramRegistryData),
      control,
    );
  }
}
...
export class MarketsApi {
  ...
  list(control: RequestControl = {}): Promise<MarketsResponse> {
    return this.transport.get(
      "/dlmm/markets",
      currentResponse<MarketsResponse>(decodeMarketsData),
      control,
    );
  }
  ...
  snapshot(
    ...
    return this.transport.get(
      `/dlmm/markets/${encodeURIComponent(marketId)}/snapshot`,
      currentResponse<MarketSnapshotResponse>(decodeMarketSnapshotData(marketId)),
      options,
    );
  }
  ...
  chart(market: string, options: ChartRequest = {}): Promise<ChartResponse> {
    ...
    return this.transport.get(
      path,
      currentResponse<ChartResponse>(decodeChartData),
      {
        ...options,
        query: {
          windowMs,
...
export class ContractsApi {
  show(): never {
    throw new AmebaUnsupportedCapabilityError(
      "contracts.show",
      "the current backend exposes the canonical market snapshot but no single-contract route",
    );
  }
  ...
  quote(): never {
    throw new AmebaUnsupportedCapabilityError(
      "contracts.quote",
      "use the canonical market snapshot; the SDK does not invent a DLMM fair price",
    );
  }
}
...
export class HistoryApi {
  ...
  ledger(request: HistoryRequest): Promise<LedgerResponse> {
    ...
    return this.transport.get(
      `/users/${encodeURIComponent(owner)}/ledger`,
      currentResponse<LedgerResponse>(decodeLedgerData(owner)),
      {
        ...request,
        query: {
          limit:
...
export class OracleApi {
  ...
  state(control: RequestControl = {}): Promise<OracleStateResponse> {
    return this.transport.get(
      "/dlmm/oracle/state",
      currentResponse<OracleStateResponse>(decodeOracleStateData),
      control,
    );
  }
  ...
  markets(
    marketOrControl: string | RequestControl = {},
    maybeControl: RequestControl = {},
  ): Promise<OracleMarketsResponse | OracleMarketResponse> {
    if (typeof marketOrControl === "string") {
      const marketId = validateInputCurrentProductId(marketOrControl, "market");
      return this.transport.get(
        `/dlmm/oracle/markets/${encodeURIComponent(marketId)}`,
        currentResponse<OracleMarketResponse>(decodeOracleMarketData(marketId)),
        maybeControl,
      );
    }
    return this.transport.get(
      "/dlmm/oracle/markets",
      currentResponse<OracleMarketsResponse>(decodeOracleMarketsData),
      marketOrControl,
    );
  }
  ...
  latest(market?: string, control: RequestControl = {}): Promise<OracleLatestResponse> {
    ...
    return this.transport.get(
      path,
      currentResponse<OracleLatestResponse>(decodeOracleLatestData(marketId)),
      control,
    );
  }
  ...
  history(market?: string, control: RequestControl = {}): Promise<OracleHistoryResponse> {
    ...
    return this.transport.get(
      path,
      currentResponse<OracleHistoryResponse>(decodeOracleHistoryData(marketId)),
      control,
    );
  }
  ...
  prepareDraft(
    request: OracleDraftPrepareRequest,
    control: RequestControl = {},
  ): Promise<OracleDraftPrepareResponse> {
    let normalizedRequest: OracleDraftPrepareRequest;
    try {
      normalizedRequest = validateCurrentOracleActionRequest(request);
    } catch (cause) {
      throw new AmebaInputError(
        cause instanceof Error ? cause.message : "current Oracle action request is invalid",
      );
    }
    return this.transport.post(
      "/dlmm/oracle/drafts/prepare",
      normalizedRequest,
      currentResponse<OracleDraftPrepareResponse>(decodeOracleDraftPrepareData(normalizedRequest)),
      control,
    );
  }
}

type ValidationContext = { method: HttpMethod; url: string };

/** Validate the public wrapper, then retain the exact portable material for independent wallet review. */
function decodeCurrentWriterPrepareResponse(payload: JsonValue, expectedIntent: unknown, context: ValidationContext): CurrentWriterFundsPrepareResponse {
  let portable: CurrentWriterFundsPrepareResponse | undefined;
  currentResponse((data) => {
    ...
    portable = material as unknown as CurrentWriterFundsPrepareResponse;
  })(payload, context);
  return portable!;
}

function decodeCurrentWriterLiquidity(payload: JsonValue, expected: CurrentWriterLiquidityReadRequest, context: ValidationContext): void {
  const path = "$.data";
  ...
  for (const field of ["market", "pool", "policyAddress", "positionAddress", "policyAuthority", "managementAuthority"] as const) {
    if (field === "managementAuthority" && root[field] === null) continue;
    if (base58Address(root[field], `${path}.${field}`, context) === SYSTEM_PROGRAM_ID) fail(`${path}.${field}`, "nonzero address required", root[field], context);
  }
  ...
  if (root.reasonCode !== null) nonemptyStringValue(root.reasonCode, `${path}.reasonCode`, context);
  if ((canManage && (lifecycle !== "sealed" || root.managementAuthority !== expected.owner))
    || (lifecycle === "sealed" && root.managementAuthority === null)) fail(path, "management authority and policy lifecycle disagree", root, context);
  ...
  for (const field of amounts) canonicalU64(accounting[field], `${path}.accounting.${field}`, context);
  for (const field of nullableAmounts) if (accounting[field] !== null) canonicalU64(accounting[field], `${path}.accounting.${field}`, context);
  if (root.budget !== null) {
    ...
    for (const field of budgetAmounts) canonicalU64(budget[field], `${path}.budget.${field}`, context);
    if (BigInt(budget.reserveReleaseSpendRatioPpm as string) > 1_000_000n) fail(`${path}.budget.reserveReleaseSpendRatioPpm`, "ratio exceeds one million ppm", budget.reserveReleaseSpendRatioPpm, context);
  ...
  if (bins.length > 32 || (!initialized && bins.length !== 0)) fail(`${path}.bins`, "position bin inventory is invalid", root.bins, context);
  let previous = 0;
...
function decodeCurrentWriterRefunds(payload: JsonValue, expectedOwner: string, limit: number, context: ValidationContext): void {
  ...
  if (rows.length > limit) fail("$.data.rows", "refund page exceeds requested limit", root.rows, context);
  ...
  rows.forEach((value, index) => {
    ...
    if (identities.has(bid)) fail(`${path}.bid`, "duplicate bid candidate", row.bid, context);
    identities.add(bid);
    for (const field of ["auction", "sleeve", "refundTokenAccount"] as const) {
      if (row[field] !== null) base58Address(row[field], `${path}.${field}`, context);
    }
    ...
    if (row.remainingRefundAtoms !== null) canonicalU64(row.remainingRefundAtoms, `${path}.remainingRefundAtoms`, context);
    ...
    if (row.evidenceDigest !== null) sha256Digest(row.evidenceDigest, `${path}.evidenceDigest`, context);
    if (row.reasonCode !== null) nonemptyStringValue(row.reasonCode, `${path}.reasonCode`, context);
  ...
  if (root.nextCursor !== null) {
    ...
    if (cursor.length > 2048 || rows.length === 0) fail("$.data.nextCursor", "invalid bounded continuation", root.nextCursor, context);
  }
}

type CurrentDataDecoder = (
  data: JsonObject,
...
} as const satisfies Readonly<Record<OracleDraftPrepareRequest["actionType"], number>>);
...
} as const satisfies Readonly<Record<OracleDraftPrepareRequest["actionType"], string>>);

const CURRENT_ORACLE_DIRECT_ACTIONS = new Set<OracleDraftPrepareRequest["actionType"]>([
...
function currentResponse<T>(decodeData: CurrentDataDecoder): ResponseDecoder<T> {
  return (payload, context) => {
    const root = exactObject(payload, ["ok", "data"], ["ok", "data"], "$", context);
    if (root.ok !== true) fail("$.ok", "expected true", root.ok, context);
...
function decodeCurrentFreshness(data: JsonObject, context: ValidationContext, path: string): void {
  if (Object.hasOwn(data, "freshness")) {
    ...
    for (const field of ["readReady", "tradeReady"]) {
      ...
      if (ready && (age > maximumAge || !healthy)) fail(`${freshnessPath}.${field}`, "stale or unhealthy snapshot cannot be ready", ready, context);
    }
    if (freshness.lastAttemptAt !== null) isoTimestamp(freshness.lastAttemptAt, `${freshnessPath}.lastAttemptAt`, context);
    if (freshness.lastErrorCode !== null) stringPattern(freshness.lastErrorCode, /^[A-Za-z0-9_.:-]{1,128}$/u, `${freshnessPath}.lastErrorCode`, context, "a bounded error code");
  }

}

function decodeChainIdentityData(data: JsonObject, context: ValidationContext, path: string): void {
  ...
  const mainnet = (data.protocol as JsonObject).cluster === MAINNET_PROFILE.network;
  ...
  for (const [field, expected] of Object.entries({liveReleaseLabel:live.releaseLabel, liveSourceCommit:live.artifactSourceCommit, liveReadProfileId:live.liveReadProfileId, reviewedBridgeSourceCommit:live.artifactSourceCommit, deploymentProvenance:live.provenance, writeCompatibility:live.writeCompatibility})) {
    if (Object.hasOwn(identity,field)) exactString(identity[field],expected,`${identityPath}.${field}`,context);
  }
  ...
  if (data.freshness) exactString((data.freshness as JsonObject).observedAtSlot, identity.observedSlot as string, `${path}.freshness.observedAtSlot`, context);
  ...
  if (program.programDataBytes !== live.programDataAccountBytes) fail(`${programPath}.programDataBytes`, "expected the exact selected ProgramData allocation", program.programDataBytes, context);
  ...
  if (program.payloadBytes !== live.programDataPayloadBytes) fail(`${programPath}.payloadBytes`, "expected the selected payload length", program.payloadBytes, context);
  exactString(program.payloadSha256, live.programDataPayloadSha256, `${programPath}.payloadSha256`, context);

  if (Object.hasOwn(program,"rawAccountSha256")) exactString(program.rawAccountSha256,live.programDataAccountSha256,`${programPath}.rawAccountSha256`,context);
  if (Object.hasOwn(identity,"governance")) {
    if (!mainnet) fail(`${identityPath}.governance`, "Mainnet governance cannot describe Devnet", identity.governance, context);
    ...
    for (const [field,expected] of Object.entries({network:MAINNET_PROFILE.network,genesisHash:MAINNET_PROFILE.genesisHash,
      ...
      artifactSha256:MAINNET_PROFILE.artifactSha256,gate:MAINNET_PROFILE.gate})) exactString(governance[field],expected,`${gp}.${field}`,context);
    if (![0,1,2].includes(governance.gateStatus as number) || typeof governance.observedSlot !== "number"
      || !Number.isSafeInteger(governance.observedSlot) || governance.observedSlot < MAINNET_PROFILE.deployedSlot) fail(gp,"invalid finalized gate status/slot",governance,context);
  ...
  if (!Object.hasOwn(identity,"collateral") && !Object.hasOwn(identity,"light")) return;
  ...
  if (collateral.decimals !== 6) fail(`${collateralPath}.decimals`, "current collateral mint must use exactly 6 decimals", collateral.decimals, context);
  const custodyAuthority = base58Address(collateral.custodyAuthority, `${collateralPath}.custodyAuthority`, context);
  if (custodyAuthority !== vaultConfig) fail(`${collateralPath}.custodyAuthority`, "must equal the current VaultConfig PDA", custodyAuthority, context);
  ...
  if (trees.length !== CURRENT_LIGHT_STATE_CONTEXTS.length) fail(`${lightPath}.stateTrees`, "expected the five authorized StateV2 contexts", trees, context);
...
function validateLinkedProgramIdentity(
  ...
  if (program.programDataBytes !== expected.programDataBytes) fail(`${path}.programDataBytes`, "expected the reviewed Light ProgramData allocation", program.programDataBytes, context);
  ...
  if (program.payloadBytes !== expected.payloadBytes) fail(`${path}.payloadBytes`, "expected the reviewed Light program payload length", program.payloadBytes, context);
  exactString(program.payloadSha256, expected.payloadSha256, `${path}.payloadSha256`, context);
}

function decodeProgramRegistryData(data: JsonObject, context: ValidationContext, path: string): void {
  ...
  if (programs.length !== 1) fail(`${path}.programs`, "expected the one current Spread deployment", programs, context);
  programs.forEach((program, index) => validateCurrentProtocol(program, `${path}.programs[${index}]`, context));
}

function decodeMarketsData(data: JsonObject, context: ValidationContext, path: string): void {
  ...
  markets.forEach((market, index) => {
    ...
    const marketId = objectValue(market, `${path}.markets[${index}]`, context).marketId as string;
    if (seenProducts.has(marketId)) fail(`${path}.markets[${index}].marketId`, "duplicate product market row", marketId, context);
    seenProducts.add(marketId);
  });
}

function validateMarketSummary(value: JsonValue, path: string, context: ValidationContext): void {
  ...
  for (const field of ["name", "displayName", "symbol", "title", "subtitle"] as const) {
  ...
  if (expiries.length === 0) fail(`${path}.expiries`, "current product rows require at least one exact series", expiries, context);
  expiries.forEach((expiry, index) => {
    validateMarketExpiry(expiry, marketId, `${path}.expiries[${index}]`, context);
    const seriesId = objectValue(expiry, `${path}.expiries[${index}]`, context).expiryId as string;
    if (seenSeries.has(seriesId)) fail(`${path}.expiries[${index}].expiryId`, "duplicate current series row", seriesId, context);
    seenSeries.add(seriesId);
  });
}

function validateMarketExpiry(value: JsonValue, marketId: string, path: string, context: ValidationContext): void {
  ...
  if (expiry.fairPrice !== null) {
    if (typeof expiry.fairPrice === "string") canonicalDecimalText(expiry.fairPrice, `${path}.fairPrice`, context);
    else finiteNumberValue(expiry.fairPrice, `${path}.fairPrice`, context, 0);
  }
  if (expiry.settlementUtc !== null) isoTimestamp(expiry.settlementUtc, `${path}.settlementUtc`, context);
  nonemptyStringValue(expiry.poolStatus, `${path}.poolStatus`, context);
}

function decodeMarketSnapshotData(expectedMarketId: string): CurrentDataDecoder {
...
function decodeChartData(data: JsonObject, context: ValidationContext, path: string): void {
  ...
  arrayValue(data.points, `${path}.points`, context).forEach((point, index) => {
    ...
    if (row.schemaVersion !== 1) fail(`${pointPath}.schemaVersion`, "expected current chart schema version 1", row.schemaVersion, context);
    currentProductId(row.marketId, `${pointPath}.marketId`, context);
    for (const field of ["expiryId", "expiryLabel", "settlementUtc"] as const) {
      if (row[field] !== null) fail(`${pointPath}.${field}`, "market-wide chart points require null expiry metadata", row[field], context);
    }
    ...
    ] as const) canonicalUnsignedDecimal(row[field], `${pointPath}.${field}`, context);
  });
}

function decodeCurrentCollectiveOperationSubmitData(
  data: JsonObject,
  context: ValidationContext,
  path: string,
): void {
...
function decodeCurrentCollectiveOperationStatusData(
  operationKinds: readonly string[],
): (data: JsonObject, context: ValidationContext, path: string) => void {
  return (data, context, path): void => {
    exactObject(data, ["freshness",
    ...
    enumString(
      data.status,
      ["prepared", "submitted", "pending", "confirmed", "failed"],
      `${path}.status`,
      context,
    );
    const batchCount = safeIntegerValue(data.batchCount, `${path}.batchCount`, context, 1, 256);
    const nextBatchIndex = safeIntegerValue(
      data.nextBatchIndex,
      `${path}.nextBatchIndex`,
      context,
      0,
    ...
    submissions.forEach((value, index) => {
      ...
      const batchIndex = safeIntegerValue(
        submission.batchIndex,
        `${submissionPath}.batchIndex`,
        context,
        0,
        batchCount - 1,
      );
...
function validateCurrentConfirmation(value: JsonValue | undefined, path: string, context: ValidationContext): void {
  ...
  if (confirmation.blockTime !== null) canonicalU64(confirmation.blockTime, `${path}.blockTime`, context);
}

function decodeLedgerData(expectedOwner: string): CurrentDataDecoder {
  return (data, context, path) => {
    ...
    if (owner !== expectedOwner) fail(`${path}.ledger.ownerPubkey`, "must match the requested owner", owner, context);
    const summaries = arrayValue(ledger.summaries, `${path}.ledger.summaries`, context);
    if (summaries.length > 1) fail(`${path}.ledger.summaries`, "must contain at most the canonical player-ledger balance", summaries, context);
    summaries.forEach((value, index) => {
      ...
      if (balanceOwner !== expectedOwner) fail(`${summaryPath}.balance.ownerPubkey`, "must match the requested owner", balanceOwner, context);
    ...
    if (events.length !== 0) fail(`${path}.ledger.events`, "current ledger event history is not exposed by rc.44", events, context);
  };
}

function decodeOracleStateData(data: JsonObject, context: ValidationContext, path: string): void {
  ...
  if (state.latest !== null) validateOracleFact(state.latest, `${path}.state.latest`, context);
...
function decodeOracleMarketsData(data: JsonObject, context: ValidationContext, path: string): void {
  exactObject(data, ["freshness", "protocol", "markets"], ["protocol", "markets"], path, context);
  validateOracleFacts(data.markets, `${path}.markets`, context);
}

function decodeOracleMarketData(expectedMarketId: string): CurrentDataDecoder {
  return (data, context, path) => {
    exactObject(data, ["freshness", "protocol", "market", "months", "latest", "history"], ["protocol", "market", "months", "latest", "history"], path, context);
    validateMarketSummary(data.market as JsonValue, `${path}.market`, context);
    const market = objectValue(data.market, `${path}.market`, context);
    if (market.marketId !== expectedMarketId) fail(`${path}.market.marketId`, "must match the requested product", market.marketId, context);
    validateOracleFacts(data.months, `${path}.months`, context, expectedMarketId);
    if (data.latest !== null) validateOracleFact(data.latest, `${path}.latest`, context, expectedMarketId);
    validateOracleFacts(data.history, `${path}.history`, context, expectedMarketId);
  };
}

function decodeOracleLatestData(expectedMarketId?: string): CurrentDataDecoder {
  return (data, context, path) => {
...
function decodeOracleHistoryData(expectedMarketId?: string): CurrentDataDecoder {
  return (data, context, path) => {
...
function validateOracleFacts(
  ...
  facts.forEach((fact, index) => {
    ...
    if (seen.has(key)) fail(`${path}[${index}].expiryId`, "duplicate current Oracle series fact", identity.expiryId, context);
    seen.add(key);
  });
  return facts;
}

function validateOracleFact(
  value: JsonValue | undefined,
  path: string,
  context: ValidationContext,
  expectedMarketId?: string,
): { readonly marketId: string; readonly expiryId: string } {
  ...
  if (fact.lastUpdatedSlot !== null) canonicalU64(fact.lastUpdatedSlot, `${path}.lastUpdatedSlot`, context);
  if (fact.settlementStatus !== null) jsonPrimitiveValue(fact.settlementStatus, `${path}.settlementStatus`, context);
  return Object.freeze({ marketId, expiryId });
}

function decodeOracleDraftPrepareData(
  expectedRequest: OracleDraftPrepareRequest,
): CurrentDataDecoder {
  return (data, context, path) => {
...
function validateOracleDraftPlan(
  value: JsonValue | undefined,
  expectedRequest: OracleDraftPrepareRequest,
  path: string,
  context: ValidationContext,
  ...
  validateExactJsonProjection(
    plan.request,
    redactCurrentOracleActionRequest(expectedRequest) as unknown as JsonValue,
    `${path}.request`,
    context,
  );
  ...
  const logical = validateOracleInstructionManifest(
    plan.logicalInstruction,
    { actionType: expectedRequest.actionType, tag: logicalTag, instructionName: logicalName, wrapped: false },
    `${path}.logicalInstruction`,
    context,
  );
  const outer = validateOracleInstructionManifest(
    ...
    `${path}.outerInstruction`,
    context,
  );
  const currentTags = arrayValue(plan.currentInstructionTags, `${path}.currentInstructionTags`, context);
  if (currentTags.length !== 1 || currentTags[0] !== logicalTag) fail(`${path}.currentInstructionTags`, `expected only logical Oracle tag ${logicalTag}`, currentTags, context);
  const transportTags = arrayValue(plan.transportInstructionTags, `${path}.transportInstructionTags`, context);
  if (transportTags.length !== 1 || transportTags[0] !== transportTag) fail(`${path}.transportInstructionTags`, `expected only transport tag ${transportTag}`, transportTags, context);
  ...
  if (instructions.length !== prefix.length + 1) fail(`${path}.instructions`, "Oracle instruction count differs from exact action grammar", instructions, context);
  prefix.forEach((expected, index) => validateExactJsonProjection(instructions[index], expected as unknown as JsonValue, `${path}.instructions[${index}]`, context));
  validateExactJsonProjection(instructions[instructions.length - 1], plan.outerInstruction, `${path}.instructions`, context);
  const actionInstructions: readonly ValidatedOracleManifest[] = [...prefix.map(expected => ({
    object: expected as unknown as JsonObject, programId: expected.programId, dataBase64: expected.dataBase64,
  ...
  if (setupBatches.length !== 0) fail(`${path}.setupInstructionBatches`, "current Oracle drafts have no setup instruction batches", setupBatches, context);
  const setupTransactions = arrayValue(plan.setupTransactions, `${path}.setupTransactions`, context);
  if (setupTransactions.length !== 0) fail(`${path}.setupTransactions`, "current Oracle drafts have no setup transactions", setupTransactions, context);

  const oracle = validateOraclePlanIdentity(plan.oracle, expectedRequest, `${path}.oracle`, context);
  const proof = validateOracleDraftProofFacts(
    plan.proofFacts,
    expectedRequest,
    oracle,
    logicalTag,
  ...
  let compressedWitness: JsonObject | null = null;
  let decodedOuter: DecodedOracleCompressedEnvelope | null = null;
  let lookupWitness: JsonObject | null = null;
  if (direct) {
    if (plan.compressedTransport !== null) fail(`${path}.compressedTransport`, "direct Oracle actions must not carry a tag-205 witness", plan.compressedTransport, context);
    if (plan.transactionLookupTable !== null) fail(`${path}.transactionLookupTable`, "direct Oracle actions must not use a compressed-state lookup table", plan.transactionLookupTable, context);
    ...
    compressedWitness = validateOracleCompressedWitness(
      plan.compressedTransport,
      expectedRequest,
      outer,
      decodedOuter,
      proof,
      `${path}.compressedTransport`,
      context,
    );
    lookupWitness = validateOracleLookupTableWitness(plan.transactionLookupTable, `${path}.transactionLookupTable`, context);
  }
  const transportMetrics = validateOracleTransportMetrics(
    plan.transportMetrics,
    outer,
    expectedRequest.ownerPubkey,
    plan.transactionLookupTable,
    `${path}.transportMetrics`,
    context,
  );
  validateOracleLookupProofBinding(
    proof,
    plan.transactionLookupTable,
    transportMetrics,
    `${path}.proofFacts`,
  ...
  const parsedTransaction = validateOracleDraftTransaction(
    plan.transaction,
    actionInstructions,
    expectedRequest.ownerPubkey,
    proof.recentBlockhash,
    transportMetrics,
    lookupWitness,
    `${path}.transaction`,
    context,
  );
  if (parsedTransaction.version !== 0) fail(`${path}.transaction.serializedTransactionBase64`, "Oracle prepares require the exact v0 transport", parsedTransaction.version, context);
  ...
  if (lookupWitness !== null) validateOracleLookupDigestBindings(lookupWitness, `${path}.transactionLookupTable`, context);
  ...
  exactString(
    plan.preparedPlanDigest,
    preparedPlanSha256("oracle_draft", planWithoutDigest as JsonObject),
    `${path}.preparedPlanDigest`,
    context,
  );
}

function validateOracleDraftDeployment(
  value: JsonValue | undefined,
  path: string,
  context: ValidationContext,
): void {
  ...
  if (deployment.schemaVersion !== 2) fail(`${path}.schemaVersion`, "expected current deployment schema 2", deployment.schemaVersion, context);
  const exactFields: Readonly<Record<string, string>> = {
  ...
  for (const [field, expected] of Object.entries(exactFields)) exactString(deployment[field], expected, `${path}.${field}`, context);
  if (deployment.programDataBytes !== 1_241_821) fail(`${path}.programDataBytes`, "expected exact rc.44 ProgramData allocation", deployment.programDataBytes, context);
  if (deployment.deployedSlot !== 487_702_729) fail(`${path}.deployedSlot`, "expected exact rc.44 deployment slot", deployment.deployedSlot, context);
  if (deployment.programPayloadBytes !== 1_142_664) fail(`${path}.programPayloadBytes`, "expected exact rc.44 payload length", deployment.programPayloadBytes, context);
}

interface ValidatedOracleManifest {
...
function validateOracleInstructionManifest(
  value: JsonValue | undefined,
  expected: {
    readonly actionType: OracleDraftPrepareRequest["actionType"];
    readonly tag: number;
  ...
  if (data.length < 1 || data[0] !== expected.tag) fail(`${path}.dataBase64`, `expected instruction tag ${expected.tag}`, manifest.dataBase64, context);
  ...
  if (accounts.length < 1 || accounts.length > 96) fail(`${path}.accounts`, "Oracle instruction account count is outside 1..96", manifest.accounts, context);
  ...
  if (decoded.tag !== expected.tag) fail(`${path}.decodedParams.tag`, `expected tag ${expected.tag}`, decoded.tag, context);
  ...
  if (expected.wrapped) {
    ...
    if (decoded.logicalTag !== expected.logicalTag) fail(`${path}.decodedParams.logicalTag`, `expected logical tag ${expected.logicalTag}`, decoded.logicalTag, context);
  }
  return Object.freeze({ object: manifest, programId: CURRENT_PROGRAM_ID, dataBase64: manifest.dataBase64 as string, data, accounts: Object.freeze(accounts) });
}

function validateOraclePlanIdentity(
  value: JsonValue | undefined,
  request: OracleDraftPrepareRequest,
  path: string,
  context: ValidationContext,
...
function validateOracleDraftProofFacts(
  value: JsonValue | undefined,
  request: OracleDraftPrepareRequest,
  oracle: JsonObject,
  logicalTag: number,
  ...
  if ([131, 133, 138, 139].includes(logicalTag)) {
    for (const field of ["expectedAmbaAmountAtomic", "expectedSambaAmountAtomic", "earliestExecutionTs"] as const) {
  ...
  exactString(proof.marketAddress, oracle.market as string, `${path}.marketAddress`, context);
  if (logicalTag === 155) {
    if (proof.oracleMonth !== null) fail(`${path}.oracleMonth`, "global reward funding has no month-scoped proof fact", proof.oracleMonth, context);
  } else {
    exactString(proof.oracleMonth, oracle.oracleMonth as string, `${path}.oracleMonth`, context);
  }
  if (proof.logicalTag !== logicalTag) fail(`${path}.logicalTag`, `expected logical tag ${logicalTag}`, proof.logicalTag, context);
  if (proof.transportTag !== transportTag) fail(`${path}.transportTag`, `expected transport tag ${transportTag}`, proof.transportTag, context);
  for (const field of ["logicalInstructionSha256", "outerInstructionSha256", "plannerFactsSha256"] as const) sha256Digest(proof[field], `${path}.${field}`, context);
  base58Address(proof.recentBlockhash, `${path}.recentBlockhash`, context);
  const compressed = transportTag === 205;
  for (const field of ["witnessDigest", "providerOriginSha256"] as const) {
    if (compressed) sha256Digest(proof[field], `${path}.${field}`, context);
    else if (proof[field] !== null) fail(`${path}.${field}`, "direct Oracle action must use null compressed proof summary", proof[field], context);
  }
  for (const field of ["proofContextSlot", "stateFinalizedSlot"] as const) {
    if (compressed) canonicalU64(proof[field], `${path}.${field}`, context);
    else if (proof[field] !== null) fail(`${path}.${field}`, "direct Oracle action must use null compressed slot summary", proof[field], context);
  }
  for (const field of [
    "lookupTableAddress", "lookupTableAddressesSha256", "lookupTableAccountDataSha256",
    "lookupTableObservedFinalizedSlot", "lookupTableWritableIndexes", "lookupTableReadonlyIndexes",
    ...
    if (compressed) {
      if (typeof proof[field] !== "string") fail(`${path}.${field}`, "wrapped Oracle action requires lookup-table proof facts", proof[field], context);
    } else if (proof[field] !== null) {
      fail(`${path}.${field}`, "direct Oracle action must use null lookup-table proof facts", proof[field], context);
    }
  }
  for (const field of ["serializedByteLength", "noLookupTableSerializedByteLength", "theoreticalAltMinimizedByteLength"] as const) {
  ...
  if ([131, 133, 138, 139, 176, 177].includes(logicalTag)) {
    ...
    if (typeof proof[field] !== "string" || proof[field].length > 100_000) fail(path, "Oracle evidence frame is missing or oversized", proof[field], context);
    let frame: unknown;
    try { frame = JSON.parse(proof[field] as string); } catch { fail(path, "Oracle evidence is not JSON", proof[field], context); }
    const evidence = exactObject(frame as JsonValue, ["readWindowStartSlot", "readWindowEndSlot", clockField, "accountFacts"], ["readWindowStartSlot", "readWindowEndSlot", clockField, "accountFacts"], path, context);
    ...
    if (clockField === "currentSlot" && (BigInt(evidence[clockField] as string) < BigInt(start) || BigInt(evidence[clockField] as string) > BigInt(end))) fail(path, "dispute economic slot is outside evidence frame", evidence[clockField], context);
    const facts = arrayValue(evidence.accountFacts, path, context);
    if (facts.length < 1 || facts.length > 32) fail(path, "Oracle account count invalid", facts, context);
    ...
    for (const value of facts) {
      ...
      if (addresses.has(address)) fail(path, "Oracle evidence repeats an account", fact.address, context);
      addresses.add(address);
      safeIntegerValue(fact.observedSlot, path, context, start, end);
      if (fact.owner === null) {
        if (fact.executable !== null || fact.dataBase64 !== null || fact.dataSha256 !== null) fail(path, "Oracle absence has partial account data", fact, context);
      } else {
        base58Address(fact.owner, path, context); booleanValue(fact.executable, path, context);
...
interface DecodedOracleCompressedEnvelope {
  readonly coreAccountCount: number;
  readonly rentPayerIndex: number;
  readonly proof: Uint8Array | null;
  readonly accesses: readonly JsonObject[];
  readonly inner: Uint8Array;
  readonly logicalTag: number;
}

function decodeOracleCompressedEnvelopeForClient(
  bytes: Uint8Array,
  path: string,
  context: ValidationContext,
): DecodedOracleCompressedEnvelope {
  let offset = 0;
  const requireBytes = (length: number, label: string): void => {
    if (offset + length > bytes.length) fail(path, `tag 205 is truncated while reading ${label}`, bytesToBase64(bytes), context);
  };
  const u8 = (label: string): number => { requireBytes(1, label); return bytes[offset++]!; };
  const u16 = (label: string): number => { requireBytes(2, label); const result = bytes[offset]! | (bytes[offset + 1]! << 8); offset += 2; return result; };
  const u32 = (label: string): number => { requireBytes(4, label); const result = (bytes[offset]! | (bytes[offset + 1]! << 8) | (bytes[offset + 2]! << 16) | (bytes[offset + 3]! << 24)) >>> 0; offset += 4; return result; };
  const slice = (length: number, label: string): Uint8Array => { requireBytes(length, label); const result = bytes.slice(offset, offset + length); offset += length; return result; };
  if (u8("outer tag") !== 205) fail(path, "expected ExecuteCompressedStateV1 tag 205", bytes[0], context);
  ...
  if (proofOption === 1) proof = slice(128, "validity proof");
  else if (proofOption !== 0) fail(path, "tag 205 proof option is noncanonical", proofOption, context);
  const accessCount = u8("access count");
  if (accessCount < 1 || accessCount > 8) fail(path, "tag 205 access count is outside 1..8", accessCount, context);
  const compactLengths: Readonly<Record<number, number>> = Object.freeze({ 1: 44, 2: 152, 3: 112, 4: 104, 5: 81, 6: 16, 7: 17, 8: 115, 9: 190, 10: 96 });
  const accesses: JsonObject[] = [];
  for (let index = 0; index < accessCount; index += 1) {
    ...
    if (kindByte === 2) {
      ...
      if (compactLengths[domain] === undefined) fail(path, "tag 205 initialize domain is unknown", domain, context);
    ...
    if (kindByte !== 0 && kindByte !== 1) fail(path, "tag 205 access kind is invalid", kindByte, context);
    ...
    if (treeInfo.proveByIndex !== 0 && treeInfo.proveByIndex !== 1) fail(path, "tag 205 proveByIndex is noncanonical", treeInfo.proveByIndex, context);
    const outputStateTreeIndex = kindByte === 1 ? u8("output state tree index") : undefined;
    const domain = u8(`access ${index} domain`);
    const compactLength = compactLengths[domain];
    if (compactLength === undefined) fail(path, "tag 205 compact domain is unknown", domain, context);
  ...
  if (innerLength < 1 || innerLength > 12_288) fail(path, "tag 205 inner instruction length is outside 1..12288", innerLength, context);
  const inner = slice(innerLength, "inner instruction");
  if (offset !== bytes.length) fail(path, "tag 205 has trailing bytes", bytesToBase64(bytes.slice(offset)), context);
  if (rentPayerIndex >= coreAccountCount) fail(path, "tag 205 rent payer is outside the core prefix", rentPayerIndex, context);
  accesses.forEach((access) => {
    if ((access.accountIndex as number) >= coreAccountCount) fail(path, "tag 205 access is outside the core prefix", access.accountIndex, context);
  });
  return Object.freeze({ coreAccountCount, rentPayerIndex, proof, accesses: Object.freeze(accesses), inner, logicalTag: inner[0]! });
}

function validateOracleCompressedWitness(
  value: JsonValue | undefined,
  request: OracleDraftPrepareRequest,
  outer: ValidatedOracleManifest,
  decodedOuter: DecodedOracleCompressedEnvelope,
  proofFacts: JsonObject,
  path: string,
  context: ValidationContext,
): JsonObject {
  ...
  for (const field of ["providerOriginSha256", "proofSha256", "logicalInstructionSha256", "outerInstructionSha256", "witnessDigest"] as const) sha256Digest(witness[field], `${path}.${field}`, context);
  ...
  exactString(witness.providerOriginSha256, proofFacts.providerOriginSha256 as string, `${path}.providerOriginSha256`, context);
  exactString(witness.proofContextSlot, proofFacts.proofContextSlot as string, `${path}.proofContextSlot`, context);
  exactString(witness.stateFinalizedSlot, proofFacts.stateFinalizedSlot as string, `${path}.stateFinalizedSlot`, context);
  exactString(witness.logicalInstructionSha256, proofFacts.logicalInstructionSha256 as string, `${path}.logicalInstructionSha256`, context);
  exactString(witness.outerInstructionSha256, proofFacts.outerInstructionSha256 as string, `${path}.outerInstructionSha256`, context);
  exactString(witness.witnessDigest, proofFacts.witnessDigest as string, `${path}.witnessDigest`, context);
  ...
  if (coreAccountCount !== decodedOuter.coreAccountCount || rentPayerIndex !== decodedOuter.rentPayerIndex) fail(path, "witness core/rent indexes do not match tag-205 bytes", witness, context);
  const coreAccounts = arrayValue(witness.coreAccounts, `${path}.coreAccounts`, context);
  if (coreAccounts.length !== coreAccountCount || outer.accounts.length < coreAccountCount) fail(`${path}.coreAccounts`, "witness core prefix length does not match the outer instruction", coreAccounts, context);
  coreAccounts.forEach((value, index) => {
    ...
    if (item.isSigner !== outerMeta.isSigner || item.isWritable !== outerMeta.isWritable) fail(itemPath, "witness core flags do not match outer metas", item, context);
  });
  const accesses = arrayValue(witness.accesses, `${path}.accesses`, context);
  if (accesses.length !== decodedOuter.accesses.length) fail(`${path}.accesses`, "witness access count does not match tag-205 bytes", accesses, context);
  ...
  if (existing + initialize !== accesses.length) fail(path, "existing and initialize access counts must cover the exact access list", witness, context);
  if (decodedOuter.proof === null) fail(path, "wrapped Oracle transport requires an exact validity proof", witness, context);
  return witness;
}

function validateOracleProofTreeFact(value: JsonValue, path: string, context: ValidationContext): void {
  ...
  if (kind === "address_v2") {
    ...
    if (object.cpiContext !== null) fail(`${path}.cpiContext`, "address-v2 proof context must be null", object.cpiContext, context);
  } else {
...
function validateOracleAccessFact(
  value: JsonValue,
  decoded: JsonObject,
  path: string,
  context: ValidationContext,
  ...
  for (const field of ["kind", "domain", "accountIndex"] as const) {
    if (access[field] !== decoded[field]) fail(`${path}.${field}`, "witness access does not match tag-205 bytes", access[field], context);
  }
  if (kind === "readOnly" || kind === "mutable") {
    validatePackedTreeInfo(access.treeInfo, decoded.treeInfo as JsonObject, `${path}.treeInfo`, context);
    exactString(access.revision, decoded.revision as string, `${path}.revision`, context);
    canonicalBase64(access.compactDataBase64, `${path}.compactDataBase64`, context);
    exactString(access.compactDataBase64, decoded.compactDataBase64 as string, `${path}.compactDataBase64`, context);
  }
  const numericFields = kind === "initialize"
    ? ["addressTreeAccountIndex", "addressQueueAccountIndex", "addressRootIndex", "outputStateTreeIndex"] as const
    : kind === "mutable"
      ? ["outputStateTreeIndex"] as const
      : [] as const;
  numericFields.forEach((field) => {
    safeIntegerValue(access[field], `${path}.${field}`, context, 0, field === "addressRootIndex" ? 65_535 : 255);
    if (access[field] !== decoded[field]) fail(`${path}.${field}`, "witness packed access field does not match tag-205 bytes", access[field], context);
  });
}

function validatePackedTreeInfo(
  value: JsonValue | undefined,
  decoded: JsonObject,
  path: string,
  context: ValidationContext,
...
function validateOracleLookupTableWitness(
  value: JsonValue | undefined,
  path: string,
  context: ValidationContext,
): JsonObject {
  ...
  if (lookup.authority !== null) base58Address(lookup.authority, `${path}.authority`, context);
  ...
  if (BigInt(observedSlot) <= BigInt(lastExtendedSlot)) fail(`${path}.observedFinalizedSlot`, "lookup table must be active at a later finalized slot", lookup.observedFinalizedSlot, context);
  ...
  if (addresses.length < 1 || addresses.length > 256 || new Set(addresses).size !== addresses.length) fail(`${path}.addresses`, "lookup table must contain 1..256 unique addresses", lookup.addresses, context);
  ...
  if (startIndex > addresses.length) fail(`${path}.lastExtendedSlotStartIndex`, "lookup extension index exceeds address count", startIndex, context);
  ...
  if (all.length < 1 || new Set(all).size !== all.length) fail(path, "lookup writable/readonly indexes must be nonempty and disjoint", lookup, context);
  safeIntegerValue(lookup.serializedByteLength, `${path}.serializedByteLength`, context, 1, 1_232);
  return lookup;
}

function validateOracleLookupIndexes(
  value: JsonValue | undefined,
  addressCount: number,
  path: string,
  context: ValidationContext,
  ...
  if (new Set(indexes).size !== indexes.length) fail(path, "lookup indexes must be unique", indexes, context);
  return indexes;
}

function validateOracleTransportMetrics(
  value: JsonValue | undefined,
  outer: ValidatedOracleManifest,
  ownerPubkey: string,
  lookupValue: JsonValue | undefined,
  path: string,
  context: ValidationContext,
): JsonObject {
  ...
  if (metrics.outerInstructionDataBytes !== outer.data.length) fail(`${path}.outerInstructionDataBytes`, "must equal outer instruction data bytes", metrics.outerInstructionDataBytes, context);
  if (metrics.outerAccountMetaCount !== outer.accounts.length) fail(`${path}.outerAccountMetaCount`, "must equal outer instruction account count", metrics.outerAccountMetaCount, context);
  const uniqueKeys = new Set([ownerPubkey, outer.programId, ...outer.accounts.map((meta) => meta.pubkey)]).size;
  if (metrics.outerUniqueAccountKeys !== uniqueKeys) fail(`${path}.outerUniqueAccountKeys`, "must equal exact outer transaction key cardinality", metrics.outerUniqueAccountKeys, context);
  ...
  if (metrics.packetDataSizeLimit !== 1_232) fail(`${path}.packetDataSizeLimit`, "expected Solana packet limit 1232", metrics.packetDataSizeLimit, context);
  if (theoretical > noLookup) fail(`${path}.theoreticalAltMinimizedByteLength`, "theoretical ALT minimum cannot exceed the no-lookup transaction", theoretical, context);
  if (lookupValue === null) {
    if (serialized !== noLookup) fail(`${path}.serializedByteLength`, "direct transaction length must equal no-lookup length", serialized, context);
  } else {
    const lookup = objectValue(lookupValue, `${path}.transactionLookupTable`, context);
    if (lookup.serializedByteLength !== serialized) fail(`${path}.serializedByteLength`, "must match lookup witness serialized length", serialized, context);
  }
  return metrics;
}

function validateOracleLookupProofBinding(
  proof: JsonObject,
  lookupValue: JsonValue | undefined,
  metrics: JsonObject,
  path: string,
  ...
  metricFields.forEach((field) => {
    if (proof[field] !== metrics[field]) fail(`${path}.${field}`, `must equal transportMetrics.${field}`, proof[field], context);
  });
  if (lookupValue === null) {
    for (const field of [
      "lookupTableAddress", "lookupTableAddressesSha256", "lookupTableAccountDataSha256",
      ...
      if (proof[field] !== null) fail(`${path}.${field}`, "direct Oracle action has no lookup table", proof[field], context);
    }
    return;
  }
  const lookup = objectValue(lookupValue, `${path}.transactionLookupTable`, context);
  const expected: Readonly<Record<string, JsonValue | undefined>> = {
  ...
  for (const [field, expectedValue] of Object.entries(expected)) {
    if (proof[field] !== expectedValue) fail(`${path}.${field}`, "must bind the exact lookup-table witness", proof[field], context);
  }
}

function validateOracleDraftTransaction(
  value: JsonValue | undefined,
  actionInstructions: readonly ValidatedOracleManifest[],
  expectedOwner: string,
  expectedRecentBlockhash: JsonValue | undefined,
  transportMetrics: JsonObject,
  lookupValue: JsonObject | null,
  path: string,
  context: ValidationContext,
  ...
  if (serialized.length > 1_232) fail(`${path}.serializedTransactionBase64`, "serialized Oracle transaction exceeds the Solana packet bound", transaction.serializedTransactionBase64, context);
  if (serialized.length !== transportMetrics.serializedByteLength) fail(`${path}.serializedTransactionBase64`, "serialized bytes must match transport metrics", transaction.serializedTransactionBase64, context);
  ...
  exactString(transaction.recentBlockhash, expectedRecentBlockhash as string, `${path}.recentBlockhash`, context);
  ...
  if (signatures.length !== 0) fail(`${path}.backendPartialSignatures`, "current public Oracle actions have no backend signer", signatures, context);
  const approved = arrayValue(transaction.approvedInstructions, `${path}.approvedInstructions`, context);
  if (approved.length !== actionInstructions.length) fail(`${path}.approvedInstructions`, "Oracle draft requires exactly one approved transport instruction", approved, context);
  ...
  if (accounts.length !== outer.accounts.length) fail(`${instructionPath}.accounts`, "approved account count must match the outer manifest", accounts, context);
  accounts.forEach((value, index) => {
    ...
    if (meta.isSigner !== expected.isSigner || meta.isWritable !== expected.isWritable) fail(metaPath, "approved meta flags must match the outer manifest", meta, context);
  });
  });
  const parsed = validateSerializedApprovedTransaction(
    serialized,
    transaction.recentBlockhash as string,
    expectedOwner,
    approved,
    0,
    lookupValue ?? undefined,
    true,
    `${path}.serializedTransactionBase64`,
    context,
  );
...
function validateOracleCompressedDigestBindings(
  witness: JsonObject,
  decodedOuter: DecodedOracleCompressedEnvelope,
  logical: ValidatedOracleManifest,
  outer: ValidatedOracleManifest,
  path: string,
  context: ValidationContext,
): void {
  if (decodedOuter.proof === null) fail(`${path}.proofSha256`, "tag 205 is missing its validity proof", null, context);
  ...
  exactString(
    ...
    sha256Hex("ameba-spread-v2/current-compressed-instruction-witness-v1\0", JSON.stringify(orderedFacts)),
    `${path}.witnessDigest`,
    context,
  );
}

function validateOracleCompressedAccessBindings(
  witness: JsonObject,
  decodedOuter: DecodedOracleCompressedEnvelope,
  outer: ValidatedOracleManifest,
  path: string,
  ...
  const treeInfos = arrayValue(witness.treeInfos, `${path}.treeInfos`, context);
  const accesses = arrayValue(witness.accesses, `${path}.accesses`, context);
  if (roots.length !== accesses.length) fail(path, "proof arrays must map one-to-one to ordered compressed accesses", witness, context);
  ...
  const coreCount = witness.coreAccountCount as number;
  const fixedSuffix = [
    SYSTEM_PROGRAM_ID,
  ...
  fixedSuffix.forEach((address, offset) => {
    ...
    if (meta === undefined || meta.pubkey !== address || meta.isSigner || meta.isWritable) fail(`${path}.coreAccounts`, "tag-205 fixed Light suffix differs from the authorized topology", outer.object.accounts, context);
  });
  const packedStart = coreCount + fixedSuffix.length;
  const usedPackedIndexes = new Set<number>();
  const accessAccountIndexes = new Set(accesses.map((value, index) => objectValue(value, `${path}.accesses[${index}]`, context).accountIndex as number));
  ...
  logicalAccounts.forEach((account, index) => {
    ...
    if (witnessCore.address !== account.pubkey || witnessCore.isSigner !== account.isSigner || witnessCore.isWritable !== expectedWritable) fail(`${path}.coreAccounts[${index}]`, "tag-205 core role/flag binding is inconsistent", witnessCore, context);
  });
  accesses.forEach((value, index) => {
    ...
    if (access.kind === "initialize") {
      ...
      if (access.addressRootIndex !== rootIndex) fail(`${path}.accesses[${index}].addressRootIndex`, "must bind rootIndices[proofIndex]", access.addressRootIndex, context);
      bindOraclePackedAccount(outer, access.addressTreeAccountIndex as number, tree.stateTree as string, true, packedStart, usedPackedIndexes, `${path}.accesses[${index}].addressTreeAccountIndex`, context);
      bindOraclePackedAccount(outer, access.addressQueueAccountIndex as number, tree.queue as string, true, packedStart, usedPackedIndexes, `${path}.accesses[${index}].addressQueueAccountIndex`, context);
      bindOraclePackedAccount(outer, access.outputStateTreeIndex as number, witness.outputQueue as string, true, packedStart, usedPackedIndexes, `${path}.accesses[${index}].outputStateTreeIndex`, context);
    } else {
      ...
      if (packedTree.rootIndex !== rootIndex || packedTree.leafIndex !== leafIndex) fail(`${path}.accesses[${index}].treeInfo`, "packed root/leaf indexes must bind the proof arrays", packedTree, context);
      const writable = access.kind === "mutable";
      bindOraclePackedAccount(outer, packedTree.treeAccountIndex as number, tree.stateTree as string, writable, packedStart, usedPackedIndexes, `${path}.accesses[${index}].treeInfo.treeAccountIndex`, context);
      bindOraclePackedAccount(outer, packedTree.queueAccountIndex as number, tree.queue as string, writable, packedStart, usedPackedIndexes, `${path}.accesses[${index}].treeInfo.queueAccountIndex`, context);
      if (writable) bindOraclePackedAccount(outer, access.outputStateTreeIndex as number, tree.queue as string, true, packedStart, usedPackedIndexes, `${path}.accesses[${index}].outputStateTreeIndex`, context);
    }
  });
  for (let index = packedStart; index < outer.accounts.length; index += 1) {
    if (!usedPackedIndexes.has(index)) fail(`${path}.accesses`, "outer suffix contains an unbound packed Light account", outer.accounts[index]!.pubkey, context);
  }
  if (BigInt(witness.stateFinalizedSlot as string) < BigInt(witness.proofContextSlot as string)) fail(`${path}.stateFinalizedSlot`, "must not precede the proof context slot", witness.stateFinalizedSlot, context);
  roots.forEach((root, index) => {
    if (root === "0".repeat(64)) fail(`${path}.roots[${index}]`, "proof root must be nonzero", root, context);
  });
}

function bindOraclePackedAccount(
  outer: ValidatedOracleManifest,
  index: number,
  address: string,
  writable: boolean,
  ...
  if (!Number.isSafeInteger(index) || index < packedStart || index >= outer.accounts.length) fail(path, "packed account index is outside the exact suffix", index, context);
  const meta = outer.accounts[index]!;
  if (meta.pubkey !== address || meta.isSigner || (writable && !meta.isWritable)) fail(path, "packed account identity/flags do not match the proof tree", meta as unknown as JsonValue, context);
  used.add(index);
}

function validateOracleLookupDigestBindings(lookup: JsonObject, path: string, context: ValidationContext): void {
  const addresses = arrayValue(lookup.addresses, `${path}.addresses`, context).map((value) => value as string);
  const count = new Uint8Array(4);
  writeU32Le(count, 0, addresses.length);
  exactString(
    lookup.addressesSha256,
    sha256Hex("ameba-spread-v2/current-oracle-lookup-table-addresses-v1\0", count, ...addresses.map(decodeBase58)),
    `${path}.addressesSha256`,
    context,
  );
  const accountData = new Uint8Array(56 + addresses.length * 32);
  writeU32Le(accountData, 0, 1);
  writeU64Le(accountData, 4, BigInt(lookup.deactivationSlot as string));
  writeU64Le(accountData, 12, BigInt(lookup.lastExtendedSlot as string));
  accountData[20] = lookup.lastExtendedSlotStartIndex as number;
  accountData[21] = lookup.authority === null ? 0 : 1;
  if (lookup.authority !== null) accountData.set(decodeBase58(lookup.authority as string), 22);
...
function validateOracleParsedTransportMetrics(
  parsed: ParsedSolanaTransaction,
  metrics: JsonObject,
  lookup: JsonObject | null,
  path: string,
  context: ValidationContext,
): void {
  if (parsed.serializedByteLength !== metrics.serializedByteLength) fail(`${path}.serializedByteLength`, "must equal the parsed transaction bytes", metrics.serializedByteLength, context);
  if (lookup === null) {
    if (parsed.lookup !== null || parsed.loadedWritableCount !== 0 || parsed.loadedReadonlyCount !== 0) fail(path, "direct Oracle action must not load ALT accounts", metrics, context);
  } else {
    if (parsed.lookup === null) fail(path, "wrapped Oracle action must use its exact authorized ALT", metrics, context);
    exactString(parsed.lookup.address, lookup.address as string, `${path}.lookupTableAddress`, context);
    validateExactJsonProjection(parsed.lookup.writableIndexes as unknown as JsonValue, lookup.writableIndexes, `${path}.lookupWritableIndexes`, context);
    validateExactJsonProjection(parsed.lookup.readonlyIndexes as unknown as JsonValue, lookup.readonlyIndexes, `${path}.lookupReadonlyIndexes`, context);
  }
  ...
  const noLookup = shortVectorLength(signatureCount) + signatureCount * 64 + 1 + 3
    ...
    + instructionBytes + 1;
  if (metrics.noLookupTableSerializedByteLength !== noLookup) fail(`${path}.noLookupTableSerializedByteLength`, "does not match exact no-lookup v0 compilation", metrics.noLookupTableSerializedByteLength, context);
  ...
  if (metrics.theoreticalAltMinimizedByteLength !== theoretical) fail(`${path}.theoreticalAltMinimizedByteLength`, "does not match exact all-eligible-account ALT compilation", metrics.theoreticalAltMinimizedByteLength, context);
}
...
interface ParsedSolanaTransaction {
  ...
  readonly version: null | 0;
  readonly payer: string;
  readonly recentBlockhash: string;
  readonly signatures: readonly Uint8Array[];
  readonly accountFlags: ReadonlyMap<string, { readonly isSigner: boolean; readonly isWritable: boolean }>;
  readonly instructions: readonly ParsedSolanaInstruction[];
  readonly lookup: null | {
    readonly address: string;
    readonly writableIndexes: readonly number[];
    readonly readonlyIndexes: readonly number[];
  };
  readonly staticAccountCount: number;
  readonly loadedWritableCount: number;
  readonly loadedReadonlyCount: number;
  readonly compiledInstructionBytes: number;
}

function validateApprovedInstruction(
  value: JsonValue,
  path: string,
  context: ValidationContext,
  requireCurrentProgram: boolean,
) {
  const instruction = exactObject(
    value,
    ["programId", "accounts", "dataBase64"],
    ["programId", "accounts", "dataBase64"],
    path,
    context,
  );
  ...
  return Object.freeze({
    programId,
    dataBase64: instruction.dataBase64 as string,
    data: bytes,
    tag: bytes[0]!,
...
function validateSerializedApprovedTransaction(
  serialized: Uint8Array,
  expectedRecentBlockhash: string,
  expectedPayer: string,
  approvedValues: readonly JsonValue[],
  expectedVersion: null | 0,
  lookupWitness: JsonObject | undefined,
  requireCurrentProgram: boolean,
  path: string,
  context: ValidationContext,
): ParsedSolanaTransaction {
  ...
  const approved = approvedValues.map((value, index) => validateApprovedInstruction(
    value,
    `${path}.approvedInstructions[${index}]`,
    context,
  ...
  approved.forEach((expected, index) => {
    ...
    if (actual.programId !== expected.programId) fail(path, `serialized instruction ${index} program differs from its approved manifest`, actual.programId, context);
    if (!bytesEqual(actual.data, expected.data)) fail(path, `serialized instruction ${index} bytes differ from its approved manifest`, bytesToBase64(actual.data), context);
    if (
      actual.accountAddresses.length !== expected.accounts.length
      || actual.accountAddresses.some((address, accountIndex) => address !== expected.accounts[accountIndex]?.address)
    ) {
      fail(path, `serialized instruction ${index} account order differs from its approved manifest`, actual.accountAddresses as unknown as JsonValue, context);
    }
  });

  const expectedFlags = new Map<string, { isSigner: boolean; isWritable: boolean }>();
  const addExpected = (address: string, isSigner: boolean, isWritable: boolean): void => {
  ...
  if (parsed.accountFlags.size !== expectedFlags.size) {
    fail(path, "serialized account-key set contains missing or extra identities", [...parsed.accountFlags.keys()] as unknown as JsonValue, context);
  }
  for (const [address, expected] of expectedFlags) {
    ...
    if (actual === undefined || actual.isSigner !== expected.isSigner || actual.isWritable !== expected.isWritable) {
      fail(path, `serialized signer/writable policy differs for ${address}`, actual as unknown as JsonValue, context);
    }
  }
  return parsed;
}
...
function parseSolanaTransaction(
  ...
  const u8 = (label: string): number => {
    requireBytes(1, label);
    return bytes[offset++]!;
  };
  const slice = (length: number, label: string): Uint8Array => {
    requireBytes(length, label);
    const result = bytes.slice(offset, offset + length);
    offset += length;
    return result;
  };
  const shortVector = (label: string): number => {
    let value = 0;
    let multiplier = 1;
    let count = 0;
    let finalPayload = 0;
    while (true) {
      ...
      count += 1;
      if (count > 3 || value > 65_535) fail(path, `${label} short-vector is outside the current bound`, value, context);
      if ((byte & 0x80) === 0) break;
      multiplier *= 128;
    }
    if (count > 1 && finalPayload === 0) fail(path, `${label} short-vector is not minimally encoded`, value, context);
    return value;
  };

  const signatureCount = shortVector("signature count");
  if (signatureCount < 1 || signatureCount > 32) fail(path, "serialized transaction signature count is outside 1..32", signatureCount, context);
  ...
  const version: null | 0 = (firstMessageByte & 0x80) === 0
    ? null
  ...
  if (
    ...
    fail(path, "serialized transaction header/account counts are inconsistent", { staticCount, numRequiredSignatures } as unknown as JsonValue, context);
  }
  ...
  if (instructionCount < 1 || instructionCount > 64) fail(path, "serialized instruction count is outside 1..64", instructionCount, context);
  const compiled: { programIndex: number; accountIndexes: readonly number[]; data: Uint8Array }[] = [];
  ...
  let lookup: ParsedSolanaTransaction["lookup"] = null;
  let loadedWritable: string[] = [];
  let loadedReadonly: string[] = [];
  if (version === 0) {
    const lookupCount = shortVector("address lookup count");
    if (lookupCount > 1) fail(path, "current Oracle transport admits at most one address lookup table", lookupCount, context);
    if (lookupCount === 1) {
      if (lookupWitness === undefined) fail(path, "serialized v0 transaction unexpectedly uses an address lookup table", lookupCount, context);
      ...
      exactString(address, lookupWitness.address as string, path, context);
      ...
      if (JSON.stringify(writableIndexes) !== JSON.stringify(expectedWritable) || JSON.stringify(readonlyIndexes) !== JSON.stringify(expectedReadonly)) {
        fail(path, "serialized ALT writable/readonly indexes differ from the attested lookup witness", { writableIndexes, readonlyIndexes } as unknown as JsonValue, context);
      }
  ...
  if (offset !== bytes.length) fail(path, "serialized transaction has trailing bytes", bytesToBase64(bytes.slice(offset)), context);

  const allKeys = [...staticKeys, ...loadedWritable, ...loadedReadonly];
  if (new Set(allKeys).size !== allKeys.length) fail(path, "serialized transaction account keys are not unique", allKeys as unknown as JsonValue, context);
  const accountFlags = new Map<string, { readonly isSigner: boolean; readonly isWritable: boolean }>();
  ...
  const instructions = Object.freeze(compiled.map((instruction, index): ParsedSolanaInstruction => {
    const programId = allKeys[instruction.programIndex];
    if (programId === undefined) fail(path, `serialized instruction ${index} program index is out of range`, instruction.programIndex, context);
...
function validateSetupTransactionBytes(
  bytes: Uint8Array,
  kind: "protocol_state_load" | "light_ata_load",
  expectedPayer: string,
  expectedRecentBlockhash: string,
  path: string,
  context: ValidationContext,
  ...
  if (parsed.version !== null) fail(path, "setup transactions must use the canonical legacy wire format", parsed.version, context);
  exactString(parsed.payer, expectedPayer, path, context);
  exactString(parsed.recentBlockhash, expectedRecentBlockhash, path, context);
  if (parsed.signatures.some((signature) => signature.some((byte) => byte !== 0))) fail(path, "setup transaction must be unsigned", bytesToBase64(bytes), context);
  if (kind === "protocol_state_load") {
    if (parsed.instructions.length !== 1) fail(path, "protocol_state_load must contain exactly one tag-219 instruction", parsed.instructions.length, context);
    const instruction = parsed.instructions[0]!;
    exactString(instruction.programId, CURRENT_PROGRAM_ID, path, context);
    if (instruction.data.length < 153 || instruction.data.length > 16_384 || instruction.data[0] !== 219) fail(path, "protocol_state_load must contain the bounded rc.44 tag-219 payload", bytesToBase64(instruction.data), context);
    ...
    if (instruction.accountAddresses.length !== 13 || instruction.accountAddresses[0] !== expectedPayer) fail(path, "tag-219 must contain the exact 13-account payer-bound grammar", instruction.accountAddresses as unknown as JsonValue, context);
    const fixed = [
    ...
    if (!CURRENT_LIGHT_STATE_CONTEXTS.some((entry) => entry.stateTree === tree && entry.queue === queue && entry.cpiContext === cpi)) fail(path, "tag-219 tree/queue/CPI tuple is not authorized", { tree, queue, cpi } as unknown as JsonValue, context);
    ...
    instruction.accountAddresses.forEach((address, index) => {
      ...
      if (actual?.isSigner !== expected[0] || actual.isWritable !== expected[1]) fail(path, `tag-219 account ${index} flags are not canonical`, actual as unknown as JsonValue, context);
    });
    return;
  }
  if (kind === "light_ata_load") {
    if (parsed.instructions.length < 2 || parsed.instructions.length > 8) fail(path, "light_ata_load requires compute budget followed by 1..7 Light Token instructions", parsed.instructions.length, context);
    ...
    if (compute.data.length !== 5 || compute.data[0] !== 2) fail(path, "light_ata_load compute-unit limit is not canonical", bytesToBase64(compute.data), context);
    const units = readU32Le(compute.data, 1);
    if (units < 50_000 || units > 1_400_000) fail(path, "light_ata_load compute-unit limit is outside the current bound", units, context);
    parsed.instructions.slice(1).forEach((instruction, index) => {
      ...
      if (instruction.data.length < 1 || instruction.data.length > 16_384 || !instruction.accountAddresses.includes(expectedPayer)) fail(path, `Light ATA load instruction ${index} is not payer-bound`, bytesToBase64(instruction.data), context);
    });
    const payerFlags = parsed.accountFlags.get(expectedPayer);
    if (payerFlags?.isSigner !== true || payerFlags.isWritable !== true) fail(path, "Light ATA load payer must be the writable signer", payerFlags as unknown as JsonValue, context);
    return;
  }
}

function shortVectorLength(value: number): number {
  let remaining = value;
  let length = 0;
  do {
    length += 1;
...
function sha256Bytes(...parts: readonly (Uint8Array | string)[]): Uint8Array {
...
function sha256Hex(...parts: readonly (Uint8Array | string)[]): string {
...
function oracleInstructionSha256(manifest: ValidatedOracleManifest): string {
  const parts: Uint8Array[] = [decodeBase58(manifest.programId), manifest.data];
...
function oracleOperationId(instructions: readonly ValidatedOracleManifest[], proofFacts: JsonObject): string {
  const parts: (Uint8Array | string)[] = ["oracle_draft", Uint8Array.of(instructions.length)];
...
function preparedPlanSha256(operation: string, planWithoutDigest: JsonObject): string {
  return sha256Hex(
    "ameba-spread-v2/current-prepared-plan-v1\0",
    operation,
    "\0",
    JSON.stringify(canonicalJsonValue(planWithoutDigest)),
  );
}
...
function writeU64Le(bytes: Uint8Array, offset: number, value: bigint): void {
  for (let index = 0; index < 8; index += 1) bytes[offset + index] = Number((value >> BigInt(index * 8)) & 0xffn);
}

function validateExactJsonProjection(
  value: JsonValue | undefined,
  ...
  if (Array.isArray(expected)) {
    ...
    if (actual.length !== expected.length) fail(path, "array length does not match the SDK-issued projection", actual, context);
  ...
  if (value !== expected) fail(path, "value does not match the SDK-issued projection", value, context);
}

function validateCurrentProtocol(value: JsonValue | undefined, path: string, context: ValidationContext): void {
...
function exactObject(
  ...
  if (unknown !== undefined) fail(`${path}.${unknown}`, "unexpected field", object[unknown], context);
  const missing = required.find((field) => !Object.hasOwn(object, field));
  if (missing !== undefined) fail(`${path}.${missing}`, "required field is missing", undefined, context);
  return object;
}

function objectValue(value: JsonValue | undefined, path: string, context: ValidationContext): JsonObject {
  if (!isObject(value)) fail(path, "expected an object", value, context);
  return value;
}

function arrayValue(value: JsonValue | undefined, path: string, context: ValidationContext): JsonValue[] {
  if (!Array.isArray(value)) fail(path, "expected an array", value, context);
  return value;
}

function nonemptyStringValue(value: JsonValue | undefined, path: string, context: ValidationContext): string {
...
function currentProductId(value: JsonValue | undefined, path: string, context: ValidationContext): string {
...
function exactString(value: JsonValue | undefined, expected: string, path: string, context: ValidationContext): string {
  if (value !== expected) fail(path, `expected ${JSON.stringify(expected)}`, value, context);
  return expected;
}
...
function enumString<const T extends string>(
  ...
  if (typeof value !== "string" || !allowed.includes(value as T)) fail(path, `expected ${allowed.join(" or ")}`, value, context);
  return value as T;
}
...
function stringPattern(
  ...
  if (typeof value !== "string" || !pattern.test(value)) fail(path, `expected ${expectation}`, value, context);
  return value;
}

function booleanValue(value: JsonValue | undefined, path: string, context: ValidationContext): boolean {
  if (typeof value !== "boolean") fail(path, "expected a boolean", value, context);
  return value;
}
...
function exactBoolean(
  ...
  if (value !== expected) fail(path, `expected ${expected}`, value, context);
  return expected;
}
...
function finiteNumberValue(
  ...
  if (typeof value !== "number" || !Number.isFinite(value)) fail(path, "expected a finite number", value, context);
  if (minimum !== undefined && (inclusive ? value < minimum : value <= minimum)) fail(path, `expected a number ${inclusive ? ">=" : ">"} ${minimum}`, value, context);
  return value;
}
...
function canonicalUnsignedDecimal(value: JsonValue | undefined, path: string, context: ValidationContext): string {
  return stringPattern(value, /^(0|[1-9][0-9]*)$/, path, context, "a canonical unsigned decimal string");
}

function canonicalU64(value: JsonValue | undefined, path: string, context: ValidationContext): string {
  const decimal = canonicalUnsignedDecimal(value, path, context);
  if (BigInt(decimal) > 18_446_744_073_709_551_615n) {
    fail(path, "expected a canonical u64 decimal string", value, context);
  }
  return decimal;
}

function canonicalPositiveU64(value: JsonValue | undefined, path: string, context: ValidationContext): string {
  const decimal = canonicalU64(value, path, context);
  if (decimal === "0") fail(path, "expected a positive canonical u64 decimal string", value, context);
  return decimal;
}

function sha256Digest(value: JsonValue | undefined, path: string, context: ValidationContext): string {
  return stringPattern(value, /^[0-9a-f]{64}$/, path, context, "a lowercase SHA-256 digest");
}

function decimalAmount(value: JsonValue | undefined, path: string, context: ValidationContext, positive = false): string {
  const amount = canonicalUnsignedDecimal(value, path, context);
  if (positive && amount === "0") fail(path, "expected a positive decimal amount", value, context);
  return amount;
}

function canonicalDecimalText(value: JsonValue | undefined, path: string, context: ValidationContext): string {
  return stringPattern(value, /^(?:0|[1-9][0-9]*)(?:\.[0-9]+)?$/, path, context, "a canonical nonnegative decimal string");
}

function base58Address(value: JsonValue | undefined, path: string, context: ValidationContext): string {
...
function decodeBase58(value: string): Uint8Array {
  ...
  for (let index = 0; index < bodyLength; index += 1) result[result.length - 1 - index] = bytes[index]!;
  return result;
}
...
function canonicalBase64(value: JsonValue | undefined, path: string, context: ValidationContext): Uint8Array {
...
function isoTimestamp(value: JsonValue | undefined, path: string, context: ValidationContext): string {
...
function stringArrayValue(value: JsonValue | undefined, path: string, context: ValidationContext): readonly string[] {
  return arrayValue(value, path, context).map((entry, index) => nonemptyStringValue(entry, `${path}[${index}]`, context));
}

function jsonObjectValue(value: JsonValue | undefined, path: string, context: ValidationContext): JsonObject {
  const object = objectValue(value, path, context);
  validateJsonValue(object, path, context);
  return object;
}

function jsonPrimitiveValue(value: JsonValue | undefined, path: string, context: ValidationContext): void {
...
function jsonArrayValue(value: JsonValue | undefined, path: string, context: ValidationContext): readonly JsonValue[] {
  const array = arrayValue(value, path, context);
  array.forEach((entry, index) => validateJsonValue(entry, `${path}[${index}]`, context));
  return array;
}

function validateJsonValue(value: JsonValue | undefined, path: string, context: ValidationContext, depth = 0): void {
  if (depth > 64) fail(path, "JSON nesting exceeds the current response bound", value, context);
  if (value === undefined) fail(path, "undefined is not valid JSON", value, context);
  if (typeof value === "number" && !Number.isFinite(value)) fail(path, "JSON number must be finite", value, context);
  if (Array.isArray(value)) {
    value.forEach((entry, index) => validateJsonValue(entry, `${path}[${index}]`, context, depth + 1));
  } else if (isObject(value)) {
    for (const [field, entry] of Object.entries(value)) validateJsonValue(entry, `${path}.${field}`, context, depth + 1);
  }
}
...
function fail(
  ...
  throw new AmebaResponseValidationError(path, message, {
    details: { received: describe(value as JsonValue | undefined) },
    context,
  });
}
...
function productSegment(raw: string, label: string): string {
...
function nonempty(raw: string, label: string): string {
  const value = raw.trim();
  if (!value) throw new AmebaInputError(`${label} must not be empty`);
  return value;
}

function optionalNonempty(raw: string | undefined, label: string): string | undefined {
  return raw === undefined ? undefined : nonempty(raw, label);
}

function rejectUnknownInputFields(
  request: object,
  ...
  if (unknown.length > 0) throw new AmebaInputError(`${label} contains unsupported fields: ${unknown.join(", ")}`);
  const missing = allowedFields.find((field) => !Object.hasOwn(request, field));
  if (missing !== undefined) throw new AmebaInputError(`${label} requires ${missing}`);
}

function validateInputPositiveU64(value: unknown, label: string): string {
...
function validateCurrentCollectiveOperationSubmitRequest(
  request: CurrentCollectiveOperationSubmitRequest,
): void {
  ...
  const preparedBytes = validateInputBase64(
    request.serializedTransactionBase64,
    "serializedTransactionBase64",
  );
  const signedBytes = validateInputBase64(
...
function validateSignedTransactionBinding(
  request: CurrentCollectiveOperationSubmitRequest,
...
function lowercaseSha256Input(value: string, label: string): string {
...
function validateInputCurrentProductId(value: string, label: string): string {
...
function validateInputCurrentSeriesId(value: string, marketId: string | undefined, label: string): string {
...
function validateInputBase64(value: string, label: string): Uint8Array {
...
function finiteNumber(value: number, label: string): number {
  if (!Number.isFinite(value)) throw new AmebaInputError(`${label} must be finite`);
  return value;
}

function chartRangeWindowMs(range: ChartRequest["range"]): number | undefined {
  switch (range) {
    case "1h":
      return 60 * 60 * 1_000;
    case "24h":
