...
export const CURRENT_FINALIZED_OBSERVATION_SCHEMA_VERSION = 1;
export const CURRENT_FINALIZED_OBSERVATION_SOURCE = "current_finalized_rpc";
export const CURRENT_FINALIZED_OBSERVATION_COMMITMENT = "finalized";
export const CURRENT_FINALIZED_OBSERVATION_DIGEST_DOMAIN = "ameba_lean:current_finalized_observation:v1";
export const CURRENT_FINALIZED_OBSERVATION_DEVNET_GENESIS_HASH = "EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG";
export class CurrentFinalizedObservationValidationError extends Error {
    code = "CURRENT_FINALIZED_OBSERVATION_INVALID";
    constructor(message) {
        super(message);
 ...
 */
export function currentFinalizedObservationPreimage(input) {
    ...
    observation.orderedAccounts.forEach((account, index) => {
        fields.push([`orderedAccounts[${index}].address`, account.address], [`orderedAccounts[${index}].owner`, account.owner === null ? "null" : `pubkey:${account.owner}`], [`orderedAccounts[${index}].executable`, account.executable === null ? "null" : String(account.executable)], [`orderedAccounts[${index}].dataLength`, account.dataLength ?? "null"], [`orderedAccounts[${index}].dataSha256`, account.dataSha256 === null ? "null" : `sha256:${account.dataSha256}`]);
    });
    ...
    for (const [name, value] of fields)
        parts.push(netstring(name, encoder), netstring(value, encoder));
...
export function computeCurrentFinalizedObservationDigest(input) {
 ...
 */
export function validateCurrentFinalizedObservation(value) {
...
function canonicalObservation(value, requireDigest, requireAdmissibleBlockTime) {
    ...
    if (object.schemaVersion !== CURRENT_FINALIZED_OBSERVATION_SCHEMA_VERSION)
        invalid("schemaVersion must be 1");
    if (object.observationSource !== CURRENT_FINALIZED_OBSERVATION_SOURCE)
        invalid("observationSource is not current_finalized_rpc");
    if (object.commitment !== CURRENT_FINALIZED_OBSERVATION_COMMITMENT)
        invalid("commitment is not finalized");
    ...
    if (observedAtSlot !== currentFinalizedSlot)
        invalid("observedAtSlot must equal currentFinalizedSlot");
    ...
    if (provider.kind !== "solana_json_rpc")
        invalid("stateProvider.kind is not solana_json_rpc");
    ...
    const seen = new Set();
    const orderedAccounts = object.orderedAccounts.map((entry, index) => {
        ...
        if (seen.has(address))
            invalid("orderedAccounts addresses must be unique");
        seen.add(address);
        ...
        if (typeof account.executable !== "boolean")
            invalid(`orderedAccounts[${index}].executable must be boolean`);
...
function exactObject(value, required, optional = []) {
    if (value === null || typeof value !== "object" || Array.isArray(value))
        invalid("observation value is not an object");
    const object = value;
    const allowed = new Set([...required, ...optional]);
    for (const key of Object.keys(object))
        if (!allowed.has(key))
            invalid(`unexpected key ${key}`);
    for (const key of required)
        if (!Object.prototype.hasOwnProperty.call(object, key))
            invalid(`missing key ${key}`);
    return object;
}
function canonicalProviderOrigin(value) {
    if (typeof value !== "string")
        invalid("stateProvider.origin must be a string");
    let parsed;
    try {
    ...
    if (parsed.protocol !== "https:"
        || parsed.origin !== value
        || authority === null
        || (port !== undefined && (!/^[1-9][0-9]*$/u.test(port)
            || Number(port) > 65_535
            || Number(port) === 443
...
function canonicalPubkey(value, label) {
    ...
    try {
        ...
        if (key.toBase58() !== value)
            invalid(`${label} is not canonical Base58`);
        return value;
    }
    catch (cause) {
        if (cause instanceof CurrentFinalizedObservationValidationError)
            throw cause;
...
function lowercaseSha256(value, label) {
    if (typeof value !== "string" || !/^[0-9a-f]{64}$/u.test(value))
        invalid(`${label} is not a lowercase SHA-256 digest`);
    return value;
}
function netstring(value, encoder) {
...
//# sourceMappingURL=current-finalized-observation.js.map