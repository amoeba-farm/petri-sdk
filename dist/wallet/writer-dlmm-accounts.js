...
function invalid(message) { walletError("CURRENT_WALLET_ACCOUNT_INVALID", message); }
function start(address, accounts, size, discriminator) {
    const reader = new WalletWriterAccountReader(requireCurrentProgramData(accounts, address, size, discriminator));
    if (!reader.bool())
        invalid("writer liquidity account is uninitialized");
    const bump = reader.u8();
    if (String.fromCharCode(...reader.bytes(3)) !== discriminator || reader.u8() !== 1)
        invalid("writer liquidity account version differs");
    return { reader, bump };
}
...
export const deriveWalletWriterDlmmPolicy = (sleeve) => pda("writer-dlmm-policy", [sleeve])[0];
export const deriveWalletWriterDlmmPosition = (pool, sleeve) => pda("writer-dlmm-position", [pool, sleeve])[0];
export function decodeWalletWriterDlmmPolicy(address, accounts) {
    ...
    if (!address.equals(expected) || bump !== expectedBump || seriesCount < 1 || seriesCount > 20 || appendedSeriesCount > seriesCount
        ...
        || !expectedPolicyHash.some(value => value !== 0) || reserveReleaseSpendRatioPpm > 1000000n || priceSeparationTicks === 0
...
export function decodeWalletWriterDlmmPosition(address, accounts) {
    ...
    if (!address.equals(expected) || bump !== expectedBump || seriesIndex >= 20 || binCount > 32
        ...
        invalid("writer position identity or native inventory differs");
...
export function decodeWalletWriterPolicyRegistry(address, accounts) {
    ...
    if (!expected.equals(address) || bump !== expectedBump || policyAuthority.equals(PublicKey.default))
        invalid("writer policy registry identity differs");
    return Object.freeze({ address, vaultConfig, policyAuthority });
}
export function decodeWalletWriterPolicySnapshot(address, accounts) {
    ...
    if (!expected.equals(address) || bump !== expectedWithBump[1] || securityMode > 1 || maxSeries !== 20 || primaryFeeBps > 10_000)
        invalid("writer policy snapshot identity differs");
    return Object.freeze({ address, sleeve, registry, policyVersion, policyHash, seriesFamilyHash, createdSlot, sealedSlot });
}
//# sourceMappingURL=writer-dlmm-accounts.js.map