...
import { CURRENT_WRITER_LIQUIDITY_OPERATIONS, CURRENT_WRITER_DLMM_POLICY_OPERATIONS, isCurrentWriterDlmmOperation, validateCurrentWriterDlmmRequest } from "../protocol/writer-dlmm-public.js";
...
import { encodeBase64, decodeBase64, sha256Hex, canonicalDigest, canonicalIndex, canonicalJson, canonicalU64, exactArray, exactObject, sha256Canonical, walletError, } from "./codec.js";
import { decodeInstructionManifest, pubkey, requireWriteSet, } from "./manifest.js";
export function decodeCollectiveOperationPrepareEnvelope(value) {
    ...
    const operation = root.operationPlan.operation;
    ...
    const leanAdmission = exactObject(root.leanAdmission, [], Object.keys(root.leanAdmission), "leanAdmission");
    ...
    const stage = root.stage === undefined ? null : exactObject(root.stage, ["operation"], root.stage !== null && typeof root.stage === "object" && "seriesIndex" in root.stage ? ["seriesIndex"] : [], "stage");
    return Object.freeze({ operationPlan: plan, leanAdmission, leanAdmissionDigest, closeRequest, stage });
}
function decodeWriterPlan(value) {
    ...
    if (probe.operation === "settlement_claim_collective") {
        if (schemaVersion !== 1)
            walletError("CURRENT_WALLET_PLAN_INVALID", "collective claim requires a hot plan");
    ...
    const object = exactObject(value, schemaVersion === 1 ? baseKeys : [...baseKeys, ...v2Keys], schemaVersion === 2 ? ["coldAccountProofFacts", "canonicalOutputSetupFacts", "classicOutputSetupFacts", "transactionLookupTable"] : [], "writer operation plan");
    ...
    let setupMode = "none";
    let coldProofFacts = null;
    let canonicalOutputFacts = null;
    let classicOutputFacts = null;
    let setupSignerRoles = Object.freeze([]);
    let executionBatches = Object.freeze([instructions]);
    let actionBatchIndex = 0;
    let allManifests = instructions;
    let transactionLookupTable;
    if (object.transactionLookupTable !== undefined) {
        if (!isCurrentWriterDlmmOperation(operation))
            walletError("CURRENT_WALLET_PLAN_INVALID", "lookup table is outside writer liquidity transport");
    ...
    if (schemaVersion === 2) {
        ...
        if (object.classicOutputSetupFacts !== undefined) {
            if (!["withdraw_principal", "auction_refund", "close_finalize"].includes(operation) || setupMode === "canonical_output_create")
                walletError("CURRENT_WALLET_PLAN_INVALID", "classic output setup operation is invalid");
        ...
        if (setupMode === "writer_liquidity_compute") {
            ...
            setupSignerRoles = decodeSetupSignerRoles(object.setupSignerRoles, setupBatches, "setupSignerRoles");
...
function decodeFlatPlan(value) {
    ...
    const observedBytes = exactObject(object.observedBytes, ["mintDataBase64", "sourceLightDataBase64", "destinationLightDataBase64"], [], "observedBytes");
    for (const field of ["mintDataBase64", "sourceLightDataBase64", "destinationLightDataBase64"]) {
...
function decodeSwapPlan(value) {
    ...
    const computeInstruction = Object.freeze({
...
function writerOperation(value) {
...
function writerSemantic(operation, value) {
    if (isCurrentWriterDlmmOperation(operation)) {
        if (!value || typeof value !== "object" || Array.isArray(value) || Object.hasOwn(value, "operation"))
            walletError("CURRENT_WALLET_RESPONSE_INVALID", "writer liquidity semantic is invalid");
        try {
    ...
    if (operation === "withdraw_principal")
        fields = ["owner", "sleeve", "amountAtoms"];
    else if (operation === "auction_refund")
        fields = ["owner", "auction", "bid"];
    else if (operation === "deposit")
        fields = ["owner", "sleeve", "principalAtoms"];
    else if (operation === "bid")
        fields = ["owner", "auction", "seriesIndex", "pricePerContractAtoms", "quantityAtoms"];
    else if (operation === "close_begin")
        fields = ["owner", "sleeve", "flatParAtoms", "minimumWithdrawalAtoms"];
        ...
        fields = ["owner", "closeRequest"];
    const object = exactObject(value, fields, [], "semantic");
    const result = { owner: pubkey(object.owner, "semantic.owner").toBase58() };
    if ("sleeve" in object)
        result.sleeve = pubkey(object.sleeve, "semantic.sleeve").toBase58();
    if ("bid" in object)
        result.bid = pubkey(object.bid, "semantic.bid").toBase58();
    if (operation === "withdraw_principal")
        result.amountAtoms = canonicalU64(object.amountAtoms, "semantic.amountAtoms", true);
    if ("auction" in object)
        result.auction = pubkey(object.auction, "semantic.auction").toBase58();
    if ("closeRequest" in object)
        result.closeRequest = pubkey(object.closeRequest, "semantic.closeRequest").toBase58();
    if (operation === "deposit")
        result.principalAtoms = canonicalU64(object.principalAtoms, "semantic.principalAtoms", true);
    if (operation === "bid") {
    ...
    if ((operation === "settlement_claim_flat" || operation === "settlement_claim_collective")) {
        ...
        if (object.claimVariant !== expectedVariant)
            walletError("CURRENT_WALLET_RESPONSE_INVALID", "claimVariant differs from operation");
...
function swapSemantic(value, observation) {
    const fields = [
...
function decodeSignerRoles(value, manifests, label) {
    const roles = exactArray(value, label, 1, 8).map((entry, index) => {
        ...
        const indexes = exactArray(role.instructionIndexes, `${label}[${index}].instructionIndexes`, 1, manifests.length)
            .map((entryValue, entryIndex) => canonicalIndex(entryValue, `${label}[${index}].instructionIndexes[${entryIndex}]`, manifests.length - 1));
    ...
    manifests.forEach((manifest, instructionIndex) => manifest.accounts.filter((meta) => meta.isSigner).forEach((meta) => {
        ...
        if (!indexes.includes(instructionIndex))
...
function decodeSetupSignerRoles(value, batches, label) {
    if (batches.length === 0) {
        if (!Array.isArray(value) || value.length !== 0)
            walletError("CURRENT_WALLET_PLAN_INVALID", `${label} must be empty`);
        return Object.freeze([]);
    }
    ...
    return Object.freeze(values.map((entry, index) => {
        ...
        const expected = manifests.flatMap((manifest, instructionIndex) => manifest.accounts.some((meta) => meta.address === signer && meta.isSigner) ? [instructionIndex] : []);
...
function decodeManifestBatches(value, label, minimum, maximum, tags) {
    return Object.freeze(exactArray(value, label, minimum, maximum).map((batch, batchIndex) => Object.freeze(exactArray(batch, `${label}[${batchIndex}]`, 1, 32).map((entry, instructionIndex) => decodeInstructionManifest(entry, `${label}[${batchIndex}][${instructionIndex}]`, tags === "optional" ? {} : { instructionTag: tags })))));
}
function writerColdFacts(value) {
    const object = exactObject(value, [
        "ata", "owner", "mint", "amountAtoms", "minimumAmountAtoms", "includesColdBalance",
        "providerOriginSha256", "payer",
    ], [], "coldAccountProofFacts");
    if (object.includesColdBalance !== true)
        walletError("CURRENT_WALLET_RESPONSE_INVALID", "cold proof does not include balance");
    return Object.freeze({
...
function writerOutputFacts(value) {
    const object = exactObject(value, ["ata", "owner", "mint", "payer"], [], "canonicalOutputSetupFacts");
    return Object.freeze(Object.fromEntries(Object.entries(object).map(([key, entry]) => [key, pubkey(entry, `canonicalOutputSetupFacts.${key}`).toBase58()])));
}
function flatColdFacts(value, label) {
    const object = exactObject(value, [
        "ata", "owner", "mint", "amountAtoms", "includesColdBalance", "providerOriginSha256", "payer",
    ], [], label);
    if (object.includesColdBalance !== true)
        walletError("CURRENT_WALLET_RESPONSE_INVALID", `${label} does not include balance`);
    return Object.freeze({
...
function validateLeanAdmission(plan, admission, stageValue) {
    ...
    if (["close_basket", "close_finalize", "close_cancel"].includes(plan.operation)) {
        const stage = exactObject(stageValue, ["operation"], stageValue !== null && typeof stageValue === "object" && "seriesIndex" in stageValue ? ["seriesIndex"] : [], "stage");
        ...
        const facts = exactObject(admission.facts, [], Object.keys(admission.facts), "leanAdmission.facts");
...
function assertCanonicalJsonValue(value, label) {
    try {
        canonicalJson(value);
    }
    catch {
