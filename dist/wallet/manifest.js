...
import { canonicalIndex, canonicalPublicKeyString, decodeBase64, equalBytes, exactArray, exactObject, readU16Le, u64Le, walletError, } from "./codec.js";
...
export function decodeInstructionManifest(value, label, options = {}) {
    const tagRequired = options.instructionTag === "required";
    const object = exactObject(value, tagRequired
        ? ["programId", "instructionName", "instructionTag", "dataBase64", "accounts"]
        : ["programId", "instructionName", "dataBase64", "accounts"], options.instructionTag === undefined ? ["instructionTag"] : [], label);
...
export function manifestInstruction(manifest) {
    return new TransactionInstruction({
        ...
        data: decodeBase64(manifest.dataBase64, `${manifest.instructionName}.dataBase64`),
    });
}
...
export function requireExactInstruction(actual, expected, label) {
...
export function requireFlags(manifest, flags, label) {
...
export function requireAddress(manifest, index, expected, label) {
...
export function requireWriteSet(value, manifests, label) {
    const entries = exactArray(value, label, 1, 256).map((entry, index) => pubkey(entry, `${label}[${index}]`).toBase58());
...
export function deriveWriterPda(seed, keys, extra = []) {
    return PublicKey.findProgramAddressSync([NAMESPACE, TEXT.encode(seed), ...keys.map((key) => key.toBytes()), ...extra], CURRENT_PROGRAM_ID)[0];
}
export function deriveWriterFlatMint(sleeve) {
    return deriveWriterPda("writer_flat_mint_v1", [sleeve]);
}
export function deriveWriterSleeveUsdcVault(sleeve) {
    return deriveWriterPda("writer_sleeve_usdc_v1", [sleeve]);
}
export function deriveWriterFlatStaging(sleeve) {
    return deriveWriterPda("writer_flat_staging_v1", [sleeve]);
}
export function deriveWriterFlatBurnCustody(sleeve) {
    return deriveWriterPda("writer_flat_burn_v1", [sleeve]);
}
export function deriveWriterSeriesBook(sleeve) {
    return deriveWriterPda("writer_series_book_v1", [sleeve]);
}
export function deriveWriterSleeve(group) {
    return deriveWriterPda("writer_sleeve_v1", [group]);
}
export function deriveWriterBidIndex(auction) {
    return deriveWriterPda("writer_bid_index_v1", [auction]);
}
export function deriveWriterBid(auction, bidder, orderId) {
    return deriveWriterPda("writer_bid_v1", [auction, bidder], [u64Le(orderId)]);
}
export function deriveWriterCloseRequest(sleeve, nonce) {
    return deriveWriterPda("writer_close_v1", [sleeve], [u64Le(nonce)]);
}
export function deriveWriterCloseFlatEscrow(request) {
    return deriveWriterPda("writer_close_flat_escrow_v1", [request]);
}
export function deriveWriterRetirementCustody(sleeve, market) {
    return deriveWriterPda("writer_retirement_v1", [sleeve, market]);
}
export function deriveClassicAta(mint, owner) {
    if (!PublicKey.isOnCurve(owner.toBytes())) {
        walletError("CURRENT_WALLET_PLAN_INVALID", "classic ATA owner must be on curve");
    }
    return PublicKey.findProgramAddressSync([owner.toBytes(), SPL_TOKEN_PROGRAM_ID.toBytes(), mint.toBytes()], ASSOCIATED_TOKEN_PROGRAM_ID)[0];
}
export function deriveLightAta(mint, owner) {
    if (!PublicKey.isOnCurve(owner.toBytes())) {
        walletError("CURRENT_WALLET_PLAN_INVALID", "Light ATA owner must be on curve");
    }
    return PublicKey.findProgramAddressSync([owner.toBytes(), LIGHT_TOKEN_PROGRAM_ID.toBytes(), mint.toBytes()], LIGHT_TOKEN_PROGRAM_ID)[0];
}
export function deriveLightSplInterface(mint) {
    return PublicKey.findProgramAddressSync([TEXT.encode("pool"), mint.toBytes()], LIGHT_TOKEN_PROGRAM_ID)[0];
}
export function deriveDlmmPool(market) {
    return PublicKey.findProgramAddressSync([NAMESPACE, TEXT.encode("ameba-dlmm-pool-v1"), market.toBytes()], CURRENT_PROGRAM_ID)[0];
}
export function deriveDlmmAuthority(pool) {
    return PublicKey.findProgramAddressSync([NAMESPACE, TEXT.encode("ameba-dlmm-authority-v1"), pool.toBytes()], CURRENT_PROGRAM_ID)[0];
}
export function deriveDlmmBinPage(pool, pageIndex) {
    const bytes = new Uint8Array(2);
    new DataView(bytes.buffer).setUint16(0, canonicalIndex(pageIndex, "pageIndex", 0xffff), true);
    return PublicKey.findProgramAddressSync([NAMESPACE, TEXT.encode("ameba-dlmm-page-v1"), pool.toBytes(), bytes], CURRENT_PROGRAM_ID)[0];
}
export function buildCreateLightAta(payer, owner, mint) {
    return new TransactionInstruction({
        ...
        data: Uint8Array.of(102, 1, 3, 16, 1, 254, 2, 0, 0, 0),
    });
}
export function requireCanonicalComputeBudget(instruction) {
    ...
    const units = new DataView(instruction.data.buffer, instruction.data.byteOffset + 1, 4).getUint32(0, true);
...
export function requireCanonicalLightTransfer2(instruction, payer, owner, mint, target) {
...
export function requireCanonicalSetupBatch(input) {
    ...
    requireExactInstruction(input.batch[1], buildCreateLightAta(input.payer, input.owner, input.mint), "cold setup Light ATA create");
...
export function readManifestU16(manifest, offset) {
