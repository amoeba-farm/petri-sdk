...
import { LIGHT_COMPRESSION_AUTHORITY, LIGHT_COMPRESSION_PROGRAM, LIGHT_REGISTERED_PROGRAM, LIGHT_SYSTEM_PROGRAM, LIGHT_TOKEN_CPI_AUTHORITY, LIGHT_TOKEN_PROGRAM_ID, } from "./manifest.js";
 ...
 */
export function validateCurrentWalletLightTransfer2LoadSequence(transfers, expected) {
    ...
    for (const transfer of transfers) {
        ...
        if (totalAmount > U64_MAX)
            invalid("cold load amount exceeds the current u64 token domain");
        for (const input of decoded.inputs) {
            const identity = `${input.tree}:${input.queue}:${input.leafIndex}`;
            if (inputs.has(identity))
                invalid("cold load reuses one compressed input across batches");
            inputs.add(identity);
        }
    }
...
function decodeTransfer2Load(instruction, expected) {
    ...
    if (proof !== null && proof.every((byte) => byte === 0))
        invalid("Transfer2 full proof is all zero");
    const inputCount = cursor.vecLength("inTokenData", TRANSFER2_MAX_INPUTS);
    if (inputCount < 1)
        invalid("Transfer2 contains no compressed token input");
    const inputs = [];
    ...
    for (let index = 0; index < inputCount; index += 1) {
        ...
        if (inputAmount > U64_MAX)
            invalid("Transfer2 input amount exceeds u64");
...
function sameMeta(meta, address, isSigner, isWritable) {
...
class Cursor {
    ...
    bool(field) {
        ...
        if (value > 1)
            invalid(`${field} is not a canonical Borsh bool`);
        return value === 1;
    }
    u16(field) {
    ...
    option(field) {
        ...
        if (value > 1)
            invalid(`${field} has a noncanonical Borsh option`);
        return value === 1;
    }
    vecLength(field, maximum) {
        const value = this.u32(`${field} length`);
        if (value > maximum)
            invalid(`${field} exceeds its current bound`);
        return value;
    }
    bytes(length, field) {
    ...
    finish() {
        if (this.#offset !== this.#bytes.length)
            invalid("Transfer2 contains trailing bytes");
    }
    #require(length, field) {
        if (this.#offset + length > this.#bytes.length)
            invalid(`Transfer2 ${field} is truncated`);
    }
}
function invalid(message) {
    walletError("CURRENT_WALLET_PLAN_INVALID", `current Light Transfer2 invalid: ${message}`);
}
//# sourceMappingURL=light-transfer2.js.map