...
import { canonicalIndex, canonicalJson, canonicalU64, decodeBase64, equalBytes, exactArray, exactObject, readU16Le, readU32Le, readU64Le, sha256Hex, u64Le, walletError } from "./codec.js";
import { CURRENT_PROGRAM_ID, LIGHT_TOKEN_CPI_AUTHORITY, LIGHT_TOKEN_PROGRAM_ID, SPL_TOKEN_PROGRAM_ID, SYSTEM_PROGRAM_ID, deriveDlmmAuthority, deriveDlmmBinPage, deriveDlmmPool, deriveLightAta, deriveLightSplInterface, pubkey, requireExactInstruction } from "./manifest.js";
import { decodeCurrentWalletDlmmPage, decodeCurrentWalletDlmmPool } from "./swap-accounts.js";
const TEXT = new TextEncoder();
function invalid(message) { return walletError("CURRENT_WALLET_CLEANUP_INVALID", message); }
function uint(value, width) {
    let remaining = BigInt(value);
    if (remaining < 0n)
        invalid("cleanup integers cannot be negative");
    ...
    if (remaining !== 0n)
        invalid("cleanup integer exceeds its wire bound");
    return bytes;
}
function derive(seed, ...parts) {
    return PublicKey.findProgramAddressSync([TEXT.encode("ameba-spread-v2"), TEXT.encode(seed), ...parts], CURRENT_PROGRAM_ID);
}
function account(accounts, address, length, outer, inner, bump) {
    ...
    if (!value || value.executable || !value.owner.equals(CURRENT_PROGRAM_ID) || value.data.length !== length ||
        ...
        !equalBytes(value.data.slice(10, 13), TEXT.encode(inner)) || value.data[13] !== 1)
        invalid("cleanup account identity is invalid");
    return value.data;
}
    ...
    return new TransactionInstruction({ programId: pubkey(row.programId, "programId"),
        ...
        keys: exactArray(row.accounts, "instruction accounts", 0, 64).map((raw) => {
            ...
            if (typeof meta.isSigner !== "boolean" || typeof meta.isWritable !== "boolean")
                invalid("cleanup flags are invalid");
            return { pubkey: pubkey(meta.pubkey, "account"), isSigner: meta.isSigner, isWritable: meta.isWritable };
        }) });
}
export function validateWalletLpCleanup(raw, request, expectedMarket, expectedPosition, owner, fresh) {
    ...
    if (plan.schemaVersion !== 1 || plan.stateNamespace !== "ameba-spread-v2" || plan.operation !== "amoeba_dlmm_liquidity" ||
        ...
        request.ownerPubkey !== owner.toBase58())
        invalid("cleanup does not match the locally reviewed intent");
    ...
    const digest = (domain, value) => sha256Hex(TEXT.encode(domain + canonicalJson(value)));
    ...
    if (!position.equals(pubkey(expectedPosition, "reviewed position")) || !pool.liquidityManager.equals(owner) ||
        ...
        liquidity.marketId !== request.marketId || liquidity.expiryId !== request.expiryId)
        invalid("cleanup target or owner changed");
    ...
    if (lower < 1 || count < 1 || count > 32 || lower + count - 1 > pool.maximumBinId)
        invalid("cleanup position range is invalid");
    ...
    const entries = rows.map((value, index) => {
        ...
        if (binId < lower || binId >= lower + count || (index > 0 && binId <= request.entries[index - 1].binId))
            invalid("cleanup bins are not canonical");
        if (typeof row.shares !== "string" || !/^[1-9][0-9]*$/u.test(row.shares))
            invalid("cleanup shares must be positive canonical integers");
    ...
    for (let i = 0; i < 32; i += 1) {
        ...
        if ((i >= count && owned !== 0n) || BigInt(selected?.shares ?? "0") !== owned)
            invalid("cleanup must close exactly the owned shares");
    }
    if (readU32Le(positionBytes, 89) !== bitmap)
        invalid("cleanup position bitmap is inconsistent");
    ...
    if (pages.some((page, index) => index > 0 && page <= pages[index - 1]) ||
        pages.length > touched.length + 2 || touched.some((page) => !pages.includes(page)) || (!entries.length && pages.length))
        invalid("cleanup page route is invalid");
    const pagePairs = pages.flatMap((page) => {
        ...
        if (!new PublicKey(data.slice(14, 46)).equals(poolAddress) || readU16Le(data, 46) !== page || readU16Le(data, 48) !== page * 32 + 1)
            invalid("cleanup share page is not canonical");
...
//# sourceMappingURL=liquidity-cleanup.js.map