...
import { deriveOracleRecipeSourceIndexPda, deriveOracleBucketSourceIndexPda, decodeOracleRecipeSourceIndex, decodeOracleBucketSourceIndex, deriveOracleMemberPagePda, decodeOracleMemberPage } from "@amoeba/spread-release-tools/oracle-dlmm";
...
export async function readG3MembershipPage(input) {
    ...
    if (!Number.isSafeInteger(pages) || pages < 1 || pages > 16 || !Number.isSafeInteger(input.minimumSlot)
        || input.minimumSlot < 0)
        throw new Error("Invalid membership page bound");
    const hashes = [input.bucketId, input.expectedRecipeHash, input.expectedManifestHash].map(v => Buffer.from(v));
    if (hashes.some(v => v.length !== 32 || v.every(b => b === 0)))
        throw new Error("Membership requires exact nonzero hashes");
    ...
    if (c && (c.version !== 3 || c.scope !== scope || !Number.isSafeInteger(c.nextPage) || c.nextPage < 1
        || !Number.isSafeInteger(c.minimumSlot) || c.minimumSlot < 0))
        throw new Error("Foreign or invalid membership cursor");
    if (await input.connection.getGenesisHash() !== input.genesisHash)
        throw new Error("Membership genesis mismatch");
    ...
    if (!Number.isSafeInteger(raw.context.slot) || raw.context.slot < floor || raw.value.length !== 2)
        throw new Error("Incomplete root observation");
    const [rootInfo, bucketInfo] = raw.value;
    for (const info of raw.value)
        if (!info || info.executable || !info.owner.equals(input.programId))
            throw new Error("Invalid membership root owner");
    const root = decodeOracleRecipeSourceIndex({ ...s, data: rootInfo.data });
    if (!root.complete || !root.recipeHash.equals(hashes[1]) || !root.manifestHash.equals(hashes[2]))
        throw new Error("Unfinished or mismatched frozen root");
    ...
    if (first >= totalPages)
        throw new Error("Cursor exceeds frozen membership");
    ...
    if (!Number.isSafeInteger(batch.context.slot) || batch.context.slot < raw.context.slot || batch.value.length !== indices.length)
        throw new Error("Incomplete member-page observation");
    const ids = [];
    let previous;
    for (let j = 0; j < indices.length; j++) {
        const info = batch.value[j];
        if (!info || info.executable)
            throw new Error("Missing member page");
        ...
        for (const id of decoded.descendingIds) {
            if (previous && Buffer.compare(previous, id) <= 0)
                throw new Error("Member pages overlap or reorder sources");
            previous = id;
            if (indices[j] >= first)
                ids.push(Buffer.from(id));
        }
    }
    const cursor = end === totalPages ? null : Object.freeze({ version: 3, scope, nextPage: end, minimumSlot: batch.context.slot });
