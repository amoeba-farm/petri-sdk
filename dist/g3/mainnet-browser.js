...
import { PublicKey, VersionedTransaction, TransactionMessage, ComputeBudgetProgram, AddressLookupTableAccount, AddressLookupTableProgram } from "@solana/web3.js";
...
const hash = async (bytes) => Buffer.from(await globalThis.crypto.subtle.digest("SHA-256", new Uint8Array(typeof bytes === "string" ? Buffer.from(bytes) : bytes))).toString("hex");
...
export async function materializeMainnetG3Plan(input) {
    const plan = structuredClone(input.plan), expected = structuredClone(input.expected), connection = input.connection;
    if (connection.rpcEndpoint !== "https://api.amoeba.farm/rpc")
        throw new Error("MAINNET_BROWSER_GATEWAY_REQUIRED");
    if (!Object.hasOwn(builders, expected.operation) || plan.schemaVersion !== 3 || plan.owner !== expected.owner || plan.operation !== expected.operation
        ...
        throw new Error("MAINNET_G3_PLAN_IDENTITY_INVALID");
    ...
    if (digest !== plan.operationId || digest !== plan.preparedPlanDigest)
        throw new Error("MAINNET_G3_PLAN_DIGEST_INVALID");
    if (typeof plan.serializedTransactionBase64 !== "string" || plan.serializedTransactionBase64.length > 1644)
        throw new Error("MAINNET_G3_PACKET_INVALID");
    const bytes = Buffer.from(plan.serializedTransactionBase64, "base64");
    if (bytes.length > 1232 || bytes.toString("base64") !== plan.serializedTransactionBase64)
        throw new Error("MAINNET_G3_PACKET_INVALID");
    ...
    if (supplied.signatures.some(sig => sig.some(b => b !== 0)) || supplied.message.header.numRequiredSignatures !== 1
        || supplied.message.staticAccountKeys[0]?.toBase58() !== expected.owner)
        throw new Error("MAINNET_G3_SIGNER_INVALID");
    ...
    if (context.epoch.toString() !== plan.epoch || context.gate.toBase58() !== p.gate)
        throw new Error("MAINNET_G3_GATE_CHANGED");
    ...
    if (!Number.isSafeInteger(accounts.context.slot) || accounts.context.slot < context.finalizedObservationSlot || !program || !data || !program.executable || data.executable
        ...
        || data.data.length !== p.programDataAccountBytes || await hash(data.data) !== p.programDataAccountSha256)
        throw new Error("MAINNET_G3_EXECUTABLE_MISMATCH");
    ...
    const builder = builders[expected.operation];
    const instruction = builder({ ...resolved.input.builderInput, programId: bindAmoebaGovernanceGateContextV1(context) });
    ...
    if (instructions.flatMap(ix => ix.keys.filter(k => k.isSigner)).some(k => k.pubkey.toBase58() !== expected.owner))
        throw new Error("MAINNET_G3_SIGNER_INVALID");
    let table;
    if (plan.lookupTable) {
        ...
        if (read.context.slot < resolved.observedSlot || !read.value || read.value.executable || !read.value.owner.equals(AddressLookupTableProgram.programId)
            || read.value.data.toString("base64") !== plan.lookupTable.dataBase64)
            throw new Error("MAINNET_G3_LOOKUP_CHANGED");
        ...
        if (!table.isActive() || table.state.lastExtendedSlot >= read.context.slot)
            throw new Error("MAINNET_G3_LOOKUP_INVALID");
    }
    ...
    if (!Buffer.from(messageBytes).equals(Buffer.from(supplied.message.serialize())) || await hash(messageBytes) !== plan.messageSha256)
        throw new Error("MAINNET_G3_INTENT_MISMATCH");
    const valid = await connection.isBlockhashValid(plan.blockhash, { commitment: "finalized", minContextSlot: resolved.observedSlot });
    if (!valid.value || valid.context.slot < resolved.observedSlot || await connection.getBlockHeight({ commitment: "finalized", minContextSlot: resolved.observedSlot }) > plan.lastValidBlockHeight)
        throw new Error("MAINNET_G3_LIFETIME_EXPIRED");
    return Object.freeze({ transaction, messageBytes, owner: expected.owner, operation: expected.operation, operationId: plan.operationId,
        ...
        validateSignedTransaction: (signedBase64) => validateG3SignedMessage(plan, signedBase64) });
}
//# sourceMappingURL=mainnet-browser.js.map