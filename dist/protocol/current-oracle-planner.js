...
import { CURRENT_ORACLE_PRE_LISTING_WINDOW_SECONDS, CURRENT_ORACLE_MONTH_ACCOUNT_SIZE, CURRENT_ORACLE_SKU_COVERAGE_MANIFEST_ACCOUNT_SIZE, decodeCurrentUserCollateralAccount, decodeCurrentMarketAccount, decodeCurrentOracleMonthAccount, decodeCurrentVaultConfigAccount, CURRENT_PROTOCOL_DEVNET_GENESIS_HASH, decodeCurrentOracleSkuCoverageManifestAccount, validateUninitializedCurrentOracleMonthData, validateUninitializedCurrentOracleSkuCoverageManifestData, } from "./current.js";
...
class Reader {
    ...
    label;
    offset = 0;
    constructor(data, label, expectedLength) {
    ...
    bytes(length, field) { return this.take(length, field); }
    ...
    currentHeader(discriminator, version = 1) {
...
function enumValue(index, values, label) {
...
function assertCanonicalPda(address, expected, bump, programId, seeds, label) {
...
async function requireAcquireableInitializationTarget(input) {
    ...
    if (!info.executable && info.owner.equals(input.programId) && info.data.length === input.length) {
        try {
            ...
            throw new CurrentOraclePlannerError("CURRENT_ORACLE_ACCOUNT_ALREADY_EXISTS", `${input.label} program-owned target is initialized or not an exact uninitialized current layout: ${cause instanceof Error ? cause.message : String(cause)}`);
        }
    }
    throw new CurrentOraclePlannerError("CURRENT_ORACLE_ACCOUNT_ALREADY_EXISTS", `${input.label} is not an absent, system-zero, or exact program-owned uninitialized target`);
}
...
function challengeBond(backing, bps, minimum, maximum) {
    ...
    const proportional = (backing * BigInt(bps)) / 10000n;
    return proportional < minimum ? minimum : proportional > maximum ? maximum : proportional;
}
function equalRewardShare(budget, count, label) {
...
function currentRewardEntitlement(input) {
    ...
    if (input.kind === "opening") {
        ...
        const proposer = reward.supporterCount === 0
            ? sourceShare
            : (sourceShare * BigInt(input.pool.proposerRewardBps)) / 10000n;
...
const compressedObservationCache = new WeakMap();
function observeCompressedOnce(input, canonicalPda, domain) {
    if (input.photonConnection === undefined) {
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_COMPRESSED_STATE_UNAVAILABLE", "current compact Oracle state requires the authenticated Photon capability");
    }
...
function collectObservation(input, observation, label) {
    if (observation.transportObservation === undefined)
        return;
    ...
    if (prior === undefined)
        collector.push(observation.transportObservation);
...
async function requiredCompressedObservation(input, canonicalPda, domain, label) {
...
async function optionalCompressedObservation(input, canonicalPda, domain, label) {
    const observation = await observeCompressedOnce(input, canonicalPda, domain);
    if (observation === null)
        return null;
    collectObservation(input, observation, label);
...
async function readSkuPool(input, bucketId) {
...
async function readSkuPoolByAddress(input, address, expectedBucketId) {
    const schedule = deriveOracleUsdcRewardSchedulePda({ oracleMonthPda: input.oracleMonthAddress, programId: input.programId });
    ...
    const registeredSourceCount = reader.u32("registered_source_count");
    const registeredOpeningCount = reader.u32("registered_opening_count");
    const registeredUpdateCount = reader.u32("registered_update_count");
...
async function readSource(input, sourceId, descriptorRequired = false, collectDescriptor = descriptorRequired) {
...
async function readSourceByAddress(input, address, expectedSourceId, descriptorRequired = false, collectDescriptor = descriptorRequired) {
    ...
    const status = enumValue(reader.u8("status"), ["Candidate", "Frozen", "Inactive", "Rejected", "OpeningPending", "Active", "Merged", "TimedOut"], "OracleSource.status");
    ...
    let canonicalLocatorHash = Buffer.from(ZERO_32);
    let sourceDefinitionHash = Buffer.from(ZERO_32);
...
async function requireAcquireableSourceChallengeGuard(input, source) {
    ...
    reader.u64("last_updated_slot");
    reader.finish();
    assertCanonicalPda(address, address, bump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_SOURCE_CHALLENGE_GUARD_PDA_SEED, input.oracleMonthAddress.toBuffer(), source.address.toBuffer()], "OracleSourceChallengeGuard");
    if (!month.equals(input.oracleMonthAddress)
...
async function readOpeningClaim(input, source) {
    ...
    const status = enumValue(reader.u8("status"), ["Empty", "Pending", "Challenged", "Accepted", "Rejected", "TimedOut"], "OracleOpeningClaim.status");
    const escrowDisposition = enumValue(reader.u8("escrow_disposition"), ["Unsettled", "Refunded", "Slashed", "Transferred"], "OracleOpeningClaim.escrow_disposition");
    reader.finish();
...
async function requireAcquireableOpeningClaim(input, source) {
...
async function readUpdateClaim(input, source, claimant, claimId) {
    return readUpdateClaimByIdentity(input, source.address, source.sourceId, claimant, claimId);
}
export async function readUpdateClaimByIdentity(input, sourceAddress, expectedSourceId, claimant, claimId) {
    ...
    const storedSource = reader.pubkey("source");
    const storedSourceId = reader.bytes(32, "source_id");
    const storedClaimant = reader.pubkey("claimant");
    const priorState = reader.u64("prior_state");
    const newState = reader.u64("new_state");
    const sourceTime = reader.u64("source_time");
    const stake = reader.u64("stake");
    const status = enumValue(reader.u8("status"), ["Open", "Committed", "Revealed", "Finalized", "Rejected", "TimedOut"], "OracleUpdateClaim.status");
    const evidenceHash = reader.bytes(32, "evidence_hash");
    const archiveUrlHash = reader.bytes(32, "archive_url_hash");
    const escrowDisposition = enumValue(reader.u8("escrow_disposition"), ["Unsettled", "Refunded", "Slashed", "Transferred"], "OracleUpdateClaim.escrow_disposition");
    if (!reader.bytes(3, "account_discriminator").equals(Buffer.from("UC2")) || reader.u8("account_version") !== 2)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_ACCOUNT_LAYOUT_MISMATCH", "OracleUpdateClaimV2 marker is not UC2/v2");
...
async function readUpdateChallenge(input, claim, challengeId) {
    ...
    const reader = new Reader(await requiredProgramData({ ...input, address, label: "OracleUpdateChallenge", length: 320 }), "OracleUpdateChallenge", 320);
    const bump = reader.currentHeader();
    const month = reader.pubkey("month");
    ...
    const status = enumValue(reader.u8("status"), ["Open", "RuleReview", "RuleReviewUnresolved", "Accepted", "Rejected", "Cancelled"], "OracleUpdateChallenge.status");
    const evidenceHash = reader.bytes(32, "evidence_hash");
    const archiveUrlHash = reader.bytes(32, "archive_url_hash");
    const ruleReviewSlot = reader.u64("rule_review_slot");
    const escrowDisposition = enumValue(reader.u8("escrow_disposition"), ["Unsettled", "Refunded", "Slashed", "Transferred"], "OracleUpdateChallenge.escrow_disposition");
    ...
    reader.finish();
    assertCanonicalPda(address, address, bump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_UPDATE_CHALLENGE_PDA_SEED, input.oracleMonthAddress.toBuffer(), claim.address.toBuffer(), storedChallengeId], "OracleUpdateChallenge");
    if (!month.equals(input.oracleMonthAddress)
...
async function readUpdateChallengeGuard(input, claim, challenge) {
    const address = deriveOracleUpdateChallengeGuardPda({
        oracleMonthPda: input.oracleMonthAddress,
        updateClaimPda: claim.address,
        programId: input.programId,
    });
    const reader = new Reader(await requiredProgramData({ ...input, address, label: "OracleUpdateChallengeGuard", length: 224 }), "OracleUpdateChallengeGuard", 224);
    ...
    reader.finish();
    assertCanonicalPda(address, address, bump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_UPDATE_CHALLENGE_GUARD_PDA_SEED, input.oracleMonthAddress.toBuffer(), claim.address.toBuffer()], "OracleUpdateChallengeGuard");
    if (!month.equals(input.oracleMonthAddress)
        || !storedClaim.equals(claim.address)
        || !storedClaimId.equals(claim.claimId)
...
async function readRewardSchedule(input) {
    ...
    const bump = reader.currentHeader("URS", 2);
    const month = reader.pubkey("month");
    const authority = reader.pubkey("authority");
    const rewardVault = reader.pubkey("reward_vault");
    const phase = enumValue(reader.u8("phase"), ["Building", "Funded", "EntitlementsFinalized", "Aborted"], "OracleUsdcRewardSchedule.phase");
    reader.u16("sku_pool_count");
    reader.u32("registered_source_count");
    reader.u32("registered_opening_count");
    reader.u32("registered_update_count");
    reader.u32("registered_update_reward_units");
    const totalRewardBudget = reader.u64("total_reward_budget");
    const remainingRewardBudget = reader.u64("remaining_reward_budget");
    reader.u64("last_updated_slot");
    reader.u32("outstanding_prelisting_escrow_count");
    reader.u64("trading_fee_bounty_total");
    reader.bool("bounty_fee_sweep_finalized");
    reader.finish();
    ...
    if (!month.equals(input.oracleMonthAddress) || authority.equals(PublicKey.default) || !rewardVault.equals(deriveOracleUsdcRewardVaultPda(input.programId)) || remainingRewardBudget > totalRewardBudget)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_ACCOUNT_IDENTITY_MISMATCH", "OracleUsdcRewardSchedule identity is invalid");
    return { address, month, phase, rewardVault, totalRewardBudget, remainingRewardBudget };
}
async function readRewardVault(input) {
    const address = deriveOracleUsdcRewardVaultPda(input.programId);
    const reader = new Reader(await requiredProgramData({ ...input, address, label: "OracleUsdcRewardVault", length: 192 }), "OracleUsdcRewardVault", 192);
    ...
    reader.u64("last_updated_slot");
    reader.finish();
    assertCanonicalPda(address, address, bump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_USDC_REWARD_VAULT_PDA_SEED], "OracleUsdcRewardVault");
...
async function readOracleEconomicsConfig(input) {
    const address = deriveOracleEconomicsConfigPda(input.programId);
    const reader = new Reader(await requiredProgramData({ ...input, address, label: "OracleEconomicsConfig", length: 40 }), "OracleEconomicsConfig", 40);
    ...
    reader.u64("last_updated_slot");
    reader.finish();
    assertCanonicalPda(address, address, bump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_ECONOMICS_CONFIG_PDA_SEED], "OracleEconomicsConfig");
    if (configVersion === 0n
...
async function validateUnresolvedVotingState(input) {
    const stakingPool = deriveOracleStakingPoolPda(input.programId);
    const reader = new Reader(await requiredProgramData({ ...input, address: stakingPool, label: "OracleStakingPool", length: 160 }), "OracleStakingPool", 160);
    ...
    reader.finish();
    assertCanonicalPda(stakingPool, stakingPool, bump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_STAKING_POOL_PDA_SEED], "OracleStakingPool");
    if (!majorTokenConfig.equals(deriveOracleMajorTokenConfigPda(input.programId))
...
async function readSourceReward(input, source) {
    const schedule = deriveOracleUsdcRewardSchedulePda({ oracleMonthPda: input.oracleMonthAddress, programId: input.programId });
    ...
    const terminalStatus = enumValue(reader.u8("terminal_status"), ["Candidate", "Frozen", "Inactive", "Rejected", "OpeningPending", "Active", "Merged", "TimedOut"], "OracleUsdcSourceReward.terminal_status");
...
export async function readClassicSourceReward(input, sourceAddress) {
    const schedule = deriveOracleUsdcRewardSchedulePda({ oracleMonthPda: input.oracleMonthAddress, programId: input.programId });
    ...
    const reader = new Reader(await requiredProgramData({ ...input, address, label: "retained OracleUsdcSourceReward", length: 288 }), "retained OracleUsdcSourceReward", 288);
    ...
    const supporterCount = reader.u32("supporter_count");
    const registered = reader.bool("registered");
    const terminalStatus = enumValue(reader.u8("terminal_status"), ["Candidate", "Frozen", "Inactive", "Rejected", "OpeningPending", "Active", "Merged", "TimedOut"], "retained OracleUsdcSourceReward.terminal_status");
    const openingClaim = reader.pubkey("opening_claim");
    reader.u64("last_updated_slot");
    const mergedIntoSource = reader.pubkey("merged_into_source");
    const maxMergeDepth = reader.u8("max_merge_depth");
    reader.bool("listing_escrow_counted");
    reader.finish();
    assertCanonicalPda(address, address, bump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_USDC_SOURCE_REWARD_PDA_SEED, schedule.toBuffer(), sourceAddress.toBuffer()], "retained OracleUsdcSourceReward");
    if (!month.equals(input.oracleMonthAddress)
...
async function readSupportPositionByIdentity(input, sourceAddress, expectedSourceId, supporter) {
    ...
    const storedSourceId = reader.bytes(32, "source_id");
    reader.finish();
    if (!storedSource.equals(sourceAddress)
...
async function readUpdateRewardRegistration(input, schedule, subject, expectedRecipient) {
...
async function readOptionalSkuCoverageRecord(input, skuId, expectedSkuIndex) {
    const address = deriveOracleSkuCoverageRecordPda({
        oracleMonthPda: input.oracleMonthAddress,
        skuId,
        programId: input.programId,
    });
    const compact = await optionalCompressedObservation(input, address, CompressedStateDomain.OracleSkuCoverageRecord, "OracleSkuCoverageRecord");
    if (compact === null)
        return null;
...
async function readEmergencyDispute(input, disputeId) {
    ...
    const kind = enumValue(reader.u8("kind"), ["source", "update", "opening"], "OracleEmergencyDispute.kind");
    ...
    const status = enumValue(reader.u8("status"), ["Open", "RuleReview", "RuleReviewUnresolved", "Accepted", "Rejected", "Cancelled"], "OracleEmergencyDispute.status");
    ...
    for (let i = 0; i < 3; i += 1)
        reader.u64(`choice_power_${i}`);
    for (let i = 0; i < 3; i += 1)
        reader.u32(`choice_vote_count_${i}`);
...
async function readEmergencyVote(input, dispute, voter) {
    ...
    const bump = reader.currentHeader("OEV", 3);
    const month = reader.pubkey("month");
    const storedDispute = reader.pubkey("dispute");
    const pot = reader.pubkey("pot");
    const storedVoter = reader.pubkey("voter");
    const sambaMint = reader.pubkey("samba_mint");
    const commitHash = reader.bytes(32, "commit_hash");
    const lockedAmount = reader.u64("locked_amount");
    const snapshotPower = reader.u64("snapshot_power");
    const votingPower = reader.u64("voting_power");
    const choice = reader.u8("choice");
    const status = enumValue(reader.u8("status"), ["Empty", "Committed", "Revealed", "Expired"], "OracleEmergencyVote.status");
    const escrowDisposition = enumValue(reader.u8("escrow_disposition"), ["Unsettled", "Refunded", "Slashed", "Transferred"], "OracleEmergencyVote.escrow_disposition");
    const committedSlot = reader.u64("committed_slot");
    const revealedSlot = reader.u64("revealed_slot");
    reader.finish();
...
async function readEmergencyPot(input, dispute) {
    ...
    const sambaMint = reader.pubkey("samba_mint");
    const tokenAccount = reader.pubkey("token_account");
    const payoutMode = enumValue(reader.u8("payout_mode"), ["Open", "Redistribute", "RefundAll"], "OracleSambaEmergencyPot.payout_mode");
    ...
    if (!storedDispute.equals(dispute.address)
...
async function requireProgramAccountLength(input, address, length, label) {
    await requiredProgramData({ ...input, address, label, length });
}
function validateClassicMint(address, info) {
...
function validateClassicToken(input) {
    if (input.info === null)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_TOKEN_ACCOUNT_UNAVAILABLE", `classic SPL token account ${input.address.toBase58()} is absent; required mint ${input.mint.toBase58()}, owner ${input.owner.toBase58()}`);
    if (input.info.executable || !input.info.owner.equals(SPL_TOKEN_PROGRAM_ID) || input.info.data.length !== AccountLayout.span)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_TOKEN_ACCOUNT_INVALID", "classic SPL token account owner/layout is invalid");
    let account;
    try {
        account = unpackAccount(input.address, input.info, SPL_TOKEN_PROGRAM_ID);
    }
    catch {
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_TOKEN_ACCOUNT_INVALID", "classic SPL token account cannot be decoded");
    }
    if (!account.mint.equals(input.mint) || !account.owner.equals(input.owner) || !account.isInitialized || account.isFrozen || account.delegate !== null || account.delegatedAmount !== 0n || account.isNative || account.closeAuthority !== null)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_TOKEN_ACCOUNT_INVALID", "classic SPL token account identity/policy is invalid");
    if (input.minimum !== undefined && account.amount < input.minimum)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_TOKEN_BALANCE_INSUFFICIENT", `classic SPL token account ${input.address.toBase58()} has ${account.amount} atoms of mint ${input.mint.toBase58()}; requires ${input.minimum}`);
    return account;
}
...
async function requireUserCollateral(input, owner, minimumAvailable = 0n) {
    ...
    try {
        ...
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_COLLATERAL_INVALID", `current owner UserCollateral is not canonical: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
...
function requireCanonicalOpeningWeightScheme(month) {
    if (month.weightSchemeVersion !== 1
...
function requirePhase(month, allowed, action) {
    if (!allowed.includes(month.phase))
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_PHASE_INVALID", `${action} requires Oracle phase ${allowed.join(" or ")}; observed ${month.phase}`);
}
function rulebookScheduleBoundaries(month) {
    ...
    if (expectedListing !== month.listingTs
...
function requireOpeningResolutionComplete(month) {
    if (month.recipeHash.equals(ZERO_32)
...
async function requireGameWindow(input, month, action) {
...
async function requireCashUpdateResolutionWindow(input, month, action) {
    requirePhase(month, ["Game"], action);
    rulebookScheduleBoundaries(month);
    requireOpeningResolutionComplete(month);
    const now = await finalizedUnixTime(input);
...
async function finalizedUnixTime(input) {
...
async function readCoverageManifest(input) {
    ...
    try {
        ...
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_ACCOUNT_LAYOUT_MISMATCH", `OracleSkuCoverageManifest is not an exact current account: ${error instanceof Error ? error.message : String(error)}`);
    }
}
async function requireSourceSubmissionOrPlacementWindow(input, month, action) {
    const coverage = await readCoverageManifest(input);
    if (coverage.plannedListingTs !== coverage.plannedScrambleStartTs + BigInt(CURRENT_ORACLE_PRE_LISTING_WINDOW_SECONDS)
    ...
    if (month.phase === "SourceSubmission") {
        const reviewWindow = BigInt(ORACLE_KILL_CHALLENGE_WINDOW_SECONDS
...
async function requireKillWindow(input, month, action) {
...
async function requireOpeningWindow(input, month, action) {
...
async function exactCompressedObservationsForInstruction(input, instruction) {
    const semanticInstruction = semanticCurrentSpreadInstructionV1(instruction);
    const tag = semanticInstruction.data[0];
    if (tag !== undefined && [131, 133, 138, 139, 155, 176, 177, 181].includes(tag))
        return Object.freeze([]);
    if (input.photonConnection === undefined) {
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_COMPRESSED_STATE_UNAVAILABLE", "wrapped current Oracle action requires the authenticated Photon capability");
    }
    ...
    try {
        ...
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_COMPRESSED_STATE_UNAVAILABLE", `logical Oracle action has no exact pinned-package compressed access contract: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
    const exact = [];
    for (const request of requests) {
        ...
        if (observation !== null) {
            if (observation.transportObservation === undefined)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_STATE_INVALID", "read evidence cannot stand in for execution proof transport");
...
function exactUtcMonthIndex(timestamp, label) {
...
async function validateOracleMonthInitializationPolicy(input, params) {
    ...
    const preservedPostSubmission = BigInt(ORACLE_KILL_CHALLENGE_WINDOW_SECONDS
        + ORACLE_RESOLUTION_FREEZE_WINDOW_SECONDS
        + ORACLE_OPENING_WINDOW_SECONDS);
    ...
    const productReader = new Reader(await requiredProgramData({ ...input, address: productManifest, label: "OracleProductSkuManifest", length: 96 }), "OracleProductSkuManifest", 96);
    ...
    assertCanonicalPda(productManifest, productManifest, productBump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_PRODUCT_SKU_MANIFEST_PDA_SEED, input.market.underlyingId], "OracleProductSkuManifest");
    if (!underlyingId.equals(input.market.underlyingId)
    ...
    ladderReader.finish();
    assertCanonicalPda(ladder, ladder, ladderBump, input.programId, [CURRENT_STATE_NAMESPACE_SEED, ORACLE_MATURITY_LADDER_PDA_SEED, input.market.underlyingId], "OracleMaturityLadderRegistry");
...
async function validateActiveWeightManifest(input) {
    ...
    const manifestPhase = enumValue(manifestReader.u8("phase"), ["Collecting", "Applying", "ReadyToFinalize", "Finalized"], "OracleActiveWeightManifest.phase");
    ...
    if (!manifestMonth.equals(input.oracleMonthAddress)
...
function validateCurrentUpdateEvidence(input) {
    if (input.state === 0n
        ...
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_UPDATE_INVALID", "update evidence time is outside the finalized current evidence interval");
    }
    ...
    if (!oracleCanonicalLocatorHash(originalUrl).equals(input.source.canonicalLocatorHash)) {
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_UPDATE_INVALID", "update archive target does not match the governed source locator");
    }
    ...
    if (!input.evidenceHash.equals(expectedEvidenceHash)) {
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_UPDATE_INVALID", "update evidence hash does not bind the governed source, state, time, and archive");
    }
...
async function captureDisputeReadFrame(input) {
    ...
    if (!Number.isSafeInteger(start) || start < 0)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_STATE_INVALID", "invalid dispute read slot");
    ...
    const connection = new Proxy(original, { get(target, key) {
            if (key === "getAccountInfo")
                return async (address) => {
                    ...
                    if (!Number.isSafeInteger(response.context.slot) || response.context.slot < start)
                        throw new CurrentOraclePlannerError("CURRENT_ORACLE_STATE_INVALID", "dispute read precedes finalized floor");
            ...
            if (key === "getSlot")
                return async () => {
                    ...
                    if (!Number.isSafeInteger(observed) || observed < currentSlot)
                        throw new CurrentOraclePlannerError("CURRENT_ORACLE_STATE_INVALID", "dispute clock moved backwards");
                    currentSlot = observed;
                    return observed;
                };
    ...
    const account = async (address) => {
        ...
        if (!info)
            throw new CurrentOraclePlannerError("CURRENT_ORACLE_STATE_INVALID", "dispute anchor is absent");
        return { address, owner: info.owner, data: info.data, executable: info.executable, namespace: "ameba-spread-v2", programId: input.programId };
    };
    ...
    return { context: { ...input, connection, market, oracleMonth, vaultConfig }, finish: async () => {
            ...
            if (!Number.isSafeInteger(end) || end < currentSlot || facts.some(fact => fact.observedSlot > end))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_STATE_INVALID", "dispute finalized frame is inconsistent");
            return JSON.stringify({ readWindowStartSlot: start, readWindowEndSlot: end, currentSlot: String(currentSlot), accountFacts: facts });
        } };
}
export async function prepareCurrentOracleAction(input) {
    if (input.compressedEvidenceReader !== undefined)
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_STATE_INVALID", "entitlement read evidence is not a transaction proof transport");
    ...
    if (disputeFrame !== null)
        input = { ...disputeFrame.context, request };
    ...
    switch (request.actionType) {
        ...
        case "initialize_oracle_month_v5": {
            if (!owner.equals(input.vaultConfig.oracleAuthority))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_AUTHORITY_REQUIRED", `month initialization requires oracle authority ${input.vaultConfig.oracleAuthority.toBase58()} as signer; supplied ${owner.toBase58()}`);
            if (input.oracleMonth !== null)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_ACCOUNT_ALREADY_EXISTS", "month initialization requires an absent month");
            ...
            instructionName = "InitializeOracleMonthV5";
            tag = 181;
        ...
        case "propose_oracle_source_v3": {
            const month = requireMonth(input);
            await requireSourceSubmissionOrPlacementWindow(input, month, request.actionType);
            ...
            const bucketId = encodeCurrentOracleLabelBytes32(request.bucketId, "bucketId");
            if (!skuId.equals(bucketId))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_REQUEST_INVALID", "propose skuId must exactly equal bucketId");
            ...
            if (schedule.phase !== "Funded")
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_REWARD_INVALID", "source proposal requires the funded current reward schedule");
            const listingBond = u64(request.listingBondAtomic, "listingBondAtomic", true);
            if (listingBond !== pool.listingBond)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_BOND_MISMATCH", "listingBondAtomic differs from the frozen SKU pool");
            ...
            instructionName = "ProposeOracleSourceV3";
            tag = 182;
        ...
        case "support_oracle_source_v3": {
            ...
            if (source.proposer.equals(owner) || source.status !== "Candidate")
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_SOURCE_INVALID", "support requires a different owner and a Candidate source");
            const skuId = encodeCurrentOracleLabelBytes32(request.skuId, "skuId");
            if (!source.bucketId.equals(skuId))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_SOURCE_INVALID", "support skuId differs from the source bucketId");
            const proof = await getCurrentGovernedSkuProof(input.market.seriesIdentity.product, request.skuId);
            const schedule = await readRewardSchedule(input);
            const pool = await readSkuPool(input, source.bucketId);
            const sourceReward = await readSourceReward(input, source);
            const coverageRecord = await readOptionalSkuCoverageRecord(input, source.bucketId, proof.skuIndex);
            const stake = u64(request.stakeAtomic, "stakeAtomic", true);
            if (schedule.phase !== "Funded" || sourceReward.registered || (coverageRecord === null && source.supportStakeTotal !== 0n))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_REWARD_INVALID", "source support schedule/reward/coverage state is invalid");
            if (stake !== pool.supportBond)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_BOND_MISMATCH", "stakeAtomic differs from the frozen SKU support bond");
            ...
            instructionName = "SupportOracleSourceV3";
            tag = 183;
            Object.assign(actionFacts, { source: source.address.toBase58(), skuIndex: proof.skuIndex, stakeAtomic: stake.toString() });
            break;
        }
        case "challenge_oracle_source_v2": {
            ...
            if (u64(request.bondAtomic, "bondAtomic", true) !== requiredBond)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_BOND_MISMATCH", "source challenge bond differs from frozen economics");
            ...
            instructionName = "ChallengeOracleSourceV2";
            tag = 161;
            Object.assign(actionFacts, { source: source.address.toBase58(), comparisonSource: comparison?.address.toBase58() ?? null, requiredBondAtomic: requiredBond.toString() });
            break;
        }
        case "submit_oracle_opening_claim_v2": {
            ...
            const pool = await readSkuPool(input, source.bucketId);
            const stake = u64(request.stakeAtomic, "stakeAtomic", true);
            if (source.status !== "Frozen" || source.openingSubmitted || stake !== pool.openingBond)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_OPENING_INVALID", "opening submission source state or frozen bond is invalid");
            ...
            instructionName = "SubmitOracleOpeningClaimV2";
            tag = 162;
            Object.assign(actionFacts, { source: source.address.toBase58(), openingClaim: claimTarget.address.toBase58(), openingAttempt: claimTarget.nextAttempt, stakeAtomic: stake.toString() });
            break;
        }
        case "challenge_oracle_opening_claim_v2": {
            ...
            if (source.status !== "OpeningPending" || claim.status !== "Pending" || claim.escrowDisposition !== "Unsettled" || claim.claimant.equals(owner) || claim.challengeDeadlineSlot === 0n || slot >= claim.challengeDeadlineSlot)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_OPENING_INVALID", "opening claim is not challengeable by this owner at the finalized slot");
            const alternativeState = u64(request.alternativeOpeningStateAtomic, "alternativeOpeningStateAtomic", true);
            const alternativeTime = u64(request.alternativeSourceTimeUnix, "alternativeSourceTimeUnix", true);
            if (alternativeTime < openingWindow.openingStart || alternativeTime > openingWindow.now || alternativeTime > input.market.expiryTs)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_OPENING_INVALID", "alternative opening source time is outside the finalized current evidence interval");
            if (alternativeState === claim.openingState && alternativeTime === claim.sourceTime && oracleOpeningArchiveUrlHash(request.archiveUrl).equals(claim.archiveUrlHash))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_OPENING_INVALID", "opening challenge alternative is a no-op");
            const requiredBond = challengeBond(claim.stake, pool.challengeBondBps, pool.challengeMinBond, pool.challengeMaxBond);
            if (u64(request.bondAtomic, "bondAtomic", true) !== requiredBond)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_BOND_MISMATCH", "opening challenge bond differs from frozen economics");
            const challengeId = parseCurrentOracleHex32(request.challengeId, "challengeId");
            await Promise.all([
            ...
            instructionName = "ChallengeOracleOpeningClaimV2";
            tag = 163;
            Object.assign(actionFacts, { source: source.address.toBase58(), openingClaim: claim.address.toBase58(), requiredBondAtomic: requiredBond.toString() });
            break;
        }
        case "finalize_oracle_opening_claim_v2": {
            const month = requireMonth(input);
            await requireOpeningWindow(input, month, request.actionType);
            const source = await readSource(input, parseCurrentOracleHex32(request.sourceId, "sourceId"), true);
            const claim = await readOpeningClaim(input, source);
            const slot = BigInt(await input.connection.getSlot(input.commitment));
            if (source.status !== "OpeningPending" || source.openingSubmitted || claim.status !== "Pending" || claim.escrowDisposition !== "Unsettled" || claim.challengeDeadlineSlot === 0n || slot < claim.challengeDeadlineSlot || claim.openingState === 0n || claim.evidenceHash.equals(ZERO_32))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_OPENING_INVALID", "opening claim is not permissionlessly finalizable at the finalized slot");
            ...
            instructionName = "FinalizeOracleOpeningClaimV2";
            tag = 165;
            Object.assign(actionFacts, { source: source.address.toBase58(), openingClaim: claim.address.toBase58(), claimant: claim.claimant.toBase58() });
            break;
        }
        case "commit_oracle_update_claim_v3": {
            ...
            const stake = u64(request.stakeAtomic, "stakeAtomic", true);
            if (source.status !== "Active" || !source.openingSubmitted || source.currentState === 0n || priorState !== source.currentState || nextState === priorState || stake !== pool.updateMinBond)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_UPDATE_INVALID", "update commit state or frozen bond is invalid");
            ...
            const claimAddress = deriveOracleUpdateClaimV2Pda({ oracleMonthPda: input.oracleMonthAddress, oracleSourcePda: source.address, claimant: owner, claimId, programId: input.programId });
            await requireCreateTarget({ ...input, address: claimAddress, label: "OracleUpdateClaimV2" });
            await requireUserCollateral(input, owner, stake);
            ...
            instructionName = "CommitOracleUpdateClaimV3";
            tag = 166;
            Object.assign(actionFacts, { source: source.address.toBase58(), updateClaim: claimAddress.toBase58(), commitHashHex: commitHash.toString("hex"), stakeAtomic: stake.toString() });
            break;
        }
        case "reveal_oracle_update_claim_v3": {
            ...
            instructionName = "RevealOracleUpdateClaimV3";
            tag = 198;
            Object.assign(actionFacts, { source: source.address.toBase58(), updateClaim: claim.address.toBase58(), commitmentVerified: true });
            break;
        }
        case "challenge_oracle_update_claim_v2": {
            ...
            instructionName = "ChallengeOracleUpdateClaimV2";
            tag = 168;
            Object.assign(actionFacts, { source: source.address.toBase58(), updateClaim: claim.address.toBase58(), requiredBondAtomic: requiredBond.toString() });
            break;
        }
        case "finalize_oracle_update_claim_v2": {
            ...
            if ((request.outcome !== "accept_claim" && challengeId === undefined)
        ...
        case "commit_oracle_emergency_vote_v3": {
            ...
            const slot = BigInt(await input.connection.getSlot(input.commitment));
            if (dispute.kind !== request.disputeKind
                || dispute.status !== "Open"
        ...
        case "deposit_oracle_usdc_rewards": {
            const amount = u64(request.amountAtomic, "amountAtomic", true);
            const rewardVault = await readRewardVault(input);
            if (rewardVault.tokenAmount + amount > U64_MAX)
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_REWARD_INVALID", "reward-vault deposit would overflow the classic SPL amount");
            const funderToken = getAssociatedTokenAddressSync(input.vaultConfig.usdcMint, owner, false, SPL_TOKEN_PROGRAM_ID);
            validateClassicToken({ address: funderToken, info: await input.connection.getAccountInfo(funderToken, input.commitment), mint: input.vaultConfig.usdcMint, owner, minimum: amount });
            ...
            instructionName = "DepositOracleUsdcRewards";
            tag = 155;
            Object.assign(actionFacts, { amountAtomic: amount.toString(), rewardVault: rewardVault.address.toBase58(), rewardVaultTokenAccount: rewardVault.tokenAccount.toBase58() });
            break;
        }
        case "claim_oracle_usdc_reward": {
            ...
            const rewardVault = await readRewardVault(input);
            if (!schedule.rewardVault.equals(rewardVault.address))
                throw new CurrentOraclePlannerError("CURRENT_ORACLE_REWARD_INVALID", "reward schedule is not bound to canonical custody");
            ...
            if (entitlementAmount > schedule.remainingRewardBudget
    ...
    if (disputeFrame !== null)
        actionFacts.disputeEvidenceJson = await disputeFrame.finish();
...
export async function readCurrentOracleRewardEntitlementInputs(input) {
    const request = validateCurrentOracleActionRequest(input.request);
    if (request.actionType !== "claim_oracle_usdc_reward")
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_REQUEST_INVALID", "reward request required");
    requireMonth(input);
    const owner = new PublicKey(request.ownerPubkey);
    const schedule = await readRewardSchedule(input);
    if (schedule.phase !== "EntitlementsFinalized")
        throw new CurrentOraclePlannerError("CURRENT_ORACLE_REWARD_PHASE_INVALID", "reward entitlements are not finalized");
    const actionFacts = {};
    ...
    if (request.rewardKind === "update") {
        ...
        const claim = await readUpdateClaimByIdentity(input, originalSourceAddress, sourceId, owner, parseCurrentOracleHex32(request.claimId, "claimId"));
        ...
        if (request.rewardKind === "support") {
            ...
            for (let depth = 0; depth <= MAX_MERGE_DEPTH; depth += 1) {
                ...
                if ((depth === 0 && !retainedReward.sourceId.equals(sourceId))
        ...
        const source = await readSourceByAddress(input, canonicalSourceAddress, lineage.length === 0 ? sourceId : undefined, request.rewardKind === "opening");
        const sourceReward = await readSourceReward(input, source);
        const pool = await readSkuPool(input, source.bucketId);
        if (!sourceReward.registered
