...
import { deriveWriterDlmmPolicyPda, deriveWriterDlmmPositionPda, deriveWriterSleeveUsdcVaultPda, deriveWriterRetirementCustodyPda, deriveWriterPolicyRegistryPda, deriveWriterProtocolFeeVaultPda } from "./writer-sleeve.js";
...
import { deriveAmoebaDlmmAuthorityPda, deriveAmoebaDlmmBinPagePda, deriveAmoebaDlmmPoolPda, } from "@amoeba/spread-release-tools/dlmm-accounts";
import { deriveWriterSeriesBookPda, deriveWriterSleevePda, } from "@amoeba/spread-release-tools/writer-sleeve-instructions";
...
export const COLLECTIVE_SWAP_OPERATION_PLAN_SCHEMA_VERSION = 1;
export const COLLECTIVE_SWAP_EXACT_IN_TAG = 254;
export const COLLECTIVE_SWAP_MAX_RESERVE_PAGES = 8;
export const COLLECTIVE_SWAP_DEADLINE_TTL_SECONDS = 120n;
export const COLLECTIVE_SWAP_COMPUTE_UNIT_LIMIT = 1_000_000;
/** Bind a Lean-admitted secondary swap to exact native tag-254 bytes and metas. */
export function prepareCollectiveSwapOperation(input) {
    ...
    compileCurrentWalletUnsignedBatch(new PublicKey(semantic.trader), PublicKey.default.toBase58(), input.computeUnitLimit === undefined ? [input.instruction] : [ComputeBudgetProgram.setComputeUnitLimit({ units: input.computeUnitLimit }), input.instruction], lookup === undefined ? [] : [lookup.account]);
...
export function collectiveSwapExecutionInstructions(plan) {
    if (plan.computeUnitLimit === undefined)
        return Object.freeze([plan.instruction]);
...
export function validateCollectiveSwapOperationPlan(plan, independentlyRebuilt) {
    const rebuilt = prepareCollectiveSwapOperation(independentlyRebuilt);
    if (canonicalJson(plan) !== canonicalJson(rebuilt)) {
        throw swapPlanError("COLLECTIVE_SWAP_PLAN_MISMATCH", "collective swap plan differs from its canonical RC44 reconstruction");
    }
    return rebuilt;
}
function collectiveSwapInstructionManifest(instruction, semantic, observation) {
    ...
    semanticInstruction.keys.forEach((meta, index) => {
        const flags = index < expectedFlags.length ? expectedFlags[index] : [false, true];
...
function validateSemantic(input, observation) {
    const addressFields = [
    ...
    if (observedBlockTime === null
        ...
        throw swapPlanError("COLLECTIVE_SWAP_DEADLINE_INVALID", "deadlineTs must equal finalized observed block time plus 120 seconds");
    }
...
function requireObservedStateBinding(observation, instruction) {
    ...
    for (const index of requiredIndexes) {
        ...
        if (account === undefined || account.owner === null || account.executable === null
            ...
            throw swapPlanError("COLLECTIVE_SWAP_OBSERVATION_INVALID", `finalized observation does not bind complete state for instruction account ${index}`);
        }
    }
    for (const index of [23, 24, 26, 28, 29]) {
        const account = observed.get(instruction.keys[index].pubkey.toBase58());
        if (!account)
            throw swapPlanError("COLLECTIVE_SWAP_OBSERVATION_INVALID", "swap omits an optional custody observation");
    }
    ...
    for (const index of programOwnedIndexes) {
        ...
        if (account.owner !== AMOEBA_SPREAD_PROGRAM_ID || account.executable !== false) {
            throw swapPlanError("COLLECTIVE_SWAP_OBSERVATION_INVALID", `finalized observation has noncanonical owner state for instruction account ${index}`);
        }
    }
}
function canonicalPositiveU64(value, label) {
...
function canonicalSignerRoles(values, instructionIndex) {
...
function requireExactWriteSet(writeSet, accounts) {
...
function requireExactSignerCoverage(signers, accounts) {
...
function swapPlanError(code, message) {
    return new AmebaProtocolError(message, { code });
}
//# sourceMappingURL=collective-swap-operation.js.map