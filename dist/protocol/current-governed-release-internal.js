...
import { CURRENT_GOVERNANCE_GENERATION_1, CURRENT_GOVERNANCE_GENERATION_3, CURRENT_LIVE_DEPLOYMENT, REVIEWED_GOVERNANCE_GENERATION_2_CANDIDATE, SDK_RELEASE_TRAIN, CurrentWriteReleaseUnavailableError, } from "./release-train.js";
import { GovernedTransactionValidationError, } from "./governed-transaction-internal.js";
 ...
 */
export function currentGovernedWriteReleaseV1() {
    ...
    const train = SDK_RELEASE_TRAIN;
    const release = train.writeRelease;
    if (release.status !== "available")
        throw new CurrentWriteReleaseUnavailableError();
    const identity = selectedReleaseIdentity(train.selectedGovernanceGeneration, train.liveGovernanceIdentity);
    const liveDeployment = CURRENT_LIVE_DEPLOYMENT;
    const identityEvidence = identity;
    const runtime = runtimeReceipt;
    const document = SDK_RELEASE_TRAIN;
    const live = CURRENT_LIVE_DEPLOYMENT;
    const selected = identity;
    const reviewed = CURRENT_GOVERNANCE_GENERATION_3;
    // Older descriptors have only historical controller evidence. It cannot qualify a current release.
    const currentControllerEvidence = upstream.accountEvidence.controllerProgramData;
    ...
    const tags = [...Object.values(CURRENT_VAULT_INSTRUCTION_TAG), ...Object.values(CURRENT_AMOEBA_DLMM_INSTRUCTION_TAG)]
        ...
        .sort((a, b) => a - b);
    if (upstream.sdkDescriptorOwnership.nativePackageSourceCommit !== runtime.sourceCommit
        ...
        || !isExactWriterOperationTransportV1(runtimeReceipt.writerOperationTransport)
        || (tags.includes(CURRENT_VAULT_INSTRUCTION_TAG.ManageWriterDlmmV1) && (!isExactWriterDlmmTransportV1(release.writerDlmmTransport)
        ...
        throw new GovernedTransactionValidationError("CURRENT_GOVERNED_WRITE_RELEASE_INVALID", "current governed write release lacks exact source, payload, generation, or compatibility pins");
    }
    return {
        identity,
        ...
        assignedInstructionTags: release.assignedInstructionTags,
    };
}
...
function selectedReleaseIdentity(generation, liveIdentity) {
    ...
    throw new GovernedTransactionValidationError("CURRENT_GOVERNED_WRITE_RELEASE_INVALID", "current governed write release does not select one exact live governance identity");
}
//# sourceMappingURL=current-governed-release-internal.js.map