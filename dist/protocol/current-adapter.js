...
import { AccountLayout, getAssociatedTokenAddressSync, getAssociatedTokenAddressInterface, unpackAccount, unpackMint, } from "./current-token-primitives.js";
import { batchCpiContext1, batchCpiContext2, batchCpiContext3, batchCpiContext4, batchCpiContext5, batchMerkleTree1, batchMerkleTree2, batchMerkleTree3, batchMerkleTree4, batchMerkleTree5, batchQueue1, batchQueue2, batchQueue3, batchQueue4, batchQueue5, createBN254, deriveAddress, TreeType, } from "@lightprotocol/stateless.js";
import { AddressLookupTableAccount, AddressLookupTableProgram, ComputeBudgetProgram, PACKET_DATA_SIZE, PublicKey, SystemProgram, TransactionMessage, TransactionInstruction, VersionedTransaction, } from "@solana/web3.js";
...
import { AMOEBA_DLMM_PRICE_SCALE, ceilMulDiv, floorMulDiv, priceFromBin, } from "@amoeba/spread-release-tools/dlmm-math";
import { decodeWriterSettlementGroup, } from "@amoeba/spread-release-tools/writer-sleeve-accounts";
import { WRITER_ACCOUNT_SIZES, deriveWriterSettlementGroupPda, } from "@amoeba/spread-release-tools/writer-sleeve-instructions";
import { requiredCompressedStateAccessesV1 } from "@amoeba/spread-release-tools/compressed-state";
import { CurrentOraclePlannerError, prepareCurrentOracleAction, } from "./current-oracle-planner.js";
import { buildCurrentGovernedInstructionV1 as materializeCurrentGovernedInstructionV1, } from "./current-governed-write.js";
import { CURRENT_LIVE_DEPLOYMENT, isCurrentWriteReleaseAvailable, } from "./release-train.js";
import { currentGovernedWriteReleaseV1 } from "./current-governed-release-internal.js";
import { prepareBoundCurrentGovernedWriteV1, materializeBoundCurrentGovernedBuilderV1, isBoundCurrentGovernedProgramV1, registerCurrentSpreadGovernedOutputV1, revalidateFreshCurrentGovernedInstructionsV1, semanticCurrentSpreadInstructionV1, } from "./current-governed-write-internal.js";
import { GOVERNANCE_GATE_STATUS_V1, inspectGovernedInstructionEnvelopeV1, } from "./governance.js";
export const CURRENT_STATE_NAMESPACE = "ameba-spread-v2";
export const CURRENT_STATE_COMMITMENT = "finalized";
const BPF_UPGRADEABLE_LOADER_ID = new PublicKey("BPFLoaderUpgradeab1e11111111111111111111111");
const CURRENT_LIGHT_COMPRESSION_AUTHORITY = new PublicKey("4digEm3af65wVp8iA3UAQJrKXNnXZDKZv3m3vQWmMDR9");
export const CURRENT_LIGHT_DEFAULT_ADDRESS_TREE_V2 = new PublicKey("amt2kaJA14v3urZbZvnc5v2np8jqvc4Z8zDep5wbtzx");
const CURRENT_LIGHT_STATE_TREE_CONTEXTS = new Map([
 ...
 */
export function currentLightSetupInstructionBatches(resolution, actionInstructions) {
    if (resolution.actionBatchIndex === null)
    ...
    if (actionInstructions.length === 0 || actionOffset < 0
        || actionInstructions.some((instruction, index) => !sameInstruction(actionBatch?.[actionOffset + index], instruction))) {
        throw new CurrentSdkOperationError("CURRENT_LIGHT_LOAD_PLAN_INVALID", "Light resolution does not end in the exact action suffix");
    }
...
function assertContext(input) {
    ...
    if (commitment !== undefined && commitment !== CURRENT_STATE_COMMITMENT) {
        throw new CurrentSdkOperationError("CURRENT_COMMITMENT_INVALID", "authoritative current protocol reads require finalized commitment");
    }
}
function finalizedCommitment(commitment) {
    if (commitment !== undefined && commitment !== CURRENT_STATE_COMMITMENT) {
        throw new CurrentSdkOperationError("CURRENT_COMMITMENT_INVALID", "authoritative current protocol reads require finalized commitment");
    }
...
function validateCurrentColdLoadInstruction(input) {
    ...
    if (!instruction.programId.equals(input.expectedOwner)
        || bytes.length > 16_384
        || bytes[0] !== 219
...
function currentColdResolver(photon, expectedOwner, payer, collector) {
    if (photon === undefined)
        return undefined;
    return {
        resolveColdAccount: async (address) => {
            if (payer === undefined) {
                ...
                if (observed === null)
                    return null;
                if (!observed.canonicalAddress.equals(address)
                    ...
                    throw new CurrentSdkOperationError("CURRENT_COLD_PROOF_INVALID", "proof-only cold observation does not bind the canonical account, owner, and authorized provider");
                }
            ...
            if (resolved === null)
                return null;
            if (!resolved.canonicalAddress.equals(address)
                ...
                throw new CurrentSdkOperationError("CURRENT_COLD_PROOF_INVALID", "payer-scoped cold observation does not bind the canonical account, owner, provider, and tag-219 fee payer");
            }
...
function requireProgramAccount(kind, info, programId, expectedSize) {
...
function decodeOptionalCurrentOracleMonthTarget(input) {
    ...
    if (info === null)
        return null;
    if (!info.executable && info.owner.equals(SystemProgram.programId) && info.data.length === 0)
        return null;
...
function asSeriesId(value, marketId) {
...
function requirePositiveU64(value, code, field) {
    if (value <= 0n || value > 0xffffffffffffffffn) {
        throw new CurrentSdkOperationError(code, `${field} must be a positive u64`);
    }
}
export async function readCurrentDeploymentFacts(input) {
    assertContext(input);
    ...
    if (program !== null) {
        ...
        if (!program.owner.equals(BPF_UPGRADEABLE_LOADER_ID)
            || !program.executable
            || bytes.length !== 36
            || bytes.readUInt32LE(0) !== 2
            || !new PublicKey(bytes.subarray(4, 36)).equals(expectedProgramData)) {
            throw new CurrentSdkOperationError("CURRENT_DEPLOYMENT_LINK_INVALID", "program account is not the exact upgradeable-loader Program linked to pinned ProgramData");
        }
        if (programData === null) {
            throw new CurrentSdkOperationError("CURRENT_PROGRAMDATA_NOT_FOUND", "the linked current ProgramData account is absent");
        }
    }
    let programDataSlot = null;
    let programPayloadBytes = null;
    let programPayloadSha256 = null;
    if (programData !== null) {
        const bytes = Buffer.from(programData.data);
        if (!programData.owner.equals(BPF_UPGRADEABLE_LOADER_ID)
        ...
        if (slot !== BigInt(CURRENT_PROTOCOL_DEPLOYED_SLOT)
            || payloadSha256 !== CURRENT_PROTOCOL_PROGRAM_PAYLOAD_SHA256
            || trailingAllocation.some((byte) => byte !== 0)) {
            throw new CurrentSdkOperationError("CURRENT_PROGRAM_PAYLOAD_INVALID", "ProgramData slot, payload hash, length, or trailing allocation is not the pinned rc.44 deployment");
        }
...
export async function readCurrentMarketAccount(input) {
    assertContext(input);
...
export function decodeCurrentAmoebaDlmmPoolAccount(input) {
    assertContext(input);
...
function validateCurrentAmoebaDlmmPoolValue(pool, input) {
    ...
    if (!input.address.equals(expectedPool)
...
export function decodeCurrentAmoebaDlmmBinPageAccount(input) {
    assertContext(input);
    ...
    if (!input.address.equals(deriveAmoebaDlmmBinPagePda(input.poolAddress, input.pageIndex, input.programId)[0])
...
export function decodeCurrentAmoebaDlmmSharePageAccount(input) {
    assertContext(input);
    ...
    if (!input.address.equals(deriveAmoebaDlmmSharePagePda(input.poolAddress, input.pageIndex, input.programId)[0])
        || !page.pool.equals(input.poolAddress)
        || page.pageIndex !== input.pageIndex
        || page.firstBinId !== input.pageIndex * 32 + 1) {
...
export function decodeCurrentAmoebaDlmmPositionAccount(input) {
    assertContext(input);
    ...
    const bitmap = position.liquidityShares.reduce((value, shares, index) => value | (shares === 0n ? 0 : (1 << index)), 0) >>> 0;
    if (!input.address.equals(deriveAmoebaDlmmPositionPda(input.poolAddress, position.owner, position.positionNonce, input.programId)[0])
...
export async function discoverCurrentAmoebaDlmmPages(input) {
    assertContext(input);
    ...
    const pages = await Promise.all(pageIndexes.map(async (pageIndex) => {
        ...
        if (!reserve.account.pool.equals(input.poolAddress)
        ...
        if (reserve.resolved.wasCold !== (reserveColdWitness !== null)
            ...
            throw new CurrentSdkOperationError("CURRENT_COLD_PROOF_INVALID", "reserve and share page transport facts are not independently authenticated");
        }
        return {
            pageIndex,
...
function parseCurrentAmoebaDlmmPositionCandidate(data) {
    const bytes = Buffer.from(data);
    if (bytes.byteLength !== AMOEBA_DLMM_POSITION_ACCOUNT_SIZE
        ...
        || bytes[13] !== 1) {
        throw new CurrentSdkOperationError("CURRENT_DLMM_POSITION_IDENTITY_INVALID", "compressed position candidate header or fixed size is not current");
    }
    return Object.freeze({
...
export async function discoverCurrentAmoebaDlmmPositions(input) {
    assertContext(input);
    ...
    if (input.expectedPositionCount !== undefined
        && (!Number.isInteger(input.expectedPositionCount)
            || input.expectedPositionCount < 0
            || input.expectedPositionCount > limit)) {
        throw new CurrentSdkOperationError("CURRENT_POSITION_LIMIT_INVALID", "expected position count must be a bounded current u16 count within the discovery limit");
    }
    const filters = [
    ...
    if (input.owner !== undefined)
        filters.push({ memcmp: { offset: 46, bytes: input.owner.toBase58() } });
    ...
    if (needsColdSearch) {
        if (input.photonConnection === undefined) {
            throw new CurrentSdkOperationError("CURRENT_LIGHT_PROOF_UNAVAILABLE", "authoritative position discovery requires the canonical Photon capability when hot state is incomplete");
        }
        ...
        for (const candidate of compressed.items) {
            if (!candidate.owner.equals(input.programId) || candidate.data === null)
                continue;
            const discriminator = Buffer.from(candidate.data.discriminator);
            const body = Buffer.from(candidate.data.data);
            if (!discriminator.equals(Buffer.from(AMOEBA_DLMM_POSITION_DISCRIMINATOR))
            ...
            if (!candidatePosition.pool.equals(input.poolAddress)
                || (input.owner !== undefined && !candidatePosition.owner.equals(input.owner)))
                continue;
            const address = deriveAmoebaDlmmPositionPda(input.poolAddress, candidatePosition.owner, candidatePosition.positionNonce, input.programId)[0];
            ...
            if (!position.owner.equals(candidatePosition.owner)
...
export async function resolveCurrentAmoebaDlmmState(input) {
    assertContext(input);
    const address = input.state.kind === "pool"
        ...
        : input.state.kind === "reserve_page"
            ...
            : input.state.kind === "share_page"
                ...
                : deriveAmoebaDlmmPositionPda(input.state.poolAddress, input.state.owner, input.state.positionNonce, input.programId)[0];
    ...
    if (hot !== null) {
        ...
        if (cold === null)
            throw new CurrentSdkOperationError("CURRENT_DLMM_STATE_NOT_FOUND", "typed current DLMM state is absent hot and cold");
        data = cold.data;
        owner = cold.owner;
        executable = false;
        sourceState = "cold";
    ...
    if ((sourceState === "cold" && (coldWitness === null
...
export async function readCurrentAmoebaDlmmPool(input) {
    assertContext(input);
    ...
    const [settlementGroupAddress, expectedSettlementGroupBump] = deriveWriterSettlementGroupPda(market.underlyingId, market.expiryTs, market.collateralMint, input.programId);
    ...
    if (vaultInfo === null || settlementGroupInfo === null) {
        throw new CurrentSdkOperationError("CURRENT_DLMM_POOL_CONTEXT_UNAVAILABLE", "current VaultConfig or collective settlement group is absent");
    }
    const vaultConfig = decodeVaultConfigInfo(vaultConfigAddress, vaultInfo, input.programId);
    requireProgramAccount("WriterSettlementGroup", settlementGroupInfo, input.programId, WRITER_ACCOUNT_SIZES.settlementGroup);
    const settlementGroup = decodeWriterSettlementGroup(settlementGroupAddress, settlementGroupInfo.data, input.programId);
    ...
    if (settlementGroup.bump !== expectedSettlementGroupBump
        ...
        throw new CurrentSdkOperationError("CURRENT_DLMM_POOL_INVARIANT_INVALID", "collective settlement group is not bound to current market economics");
    }
    const monthInfo = await input.connection.getAccountInfo(oracleMonthAddress, finalizedCommitment(input.commitment));
    if (monthInfo === null) {
        throw new CurrentSdkOperationError("CURRENT_DLMM_POOL_CONTEXT_UNAVAILABLE", "current collective anchor OracleMonth is absent");
    }
    ...
    if (!oracleMonth.market.equals(settlementGroup.anchorMarket)) {
        throw new CurrentSdkOperationError("CURRENT_DLMM_POOL_INVARIANT_INVALID", "collective anchor OracleMonth does not match its settlement group");
    }
    ...
    const proofFacts = resolved.coldWitness === null
        ? Object.freeze({ sourceState: "hot", resolutionMode: "hot" })
        : Object.freeze({
            sourceState: "cold",
...
export async function resolveCurrentLightAccounts(input) {
    assertContext(input);
    ...
    for (const request of input.accounts) {
        ...
        if (input.photonConnection === undefined || request.payer === undefined) {
            throw new CurrentSdkOperationError("CURRENT_LIGHT_PROOF_UNAVAILABLE", "a hot-absent Light ATA requires the authorized payer-bound Light indexer");
        }
        ...
        if (proof === null) {
            if (request.role === "required_hot_input") {
                throw new CurrentSdkOperationError("CURRENT_LIGHT_INPUT_ABSENT", "required Light input has no authenticated hot or cold custody");
            }
            accounts.push({
                address,
                role: request.role,
    ...
    if (coldRequests.length > 1) {
        throw new CurrentSdkOperationError("CURRENT_LIGHT_COLD_BOUND_EXCEEDED", "one prepared action may load at most one cold Light ATA; additional loads require finalized sequential preparation");
    }
    let readyPlan = null;
    if (coldRequests.length === 1) {
        if (input.actionInstructions === undefined || input.actionInstructions.length === 0) {
            throw new CurrentSdkOperationError("CURRENT_LIGHT_ACTION_REQUIRED", "cold Light resolution requires the exact action for an atomic load-ready plan");
        }
        ...
        if (actionOffset < 0 || input.actionInstructions.some((instruction, index) => !sameInstruction(finalBatch?.[actionOffset + index], instruction))) {
 ...
 */
export function validateCurrentHotLightAccount(input) {
    if (input.minimumAmountAtoms < 0n || input.minimumAmountAtoms > 0xffffffffffffffffn) {
        throw new CurrentSdkOperationError("CURRENT_LIGHT_TOKEN_ACCOUNT_INVALID", "hot Light ATA minimum amount is not a u64");
    }
    const expectedAddress = getAssociatedTokenAddressInterface(input.expectedMint, input.expectedTokenOwner);
    if (!input.address.equals(expectedAddress)) {
        throw new CurrentSdkOperationError("CURRENT_LIGHT_TOKEN_ACCOUNT_INVALID", "hot Light ATA address is not canonical for the expected mint and token owner");
    }
    ...
    if (account.amount < input.minimumAmountAtoms) {
        throw new CurrentSdkOperationError("CURRENT_WRITER_LIGHT_BALANCE_INSUFFICIENT", "hot Light ATA amount is below the exact writer-close minimum");
    }
    return Object.freeze({
        address: input.address,
...
function decodeVaultConfigInfo(address, info, programId) {
...
function hasCanonicalLightCompressibleTokenLayout(data) {
    const bytes = Buffer.from(data);
...
function hasCanonicalInitializedTokenPolicy(account) {
...
function decodeRequiredCurrentTokenAccount(input) {
    ...
    if (!account.mint.equals(input.mint)
...
async function requiredMarket(input) {
...
function isCurrentActiveWeightManifestFinal(month, manifest) {
...
function manifest(instruction, decodedParams) {
    return Object.freeze({
...
function approvedInstruction(value) {
    return Object.freeze({
...
function canonicalDecodedParams(params) {
...
function manifestIdentityParts(instruction) {
    const parts = [];
...
function operationId(operation, instructions, setupInstructionBatches, proofFacts) {
    const parts = [Buffer.from(operation, "utf8"), Buffer.from([instructions.length])];
...
function cloneAndFreezeIssuedValue(value, seen = new Map()) {
    if (value === null || typeof value !== "object")
        return value;
    if (value instanceof PublicKey)
        return Object.freeze(new PublicKey(value.toBytes()));
    ...
    if (Array.isArray(value)) {
        ...
        for (const item of value)
            copy.push(cloneAndFreezeIssuedValue(item, seen));
...
function canonicalIssuedValue(value, seen = new Set()) {
    ...
    if (seen.has(value))
        throw new CurrentSdkOperationError("CURRENT_PLAN_CYCLE_INVALID", "issued plan graph must be acyclic");
    seen.add(value);
    let canonical;
    if (Array.isArray(value)) {
        ...
        canonical = Object.fromEntries(Reflect.ownKeys(value)
            ...
            .map((key) => [key, canonicalIssuedValue(value[key], seen)]));
    }
    seen.delete(value);
    return canonical;
}
 ...
 * rule.
 */
function currentPreparedPlanDigest(operation, planWithoutDigest) {
...
function issuedPlanIntegrityDigest(planValue) {
    return createHash("sha256").update(JSON.stringify(canonicalIssuedValue(planValue))).digest("hex");
}
function orderedUniqueWritableAccounts(instructions, setupInstructionBatches) {
    const seen = new Set();
    const writeSet = [];
...
function mainSignerRole(operation) {
    if (operation === "position_action")
...
function signerRolesForInstructions(operation, instructions, setupInstructionBatches) {
    const roles = [];
    const append = (manifests, role, scope, setupBatchIndex) => {
        ...
        manifests.forEach((instruction, instructionIndex) => {
            for (const account of instruction.accounts) {
                ...
                if (!prior.includes(instructionIndex))
                    prior.push(instructionIndex);
...
function plan(operation, instructions, setupInstructionBatches = [], proofFacts = {}) {
    return Object.freeze({
        ...identity(),
        operation,
...
function instructionFromManifest(value) {
...
function serializeUnsignedTransaction(instructions, feePayer, recentBlockhash) {
...
const CURRENT_LOOKUP_TABLE_ACTIVE_DEACTIVATION_SLOT = 0xffffffffffffffffn;
function shortVectorByteLength(value) {
...
function versionedTransactionByteLength(message) {
...
export function currentOracleLookupTableAddressesSha256(addresses) {
    const hash = createHash("sha256");
...
async function resolveCurrentOracleLookupTable(input) {
    ...
    try {
        ...
        throw new CurrentSdkOperationError("CURRENT_ORACLE_LOOKUP_TABLE_INVALID", `authorized Oracle lookup-table account layout is invalid: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
...
function theoreticalOracleLookupTable(instructions, feePayer) {
    const addresses = new Map();
    for (const instruction of instructions) {
        addresses.set(instruction.programId.toBase58(), instruction.programId);
        for (const meta of instruction.keys) {
            if (!meta.isSigner && !meta.pubkey.equals(feePayer))
                addresses.set(meta.pubkey.toBase58(), meta.pubkey);
        }
    }
...
function compileUnsignedV0Transaction(input) {
    ...
    if (serializedByteLength > PACKET_DATA_SIZE) {
        throw new CurrentSdkOperationError("CURRENT_ORACLE_TRANSACTION_TOO_LARGE", `current Oracle transaction is ${serializedByteLength} bytes; maximum is ${PACKET_DATA_SIZE}`, { serializedByteLength, packetDataSizeLimit: PACKET_DATA_SIZE });
    }
...
function currentOracleLookupTableFromWitness(witness) {
    exactKeys(witness, [
    ...
    if (typeof witness.address !== "string"
    ...
    if (witness.owner !== AddressLookupTableProgram.programId.toBase58()
        || deactivationSlot !== CURRENT_LOOKUP_TABLE_ACTIVE_DEACTIVATION_SLOT
...
function validateOracleApprovedTransaction(plan, owner) {
    ...
    if (serialized.toString("base64") !== plan.transaction.serializedTransactionBase64
    ...
    try {
        ...
        if (parsed.version !== 0
            ...
            || (expectedLookup !== null && (parsed.message.addressTableLookups.length !== 1
        ...
        throw new CurrentSdkOperationError("CURRENT_ORACLE_DRAFT_INVALID", `Oracle serialized transaction is not one exact v0 message: ${cause instanceof Error ? cause.message : String(cause)}`);
    }
}
function validateCurrentPlanExecutionFacts(planValue) {
    const expectedWriteSet = orderedUniqueWritableAccounts(planValue.instructions, planValue.setupInstructionBatches);
    if (JSON.stringify(planValue.writeSet) !== JSON.stringify(expectedWriteSet)) {
        throw new CurrentSdkOperationError("CURRENT_PLAN_WRITE_SET_INVALID", "operation plan write set is not the ordered unique setup-then-main writable account set");
    }
    const expectedSignerRoles = signerRolesForInstructions(planValue.operation, planValue.instructions, planValue.setupInstructionBatches);
    if (!canonicalValuesEqual(planValue.signerRoles, expectedSignerRoles)) {
        throw new CurrentSdkOperationError("CURRENT_PLAN_SIGNER_ROLES_INVALID", "operation plan signer roles do not match exact signer metas and execution ordering");
    }
...
function validateCurrentOperationPlanStructure(input) {
    ...
    for (const [index, instruction] of input.plan.instructions.entries()) {
        if (input.plan.operation === "oracle_draft" && tagSequence === "2,1,139" && index < 2)
            continue; // exact prefix is checked by Oracle validation below
    ...
    for (const batch of input.plan.setupInstructionBatches) {
        ...
        for (const [instructionIndex, instruction] of batch.entries()) {
            ...
            if ((!isComputeBudget && !isLightTokenLoad && !isCurrentColdLoad)
                || data.length < 1
                || data.length > 16_384
    ...
    if (input.plan.operation === "position_action")
        validateCurrentPositionActionPlan(input.plan);
    else
        validateCurrentOracleDraftPlan(input.plan);
    const expectedId = operationId(input.plan.operation, input.plan.instructions, input.plan.setupInstructionBatches, input.plan.proofFacts);
 ...
 */
export function validateCurrentOperationPlan(planValue) {
...
function reviveCurrentPlanJson(value, path = "plan") {
    if (value === null || typeof value === "string" || typeof value === "boolean")
        return value;
    if (typeof value === "number") {
        if (!Number.isFinite(value)) {
    ...
    for (const key of keys)
        revived[key] = reviveCurrentPlanJson(objectValue[key], `${path}.${key}`);
    return revived;
}
/** Return the complete immutable plan as a canonical JSON-safe value. */
export function currentOperationPlanToJson(planValue) {
    validateCurrentOperationPlanStructure({
        plan: planValue,
        operation: planValue.operation,
        namespace: planValue.stateNamespace,
    });
    return cloneAndFreezeIssuedValue(canonicalIssuedValue(planValue));
}
/** Validate a persisted/remote JSON plan and all of its RC44 bytes, metas, PDAs, digests, and proof facts. */
export function validateCurrentOperationPlanJson(planJson) {
...
function validateApprovedInstructions(plan, expectedFeePayer) {
    ...
    if (serialized.toString("base64") !== transaction.serializedTransactionBase64
        || transaction.backendPartialSignatures.length !== 0
        || plan.instructions[0]?.accounts[0] === undefined
        || serializeUnsignedTransaction(plan.instructions.map(instructionFromManifest), expectedFeePayer ?? new PublicKey(plan.instructions[0].accounts[0].pubkey), transaction.recentBlockhash) !== transaction.serializedTransactionBase64) {
...
function validatePortableCurrentMarket(market, programId) {
    exactKeys(market, [
    ...
    const expectedCapPrice = side === "CALL" ? 112000000n : 88000000n;
    const expectedExpiryTs = BigInt(Date.UTC(Number(yearText), Number(monthText), 1) / 1_000);
    const [, expectedBump] = PublicKey.findProgramAddressSync([CURRENT_STATE_NAMESPACE_SEED, Buffer.from("market", "ascii"), marketId], programId);
    if (market.stateNamespace !== CURRENT_STATE_NAMESPACE
        ...
        || market.strikePrice !== 100000000n
        || market.capPrice !== expectedCapPrice
        || market.contractSize !== 1000000n
        || market.maxPayoutPerContract !== 12000000n
        || market.settlementStyle !== "CashSettledMonthly"
        || market.tickSize !== 50000n
...
const CURRENT_ORACLE_ACTION_TAG = Object.freeze({
...
const CURRENT_ORACLE_DIRECT_TAGS = new Set([
    131, 133, 138, 139, 155, 176, 177, 181,
]);
function oracleInstructionSha256(value) {
    const hash = createHash("sha256");
...
function oracleTransportWitnessDigest(value) {
    const { witnessDigest: _witnessDigest, ...facts } = value;
    const hash = createHash("sha256");
    hash.update("ameba-spread-v2/current-compressed-instruction-witness-v1\0", "utf8");
    hash.update(JSON.stringify(facts), "utf8");
    return hash.digest("hex");
}
function governedPlanBinding(governance, release) {
    return Object.freeze({
        schema: "ameba.sdk.governed-plan-binding.v1",
...
function validateGovernedPlanBindingV1(value) {
    exactKeys(value, [
    ...
    if (value.schema !== "ameba.sdk.governed-plan-binding.v1"
        ...
        || BigInt(value.epoch) > 0xffffffffffffffffn
        || !Number.isSafeInteger(value.finalizedObservationSlot)
        || value.finalizedObservationSlot < 0) {
        throw new CurrentSdkOperationError("CURRENT_GOVERNED_PLAN_INVALID", "Oracle plan governance identity, epoch, or release pins are not exact");
    }
    return Object.freeze({
        release,
...
function validateGovernedTransportSemanticBindingV1(governed, semantic, binding) {
    ...
    if (!semanticInstruction.programId.equals(instruction.programId)
        ...
        throw new CurrentSdkOperationError("CURRENT_GOVERNED_PLAN_INVALID", "Oracle transport semantic view does not match the exact governed envelope");
    }
}
function validateCurrentOracleDraftPlan(plan) {
    ...
    const expectedTransportTag = isDirect ? expectedLogicalTag : 205;
    const governed = plan.governance !== undefined || plan.transportSemanticInstruction !== undefined;
    if ((plan.governance === undefined) !== (plan.transportSemanticInstruction === undefined)) {
        throw new CurrentSdkOperationError("CURRENT_GOVERNED_PLAN_INVALID", "Oracle plan must bind both governance and its Spread-validated transport semantic view");
    }
    const transportSemanticInstruction = plan.transportSemanticInstruction ?? plan.outerInstruction;
    if (plan.governance !== undefined) {
        validateGovernedTransportSemanticBindingV1(plan.outerInstruction, transportSemanticInstruction, plan.governance);
    }
    ...
    if (plan.request.actionType !== plan.actionType
    ...
    if (plan.oracle.market !== market.toBase58()
    ...
    if (isDirect) {
        ...
        const decoded = decodeCurrentExecuteCompressedStateV1(Buffer.from(transportSemanticInstruction.dataBase64, "base64"));
        if (decoded.logicalTag !== expectedLogicalTag
...
export async function readCurrentOracleState(input) {
    assertContext(input);
    const marketFacts = [];
    if (input.expiryId !== undefined) {
        ...
        if (read.market !== null)
            marketFacts.push({ address: read.address, market: read.market });
    }
    else {
    ...
    const markets = await Promise.all(marketFacts.map(async ({ address, market }) => {
        ...
        if (info === null)
            return { marketAddress: address, market, oracleMonthAddress, oracleMonth: null };
        requireProgramAccount("OracleMonth", info, input.programId, CURRENT_ORACLE_MONTH_ACCOUNT_SIZE);
        return {
...
export async function buildCurrentOracleDraft(input) {
    assertContext(input);
    ...
    if (vaultInfo === null)
        throw new CurrentSdkOperationError("CURRENT_VAULT_CONFIG_NOT_FOUND", "oracle draft requires current VaultConfig");
    ...
    if (!direct) {
        ...
        if (decodedOuter.logicalTag !== logicalTag
    ...
    let resolvedLookupTable = null;
    if (!direct) {
        if (input.oracleCompressedStateLookupTable === undefined) {
            throw new CurrentSdkOperationError("CURRENT_ORACLE_COMPRESSED_PLAN_UNAVAILABLE", "compressed Oracle prepare requires one independently authorized finalized address lookup table", {
    ...
    let transactionLookupTable = null;
    if (resolvedLookupTable !== null) {
        const lookup = compiled.message.addressTableLookups[0];
        if (compiled.message.addressTableLookups.length !== 1
            || lookup === undefined
            || !lookup.accountKey.equals(resolvedLookupTable.account.key)
            || lookup.writableIndexes.length + lookup.readonlyIndexes.length < 1
            || [...lookup.writableIndexes, ...lookup.readonlyIndexes].some((index) => !Number.isInteger(index) || index < 0 || index >= resolvedLookupTable.addresses.length)) {
    ...
    const transportMetrics = Object.freeze({
        outerInstructionDataBytes: outerTransactionInstruction.data.length,
        outerAccountMetaCount: outerTransactionInstruction.keys.length,
        outerUniqueAccountKeys: new Set([
            owner.toBase58(),
    ...
    const proofFacts = Object.freeze({
        ...
        ...([176, 177].includes(logicalTag) ? { disputeEvidenceJson: prepared.proofFacts.disputeEvidenceJson } : {}),
        recentBlockhash: latestBlockhash.blockhash,
    });
    const base = plan("oracle_draft", actionManifests, [], proofFacts);
    const preparedPlanWithoutDigest = {
        ...base,
        operation: "oracle_draft",
        actionType: prepared.actionType,
        request: prepared.request,
        oracle,
        currentInstructionTags: Object.freeze([logicalTag]),
        transportInstructionTags: Object.freeze([transportTag]),
...
const positionContexts = new WeakMap();
...
function positionRequest(value) {
    if (!value || !Object.hasOwn(POSITION_ACTIONS, value.action))
        positionRejected("unsupported collateral action");
    ...
    if (owner !== value.ownerPubkey || owner === PublicKey.default.toBase58())
        positionRejected("owner must be canonical and nonzero");
...
function positionRejected(message, code = "CURRENT_POSITION_ACTION_INVALID") {
...
export async function prepareCurrentPositionActionContext(input) {
    assertContext(input);
    ...
    if (!Number.isSafeInteger(floor) || floor < 0)
        positionRejected("finalized observation floor is invalid");
    const userCollateral = deriveUserCollateralPda(owner, input.programId);
    let builderName = "buildInitUserCollateralInstruction";
    let builderInput = { accounts: { user: owner, userCollateral } };
    let preliminaryVault = null;
    ...
    if (request.action !== "init_collateral") {
        ...
        if (!vaultInfo)
            positionRejected("current VaultConfig is absent");
    ...
    if (materialized.instructions.length !== 1)
        positionRejected("collateral action must have exactly one governed instruction");
    ...
    const observed = await input.connection.getMultipleAccountsInfoAndContext(addresses, { commitment: CURRENT_STATE_COMMITMENT, minContextSlot: Math.max(floor, materialized.governance.finalizedObservationSlot) });
    const slot = observed.context.slot;
    if (!Number.isSafeInteger(slot) || slot < floor || observed.value.length !== addresses.length)
        positionRejected("invalid finalized account observation");
    ...
    if (genesisHash !== CURRENT_PROTOCOL_DEVNET_GENESIS_HASH || blockTime === null || !Number.isSafeInteger(blockTime))
        positionRejected("finalized time or Devnet identity is unavailable");
    const origin = new URL(input.connection.rpcEndpoint).origin;
    const observationInput = {
        schemaVersion: 1, observationSource: "current_finalized_rpc", commitment: "finalized",
        observedAtSlot: String(slot), currentFinalizedSlot: String(slot), observedBlockTimeUnixSeconds: String(blockTime),
        stateProvider: { kind: "solana_json_rpc", origin,
    ...
    validatePositionAccountState(request, accountState, observation, manifest(semanticCurrentSpreadInstructionV1(instruction), { instructionName: POSITION_ACTIONS[request.action].name, tag: POSITION_ACTIONS[request.action].tag }));
...
export async function buildCurrentPositionAction(input) {
    assertContext(input);
    const request = positionRequest(input.request);
    input = { ...input, request };
    const context = input.context ?? await prepareCurrentPositionActionContext(input);
    const issued = positionContexts.get(context);
    if (!issued || issued.connection !== input.connection || !canonicalValuesEqual(request, context.request))
        positionRejected("position context was not issued for this connection and exact request");
    if (input.minimumContextSlot !== undefined && BigInt(context.observation.observedAtSlot) < BigInt(input.minimumContextSlot))
        positionRejected("position context predates requested floor");
    ...
    const withoutDigest = cloneAndFreezeIssuedValue({ ...base, operation: "position_action", actionType: request.action,
        instructionName: action.name, instructionTags: [action.tag], request, observation: context.observation,
        governance: issued.governance, semanticInstruction, accountState: issued.accountState, setupTransactions: [], transaction });
...
function validatePositionAccountState(request, accounts, observation, semantic) {
    const programId = new PublicKey(DEFAULT_AMEBA_SPREAD_PROGRAM_ID);
    const owner = new PublicKey(request.ownerPubkey);
    ...
    const account = (address) => {
        ...
        if (values.length !== 1 || facts.length !== 1)
            positionRejected("canonical account is absent or repeated in observation");
        const value = values[0], fact = facts[0];
        if (value.owner !== fact.owner || value.executable !== fact.executable)
            positionRejected("account identity differs from observation");
        if (value.dataBase64 === null) {
            if (fact.dataSha256 !== null || value.owner !== null)
                positionRejected("absent account evidence is inconsistent");
            return null;
        }
        const data = Buffer.from(value.dataBase64, "base64");
        if (data.toString("base64") !== value.dataBase64 || String(data.length) !== fact.dataLength || createHash("sha256").update(data).digest("hex") !== fact.dataSha256)
            positionRejected("account bytes differ from observation");
    ...
    if (request.action === "init_collateral") {
        if (collateralInfo !== null)
            positionRejected("collateral account is already initialized");
        expected = buildInitUserCollateralInstruction({ accounts: { user: owner, userCollateral: collateralAddress }, programId });
    }
    else {
        if (!collateralInfo)
            positionRejected(`UserCollateral ${collateralAddress.toBase58()} is absent for ${owner.toBase58()}; ${request.action} requires initialized collateral`, "CURRENT_POSITION_COLLATERAL_UNAVAILABLE");
        ...
        if (!vaultInfo)
            positionRejected("vault is absent");
        ...
        if (!userInfo)
            positionRejected(`classic SPL USDC account ${ata.toBase58()} is absent for owner ${owner.toBase58()} and mint ${vault.usdcMint.toBase58()}`, "CURRENT_POSITION_TOKEN_ACCOUNT_UNAVAILABLE");
        if (!custodyInfo)
            positionRejected(`classic SPL vault custody ${vault.vaultTokenAccount.toBase58()} is absent`);
        if (!mintInfo)
            positionRejected(`classic SPL collateral mint ${vault.usdcMint.toBase58()} is absent`);
        if (userInfo.executable || custodyInfo.executable || mintInfo.executable)
            positionRejected("canonical SPL custody or mint account is executable");
        let mint;
        let user;
        let custody;
        try {
        ...
        if (!mint.isInitialized || !user.isInitialized || user.isFrozen || !custody.isInitialized || custody.isFrozen
            ...
            || !custody.owner.equals(vaultAddress))
            positionRejected("canonical SPL custody identity/state is invalid", "CURRENT_POSITION_TOKEN_IDENTITY_INVALID");
        if (request.action === "deposit_collateral" && user.amount < request.amount)
            positionRejected(`classic SPL USDC balance at ${ata.toBase58()} is ${user.amount} atoms; deposit requires ${request.amount}`, "CURRENT_POSITION_TOKEN_BALANCE_INSUFFICIENT");
        if (request.action === "withdraw_collateral" && collateral.availableBalance < request.amount)
            positionRejected(`UserCollateral available balance is ${collateral.availableBalance} atoms; withdrawal requires ${request.amount}`, "CURRENT_POSITION_COLLATERAL_BALANCE_INSUFFICIENT");
        if (request.action === "withdraw_collateral" && custody.amount < request.amount)
            positionRejected(`classic SPL vault custody balance is ${custody.amount} atoms; withdrawal requires ${request.amount}`, "CURRENT_POSITION_CUSTODY_BALANCE_INSUFFICIENT");
    ...
    if (!canonicalValuesEqual(semantic, manifest(expected, params)))
        positionRejected("instruction differs from canonical collateral request and custody");
}
function validateCurrentPositionActionPlan(value) {
    if (!canonicalValuesEqual(value.deployment, CURRENT_PROTOCOL_DEPLOYMENT))
        positionRejected("position plan deployment differs");
    ...
    if (value.actionType !== request.action || value.instructionName !== action.name || !canonicalValuesEqual(value.instructionTags, [action.tag])
        ...
        || !canonicalValuesEqual(value.proofFacts, { actionType: request.action, instructionName: action.name, instructionTag: action.tag,
            currentObservationDigest: observation.currentObservationDigest, observedAtSlot: observation.observedAtSlot }))
        positionRejected("position plan identity or observation differs");
    ...
    if (preparedPlanDigest !== currentPreparedPlanDigest("position_action", withoutDigest))
        positionRejected("position prepared digest differs");
}
export async function readCurrentUserLedger(input) {
    assertContext(input);
    const owner = parseOwner(input.ownerPubkey);
    const address = deriveOraclePlayerLedgerPda(owner, input.programId);
    const info = await input.connection.getAccountInfo(address, finalizedCommitment(input.commitment));
    if (info === null)
        return { ...identity(), deployment: CURRENT_LIVE_DEPLOYMENT, address, found: false, ledger: null };
    ...
    const [, expectedBump] = PublicKey.findProgramAddressSync([CURRENT_STATE_NAMESPACE_SEED, ORACLE_PLAYER_LEDGER_PDA_SEED, owner.toBuffer()], input.programId);
...
export async function readCurrentUserCollateral(input) {
    assertContext(input);
    const owner = parseOwner(input.ownerPubkey);
    ...
    if (info === null)
        return { ...identity(), deployment: CURRENT_LIVE_DEPLOYMENT, address, observedAtSlot, found: false, collateral: null };
...
function validatedCurrentIdempotencyKey(value) {
    if (typeof value !== "string" || value.length < 1 || value.length > 128 || !/^[A-Za-z0-9._:-]+$/.test(value)) {
        throw new CurrentSdkOperationError("CURRENT_IDEMPOTENCY_KEY_INVALID", "idempotencyKey must be 1..128 ASCII letters, digits, dot, underscore, colon, or hyphen");
    }
    return value;
}
function validatedCurrentSubmissionReceipt(receipt, planValue, idempotencyKey) {
    exactKeys(receipt, [
    ...
    if (receipt.status !== "finalized"
        ...
        || receipt.preparedPlanDigest !== planValue.preparedPlanDigest
        ...
        throw new CurrentSdkOperationError("CURRENT_SUBMISSION_RECEIPT_INVALID", "managed submission receipt is not finalized or is not bound to the exact plan and idempotency key");
    }
    exactKeys(receipt.simulation, ["status", "unitsConsumed", "logsSha256"], "CURRENT_SUBMISSION_RECEIPT_INVALID");
    if (receipt.simulation.status !== "succeeded"
    ...
    receipt.postState.forEach((fact, index) => {
        ...
        if (fact.address !== planValue.writeSet[index]
            || !/^(0|[1-9][0-9]*)$/.test(fact.lamports)
            || typeof fact.executable !== "boolean"
            || !/^[0-9a-f]{64}$/.test(fact.dataSha256)) {
            throw new CurrentSdkOperationError("CURRENT_SUBMISSION_RECEIPT_INVALID", "post-state must attest every writable account in exact plan order");
        }
    });
    return cloneAndFreezeIssuedValue(receipt);
}
/** Stateless validation for a managed-signer finalized receipt and its exact plan. */
export function validateCurrentSubmissionReceipt(input) {
...
function currentUserCollateralDto(value, ownerPubkey) {
    return Object.freeze({
...
function snapshotCurrentOracleLookupTableAuthorization(value) {
    if (value === undefined)
        return undefined;
    let address;
    let authority;
...
export function createCurrentSdkAdapter(input) {
    ...
    const issuedOraclePlans = new WeakSet();
    const submittedOracleIdempotencyKeys = new WeakMap();
    const issuedOperationPlans = new WeakMap();
    const issuePlan = (value) => {
        issuedOperationPlans.set(value, issuedPlanIntegrityDigest(value));
        return value;
    };
    const validateIssuedPlan = (planValue) => {
        const issuedDigest = issuedOperationPlans.get(planValue);
        if (issuedDigest === undefined) {
            throw new CurrentSdkOperationError("CURRENT_PLAN_NOT_ISSUED", "operation validation accepts only the exact plan object issued by this adapter instance");
        }
    ...
    const attest = async () => {
        ...
        if (attestationCache?.observedSlot === observedSlot)
        ...
        try {
            ...
            if (attestationInFlight?.promise === pending)
                attestationInFlight = undefined;
            throw cause;
        }
        finally {
            if (attestationInFlight?.promise === pending)
    ...
    const bound = (value, commitment) => Object.assign({}, value, context(commitment));
    const afterAttestation = async (operation) => {
    ...
    const adapter = {
        ...
        readCurrentMarketAccount: (value) => afterAttestation(() => readCurrentMarketAccount(bound({ marketId: value.marketId, expiryId: value.expiryId }, value.commitment))),
        ...
        buildCurrentGovernedInstruction: (value) => afterAttestation(async () => {
            ...
            if (!Number.isSafeInteger(minimumContextSlot) || minimumContextSlot < 0) {
                throw new CurrentSdkOperationError("CURRENT_GOVERNANCE_CONTEXT_INVALID", "finalized governance observation floor is unavailable");
            }
        ...
        readCurrentOracleState: (value = {}) => afterAttestation(() => readCurrentOracleState(bound({ marketId: value.marketId, expiryId: value.expiryId }, value.commitment))),
        ...
        readCurrentUserLedger: (value) => afterAttestation(async () => currentUserLedgerDto(await readCurrentUserLedger(bound({ ownerPubkey: value.owner }, value.commitment)), value.owner)),
        readCurrentUserCollateral: (value) => afterAttestation(async () => currentUserCollateralDto(await readCurrentUserCollateral(bound({ ownerPubkey: value.owner, ...(value.minimumContextSlot === undefined ? {} : { minimumContextSlot: value.minimumContextSlot }) }, value.commitment)), value.owner)),
...
//# sourceMappingURL=current-adapter.js.map