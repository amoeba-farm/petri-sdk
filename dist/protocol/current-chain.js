...
import { accountCompressionProgram, batchAddressTree, batchCpiContext1, batchCpiContext2, batchCpiContext3, batchCpiContext4, batchCpiContext5, batchMerkleTree1, batchMerkleTree2, batchMerkleTree3, batchMerkleTree4, batchMerkleTree5, batchQueue1, batchQueue2, batchQueue3, batchQueue4, batchQueue5, lightSystemProgram, } from "@lightprotocol/stateless.js";
import { PublicKey, SystemProgram, } from "@solana/web3.js";
import { CURRENT_PROTOCOL_CLUSTER, CURRENT_PROTOCOL_DEPLOYMENT, CURRENT_PROTOCOL_DEVNET_GENESIS_HASH, CURRENT_PROTOCOL_RELEASE, CURRENT_PROTOCOL_SOURCE_COMMIT, CURRENT_VAULT_CONFIG_ACCOUNT_SIZE, decodeCurrentVaultConfigAccount, } from "./current.js";
import { CurrentSdkOperationError, readCurrentDeploymentFacts, } from "./current-adapter.js";
import { CURRENT_LIGHT_PROGRAM_DEPLOYMENT_ORDER, } from "./current-light-deployments.js";
import { DEFAULT_AMEBA_SPREAD_PROGRAM_ID, LIGHT_TOKEN_COMPRESSIBLE_CONFIG, LIGHT_TOKEN_CPI_AUTHORITY, LIGHT_TOKEN_PROGRAM_ID, LIGHT_TOKEN_RENT_SPONSOR, SPL_TOKEN_PROGRAM_ID, } from "@amoeba/spread-release-tools/oracle-dlmm";
import { deriveVaultConfigPda } from "@amoeba/spread-release-tools/oracle-dlmm";
const CURRENT_NAMESPACE_VALUE = "ameba-spread-v2";
const CURRENT_COMMITMENT = "finalized";
...
const CURRENT_LIGHT_STATE_TREE_CONTEXTS = Object.freeze([
...
const DEFAULT_DEPENDENCIES = Object.freeze({
    ...
    hashProgramPayload: (payload) => createHash("sha256").update(payload).digest("hex"),
});
...
function assertSafeSlot(slot, minimum, label) {
...
function exactPrimitiveRecord(actual, expected) {
...
function assertPinnedDeploymentFacts(facts, input) {
    if (facts.stateNamespace !== CURRENT_NAMESPACE_VALUE
        ...
        || facts.verified !== true) {
        fail("CURRENT_CHAIN_DEPLOYMENT_INVALID", "deployment reader did not return the exact verified rc.44 Program and ProgramData identity");
    }
}
function requireAccount(info, code, label) {
    if (info === null || info === undefined)
        fail(code, `${label} is absent at finalized commitment`);
    return info;
}
...
function parseUpgradeableProgram(address, info, expected) {
    ...
    if (address.toBase58() !== expected.programId
...
function assertUpgradeableProgramData(programId, programDataAddress, info, expected, hashProgramPayload) {
    const bytes = Buffer.from(info.data);
    if (!info.owner.equals(BPF_UPGRADEABLE_LOADER_ID)
        || info.executable
    ...
    if (deployedSlot !== BigInt(expected.deployedSlot)
...
function assertSystemIdentityAccount(address, info, label) {
...
function assertCompressibleConfig(info) {
    ...
    const bytes = Buffer.from(info.data);
    if (!bytes.subarray(0, 8).equals(LIGHT_COMPRESSIBLE_CONFIG_DISCRIMINATOR)
        || bytes[8] !== 1
        || (bytes[9] !== 0 && bytes[9] !== 1)
...
function assertAddressTree(info) {
    assertExactDataAccount({
        info,
...
function assertStateTree(context, info) {
    assertExactDataAccount({
        info,
        expectedOwner: LIGHT_ACCOUNT_COMPRESSION_PROGRAM_ID,
    ...
    const bytes = Buffer.from(info.data);
    if (!bytes.subarray(0, 8).equals(LIGHT_TREE_DISCRIMINATOR)
...
function assertOutputQueue(context, info) {
    assertExactDataAccount({
        info,
        expectedOwner: LIGHT_ACCOUNT_COMPRESSION_PROGRAM_ID,
    ...
    const bytes = Buffer.from(info.data);
    if (!bytes.subarray(0, 8).equals(LIGHT_QUEUE_DISCRIMINATOR)
...
function assertCpiContext(context, info) {
    assertExactDataAccount({
        info,
    ...
    const bytes = Buffer.from(info.data);
    if (!bytes.subarray(0, 8).equals(LIGHT_CPI_CONTEXT_DISCRIMINATOR)
...
export async function readCurrentChainIdentity(input, dependencies = DEFAULT_DEPENDENCIES) {
    assertInput(input);
    ...
    const vaultInfo = requireAccount(vaultSnapshot.value, "CURRENT_VAULT_IDENTITY_INVALID", "current VaultConfig");
    let vaultConfig;
    try {
    ...
    if (linkedProgramDeployments.length !== linkedPrograms.length
        || linkedPrograms.some((programId, index) => programId.toBase58() !== linkedProgramDeployments[index]?.programId)) {
    ...
    const coreAccounts = new Map();
    coreAddresses.forEach((address, index) => coreAccounts.set(address.toBase58(), coreSnapshot.value[index] ?? null));
    const getCore = (address, code, label) => requireAccount(coreAccounts.get(address.toBase58()), code, label);
    ...
    if (vaultConfig.usdcMint.equals(PublicKey.default)
    ...
    if (vaultConfig.vaultTokenAccount.equals(PublicKey.default)
    ...
    const programDataAddresses = linkedPrograms.map((programId, index) => parseUpgradeableProgram(programId, getCore(programId, "CURRENT_LIGHT_PROGRAM_INVALID", "canonical Light program"), linkedProgramDeployments[index]));
    ...
    const linkedProgramIdentities = Object.freeze(programDataAddresses.map((programDataAddress, index) => assertUpgradeableProgramData(linkedPrograms[index], programDataAddress, requireAccount(programDataSnapshot.value[index], "CURRENT_LIGHT_PROGRAM_INVALID", "Light ProgramData"), linkedProgramDeployments[index], hashProgramPayload)));
    assertSystemIdentityAccount(LIGHT_TOKEN_CPI_AUTHORITY, getCore(LIGHT_TOKEN_CPI_AUTHORITY, "CURRENT_LIGHT_SYSTEM_IDENTITY_INVALID", "Light CPI authority"), "Light CPI authority");
    assertSystemIdentityAccount(LIGHT_TOKEN_RENT_SPONSOR, getCore(LIGHT_TOKEN_RENT_SPONSOR, "CURRENT_LIGHT_SYSTEM_IDENTITY_INVALID", "Light rent sponsor"), "Light rent sponsor");
    assertCompressibleConfig(getCore(LIGHT_TOKEN_COMPRESSIBLE_CONFIG, "CURRENT_LIGHT_CONFIG_INVALID", "Light compressible config"));
    assertAddressTree(getCore(LIGHT_ADDRESS_TREE_ID, "CURRENT_LIGHT_ADDRESS_TREE_INVALID", "Light AddressV2 tree"));
...
//# sourceMappingURL=current-chain.js.map