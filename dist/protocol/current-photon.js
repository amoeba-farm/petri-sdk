...
function parseCurrentPhotonEndpoint(value, label) {
    let url;
    try {
        url = new URL(value);
    }
...
class CurrentPhotonConnectionImpl {
    ...
    async getCompressedAccountsByOwner(owner, config) {
        ...
        if (result.cursor !== null || result.items.length > limit) {
            throw photonError("CURRENT_PHOTON_OWNER_QUERY_BOUND_EXCEEDED", "compressed owner query is incomplete or exceeds its explicit bound");
        }
...
export async function createCurrentPhotonConnection(input) {
    ...
    if (stateGenesis !== CURRENT_PROTOCOL_DEVNET_GENESIS_HASH
        || photonGenesis !== CURRENT_PROTOCOL_DEVNET_GENESIS_HASH) {
        throw photonError("CURRENT_PHOTON_GENESIS_MISMATCH", "Photon and standard state transports must both attest the maintained Devnet genesis");
    }
