...
export const CURRENT_COMPRESSED_EVIDENCE_SCHEMA = "ameba-compressed-evidence-v1";
const issued = new WeakSet();
const hash = (bytes) => createHash("sha256").update(bytes).digest("hex");
const hex32 = (value) => typeof value === "string" && /^[0-9a-f]{64}$/.test(value);
function u64(value) { return typeof value === "string" && /^(0|[1-9][0-9]*)$/.test(value) && BigInt(value) <= 18446744073709551615n; }
function reject(message) { throw new Error(`CURRENT_COMPRESSED_EVIDENCE_INVALID: ${message}`); }
function keys(value, expected) {
    if (JSON.stringify(Object.keys(value).sort()) !== JSON.stringify([...expected].sort()))
        reject("unexpected JSON fields");
}
export function requireCurrentCompressedEvidenceVerifier(value) {
    if (!issued.has(value))
        reject("verifier was not issued from a trusted executable configuration");
}
export function validateCurrentCompressedEvidenceRequest(request) {
    ...
    if (request.schema !== CURRENT_COMPRESSED_EVIDENCE_SCHEMA || !u64(request.finalizedSlot) || !u64(request.minimumSlot)
        || BigInt(request.finalizedSlot) < BigInt(request.minimumSlot) || !hex32(request.contextBindingHex))
        reject("invalid request identity/slot");
    const identity = request.identity;
    if (!identity || !hex32(identity.programIdHex) || !hex32(identity.addressTreeKeyHex) || !hex32(identity.canonicalPdaHex) || !Number.isInteger(identity.domain) || identity.domain < 1 || identity.domain > 10)
        reject("invalid compressed identity");
    ...
    if (!statement || !["state_merkle", "state_queue", "address_absent"].includes(statement.kind)
        || !hex32(statement.treeKeyHex) || !hex32(statement.valueHex))
        reject("invalid proof statement");
    const state = statement.kind !== "address_absent";
    const proof = statement.kind !== "state_queue";
    keys(statement, ["kind", "treeKeyHex", "valueHex", ...(state ? ["queueKeyHex", "leafIndex"] : []), ...(proof ? ["rootHex", "rootIndex", "proofHex"] : [])]);
    if (state && (!hex32(statement.queueKeyHex) || !u64(statement.leafIndex) || BigInt(statement.leafIndex) > 4294967295n))
        reject("invalid state queue/index");
    if (proof && (!hex32(statement.rootHex) || !Number.isInteger(statement.rootIndex) || statement.rootIndex < 0 || statement.rootIndex > 65_535
        || typeof statement.proofHex !== "string" || !/^[0-9a-f]{256}$/.test(statement.proofHex)))
        reject("invalid proof/root");
    if (state) {
        ...
        if (!account || account.ownerHex !== identity.programIdHex || !hex32(account.addressHex) || account.lamports !== "0"
            ...
            || typeof account.dataHex !== "string" || account.dataHex.length > 131_072 || !/^(?:[0-9a-f]{2})+$/.test(account.dataHex))
            reject("invalid compressed account envelope");
        ...
        if (dataHash.toString("hex") !== account.dataHashHex || account.discriminatorHex !== hash("CompressedAmebaStateLeaf").slice(0, 16)
            ...
            || data.readUInt32LE(42) !== data.length - 46)
            reject("compressed data hash/domain/PDA binding failed");
    }
    const accountKeys = state ? [statement.treeKeyHex, statement.queueKeyHex] : [statement.treeKeyHex];
    if (!Array.isArray(request.accounts) || request.accounts.length !== accountKeys.length || new Set(accountKeys).size !== accountKeys.length)
        reject("invalid account inventory");
    request.accounts.forEach((account, index) => {
        ...
        if (account.keyHex !== accountKeys[index] || !hex32(account.ownerHex) || account.executable !== false
            || typeof account.dataHex !== "string" || account.dataHex.length > 32 * 1024 * 1024 || !/^(?:[0-9a-f]{2})+$/.test(account.dataHex))
            reject("invalid raw account snapshot");
    });
}
export async function createCurrentCompressedEvidenceVerifier(configuration) {
    ...
    if (!isAbsolute(configuredPath) || !hex32(expectedSha256))
        reject("trusted absolute executable and actual SHA256 required");
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 100 || timeoutMs > 60_000)
        reject("timeout outside 100..60000ms");
    ...
    async function attest() {
        ...
        if (!info.isFile() || info.size < 1 || info.size > 128 * 1024 * 1024)
            reject("invalid verifier executable size/type");
        if (hash(await readFile(executablePath)) !== expectedSha256)
            reject("verifier executable digest mismatch");
    }
    await attest();
    const verifier = Object.freeze({ executableSha256: expectedSha256,
        async verify(value) {
            ...
            const stdout = await new Promise((resolve, rejectPromise) => {
                ...
                const fail = () => { if (settled)
                    return; settled = true; clearTimeout(timer); child.kill(); rejectPromise(new Error("CURRENT_COMPRESSED_EVIDENCE_VERIFIER_FAILED: bounded native verification failed")); };
                const timer = setTimeout(fail, timeoutMs);
                child.once("error", fail);
                child.stdin.once("error", fail);
                child.stdout.on("data", (chunk) => { bytes += chunk.length; if (bytes > 16_384)
                    fail();
                else
                    chunks.push(chunk); });
                child.stderr.on("data", (chunk) => { errorBytes += chunk.length; if (errorBytes > 16_384)
                    fail(); });
                child.once("close", code => { if (settled)
                    return; if (code !== 0)
                    return fail(); settled = true; clearTimeout(timer); resolve(Buffer.concat(chunks)); });
            ...
            try {
                ...
                return reject("native result is not JSON");
            }
            if (!result || typeof result !== "object" || Array.isArray(result))
                reject("native result is not an object");
            ...
            if (result.schema !== request.schema || result.requestSha256 !== hash(encoded) || result.contextBindingHex !== request.contextBindingHex
                ...
                || result.verification !== (statement.kind === "state_queue" ? "queue_inclusion_and_bloom" : "groth16_current_root_and_bloom"))
                reject("native result does not bind exact request");
...
export async function readCurrentCompressedEvidenceVerifierSource() {
    ...
    if (bytes.length > 16_384)
        reject("source locator exceeds bound");
    ...
    if (manifest.schemaVersion !== 1 || manifest.interfaceSchema !== CURRENT_COMPRESSED_EVIDENCE_SCHEMA
        ...
        || typeof manifest.lockProvenance !== "string")
        reject("native source locator differs from agreed package contract");
    const files = [];
    for (const relativePath of ["source.json", "Cargo.toml", "Cargo.lock", "INTERFACE.md", "src/main.rs"]) {
        ...
        if (!info.isFile() || info.size < 1 || info.size > 4 * 1024 * 1024)
            reject("native source file is absent or oversized");
        files.push(Object.freeze({ relativePath, path, sha256: hash(await readFile(path)) }));
    }
    return Object.freeze({ root, manifest: Object.freeze(manifest), files: Object.freeze(files), artifactStatus: "source-only-unqualified" });
}
//# sourceMappingURL=current-compressed-evidence-verifier.js.map