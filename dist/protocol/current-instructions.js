...
import { CURRENT_VAULT_INSTRUCTION_TAG, } from "./current.js";
import { AmebaProtocolLayoutError } from "../errors.js";
import { DEFAULT_AMEBA_SPREAD_PROGRAM_ID, SPL_TOKEN_PROGRAM_ID, MIN_SETTLEMENT_SIGNER_ROTATION_DELAY_SLOTS, deriveSettlementSignerRegistryPda, deriveSettlementSignerSetPda, deriveVaultConfigPda, } from "@amoeba/spread-release-tools/oracle-dlmm";
function writable(pubkey, signer = false) {
    return { pubkey, isSigner: signer, isWritable: true };
}
function readonly(pubkey, signer = false) {
...
export function buildInitUserCollateralInstruction(input) {
    return instruction(CURRENT_VAULT_INSTRUCTION_TAG.InitUserCollateral, [writable(input.accounts.user, true), writable(input.accounts.userCollateral), readonly(SystemProgram.programId)], Buffer.alloc(0), input.programId ?? new PublicKey(DEFAULT_AMEBA_SPREAD_PROGRAM_ID));
}
...
export function buildProposeEmergencySettlementSignerRecoveryInstruction(input) {
    ...
    if (payload.length !== 282)
        throw new Error("tag 94 payload length is not canonical");
    return instruction(CURRENT_VAULT_INSTRUCTION_TAG.ProposeEmergencySettlementSignerRecovery, [
...
function instruction(tag, keys, payload, programId) {
...
export function buildWithdrawCollateralInstruction(input) {
    ...
    return instruction(CURRENT_VAULT_INSTRUCTION_TAG.WithdrawCollateral, [
    ...
    ], u64(input.amount, "amount"), input.programId ?? new PublicKey(DEFAULT_AMEBA_SPREAD_PROGRAM_ID));
}
...
class CurrentPayloadCursor {
    ...
    fail(reason) {
        throw new AmebaProtocolLayoutError("VaultInstruction", `tag ${this.tag} payload ${reason}`);
    }
    ...
    boolean(field) {
        ...
        if (value !== 0 && value !== 1)
            this.fail(`has a noncanonical bool for ${field}`);
    }
    enumByte(field, allowed) {
        const value = this.u8(field);
        if (!allowed.includes(value))
            this.fail(`has invalid ${field} discriminant ${value}`);
    }
    option(field, parseSome) {
        const discriminant = this.u8(`${field} option discriminant`);
        if (discriminant === 0)
            return;
        if (discriminant !== 1)
            this.fail(`has invalid ${field} option discriminant ${discriminant}`);
        parseSome();
    }
    boundedCount(field, maximum) {
        const count = this.u32(`${field} length`);
        if (count > maximum)
            this.fail(`${field} length ${count} exceeds ${maximum}`);
        return count;
    }
    boundedString(field, maximum) {
...
const emptyPayload = () => { };
function exactBytes(length, field = "fixed fields") {
    return (cursor) => cursor.skip(length, field);
}
function optionalPubkey(cursor, field) {
    cursor.option(field, () => cursor.skip(32, field));
}
function optionalU64(cursor, field) {
    cursor.option(field, () => cursor.skip(8, field));
}
function parseUpdateConfig(cursor) {
...
function parseInitMarketV2(cursor) {
...
function parseFixedTrailingBool(prefixLength, field) {
    return (cursor) => {
        cursor.skip(prefixLength, `${field} prefix`);
        cursor.boolean(field);
    };
}
function parseBoundedBytes32Vector(cursor, field, maximum) {
    const count = cursor.boundedCount(field, maximum);
    cursor.skip(count * 32, field);
}
function parseConfigureOracleProductSkuManifest(cursor) {
...
function parseProposeOracleSourceV3(cursor) {
...
function parsePackedStateTreeInfo(cursor) {
...
function parseCompressedMarketPageLeaf(cursor) {
    ...
    for (let index = 0; index < expiryCount; index += 1)
        parseMarketPageExpiry(cursor);
    cursor.boolean("market page active");
}
function parseCompressedSettlementLeaf(cursor) {
    ...
    const observationCount = cursor.boundedCount("settlement observations", SETTLEMENT_MAX_OBSERVATIONS);
    ...
    cursor.skip(32 + 8 + 8 + 32 + 8 + 8 + 32, "settlement digest, oracle values, submitter, slot, and signer set");
}
function parseUpsertMarketPageV2(cursor) {
...
function parseUpsertSettlementV3(cursor) {
...
function parseCompressedDomain(cursor, field) {
...
function parseCompactAccessLeaf(cursor) {
    const domain = parseCompressedDomain(cursor, "compressed state domain");
    cursor.skip(8, "compressed state revision");
    cursor.skip(COMPRESSED_DOMAIN_DATA_LENGTH[domain], "compressed state compact data");
}
function parseCompressedStateAccess(cursor) {
...
function parseExecuteCompressedStateV1(cursor) {
    ...
    if (accessCount > MAX_COMPRESSED_STATE_SESSION_RECORDS) {
        cursor.fail(`compressed state access count ${accessCount} exceeds ${MAX_COMPRESSED_STATE_SESSION_RECORDS}`);
    }
    for (let index = 0; index < accessCount; index += 1)
        parseCompressedStateAccess(cursor);
    const innerLength = cursor.u16("compressed inner instruction length");
    if (innerLength > MAX_COMPRESSED_INNER_INSTRUCTION_BYTES) {
        cursor.fail(`compressed inner instruction length ${innerLength} exceeds ${MAX_COMPRESSED_INNER_INSTRUCTION_BYTES}`);
    }
...
class CurrentCompressedEnvelopeCursor {
    ...
    fail(reason) {
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `tag 205 payload ${reason}`);
    }
    require(length, field) {
        if (!Number.isSafeInteger(length) || length < 0 || this.offset + length > this.payload.length) {
            this.fail(`is truncated while reading ${field}`);
...
function decodePackedStateTreeInfo(cursor) {
...
function decodeCompactAccessLeaf(cursor) {
    const domain = cursor.u8("compressed state domain");
    const compactLength = COMPRESSED_DOMAIN_DATA_LENGTH[domain];
    if (compactLength === undefined) {
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `tag 205 payload has invalid compressed state domain ${domain}`);
    }
    return Object.freeze({
        domain,
 ...
 */
export function decodeCurrentExecuteCompressedStateV1(data) {
    const decoded = decodeCurrentVaultInstruction(data);
    if (decoded.tag !== CURRENT_VAULT_INSTRUCTION_TAG.ExecuteCompressedStateV1) {
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `expected outer tag 205, received ${decoded.tag}`);
    }
    ...
    if (proofDiscriminant === 0) {
        ...
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `tag 205 payload has invalid validity proof option discriminant ${proofDiscriminant}`);
    }
    const accessCount = cursor.u8("compressed state access count");
    if (accessCount < 1 || accessCount > MAX_COMPRESSED_STATE_SESSION_RECORDS) {
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `tag 205 payload access count ${accessCount} is outside 1..${MAX_COMPRESSED_STATE_SESSION_RECORDS}`);
    }
    const accesses = [];
    for (let index = 0; index < accessCount; index += 1) {
        ...
        if (kind === 0) {
            ...
            if (COMPRESSED_DOMAIN_DATA_LENGTH[domain] === undefined) {
                throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `tag 205 payload has invalid initialization domain ${domain}`);
            }
            ...
            throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `tag 205 payload has invalid access kind ${kind}`);
        }
    }
    const innerLength = cursor.u16("compressed inner instruction length");
    if (innerLength === 0 || innerLength > MAX_COMPRESSED_INNER_INSTRUCTION_BYTES) {
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", `tag 205 payload inner length ${innerLength} is outside 1..${MAX_COMPRESSED_INNER_INSTRUCTION_BYTES}`);
    }
    ...
    if (logical.tag === CURRENT_VAULT_INSTRUCTION_TAG.ExecuteCompressedStateV1) {
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", "nested ExecuteCompressedStateV1 is forbidden");
    }
    if (rentPayerIndex >= coreAccountCount) {
        throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", "rent payer index is outside the core account prefix");
    }
    for (const access of accesses) {
        if (access.accountIndex >= coreAccountCount) {
            throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", "compressed access account index is outside the core account prefix");
        }
    }
    for (let index = 1; index < accesses.length; index += 1) {
        const previous = accesses[index - 1];
        const current = accesses[index];
        if (current.accountIndex < previous.accountIndex
            || (current.accountIndex === previous.accountIndex
                && !(previous.domain === 9 && current.domain === 10))) {
            throw new AmebaProtocolLayoutError("ExecuteCompressedStateV1", "compressed accesses are not in canonical account-index/domain order");
        }
    }
    return Object.freeze({
...
const CURRENT_VAULT_PAYLOAD_PARSERS = Object.freeze({
    ...
    [TAG.ManageWriterDlmmV1]: (cursor) => {
        ...
        if (selector === 0)
            ...
            if (count === 0)
                cursor.fail("writer action must contain entries");
            cursor.skip(count * 18, "writer bin entries");
        }
        else if (selector !== 2)
            cursor.fail("writer DLMM selector is invalid");
    },
    ...
    [TAG.AccumulateOracleRecipeBucketV2]: parseFixedTrailingBool(34, "finalize_collection"),
    ...
    [TAG.AccumulateOracleSettlementSourceBucket]: parseFixedTrailingBool(32, "finalize_collection"),
    [TAG.UpsertSettlementV3]: parseUpsertSettlementV3,
    [TAG.BeginOracleActiveWeights]: exactBytes(4),
    [TAG.AccumulateOracleActiveWeightGroup]: parseFixedTrailingBool(32, "finalize_collection"),
    ...
    [TAG.ResolveOracleOpeningClaimChallengeV2]: (cursor) => cursor.enumByte("opening challenge outcome", [0, 1, 2, 3, 4]),
    ...
    [TAG.SettleOracleUsdcEscrow]: (cursor) => cursor.enumByte("oracle escrow kind", [0, 1, 2, 3, 4, 5, 6]),
    ...
    [TAG.ClaimOracleUsdcReward]: (cursor) => cursor.enumByte("oracle USDC reward kind", [0, 1, 2, 3]),
    ...
    [TAG.ResolveOracleSourceChallengeV2]: (cursor) => cursor.enumByte("source challenge outcome", [0, 1, 2, 3]),
    ...
    [TAG.SettleFailedOracleMonthEscrowV2]: (cursor) => cursor.enumByte("oracle escrow kind", [0, 1, 2, 3, 4, 5, 6]),
    ...
    [TAG.IndexOracleRecipeSourceV1]: (cursor) => {
        for (const field of ["previous hash", "bucket id", "source id", "source type hash", "canonical locator hash", "source definition hash"]) {
            ...
            for (let index = 0; index < 32; index += 1)
                nonzero = cursor.u8(field) !== 0 || nonzero;
            if (!nonzero)
                cursor.fail(`${field} must be nonzero`);
        }
        const weight = cursor.u16("bucket weight bps");
        if (weight < 1 || weight > 10_000)
            cursor.fail("bucket weight must be within 1..10000 bps");
    },
    ...
    [TAG.OracleCarryForwardV1]: (cursor) => {
        ...
        if (action > 7)
            cursor.fail("invalid oracle carry action");
        if (action === 2) {
            ...
            if (count > 8)
                cursor.fail("SKU proof exceeds eight nodes");
 ...
 */
export function decodeCurrentVaultInstruction(data) {
    ...
    if (data.byteLength > CURRENT_MAX_INSTRUCTION_DATA_BYTES) {
        throw new AmebaProtocolLayoutError("VaultInstruction", `instruction data length ${data.byteLength} exceeds ${CURRENT_MAX_INSTRUCTION_DATA_BYTES}`);
    }
    const bytes = Buffer.from(data);
    const tag = bytes[0];
    const parser = CURRENT_VAULT_PAYLOAD_PARSERS[tag];
    if (parser === undefined ||
        !Object.values(CURRENT_VAULT_INSTRUCTION_TAG).includes(tag)) {
...
//# sourceMappingURL=current-instructions.js.map