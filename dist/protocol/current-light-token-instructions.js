...
import { CLASSIC_SPL_TOKEN_PROGRAM_ID, CURRENT_LIGHT_TOKEN_PROGRAM_ID, deriveLightAssociatedTokenAddress, } from "@amoeba/spread-release-tools/token-primitives";
...
const CURRENT_LIGHT_TOKEN_CPI_AUTHORITY = new PublicKey("GXtd2izAiMJPwMEjfgTRH3d7k9mjn4Jq3JrWFv9gySYy");
const CURRENT_LIGHT_TOKEN_COMPRESSIBLE_CONFIG = new PublicKey("ACXg8a7VaqecBWrSbdu73W4Pg9gsqXJ3EXAqkHyhvVXg");
const CURRENT_LIGHT_TOKEN_RENT_SPONSOR = new PublicKey("r18WwUxfG8kQ69bQPAB2jV6zGNKy3GosFGctjQoV4ti");
const PINNED_CURRENT_LIGHT_TOKEN_PROGRAM_ID = new PublicKey(CURRENT_LIGHT_PROGRAM_DEPLOYMENTS.tokenProgram.programId);
...
export const CURRENT_LIGHT_CREATE_ATA_IDEMPOTENT_DATA = Object.freeze([102, 1, 3, 16, 1, 254, 2, 0, 0, 0]);
...
export function buildCurrentCreateLightAtaIdempotentInstruction(input) {
    ...
    return new TransactionInstruction({
        ...
        data: Uint8Array.from(CURRENT_LIGHT_CREATE_ATA_IDEMPOTENT_DATA),
    });
}
 ...
 */
export function buildCurrentClassicSplToLightAtaRebridgeV1(input) {
    if (!CURRENT_LIGHT_TOKEN_PROGRAM_ID.equals(PINNED_CURRENT_LIGHT_TOKEN_PROGRAM_ID)) {
        throw new CurrentLightRebridgeValidationError("Light token primitive does not match the pinned current deployment identity");
    }
    ...
    const source = exactNondefaultKey(input.sourceClassicSplAccount, "source classic-SPL account");
    const mint = exactNondefaultKey(input.mint, "mint");
    const amountAtoms = input.amountAtoms;
    const decimals = input.decimals;
    if (typeof amountAtoms !== "bigint"
        || amountAtoms <= 0n
        || amountAtoms > 0xffffffffffffffffn) {
    ...
    const [splInterface, splInterfaceBump] = PublicKey.findProgramAddressSync([LIGHT_SPL_INTERFACE_POOL_SEED, mint.toBytes()], CURRENT_LIGHT_TOKEN_PROGRAM_ID);
    if (source.equals(destination)
        || source.equals(mint)
        || source.equals(splInterface)) {
        throw new CurrentLightRebridgeValidationError("re-bridge source collides with a canonical mint, interface, or destination account");
    }
    ...
    const transfer = new TransactionInstruction({
        ...
        data: encodeClassicSplToLightTransfer2V1(amountAtoms, splInterfaceBump, decimals),
    });
    return Object.freeze({
        ...
        instructions: Object.freeze([createDestination, transfer]),
    });
}
function encodeClassicSplToLightTransfer2V1(amount, splInterfaceBump, decimals) {
...
function writeCompression(data, view, offset, value) {
...
function exactNondefaultKey(value, label) {
...
//# sourceMappingURL=current-light-token-instructions.js.map