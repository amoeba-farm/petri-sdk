...
import { ACCOUNT_SIZE, MINT_SIZE, unpackAccount, unpackMint, } from "./current-token-primitives.js";
import { CURRENT_STATE_NAMESPACE_SEED, DEFAULT_AMEBA_SPREAD_PROGRAM_ID, LIGHT_TOKEN_CPI_AUTHORITY, MARKET_PDA_SEED, MIN_SETTLEMENT_SIGNER_ROTATION_DELAY_SLOTS, ORACLE_MONTH_PDA_SEED, SETTLEMENT_SIGNER_REGISTRY_PDA_SEED, SETTLEMENT_SIGNER_SET_PDA_SEED, SETTLEMENT_V2_PDA_SEED, SPL_TOKEN_PROGRAM_ID, USER_COLLATERAL_PDA_SEED, VAULT_PDA_SEED, } from "@amoeba/spread-release-tools/oracle-dlmm";
import { deriveMarketPda, deriveContractMintPda, deriveLightSplInterfacePda, deriveOracleMonthPda, deriveOracleSkuCoverageManifestPda, deriveSettlementRecordV2Pda, deriveSettlementSignerRegistryPda, deriveSettlementSignerSetPda, deriveUserCollateralPda, deriveVaultConfigPda, hashBuffers, } from "@amoeba/spread-release-tools/oracle-dlmm";
...
import { AmebaAccountNamespaceMismatchError, AmebaMintOwnerMismatchError, AmebaProgramIdMismatchError, AmebaProtocolDeploymentError, AmebaProtocolIdentityError, AmebaProtocolLayoutError, } from "../errors.js";
/** Historical RC44 reader-semantic source. It is not the live byte source. */
export const CURRENT_PROTOCOL_SOURCE_COMMIT = "1b2230d96e51f6582155d8284900fbfc11ff1f18";
export const CURRENT_PROTOCOL_RELEASE = "v0.1.0-rc.44";
export const CURRENT_PROTOCOL_CLUSTER = "devnet";
export const CURRENT_PROTOCOL_DEVNET_GENESIS_HASH = "EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG";
export const CURRENT_PROTOCOL_SCHEMA_VERSION = 2;
export const CURRENT_PROTOCOL_PROGRAMDATA_ADDRESS = "2DBN762WGdNc85xiVdvQX7Lo4TXAq3WhVz9mBQcaU3a3";
export const CURRENT_PROTOCOL_PROGRAMDATA_BYTES = 1_241_821;
export const CURRENT_PROTOCOL_UPGRADE_AUTHORITY = "D5jhTM3kYHdKixrc52Kn657gBHytQLhQqsTmJBcNFVdq";
export const CURRENT_PROTOCOL_DEPLOYED_SLOT = 487_702_729;
export const CURRENT_PROTOCOL_FINALIZED_VERIFICATION_CONTEXT_SLOT = 487_703_026;
export const CURRENT_PROTOCOL_PROGRAM_PAYLOAD_BYTES = 1_142_664;
export const CURRENT_PROTOCOL_PROGRAM_PAYLOAD_SHA256 = "3bc781297d83e72faad1c63274f7f3745ef46bb60dcff16dd28a045b734a6c0b";
export const CURRENT_PROTOCOL_BUILD_RECEIPT_SHA256 = "3747c4fefbd233eb87e329516ab2981ae4017c6fe606d7cf497efa3e8973a322";
export const CURRENT_PROTOCOL_DEPLOYMENT_RECEIPT_SIGNATURE = "24xVihYoZtcdTLaSxH2Q4NWpSt7d9YKtSAwitfXNQzNoigLYtWX31Ksd7iUYHsGZzcX3vMJmjYVLzyZXVdzsdZ65";
export const CURRENT_PROTOCOL_FORMAL_RECEIPT_INTERNAL_SHA256 = "c131deef09c3959829b339c9e8093db69d3fd1e735ace5b017631b064fc93f31";
export const CURRENT_PROTOCOL_FORMAL_RECEIPT_FILE_SHA256 = "8e90997c06c1eae7c7a5424d85c0836657727018b04c7a8a68135edb3cfdad94";
export const CURRENT_MARKET_ACCOUNT_SIZE = 285;
export const CURRENT_ORACLE_MONTH_ACCOUNT_SIZE = 299;
export const CURRENT_VAULT_CONFIG_ACCOUNT_SIZE = 135;
export const CURRENT_USER_COLLATERAL_ACCOUNT_SIZE = 58;
export const CURRENT_SETTLEMENT_RECORD_V2_ACCOUNT_SIZE = 160;
export const CURRENT_SETTLEMENT_SIGNER_REGISTRY_ACCOUNT_SIZE = 128;
export const CURRENT_SETTLEMENT_SIGNER_SET_ACCOUNT_SIZE = 416;
export const CURRENT_ORACLE_SKU_COVERAGE_MANIFEST_ACCOUNT_SIZE = 128;
export const CURRENT_ORACLE_ACTIVE_WEIGHT_MANIFEST_ACCOUNT_SIZE = 337;
/** rc.44 production schedule: seven scramble days plus one opening day. */
export const CURRENT_ORACLE_PRE_LISTING_WINDOW_SECONDS = 8n * 86400n;
const CURRENT_PROGRAM_ID = new PublicKey(DEFAULT_AMEBA_SPREAD_PROGRAM_ID);
function assertCanonicalCurrentProgramId(programId) {
...
function assertCurrentProgramAccount(label, input) {
...
export const HISTORICAL_RC44_PROTOCOL_FINALIZED_VERIFICATION_CONTEXT_SLOT = CURRENT_PROTOCOL_FINALIZED_VERIFICATION_CONTEXT_SLOT;
export const HISTORICAL_RC44_PROTOCOL_PROGRAM_PAYLOAD_BYTES = CURRENT_PROTOCOL_PROGRAM_PAYLOAD_BYTES;
export const HISTORICAL_RC44_PROTOCOL_PROGRAM_PAYLOAD_SHA256 = CURRENT_PROTOCOL_PROGRAM_PAYLOAD_SHA256;
export const HISTORICAL_RC44_PROTOCOL_BUILD_RECEIPT_SHA256 = CURRENT_PROTOCOL_BUILD_RECEIPT_SHA256;
export const HISTORICAL_RC44_PROTOCOL_DEPLOYMENT_RECEIPT_SIGNATURE = CURRENT_PROTOCOL_DEPLOYMENT_RECEIPT_SIGNATURE;
export const HISTORICAL_RC44_PROTOCOL_FORMAL_RECEIPT_INTERNAL_SHA256 = CURRENT_PROTOCOL_FORMAL_RECEIPT_INTERNAL_SHA256;
export const HISTORICAL_RC44_PROTOCOL_FORMAL_RECEIPT_FILE_SHA256 = CURRENT_PROTOCOL_FORMAL_RECEIPT_FILE_SHA256;
...
export { CURRENT_VAULT_INSTRUCTION_TAG, CURRENT_AMOEBA_DLMM_INSTRUCTION_TAG } from "./current-instruction-tags.js";
class Reader {
    label;
    data;
    offset = 0;
    ...
    bool(field) {
        ...
        if (value !== 0 && value !== 1)
            throw new Error(`${field} is not a canonical bool`);
        return value === 1;
    }
    u16(field) {
    ...
    exactLengthWithZeroPadding(length) {
    ...
    require(length, field) {
        if (this.offset + length > this.data.length)
            throw new Error(`account ended before ${field}`);
    }
}
function header(reader, discriminator, expectedVersion = 1) {
    ...
    if (discriminator !== undefined) {
        ...
        if (reader.u8("account_version") !== expectedVersion)
            throw new Error("account version is not current");
    }
    return bump;
}
function uninitializedHeader(reader) {
    if (reader.bool("is_initialized"))
...
function enumValue(value, values, field) {
    ...
    if (result === undefined)
        throw new Error(`${field} has an unknown enum value`);
    return result;
}
function decodeLayout(label, data, decode) {
    try {
        return decode();
    }
    catch (error) {
        if (error instanceof AmebaProtocolLayoutError)
            throw error;
...
function assertPda(label, address, expected, bump, expectedBump) {
...
function expectedPda(programId, seeds) {
    return PublicKey.findProgramAddressSync(seeds.map((seed) => Buffer.from(seed)), programId);
}
function u64Seed(value, field) {
    const integer = typeof value === "bigint" ? value : BigInt(value);
    if (integer < 0n || integer > 0xffffffffffffffffn)
        throw new RangeError(`${field} must fit u64`);
    const output = Buffer.alloc(8);
    output.writeBigUInt64LE(integer);
    return output;
}
const CURRENT_PRODUCT_POLICY = Object.freeze({
    RAMX: Object.freeze({ product: "ramx", underlyingId: "ram-standardized-baskets" }),
    NANDX: Object.freeze({ product: "nandx", underlyingId: "nand-standardized-baskets" }),
});
function decodeCurrentMarketSeriesIdentity(marketId, underlyingId, expiryTs, optionKind, strikePrice, capPrice, contractSize, maxPayoutPerContract) {
    ...
    if (match === null)
        throw new Error("market_id does not use the current PRODUCT-YYYYMM-SIDE-NN grammar");
    const [productLabel, yearText, monthText, side, ordinalText] = match.slice(1);
    ...
    const expectedCap = side === "CALL" ? 112000000n : 88000000n;
    if (ordinal !== 1
...
export function decodeCurrentMarketAccount(input) {
    ...
    return decodeLayout("Market", data, () => {
        ...
        const optionKind = enumValue(reader.u8("option_kind"), ["CallSpread", "PutSpread"], "option_kind");
        const settlementStyle = enumValue(reader.u8("settlement_style"), ["CashSettledMonthly"], "settlement_style");
        ...
        if (mintAccounting.totalBurned > mintAccounting.totalConsumed ||
        ...
        const expectedMaximumPayout = spreadWidth * contractSize / 1000000n;
        const maximumBinId = maxPayoutPerContract / 50000n;
        if (expiryTs === 0n
            || contractSize === 0n
        ...
        const priceDisplayDecimals = derivedPriceDisplayDecimals;
        const quoteDisplayDecimals = mintAccounting.decimals;
...
export function validateUninitializedCurrentOracleMonthData(data) {
    decodeLayout("UninitializedOracleMonth", data, () => {
        ...
        enumValue(reader.u8("phase"), ["Uninitialized", "Scramble", "Game", "Settled", "Closed", "Opening", "SourceSubmission"], "phase");
        ...
        enumValue(reader.u8("settlement_status"), ["Final", "Provisional", "FrozenPendingEvidence"], "settlement_status");
...
export function decodeCurrentOracleMonthAccount(input) {
    ...
    return decodeLayout("OracleMonth", data, () => {
        ...
        const phase = enumValue(reader.u8("phase"), ["Uninitialized", "Scramble", "Game", "Settled", "Closed", "Opening", "SourceSubmission"], "phase");
        ...
        const settlementStatus = enumValue(reader.u8("settlement_status"), ["Final", "Provisional", "FrozenPendingEvidence"], "settlement_status");
        ...
        assertPda("OracleMonth", address, deriveOracleMonthPda({ marketPda: market, expiryTs, programId }), bump, expectedBump);
        return {
            bump,
            market,
            authority,
...
/** Internal initialization preflight: exact fixed-codec shape with is_initialized=false. */
export function validateUninitializedCurrentOracleSkuCoverageManifestData(data) {
...
export function decodeCurrentOracleSkuCoverageManifestAccount(input) {
...
export function decodeCurrentOracleActiveWeightManifestAccount(input) {
    const { address, data, month, programId } = input;
    ...
    return decodeLayout("OracleActiveWeightManifest", data, () => {
        ...
        const phase = enumValue(reader.u8("phase"), ["Collecting", "Applying", "ReadyToFinalize", "Finalized"], "phase");
...
export function decodeCurrentVaultConfigAccount(input) {
    const { address, data, programId } = input;
    ...
    return decodeLayout("VaultConfig", data, () => {
        ...
        if (bytes[130] !== 0 && bytes[130] !== 1)
            throw new Error("paused is not a canonical bool");
        const value = {
            bump: bytes[1],
...
export function decodeCurrentContractMintAccount(input) {
    ...
    return decodeLayout("ContractMint", data, () => {
        ...
        if (accounting.discriminator !== "MNT" ||
...
export function decodeCurrentLightSplInterfaceAccount(input) {
    ...
    return decodeLayout("LightSplInterface", data, () => {
        ...
        if (data.byteLength !== ACCOUNT_SIZE ||
            input.executable === true ||
        ...
        const account = unpackAccount(address, {
            data: Buffer.from(data),
            executable: false,
            lamports: 0,
            owner: input.accountOwner,
...
export function decodeCurrentUserCollateralAccount(input) {
    const { address, data, programId } = input;
...
export function decodeCurrentSettlementRecordV2Account(input) {
    const { address, data, programId } = input;
    ...
    return decodeLayout("SettlementRecordV2", data, () => {
        ...
        if (signedLeafCommitment.equals(Buffer.alloc(32)) ||
        ...
        assertPda("SettlementRecordV2", address, deriveSettlementRecordV2Pda({ marketPda: market, oracleMonthPda: oracleMonth, programId }), bump, expectedBump);
...
export function decodeCurrentSettlementSignerRegistryAccount(input) {
    const { address, data, programId } = input;
    ...
    return decodeLayout("SettlementSignerRegistry", data, () => {
        ...
        if (value.currentSet.equals(PublicKey.default) ||
        ...
        assertPda("SettlementSignerRegistry", address, deriveSettlementSignerRegistryPda(programId), value.bump, expectedBump);
        return value;
    });
}
export function decodeCurrentSettlementSignerSetAccount(input) {
    const { address, data, programId } = input;
    ...
    return decodeLayout("SettlementSignerSet", data, () => {
        ...
        if (signers.some((signer) => signer.equals(PublicKey.default)) ||
        ...
        if (!value.setHash.equals(expectedHash))
            throw new Error("signer set hash is invalid");
        const [, expectedBump] = expectedPda(programId, [
            CURRENT_STATE_NAMESPACE_SEED,
            SETTLEMENT_SIGNER_SET_PDA_SEED,
            u64Seed(value.version, "signer set version"),
        ]);
        assertPda("SettlementSignerSet", address, deriveSettlementSignerSetPda(value.version, programId), value.bump, expectedBump);
        return value;
    });
}
/**
 ...
 */
export async function discoverCurrentMarkets(connection, options = {}) {
...
export function validateCurrentDeploymentIdentity(input) {
    if (input.cluster !== CURRENT_PROTOCOL_CLUSTER ||
...
export function assertCurrentProgramOwner(owner, programId = new PublicKey(DEFAULT_AMEBA_SPREAD_PROGRAM_ID)) {
    assertCanonicalCurrentProgramId(programId);
