...
import { CURRENT_LIGHT_TOKEN_ACCOUNT_SIZE, CURRENT_LIGHT_TOKEN_PROGRAM_ID, deriveLightAssociatedTokenAddress, deriveClassicAssociatedTokenAddress, createClassicAssociatedTokenAccountIdempotentInstruction, CLASSIC_ASSOCIATED_TOKEN_PROGRAM_ID, } from "@amoeba/spread-release-tools/token-primitives";
import { AMOEBA_SPREAD_PROGRAM_ID } from "./identity.js";
import { CURRENT_PROTOCOL_RELEASE, CURRENT_PROTOCOL_SOURCE_COMMIT, CURRENT_VAULT_INSTRUCTION_TAG, } from "./current.js";
...
import { isCurrentWriterDlmmOperation } from "./writer-dlmm-public.js";
...
import { decodeCurrentCollectiveLookupTableV1 } from "./current-collective-lookup-table.js";
import { compileCurrentWalletUnsignedBatch } from "../wallet/transaction.js";
export const WRITER_OPERATION_PLAN_SCHEMA_VERSION = 1;
export const WRITER_OPERATION_PLAN_SETUP_SCHEMA_VERSION = 2;
export const WRITER_INSTRUCTION_OUTER_BYTE_CAP = 16_384;
...
export function validateCurrentWriterPublicOperationRequest(value) {
    ...
    const addressField = (field) => {
        if (typeof request[field] !== "string")
            throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", `${field} must be canonical base58`);
        return canonicalPublicKey(request[field], field).toBase58();
    };
    const owner = addressField("owner");
    if (operation === "auction_refund")
        return Object.freeze({ operation, owner,
            auction: addressField("auction"),
            bid: addressField("bid") });
    if (typeof request.amountAtoms !== "string" || !/^[1-9][0-9]*$/.test(request.amountAtoms)
        || BigInt(request.amountAtoms) > 18446744073709551615n) {
...
const WRITER_TAG_NAMES = new Map(Object.entries(CURRENT_VAULT_INSTRUCTION_TAG)
...
export function writerInstructionManifest(instruction) {
 ...
 */
export function prepareWriterOperation(input) {
    ...
    const semanticInstructions = Object.freeze(input.instructions.map(semanticCurrentSpreadInstructionV1));
    ...
    if (isCurrentWriterDlmmOperation(input.operation)) {
        ...
        const setupInstructionBatches = Object.freeze([Object.freeze(setup.map((instruction, index) => Object.freeze({
        ...
        const setupSignerRoles = Object.freeze([]);
        ...
        compileCurrentWalletUnsignedBatch(new PublicKey(semantic.owner), PublicKey.default.toBase58(), [...setup, ...input.instructions], lookup === null ? [] : [lookup.account]);
        const fields = { operation: input.operation, currentObservationDigest: observation.currentObservationDigest,
            leanAdmissionDigest: input.leanAdmissionDigest, semantic, setupMode: "writer_liquidity_compute",
    ...
    if (input.transactionLookupTable !== undefined)
        throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "lookup witness is only supported by writer liquidity transport");
    ...
    if (input.coldAccount !== undefined && input.canonicalOutput !== undefined) {
        throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "writer schema-v2 setup must select exactly one setup mode");
    }
    if (input.coldAccount === undefined && input.canonicalOutput === undefined && input.classicOutput === undefined) {
        if (lightAccount !== null)
            requireHotLightAccount(observation, lightAccount.target);
    ...
    if (lightAccount === null && input.classicOutput === undefined) {
        throw writerPlanError("WRITER_PLAN_COLD_ACCOUNT_INVALID", `${input.operation} does not have one supported cold Light ATA`);
    }
    let setupMode;
    let setupFacts;
    ...
    if (input.coldAccount !== undefined) {
        if (lightAccount === null)
            throw writerPlanError("WRITER_PLAN_COLD_ACCOUNT_INVALID", "operation has no Light input");
        if (!["close_begin", "close_basket", "withdraw_principal", "settlement_claim_collective", "settlement_claim_flat"].includes(input.operation)) {
            throw writerPlanError("WRITER_PLAN_COLD_ACCOUNT_INVALID", "cold-load setup requires an exact principal withdrawal, settlement claim or close input");
        }
        requireColdLightAccount(observation, lightAccount.target);
        const coldAccountProofFacts = canonicalWriterColdProof(input.coldAccount.proofFacts, lightAccount);
        setupMode = "cold_load";
        setupFacts = Object.freeze({ coldAccountProofFacts });
        setupInstructions = canonicalWriterColdSetupBatches(input.coldAccount.setupInstructionBatches, lightAccount, coldAccountProofFacts);
    }
    else if (input.canonicalOutput !== undefined) {
        if (lightAccount === null)
            throw writerPlanError("WRITER_PLAN_CANONICAL_OUTPUT_INVALID", "operation has no Light output");
        if (input.operation !== "close_cancel" || input.canonicalOutput === undefined) {
            throw writerPlanError("WRITER_PLAN_CANONICAL_OUTPUT_INVALID", "canonical-output setup is admitted only for close cancellation");
        }
        const canonicalOutputSetupFacts = canonicalWriterOutputFacts(input.canonicalOutput.setupFacts, lightAccount);
    ...
    if (input.classicOutput !== undefined) {
        if (input.canonicalOutput !== undefined)
            throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "Light and classic output setup cannot be mixed");
    ...
    const setupInstructionBatches = Object.freeze(setupInstructions.map((batch) => Object.freeze(batch.map(writerSetupInstructionManifest))));
    const setupSignerRoles = Object.freeze(setupInstructions.map((batch, setupBatchIndex) => Object.freeze({
        pubkey: setupPayer.toBase58(),
        role: "payer",
        setupBatchIndex,
        instructionIndexes: Object.freeze(batch.flatMap((instruction, instructionIndex) => instruction.keys.some((meta) => meta.isSigner && meta.pubkey.equals(setupPayer))
            ? [instructionIndex]
            : [])),
    })));
    const executionInstructionBatches = Object.freeze(setupInstructionBatches.map((batch, index) => Object.freeze(index === setupInstructionBatches.length - 1 ? [...batch, ...instructions] : [...batch])));
...
function writerLightAccountExpectation(operation, instructions) {
    if (!["withdraw_principal", "close_begin", "close_basket", "close_cancel", "settlement_claim_collective", "settlement_claim_flat"].includes(operation))
        return null;
    if (instructions.length !== 1) {
        throw writerPlanError("WRITER_PLAN_COLD_ACCOUNT_INVALID", "a cold-capable writer stage must contain exactly one native writer instruction");
    }
    ...
    switch (operation) {
        ...
        case "settlement_claim_flat": {
            ...
            minimumRequirement = Object.freeze({ kind: "instruction_exact",
        ...
        case "close_basket":
            ...
            mintIndex = 5;
            requiredOwner = instruction.keys[0].pubkey;
            minimumRequirement = Object.freeze({ kind: "positive" });
            break;
        case "close_cancel": {
            if (instruction.data[1] === 0xff) {
                requireWriterInstructionShape(instruction, 11, CURRENT_VAULT_INSTRUCTION_TAG.ProcessWriterCloseCancellationV1);
                targetIndex = 5;
                mintIndex = 3;
            }
            else {
                requireWriterInstructionShape(instruction, 13, CURRENT_VAULT_INSTRUCTION_TAG.ProcessWriterCloseCancellationV1);
                targetIndex = 7;
                mintIndex = 5;
            }
            ...
            requiredOwner = null;
            minimumRequirement = Object.freeze({ kind: "zero" });
            break;
        }
        default:
            return null;
    ...
    if (!payerMeta.isSigner
        ...
        || mintMeta.isSigner) {
        throw writerPlanError("WRITER_PLAN_COLD_ACCOUNT_INVALID", "writer Light ATA payer signer, target, or mint flags are not canonical");
    }
    return Object.freeze({
...
function requireWriterInstructionShape(instruction, accountCount, tag) {
    if (instruction.keys.length !== accountCount || instruction.data[0] !== tag) {
        throw writerPlanError("WRITER_PLAN_COLD_ACCOUNT_INVALID", "writer Light ATA stage does not have the exact RC44 account grammar");
    }
}
function canonicalWriterColdProof(proof, expected) {
    ...
    const minimumAmount = canonicalU64(proof.minimumAmountAtoms, "coldAccountProofFacts.minimumAmountAtoms");
    ...
    if (!PublicKey.isOnCurve(owner.toBytes())) {
        throw writerPlanError("WRITER_PLAN_COLD_PROOF_INVALID", "cold writer proof owner must be an on-curve wallet");
    }
    ...
    if (!ata.equals(expected.target) || !ata.equals(derivedAta)
        ...
        throw writerPlanError("WRITER_PLAN_COLD_PROOF_INVALID", "cold writer proof facts do not bind the exact payer, Light ATA, mint, owner, amount, or minimum");
    }
    return Object.freeze({
        ata: ata.toBase58(),
...
function canonicalWriterColdSetupBatches(batches, expected, proof) {
    if (batches.length < 1 || batches.length > 8 || batches.some((batch) => batch.length !== 3)) {
        throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "cold writer setup must contain 1..8 exact compute/create/Transfer2 batches");
    }
    const owner = canonicalPublicKey(proof.owner, "coldAccountProofFacts.owner");
    ...
    const canonical = Object.freeze(batches.map((batch) => {
        ...
        if (!sameNativeInstruction(create, officialCreate)) {
            throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "cold writer setup idempotent ATA create is not the exact SDK-native v0.23.3 instruction");
        }
        return Object.freeze([...batch]);
    }));
    try {
        validateCurrentLightTransfer2LoadSequence(canonical.map((batch) => batch[2]), {
            payer: expected.payer,
            owner,
            mint: expected.mint,
        ...
        throw writerPlanError("WRITER_PLAN_SETUP_INVALID", `cold writer Transfer2 payload is not exact: ${detail}`);
    }
    return canonical;
}
function canonicalWriterOutputFacts(facts, expected) {
    ...
    if (!PublicKey.isOnCurve(owner.toBytes())) {
        throw writerPlanError("WRITER_PLAN_CANONICAL_OUTPUT_INVALID", "canonical cancellation output owner must be an on-curve wallet");
    }
    const derived = deriveLightAssociatedTokenAddress(mint, owner);
    if (!ata.equals(derived) || !ata.equals(expected.target)
        || !mint.equals(expected.mint) || !payer.equals(expected.payer)) {
        throw writerPlanError("WRITER_PLAN_CANONICAL_OUTPUT_INVALID", "cancellation output facts do not bind the exact payer, owner, mint, or canonical Light ATA");
    }
    return Object.freeze({
        ata: ata.toBase58(),
        owner: owner.toBase58(),
...
function canonicalWriterOutputSetupBatch(facts) {
...
function requireWriterComputeBudget(instruction) {
    ...
    if (!instruction.programId.equals(COMPUTE_BUDGET_PROGRAM)
        ...
        throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "writer setup ComputeBudget SetComputeUnitLimit is not canonical");
    }
}
...
function canonicalWriterClassicOutputFacts(value, operation, instruction, observation) {
    if (!["withdraw_principal", "auction_refund", "close_finalize"].includes(operation))
        throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "classic output creation requires withdrawal/refund/close finalization");
    const owner = canonicalPublicKey(value.owner, "classicOutput.owner");
    const payer = canonicalPublicKey(value.payer, "classicOutput.payer");
    const mint = canonicalPublicKey(value.mint, "classicOutput.mint");
    const ata = canonicalPublicKey(value.ata, "classicOutput.ata");
    ...
    if (!payer.equals(instruction.keys[0].pubkey) || !ata.equals(instruction.keys[destinationIndex].pubkey)
        ...
        || (operation !== "auction_refund" && !owner.equals(payer)))
        throw writerPlanError("WRITER_PLAN_SETUP_INVALID", "classic output identity differs from exact native destination");
...
function sameNativeInstruction(left, right) {
...
function writerSetupInstructionManifest(instruction) {
    return Object.freeze({
        programId: instruction.programId.toBase58(),
...
function canonicalPublicKey(value, path) {
    let key;
    try {
...
function requireWriterSemanticBinding(operation, semantic, instructions, observation, seriesBookBase64) {
    ...
    if (instructions.length !== 1) {
        throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", "one public writer semantic must bind exactly one native instruction");
    }
    const instruction = instructions[0];
    ...
    if (operation === "auction_refund") {
        ...
        if (instruction.data.length !== 1)
            throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", "refund has no caller-selected amount or destination");
        return;
    }
    ...
    if (operation === "close_begin") {
        ...
        requireSemanticU64(semantic, "minimumWithdrawalAtoms", instructionU64(instruction, 9));
        return;
    }
    ...
    if (operation === "settlement_claim_collective") {
        ...
        if (!book.sleeve.equals(instruction.keys[2].pubkey)
            ...
            || book.records.some((record) => !record.retirementCustody.equals(deriveWriterRetirementCustodyPda(book.sleeve, record.market, new PublicKey(AMOEBA_SPREAD_PROGRAM_ID))[0]))
    ...
    throw writerPlanError("WRITER_PLAN_SEMANTIC_UNBOUND", `${operation} has no reviewed native semantic binding and is not admitted by the public plan surface`);
}
function instructionU64(instruction, offset) {
    const bytes = Buffer.from(instruction.data);
...
function requireExactSemanticKeys(semantic, expected) {
    const actual = Object.keys(semantic).sort();
    const required = [...expected].sort();
    if (canonicalJson(actual) !== canonicalJson(required)) {
        throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", "writer semantic fields are not the exact public operation schema");
    }
}
function requireSemanticPublicKey(semantic, field, expected) {
    ...
    if (!key.equals(expected)) {
        throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", `${field} does not match the exact native instruction account`);
    }
}
function requireSemanticU64(semantic, field, expected) {
    const value = semantic[field];
    if (typeof value !== "string"
        || canonicalU64(value, `semantic.${field}`) !== expected) {
        throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", `${field} does not match the exact native instruction payload`);
    }
}
function requireSemanticInteger(semantic, field, expected) {
    const value = semantic[field];
    if (!Number.isSafeInteger(value) || value !== expected) {
        throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", `${field} does not match the exact native instruction payload`);
    }
}
function requireSemanticLiteral(semantic, field, expected) {
    if (semantic[field] !== expected) {
        throw writerPlanError("WRITER_PLAN_SEMANTIC_MISMATCH", `${field} does not match the exact native instruction operation`);
    }
}
function requireObservedInstructionAccounts(observation, instructions) {
    ...
    const required = new Set(instructions.flatMap((instruction) => instruction.keys.map((account) => account.pubkey.toBase58())));
    for (const address of required) {
        const account = observed.get(address);
        if (account === undefined) {
            throw writerPlanError("WRITER_PLAN_OBSERVATION_INVALID", `finalized observation does not bind instruction account ${address}`);
        }
        ...
        if (!complete && !absent) {
            throw writerPlanError("WRITER_PLAN_OBSERVATION_INVALID", `finalized observation contains partial state for instruction account ${address}`);
        }
    }
}
function requireHotLightAccount(observation, target) {
    ...
    if (account === undefined
        ...
        throw writerPlanError("WRITER_PLAN_OBSERVATION_INVALID", `writer Light ATA ${address} is neither canonical hot state nor an authenticated cold load`);
    }
}
function requireColdLightAccount(observation, target) {
    const address = target.toBase58();
    const account = observation.orderedAccounts.find((candidate) => candidate.address === address);
    if (account === undefined || [
        account.owner,
        ...
        throw writerPlanError("WRITER_PLAN_OBSERVATION_INVALID", `cold writer Light ATA ${address} must be explicitly absent from finalized hot state`);
    }
}
function requireCanonicalOutputLightAccount(observation, target) {
    const address = target.toBase58();
    const account = observation.orderedAccounts.find((candidate) => candidate.address === address);
    if (account === undefined) {
        throw writerPlanError("WRITER_PLAN_OBSERVATION_INVALID", `canonical cancellation output ${address} is missing from the finalized observation`);
    }
    const fields = [account.owner, account.executable, account.dataLength, account.dataSha256];
    const absent = fields.every((value) => value === null);
    ...
    if (!absent && !canonicalHot) {
        throw writerPlanError("WRITER_PLAN_OBSERVATION_INVALID", `canonical cancellation output ${address} is neither absent nor exact hot Light state`);
    }
}
/** Exact portable-plan validation used before bounded submission. */
export function validateWriterOperationPlan(plan, independentlyRebuilt) {
...
function canonicalSignerRoles(values, instructionCount) {
...
function requireOperationTags(operation, instructions) {
...
function requireExactWriteSet(writeSet, instructions) {
    const exact = [...new Set(instructions.flatMap((instruction) => instruction.accounts.filter((meta) => meta.isWritable).map((meta) => meta.address)))].sort();
...
function requireExactSignerCoverage(signerRoles, instructions) {
...
function writerPlanError(code, message) {
    return new AmebaProtocolError(message, { code });
}
//# sourceMappingURL=writer-operation.js.map